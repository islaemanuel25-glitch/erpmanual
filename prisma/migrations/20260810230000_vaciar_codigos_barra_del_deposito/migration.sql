...
-- falle por checksum distinto en la próxima corrida. Una migración aplicada es
-- historia: no se toca, se agrega otra.
--
-- ── EL CRITERIO ES QUIÉN CREÓ EL PRODUCTO ───────────────────────────────────
--
-- Decisión de negocio del 2026-08-10, escrita en docs/decisions/DEC-0006. En
-- una línea: el único local que crea y toca productos es Casiano Casas, y lo
-- suyo no se toca; el resto es del depósito y se vacía.
--
...
--
-- ⚠ ESTE CRITERIO REEMPLAZA AL ANTERIOR, Y NO SON COMPATIBLES. La migración
-- 20260810210000 vació solo los que no tenían ninguna venta, con el argumento
-- de que vaciar un atajo en uso le rompe el trabajo a quien atiende. Este vacía
-- por creador, sin mirar ventas, y la oposición es total: LOS 60 TIENEN VENTAS.
-- Tiene que ser así — los que no vendían nunca ya los vació la migración
-- anterior, así que lo que queda es exactamente lo que se usa. Entre ellos 'xl'
-- con 172 ventas y BARRATREMBLAY con 201, las dos del mismo día de la medición.
-- Es una decisión tomada a sabiendas.
--
...
--
-- Queda afuera 'pollo trozado' (id 2387), el único de Casiano Casas.
--
-- NO se toca el código de nivel ubicación (ProductoLocal.codigo_barra_propio).
-- Es otro campo, de otra tabla, y está VACÍO en las 11.651 filas de producción:
-- nadie lo usó nunca.
--
...
-- ── REPONER ─────────────────────────────────────────────────────────────────
--
-- docs/business-rules/codigos-vaciados-deposito-2026-08-10.md, con el UPDATE
-- listo para copiar, uno por uno o todos juntos.
