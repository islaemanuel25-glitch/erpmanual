 ...
 *
 * EL TEMPLATE LITERAL DE VARIAS LÍNEAS, QUE ANTES ERA UN AGUJERO. Hasta esta
 * corrección las comillas se reiniciaban en CADA salto de línea, así que un
 * backtick abierto en un renglón y cerrado en otro perdía el estado. Si adentro
 * de ese literal había un `{/*`, se tomaba por comentario y **se blanqueaba todo
 * el código que venía después**. Medido: un archivo con un literal multilínea que
 * menciona `{/*` perdía un `text-[17px]` real que estaba dos renglones más abajo.
 *
 * Es la peor familia de defecto de este proyecto —la que da VERDE—: la cuenta
 * baja sin motivo y nadie se entera, porque un número que baja no frena a nadie.
 *
 * Se arregla distinguiendo qué comilla puede cruzar un renglón. El backtick SÍ:
 * un template literal de varias líneas es JavaScript válido y corriente. Las
 * comillas simples y dobles NO: un salto de línea sin escapar adentro de ellas es
 * un error de sintaxis, así que reiniciarlas en el salto es lo correcto y además
 * protege de que una comilla desbalanceada se coma el resto del archivo.
 *
 * Y ADEMÁS FALLA CERRADO. Si al terminar el archivo quedó un `{/*` sin cerrar o
 * un backtick sin cerrar, este reconocedor no entendió lo que leyó: en vez de
 * devolver un texto blanqueado de más —que es exactamente una cuenta que baja
 * sin motivo— tira. El que lo llama ya sabe frenar; un contador que no puede
 * medir tiene que decirlo, no contestar un número menor.
 *
...
export function sinComentariosJsx(contenido) {
  ...
  while (i < src.length) {
    ...
    if (c === "\n") {
      salida += "\n";
      // El backtick cruza renglones; `'` y `"` no. Ver el encabezado.
  ...
  // FALLA CERRADO. Los dos casos significan lo mismo: el reconocedor no entendió
  // el archivo. Devolver `salida` sería devolver texto blanqueado de más, y eso
  // se ve como una cuenta que baja —el síntoma que nadie mira—.
  if (dentro) {
    throw new Error(
      "sinComentariosJsx: quedó un `{/*` sin cerrar. Blanquear hasta el final del " +
        "archivo haría desaparecer hardcodeo real del conteo, así que no se cuenta."
    );
  }
  if (comilla === "`") {
    throw new Error(
      "sinComentariosJsx: quedó un template literal sin cerrar. El reconocedor no " +
        "puede saber qué es código y qué es texto, así que no se cuenta."
    );
  }
