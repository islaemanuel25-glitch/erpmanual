...
test("un import multilínea del kit SÍ cuenta como usarlo", () => {
  // `import {\n  algo,\n} from ".../SunmiModalLayout"` no matchea una expresión
  // de una sola línea. Se mira también el `from`.
  const conKit = ARCHIVO.replace(
    "import SunmiButton",
    'import {\n  algo,\n} from "@/components/sunmi/SunmiModalLayout";\nimport SunmiButton'
  );
  assert.equal(contar(conKit).conteo.modal, 0);
});
