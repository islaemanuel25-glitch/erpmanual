...
// ══════════════════════════════════════════════════════════════════════════
// EL TEMA PARALELO SE ANCLA: `sunmi-pos-*` NO ES `pos-*`
// ══════════════════════════════════════════════════════════════════════════
//
// El patrón empezaba con `\b`, que matchea entre un guión y una letra, así que
// `sunmi-pos-text-danger` —una clase legítima del sistema general, definida en
// `styles/sunmi.css:407`— contenía `pos-text-danger` y se contaba como tema
// paralelo. Medido el 2026-09-07: 32 ocurrencias mal contadas, el 24 % de la
// categoría, iguales en la línea de base y en el árbol de hoy.

const temaParalelo = (jsx) =>
  contarArchivo("app/x.jsx", jsx, OPCIONES).hallazgos
    .filter((h) => h.categoria === "tema-paralelo")
    .map((h) => h.que);

test("LA CLASE DEL TEMA PARALELO SE CUENTA", () => {
  assert.deepEqual(temaParalelo('<b className="pos-text-danger">x</b>'), ["pos-text-danger"]);
  assert.deepEqual(temaParalelo('<b className="pos-surface-alta">x</b>'), ["pos-surface-alta"]);
});

test("`sunmi-pos-text-danger` NO SE CUENTA: es del sistema general", () => {
  assert.deepEqual(temaParalelo('<b className="sunmi-pos-text-danger">x</b>'), []);
});

test("y ningún otro prefijo la despierta", () => {
  // Los casos cercanos, para que el falso positivo no se reabra por otro lado.
  for (const clase of [
    "sunmi-pos-text-danger",
    "sunmi-pos-surface-alta",
    "mi-pos-text-danger",
    "xpos-text-danger",
    "_pos-text-danger",
    "algo_pos-bg-base",
  ]) {
    assert.deepEqual(temaParalelo(`<b className="${clase}">x</b>`), [], clase);
  }
});

test("pero sí cuenta cuando empieza de verdad, esté donde esté en el atributo", () => {
  // Al principio, en el medio y al final de una lista de clases: lo que importa
  // es que la familia EMPIECE ahí, no dónde caiga en el string.
  assert.deepEqual(temaParalelo('<b className="pos-text-muted flex">x</b>'), ["pos-text-muted"]);
  assert.deepEqual(temaParalelo('<b className="flex pos-text-muted gap-2">x</b>'), ["pos-text-muted"]);
  assert.deepEqual(temaParalelo('<b className="flex pos-text-muted">x</b>'), ["pos-text-muted"]);
});

test("USAR EL TOKEN NO ES HARDCODEAR: `var(--pos-…)` no se cuenta", () => {
  // El segundo falso positivo, y el peor de los dos: `--pos-control-bg` es la
  // VARIABLE del tema, o sea exactamente lo que hay que usar. El patrón viejo la
  // contaba como violación porque `-` abre un `\b`. Diez ocurrencias en la línea
  // de base histórica, penalizando la solución en vez del problema.
  for (const jsx of [
    '<b className="hover:bg-[var(--pos-control-bg)]">x</b>',
    '<b className="file:bg-[var(--pos-control-hover)]">x</b>',
    '<b style={{ background: "var(--pos-bg-soft, rgba(0,0,0,.1))" }}>x</b>',
    '<b className="hover:bg-[var(--pos-surface-hover)]">x</b>',
  ]) {
    assert.deepEqual(temaParalelo(jsx), [], jsx);
  }
});

test("y la clase con prefijo de variante SÍ se cuenta: ahí sí empieza la familia", () => {
  // El anclaje no puede volverse tan estricto que deje pasar lo que importa.
  // Después de `:` la familia empieza de verdad, así que cuenta.
  assert.deepEqual(temaParalelo('<b className="hover:pos-text-muted">x</b>'), ["pos-text-muted"]);
  assert.deepEqual(temaParalelo('<b className="md:pos-surface-alta">x</b>'), ["pos-surface-alta"]);
});

test("una línea puede traer la buena y la mala juntas, y se cuenta una sola", () => {
  // El caso real de `app/modulos/pos-ventas/page.jsx`: conviven las dos formas.
  assert.deepEqual(
    temaParalelo('<b className="sunmi-pos-text-danger">a</b><i className="pos-text-danger">b</i>'),
    ["pos-text-danger"]
  );
});
...
// ══════════════════════════════════════════════════════════════════════════
// LA PROSA DE UN COMENTARIO JSX NO ES CÓDIGO
// ══════════════════════════════════════════════════════════════════════════
//
// `esComentario` mira cómo EMPIEZA una línea, y las del medio de un bloque
// `{/* … */}` empiezan con cualquier cosa. Medido el 2026-09-07: dos
// `rgb(51,65,85)` escritos en prosa dentro de un comentario que explicaba una
// medición se contaban como colores fijos. El mismo día volvió a pasar al
// escribir un comentario que NOMBRABA dos clases viejas.
//
// Quinta vez del patrón que CLAUDE.md tiene anotado, y la primera en la que la
// prosa no está al principio del renglón.

/** Los textos marcados de una categoría, para leer los casos de un vistazo. */
const marcados = (jsx, categoria) =>
  contar(jsx).hallazgos.filter((h) => h.categoria === categoria).map((h) => h.que);

test("A · EL HARDCODEO DE VERDAD SE SIGUE CONTANDO", () => {
  // El control. Sin esto, un blanqueo que se comiera todo también pasaría.
  assert.deepEqual(marcados(`<div style={{ color: "rgb(51,65,85)" }} />`, "color"), ["rgb(51,65,85)"]);
  assert.deepEqual(marcados(`<b className="text-[13px]">x</b>`, "medida"), ["text-[13px]"]);
});

test("B · EL MISMO, DENTRO DE UN {/* */} DE UNA LÍNEA, NO CUENTA", () => {
  assert.deepEqual(marcados(`{/* medido: rgb(51,65,85) sobre el panel */}`, "color"), []);
  assert.deepEqual(marcados(`{/* antes era text-[13px] */}`, "medida"), []);
});

test("C · NI DENTRO DE UN BLOQUE DE VARIAS LÍNEAS", () => {
  // Es el caso exacto de ImportarPedidoDesdeArchivo.jsx: la apertura arriba, la
  // prosa en el medio —sin `//` ni `*` que la delaten— y el cierre abajo.
  const jsx = [
    `<div>`,
    `  {/*`,
    `    LAS ACCIONES VAN FUERA DE LA CAJA.`,
    `    El boton queda sobre una superficie del mismo color: medido,`,
    `    rgb(51,65,85) sobre rgb(51,65,85) y sin borde.`,
    `    Antes esto usaba text-[13px] y un <button> a mano.`,
    `  */}`,
    `</div>`,
  ].join("\n");

  const { conteo } = contar(jsx);
  assert.equal(conteo.color, 0, "los dos rgb de la prosa");
  assert.equal(conteo.medida, 0, "la medida nombrada en la prosa");
  assert.equal(conteo.crudo, 0, "el <button> nombrado en la prosa");
});

test("D · CERRADO EL COMENTARIO, LO QUE SIGUE EN ESA MISMA LÍNEA SÍ CUENTA", () => {
  // La mitad que evita el arreglo que se pasa de largo: si el blanqueo siguiera
  // hasta el fin de renglón, el código de después desaparecería y el contador
  // dejaría de ver hardcodeo real.
  const jsx = `{/* nota: rgb(1,2,3) */}<b className="text-[13px]">x</b>`;
  assert.deepEqual(marcados(jsx, "color"), [], "lo de adentro no");
  assert.deepEqual(marcados(jsx, "medida"), ["text-[13px]"], "lo de afuera sí");
});

test("E · DOS BLOQUES EN EL MISMO ARCHIVO, Y EL CÓDIGO DEL MEDIO", () => {
  const jsx = [
    `{/* uno:`,
    `   rgb(1,1,1) */}`,
    `<b className="text-[13px]">visible</b>`,
    `{/* dos:`,
    `   rgb(2,2,2) */}`,
    `<i className="text-[25px]">tambien</i>`,
  ].join("\n");

  assert.deepEqual(marcados(jsx, "color"), [], "ninguno de los dos bloques");
  assert.deepEqual(marcados(jsx, "medida"), ["text-[13px]", "text-[25px]"], "el código entre bloques");
});

test("F · UN TEXTO QUE PARECE COMENTARIO NO HACE DESAPARECER CÓDIGO", () => {
  // Sería peor que el defecto original: en vez de contar de más, dejaría de
  // contar. Un `{/*` entre comillas es texto, no una apertura.
  const jsx = [
    `<p>{"{/*"}</p>`,
    `<b className="text-[13px]">tiene que contarse</b>`,
  ].join("\n");
  assert.deepEqual(marcados(jsx, "medida"), ["text-[13px]"]);

  // Y lo mismo cuando la mención está en un comentario de línea.
  const conLinea = [
    `// un bloque se abre con {/* y se cierra con */}`,
    `<b className="text-[25px]">tambien</b>`,
  ].join("\n");
  assert.deepEqual(marcados(conLinea, "medida"), ["text-[25px]"]);
});

test("y los números de línea NO se mueven", () => {
  // El blanqueo conserva largo y renglones a propósito. Si borrara, cada
  // hallazgo posterior a un comentario quedaría informado en la línea
  // equivocada y la ficha mandaría a mirar donde no está.
  const jsx = [
    `<div>`,
    `  {/*`,
    `     prosa`,
    `  */}`,
    `  <b className="text-[13px]">x</b>`,
  ].join("\n");
  const [h] = contar(jsx).hallazgos.filter((x) => x.categoria === "medida");
  assert.equal(h.linea, 5);
});

test("G · EL CASO REAL: las dos rgb de ImportarPedidoDesdeArchivo dejan de contarse", () => {
  // Se lee el archivo de verdad. Es el caso que motivó todo, y un candado con
  // un fixture inventado no probaría que el archivo real quedó limpio.
  ...
  // La premisa: el literal SIGUE estando escrito en el archivo, en la prosa.
  assert.ok(texto.includes("rgb(51,65,85)"), "si ya no está, este candado no prueba nada");

  const { hallazgos } = contarArchivo(ruta, texto, OPCIONES);
  assert.deepEqual(
    hallazgos.filter((h) => h.que === "rgb(51,65,85)"),
    [],
    "la prosa del comentario volvió a contarse como color"
  );
});

// ── EL TEMPLATE LITERAL DE VARIAS LÍNEAS ───────────────────────────────────
//
// Es la familia de defecto que da VERDE, que es la peligrosa: hasta que se
// arregló, un backtick abierto en un renglón y cerrado en otro perdía el estado
// en cada `\n`. Si adentro había un `{/*`, se tomaba por comentario y se
// blanqueaba TODO el código que venía después. La cuenta bajaba sin motivo, y
// una cuenta que baja no frena a nadie.
//
// Medido antes del arreglo: este mismo caso perdía el `text-[17px]` de abajo.

const CON_LITERAL = [
  "const q = `",
  "  texto de varias líneas que menciona {/* y sigue",
  "`;",
  "export default function B(){",
  '  return <div className="text-[17px]">real</div>;',
  "}",
  "",
].join("\n");

test("un template literal multilínea con `{/*` adentro NO se lleva puesto el código de abajo", () => {
  const salida = sinComentariosJsx(CON_LITERAL);
  assert.ok(
    salida.includes("text-[17px]"),
    "el literal multilínea blanqueó código real: la cuenta baja sin motivo y nadie se entera"
  );

  // Y el contador tiene que VERLO, no solo la función de blanqueo.
  const { hallazgos } = contarArchivo("components/sunmi/B.jsx", CON_LITERAL, {});
  assert.ok(
    hallazgos.some((h) => h.que === "text-[17px]"),
    "el hardcodeo real de abajo del literal desapareció del conteo"
  );
});

test("y las comillas simples y dobles SÍ se reinician en el salto de línea", () => {
  // Un salto sin escapar adentro de `'` o `"` es un error de sintaxis, así que
  // reiniciar es lo correcto: sin eso, una comilla desbalanceada se comería el
  // resto del archivo y volveríamos al mismo defecto por otra puerta.
  const src = ['const roto = "sin cerrar', '<div className="text-[19px]"/>', ""].join("\n");
  const { hallazgos } = contarArchivo("components/sunmi/C.jsx", src, {});
  assert.ok(
    hallazgos.some((h) => h.que === "text-[19px]"),
    "una comilla desbalanceada se llevó puesto el resto del archivo"
  );
});

test("FALLA CERRADO: lo que el reconocedor no sabe interpretar, no lo cuenta callado", () => {
  assert.throws(
    () => sinComentariosJsx("{/* comentario que nunca cierra\nmás texto\n"),
    /sin cerrar/,
    "un `{/*` sin cerrar blanquearía hasta el final: tiene que tirar, no devolver menos"
  );
  assert.throws(
    () => sinComentariosJsx("const q = `literal que nunca cierra\nmás texto\n"),
    /sin cerrar/,
    "un literal sin cerrar deja al reconocedor sin saber qué es código: tiene que tirar"
  );
});
