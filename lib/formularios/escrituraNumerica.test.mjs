...
import { alEnfocarNumero, alEscribirNumero } from "@/lib/formularios/escrituraNumerica.js";
...
// ══════════════════════════════════════════════════════════════════════════
// AL ENTRAR AL CAMPO: EL CERO QUEDA SELECCIONADO Y NADA MÁS
// ══════════════════════════════════════════════════════════════════════════

/** Un campo de mentira que anota si le pidieron seleccionar. */
const campoFalso = (value, { rompe = false } = {}) => {
  const campo = { value, seleccionado: false };
  campo.select = () => {
    if (rompe) throw new DOMException("InvalidStateError");
    campo.seleccionado = true;
  };
  return campo;
};

test("un campo que muestra 0 queda seleccionado al entrar", () => {
  const campo = campoFalso("0");
  assert.equal(alEnfocarNumero({ currentTarget: campo }), true);
  assert.equal(campo.seleccionado, true);
});

test("un valor de dos dígitos NO se selecciona", () => {
  // Quien entra a corregir un 12 no quiere que la primera tecla se lo borre.
  const campo = campoFalso("12");
  assert.equal(alEnfocarNumero({ currentTarget: campo }), false);
  assert.equal(campo.seleccionado, false);
});

test("un decimal tampoco", () => {
  const campo = campoFalso("7.5");
  assert.equal(alEnfocarNumero({ currentTarget: campo }), false);
  assert.equal(campo.seleccionado, false);
});

test("UN CAMPO VACÍO NO SE TOCA, Y NO SE LE INVENTA UN 0", () => {
  // Es la comisión heredada. Seleccionar "nada" no rompe, pero lo que importa es
  // que esta función no escribe: no hay ningún camino donde un vacío se vuelva
  // un cero.
  const campo = campoFalso("");
  assert.equal(alEnfocarNumero({ currentTarget: campo }), false);
  assert.equal(campo.seleccionado, false);
  assert.equal(campo.value, "");
});

test("no escribe el valor en ningún caso", () => {
  for (const v of ["0", "12", "", "7.5"]) {
    const campo = campoFalso(v);
    alEnfocarNumero({ currentTarget: campo });
    assert.equal(campo.value, v, `el enfoque cambió el valor ${JSON.stringify(v)}`);
  }
});

test("si `select()` no está disponible, el campo no se rompe al enfocarlo", () => {
  // En este mismo elemento `selectionStart` LANZA, así que la familia es
  // quisquillosa. Un error acá saldría al enfocar un campo, que es lo primero
  // que hace la persona.
  const campo = campoFalso("0", { rompe: true });
  assert.equal(alEnfocarNumero({ currentTarget: campo }), false);
});

test("sin evento o sin campo no hace nada", () => {
  assert.equal(alEnfocarNumero(undefined), false);
  assert.equal(alEnfocarNumero({}), false);
});

test("acepta `target` cuando no hay `currentTarget`", () => {
  const campo = campoFalso("0");
  assert.equal(alEnfocarNumero({ target: campo }), true);
});
...
test("recargo y comisión pasan los dos por la misma regla, con el evento entero", () => {
  ...
  // Y los dos entran igual: el enfoque va en los dos campos o en ninguno.
  assert.equal((form.match(/onFocus=\{alEnfocarNumero\}/g) || []).length, 2);
