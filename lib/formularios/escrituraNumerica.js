...
/**
 * Al ENTRAR a un campo que muestra exactamente `0`, deja ese cero seleccionado.
 *
 * ── POR QUÉ HACE FALTA SI LA REGLA DE ARRIBA YA LO REEMPLAZA ──────────────
 *
 * Porque lo que la persona ve antes de escribir es un `0` con el cursor al
 * costado, y eso se lee como "lo que escriba se va a sumar al lado". El
 * resultado terminaba siendo el correcto, pero recién DESPUÉS de arriesgarse a
 * teclear. Con el cero resaltado, la pantalla dice de antemano lo que va a
 * pasar.
 *
 * ── MEDIDO EN EL NAVEGADOR, SOBRE ESTE MISMO CAMPO ────────────────────────
 *
 * `scripts/sonda-escritura-en-cero.mjs` lo comprobó en Chrome sobre el input
 * real: `select()` NO lanza, y lo que queda seleccionado es exactamente `"0"`
 * —leído copiando al portapapeles, porque `selectionStart` sí lanza
 * `InvalidStateError` en un `type="number"`—. Sin esa medición no se podía
 * saber: en este mismo elemento, una API de la misma familia falla.
 *
 * El `try` está por eso: la familia es conocida por ser quisquillosa y esto
 * corre en el navegador de cada local. Si algún día `select()` no está
 * disponible, el campo se comporta como antes en vez de romperse al enfocarlo.
 *
 * @param {{ currentTarget?: HTMLInputElement, target?: HTMLInputElement }} evento
 * @returns {boolean} `true` si seleccionó el cero.
 */
export function alEnfocarNumero(evento) {
  const campo = evento?.currentTarget || evento?.target;
  if (!campo || String(campo.value) !== "0") return false;
