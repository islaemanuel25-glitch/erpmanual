...
test("LA RUTA MANDA LOS CANDIDATOS Y VUELVE A FILTRAR CONTRA ELLOS", () => {
  // Dos defensas para lo mismo, a propósito: los candidatos van EN el pedido
  // —así el modelo no tiene de dónde sacar uno que no esté— y lo que vuelve se
  // cruza igual contra esa lista. Lo que se ordena termina en un vínculo que
  // escribe costos maestros, y ahí una sola defensa no alcanza.
  const ruta = readFileSync(
    new URL("../../../app/api/compras-proveedor/importar/ordenar-candidatos/route.js", import.meta.url),
    "utf8"
  );
  assert.match(ruta, /ordenFiltrado\(\{ candidatosDelSistema: ids, ordenPropuesto: propuesto \}\)/);
  assert.match(ruta, /propuestaUtilizable/);
  // Y no escribe: el alias se aprende recién cuando una persona confirma.
  const sinComentarios = ruta.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/[^\n]*/g, "");
  for (const prohibido of ["prisma", "persistirIdentidad", "$transaction"]) {
    assert.ok(!sinComentarios.includes(prohibido), `la ruta de orden nombra "${prohibido}"`);
  }
  // La ruta no busca en el catálogo: si buscara, un id inventado se convertiría
  // en una búsqueda, y una búsqueda siempre devuelve algo.
  assert.ok(!sinComentarios.includes("findMany"));
  assert.ok(!sinComentarios.includes("buscarCandidatos"));
});
