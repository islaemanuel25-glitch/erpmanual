...
export function reversionDeProducto(producto) {
  ...
  if (filas.length === 0) {
    return {
      productoBaseId: producto?.id ?? null,
      nombre: producto?.nombre ?? null,
      revierte: false,
      motivo: MOTIVO_OMISION.SIN_APLICACION,
    };
  }
  ...
  if (costoDestino === null) {
    return {
      ...
      nombre: producto.nombre ?? null,
      revierte: false,
      motivo: MOTIVO_OMISION.SIN_DATO_PREVIO,
    };
  }
  ...
  if (!iguales(producto.precioCostoActual, ultima.costoAplicado)) {
    // El nombre viaja también en los omitidos: son los que la persona tiene que
    // mirar, y una lista de números de producto no se puede mirar.
    return {
      productoBaseId: producto.id,
      nombre: producto.nombre ?? null,
      revierte: false,
      motivo: MOTIVO_OMISION.COSTO_CAMBIADO,
    };
  }
