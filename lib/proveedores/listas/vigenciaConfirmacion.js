...
/**
 * ¿CUÁL DE LAS LECTURAS ES LA QUE SE CONFIRMÓ?
 *
 * La fila guarda el multiplicador elegido, no la clave de la tarjeta. Traducir
 * uno en la otra es lo que permite abrir una fila resuelta mostrando qué se
 * decidió, en vez de mostrarla en blanco como si nadie hubiera decidido nada
 * —que es lo que hacía creer que la pantalla estaba rota—.
 *
 * Devuelve null cuando la confirmación venció (se revinculó después) o cuando el
 * multiplicador guardado ya no corresponde a ninguna lectura posible: eso pasa
 * de verdad, porque corregir el `factor_pack` del producto cambia qué lecturas
 * existen. Marcar en ese caso una tarjeta cualquiera sería peor que no marcar
 * ninguna.
 */
export function claveDeLaConfirmacion(fila, evaluadas = []) {
  if (!multiplicadorConfirmadoUsable(fila)) return null;
  const m = Number(fila.multiplicadorConfirmado);
  const elegida = evaluadas.find((h) => Number(h?.multiplicador) === m);
  return elegida?.clave ?? null;
}
