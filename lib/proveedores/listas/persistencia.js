...
/** Qué se hará con el precio de venta al aplicar. Todavía no se usa. */
 ...
 * sabe que no sirve.
 */
export function validarArchivo({ nombre, tamano, mime, extensionesPermitidas = LIMITES.extensiones } = {}) {
  ...
  if (bytes > LIMITES.tamanoMaxBytes) {
    return {
      ok: false,
      codigo: ERROR_UPLOAD.DEMASIADO_GRANDE,
      error: `El archivo supera el límite de ${Math.round(LIMITES.tamanoMaxBytes / 1024 / 1024)} MB.`,
    };
  }
  ...
  if (m && !LIMITES.mimes.includes(m)) {
    return {
      ok: false,
      codigo: ERROR_UPLOAD.MIME_INVALIDO,
      error: "El tipo de archivo no corresponde a una planilla.",
    };
  }
