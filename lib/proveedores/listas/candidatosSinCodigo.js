...
/**
 * PISO para que una fila entre siquiera en la lista.
 *
 * El precio confirma, no identifica. Que un costo caiga en la banda del 10 al
 * 20 % lo cumplen muchísimas de las 626 filas sin machear, así que el precio
 * SOLO no alcanza para mostrar: medido sobre los 59 productos sin código, de 287
 * tarjetas 276 entraban únicamente por precio, y 56 de los 59 llegaban al tope
 * de 5. Eran cinco tarjetas de ruido por producto.
 *
 * Con un piso bajo de parecido, la señal de precio sigue haciendo lo suyo
 * —distinguir entre los que se parecen— pero deja de arrastrar filas que no
 * tienen nada que ver con el producto.
 *
 * El valor salió de medir, no de elegirlo: es el más alto que conserva las tres
 * sugerencias buenas que ya salían y el más bajo que deja las tarjetas de
 * solo-precio en cero. Ver la nota del commit.
 */
export const PISO_PARECIDO = 0.3;
...
export function candidatosParaProducto({ producto, filas = [], recargoPct, rango } = {}) {
  ...
  for (const fila of filas) {
    ...
    // El piso va ANTES de calcular nada: sin un mínimo de parecido la fila no
    // entra, aunque el precio cierre. Además ahorra correr `analizarFila` sobre
    // las 626 por cada uno de los 59 productos.
    if (parecido < PISO_PARECIDO) continue;

    const analisis = analizarFila({ fila, base: producto, recargoPct, rango });
    const coherente = precioCoherente(analisis);

    // Pasado el piso, hace falta al menos una de las dos señales. Un parecido
    // apenas por encima del piso, con el precio incoherente, no dice nada.
