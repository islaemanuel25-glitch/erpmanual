      ...
      for (const fila of [false, true])
        for (const codigo of [false, true]) {
          const g = grupoDeProducto({
            tieneFilaAplicada: aplicada, tieneFilaEnCola: cola, tieneFila: fila, tieneCodigoActivo: codigo,
          });
          combinaciones++;
          assert.ok(validos.has(g), `combinación sin grupo: ${JSON.stringify({ aplicada, cola, fila, codigo })}`);
        }
  assert.equal(combinaciones, 16);
});
...
function evaluar(where, p) {
  return Object.entries(where).every(([clave, cond]) => {
    ...
    if (clave === "codigosProveedor") {
      if (cond.some) return p.tieneCodigoActivo;
    ...
    for (const tieneFilaEnCola of [false, true])
      for (const tieneFila of [false, true])
        for (const tieneCodigoActivo of [false, true]) {
          ...
          const p = { tieneFilaAplicada, tieneFilaEnCola, tieneFila, tieneCodigoActivo };
