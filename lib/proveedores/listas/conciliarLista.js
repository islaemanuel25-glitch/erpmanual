 ...
 *   3. código de barras — que NO machea, solo sugiere;
 *   4. nombre — no se implementa. Genera falsos positivos que nadie revisa.
 *
 * Cualquiera de los dos primeros caminos que dé más de un candidato devuelve
 * AMBIGUA con todos los candidatos. Elegir uno sería adivinar a qué producto se
 * le cambia el costo.
 */
export function macheePorCodigo(indice, fila) {
  ...
  const exacto = indice.exacto.get(claves.normalizado) ?? [];
  if (exacto.length === 1) {
...
export function sugerirPorCodigoBarra(indiceBarra, fila) {
  ...
  const candidatos = indiceBarra.get(normalizado) ?? [];
...
export function conciliarFila({ fila, indice, indiceBarra, productosPorId, contexto, config }) {
  ...
  if (machee.tipo === TIPO_COINCIDENCIA.NINGUNA) {
    // Sin vínculo interno, el código de barras SUGIERE pero no machea: la fila
    // sigue NO_MACHEADA hasta que alguien confirme.
    const sugerencia = sugerirPorCodigoBarra(indiceBarra, fila);
    return {
      ...salida,
      sugerencia,
  ...
  const vinculo = machee.candidatos[0];
  const producto = productosPorId.get(vinculo.productoBaseId) ?? null;
  if (!producto) {
    return {
      ...salida,
      estado: ESTADO_LINEA.NO_MACHEADO,
...
export function conciliarLista({
  ...
  return {
    ...
    resumen: {
      ...
      porEstado,
      sugerenciasCodigoBarras: resultado.filter((r) => r.sugerencia).length,
