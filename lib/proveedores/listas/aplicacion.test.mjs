...
//
// Lo que la persona confirma es QUÉ ES el producto respecto de lo que cotiza el
// proveedor, no un multiplicador. La cantidad contenida solo multiplica cuando
// el archivo demuestra que el precio es por unidad suelta.

// Fila "DI" confirmada como la misma presentación: el display ya es el pack.
function filaDisplayConfirmada(extra = {}) {
  return {
    ...
    baseConfirmada: "MISMA_PRESENTACION", unidadesConfirmadas: 12,
...
test("DI sin confirmar no es aplicable: el resolvedor no sabe qué es un display", () => {
  const fila = filaDisplayConfirmada({ baseConfirmada: null });
  assert.equal(revalidarFila({ fila, base: producto(), contexto: CONTEXTO, config: CONFIG_ARCOR, recargoPct: RECARGO }).aplicable, false);
});

test("un display confirmado NO se multiplica por la cantidad que trae", () => {
...
test("cambiar la cantidad contenida no mueve el costo del display", () => {
  const a = revalidarFila({ fila: filaDisplayConfirmada({ unidadesConfirmadas: 12 }), base: producto(), contexto: CONTEXTO, config: CONFIG_ARCOR, recargoPct: RECARGO });
  const b = revalidarFila({ fila: filaDisplayConfirmada({ unidadesConfirmadas: 18 }), base: producto(), contexto: CONTEXTO, config: CONFIG_ARCOR, recargoPct: RECARGO });
  assert.equal(a.costoNuevo, b.costoNuevo, "la cantidad es un dato, no un factor");
});

test("con precio por unidad y relación por unidad suelta sí se multiplica", () => {
  // 100 × 1,05 × 12 = 1260, y el archivo dice UN, que es lo que lo demuestra.
  const fila = filaDisplayConfirmada({
    unidadProveedor: "UN", precioConIva: 100,
    baseConfirmada: "POR_UNIDAD_SUELTA", unidadesConfirmadas: 12,
  });
  const r = revalidarFila({ fila, base: producto(), contexto: CONTEXTO, config: CONFIG_ARCOR, recargoPct: RECARGO });
  assert.equal(r.aplicable, true);
  assert.equal(r.costoNuevo, 1260);
...
test("pedir multiplicar sobre un display no da costo por ningún camino", () => {
  const fila = filaDisplayConfirmada({ baseConfirmada: "POR_UNIDAD_SUELTA", unidadesConfirmadas: 12 });
  // 1200 × 1,05 × 12 = 15120, que no coincide con la propuesta guardada: se cae.
  const r = revalidarFila({ fila, base: producto(), contexto: CONTEXTO, config: CONFIG_ARCOR, recargoPct: RECARGO });
  assert.equal(r.aplicable, false);
});

test("confirmar la presentación NO saltea el resto de las revalidaciones", () => {
...
test("una cantidad inválida no habilita la multiplicación", () => {
  for (const u of [0, -1, 1.5, "doce", 99999, null]) {
    const fila = filaDisplayConfirmada({
      unidadProveedor: "UN", precioConIva: 100,
      baseConfirmada: "POR_UNIDAD_SUELTA", unidadesConfirmadas: u,
    });
    assert.equal(revalidarFila({ fila, base: producto(), contexto: CONTEXTO, config: CONFIG_ARCOR, recargoPct: RECARGO }).aplicable, false, String(u));
  }
});
