...
// ── Armado confirmado a mano ────────────────────────────────────────────────
//
// El caso real de la lista de Arcor: el proveedor cotiza por display ("DI"), el
// resolvedor no sabe cuántas unidades es un display y la fila queda trabada. Con
// el armado confirmado, la aplicación tiene que poder seguir.

// Fila "DI" confirmada sin conversión: el precio del display YA es el del bulto.
function filaConfirmada(extra = {}) {
  return {
    id: 2,
    estado: "LISTO_PARA_ACTUALIZAR",
    aplicada: false,
...
test("DI sin confirmar no es aplicable: el resolvedor no sabe qué es un display", () => {
  const fila = filaConfirmada({ factorConfirmado: null });
  const r = revalidarFila({ fila, base: producto(), contexto: CONTEXTO, config: CONFIG_ARCOR, recargoPct: RECARGO });
  assert.equal(r.aplicable, false);
});

test("DI con armado confirmado sí es aplicable", () => {
  // 1200 × 1,05 = 1260, sin multiplicar: el display ya es el bulto.
  const r = revalidarFila({ fila: filaConfirmada(), base: producto(), contexto: CONTEXTO, config: CONFIG_ARCOR, recargoPct: RECARGO });
  assert.equal(r.aplicable, true);
  assert.equal(r.costoNuevo, 1260);
});

test("el armado confirmado multiplica cuando el proveedor cotiza la unidad", () => {
  // 100 × 1,05 × 12 = 1260, con el factor que eligió la persona.
  const fila = filaConfirmada({ precioConIva: 100, factorConfirmado: 12 });
  const r = revalidarFila({ fila, base: producto(), contexto: CONTEXTO, config: CONFIG_ARCOR, recargoPct: RECARGO });
  assert.equal(r.aplicable, true);
  assert.equal(r.costoNuevo, 1260);
});

test("confirmar el armado NO saltea el resto de las revalidaciones", () => {
  // El producto dejó de pertenecer a esta ubicación.
  const ajeno = producto({ creadoEnLocalId: 99 });
  assert.equal(
    revalidarFila({ fila: filaConfirmada(), base: ajeno, contexto: CONTEXTO, config: CONFIG_ARCOR, recargoPct: RECARGO }).aplicable,
    false
  );
  // El costo del producto se movió desde que se confirmó.
  const movido = producto({ precio_costo: 1111 });
  assert.equal(
    revalidarFila({ fila: filaConfirmada(), base: movido, contexto: CONTEXTO, config: CONFIG_ARCOR, recargoPct: RECARGO }).aplicable,
    false
  );
  // El producto ya no existe.
  assert.equal(
    revalidarFila({ fila: filaConfirmada(), base: null, contexto: CONTEXTO, config: CONFIG_ARCOR, recargoPct: RECARGO }).aplicable,
    false
  );
  // La ficha cambió de armado entre confirmar y aplicar.
  const otroFactor = producto({ factor_pack: 24 });
  assert.equal(
    revalidarFila({ fila: filaConfirmada(), base: otroFactor, contexto: CONTEXTO, config: CONFIG_ARCOR, recargoPct: RECARGO }).aplicable,
    false
  );
});

test("un factor confirmado inválido no habilita nada", () => {
  for (const f of [0, -1, 1.5, "doce", 99999]) {
    const r = revalidarFila({
      fila: filaConfirmada({ factorConfirmado: f }), base: producto(),
      contexto: CONTEXTO, config: CONFIG_ARCOR, recargoPct: RECARGO,
    });
    assert.equal(r.aplicable, false, String(f));
  }
});
