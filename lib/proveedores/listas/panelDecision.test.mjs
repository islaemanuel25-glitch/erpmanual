...
// ── El `where` de la cola dice lo mismo que el predicado ────────────────────
//
// Son dos implementaciones de la misma regla —una en JS y otra que va a SQL— y no
// hay forma de evitarlo: Postgres no puede llamar a `formaDelPanel`. Lo que sí se
// puede es que no se separen en silencio. Este candado las compara sobre todas
// las combinaciones que importan.
//
// El intérprete de abajo entiende solo los operadores que el filtro usa. Si
// alguien agrega uno, explota con "operador desconocido" en vez de dar un
// resultado inventado: es preferible que el candado se caiga a que mienta.

/** La referencia de campo de Prisma, simulada. */
const REF_VINCULADO = { __campo: "vinculadoEn" };
...
/** Evalúa un `where` de Prisma contra una fila en memoria. */
function evaluar(where, fila) {
  return Object.entries(where).every(([clave, cond]) => {
    if (clave === "OR") return cond.some((c) => evaluar(c, fila));
    if (clave === "AND") return cond.every((c) => evaluar(c, fila));
    if (clave === "NOT") return !evaluar(cond, fila);

    const actual = valorDe(fila, clave);
    if (cond === null) return actual === null;
    if (typeof cond !== "object" || cond instanceof Date) return actual === cond;

    return Object.entries(cond).every(([op, arg]) => {
      if (op === "in") return actual !== null && arg.includes(actual);
      if (op === "not") return arg === null ? actual !== null : actual !== arg;
      if (op === "lte") {
        // Semántica de SQL: cualquier comparación con NULL no es verdadera.
        const izq = comparable(actual);
        const der = comparable(arg?.__campo ? valorDe(fila, arg.__campo) : arg);
        if (izq === null || der === null) return false;
        return izq <= der;
      }
      throw new Error(`operador desconocido en el filtro de la cola: ${op}`);
    });
  });
}

test("el where de la cola dice lo mismo que esDeLaCola, en todas las combinaciones", () => {
  const filtro = filtroDeLaCola(REF_VINCULADO);

  const estados = [...Object.values(ESTADO_LINEA), null];
  const veredictos = ["RECOMENDADA", "AMBIGUA", "REVISAR", null];
  // Las cuatro situaciones de la vigencia, incluido el empate exacto.
  const fechas = [
    { confirmadoEn: null, vinculadoEn: null },
    { confirmadoEn: T(DESPUES), vinculadoEn: null },
    { confirmadoEn: T(DESPUES), vinculadoEn: T(ANTES) },
    { confirmadoEn: T(ANTES), vinculadoEn: T(DESPUES) },
    { confirmadoEn: T(ANTES), vinculadoEn: T(ANTES) },
  ];

  let combinaciones = 0;
  for (const estado of estados) {
    for (const resultadoInterpretacion of veredictos) {
      for (const f of fechas) {
        for (const aplicada of [false, true]) {
          for (const excluidaManual of [false, true]) {
            for (const productoBaseId of [null, 10]) {
              const fila = { estado, resultadoInterpretacion, aplicada, excluidaManual, productoBaseId, ...f };
              combinaciones++;
              assert.equal(
                evaluar(filtro, fila),
                esDeLaCola(fila),
                `discrepan en ${JSON.stringify({ estado, resultadoInterpretacion, aplicada, excluidaManual, productoBaseId, conf: f.confirmadoEn, vinc: f.vinculadoEn })}`
              );
            }
          }
        }
  ...
  assert.ok(combinaciones > 500, `se probaron pocas combinaciones: ${combinaciones}`);
});
...
// ── LA TERCERA FORMA: un producto sin fila del archivo ──────────────────────
//
// Lo que se protege sobre todo es que NO se toquen las dos formas que están en
// producción: esto es una función aparte, con otra entrada —un producto, no una
// fila— y el candado de abajo lo fija.

test("la tercera forma no toca las dos que ya andan", () => {
  // Si alguien fusionara las funciones, `formaDelPanel` empezaría a entender de
  // productos para nada y este candado se pondría rojo.
  const fila = formaDelPanel({ estado: ESTADO_LINEA.FACTOR_DUDOSO, tieneProducto: true });
  assert.equal(fila.pregunta, PREGUNTA.INTERPRETACION);
  assert.equal(fila.origen, undefined, "la forma de fila no habla de origen");

  const producto = formaDelPanelDeProducto({ grupo: "NO_TRAIDO" });
  assert.equal(producto.origen, ORIGEN_PANEL.SISTEMA);
});

test("sin código y con candidatos: se elige una FILA del archivo", () => {
  const f = formaDelPanelDeProducto({ grupo: "SIN_CODIGO", candidatos: 3 });
  assert.equal(f.pregunta, PREGUNTA.FILA);
  assert.equal(f.origen, ORIGEN_PANEL.SISTEMA);
});

test("sin código y SIN candidatos: se muestra, pero no hay nada que elegir", () => {
  // Esconder el producto sería peor: es el que sigue con el precio viejo y nadie
  // se enteraría de que ninguna fila se le parece.
  const f = formaDelPanelDeProducto({ grupo: "SIN_CODIGO", candidatos: 0 });
  assert.equal(f.pregunta, null);
  assert.equal(f.origen, ORIGEN_PANEL.SISTEMA);
});

test("la lista no lo trajo: se elige un destino, y los tres existen siempre", () => {
  const f = formaDelPanelDeProducto({ grupo: "NO_TRAIDO" });
  assert.equal(f.pregunta, PREGUNTA.DESTINO);
  assert.deepEqual(Object.keys(DESTINO).sort(), ["DEJAR_COMO_ESTA", "DISCONTINUADO", "REVISAR_CODIGO"]);
  for (const d of Object.values(DESTINO)) {
    assert.ok(TEXTO_DESTINO[d]?.length > 3, d + " sin texto");
    assert.ok(DETALLE_DESTINO[d]?.length > 20, d + " sin explicación");
  }
});

test("un grupo que SÍ tiene fila no le corresponde a la forma de producto", () => {
  // Su panel es el de siempre. Devolver null es decir "esto no me toca".
  for (const g of ["ACTUALIZADO", "NECESITA_DECISION", "LISTO_PARA_APLICAR"]) {
    const f = formaDelPanelDeProducto({ grupo: g });
    assert.equal(f.pregunta, null, g);
    assert.equal(f.origen, ORIGEN_PANEL.COMPARACION, g);
  }
});

test("ningún destino toca un costo", () => {
  // Los tres son decisiones sobre el producto, no sobre su precio. Si algún día
  // uno escribiera un costo, tendría que pasar por el endpoint de aplicar.
  for (const d of Object.values(DESTINO)) {
    assert.ok(!/costo|precio/i.test(TEXTO_DESTINO[d]), d + " promete tocar el precio");
  }
});
