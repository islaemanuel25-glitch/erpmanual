...
// ── CAMBIAR LA LECTURA DE UNA FILA YA RESUELTA ─────────────────────────────
//
// Confirmar dejaba la fila en LISTO_PARA_ACTUALIZAR, y ese mismo éxito la volvía
// intocable: la guarda solo aceptaba FACTOR_DUDOSO. La única forma de corregir
// una lectura equivocada era no haberla confirmado nunca.

const resuelta = (extra = {}) => ({
  id: 9, estado: "LISTO_PARA_ACTUALIZAR", aplicada: false, excluidaManual: false,
  productoBaseId: 40, unidadProveedor: "DI", unidadesPorBulto: 16,
  descripcionProveedor: "TRAVIATA 3x108g", precioConIva: 1733.333349,
  confirmadoEn: "2026-08-09T03:00:00Z", vinculadoEn: null, multiplicadorConfirmado: 16,
  ...extra,
});

test("una fila confirmada y NO aplicada se puede volver a confirmar", () => {
  const r = puedeConfirmarse(resuelta(), ABIERTA);
  assert.equal(r.ok, true);
});

test("APLICADA sigue siendo intocable", () => {
  const r = puedeConfirmarse(resuelta({ aplicada: true }), ABIERTA);
  assert.equal(r.ok, false);
  assert.equal(r.motivo, MOTIVO_NO_CONFIRMABLE.FILA_APLICADA);
});

test("una fila resuelta SIN confirmación de persona no se toca", () => {
  // La resolvió el motor, no alguien. No hay decisión que cambiar: si el motor
  // se equivocó, lo que hay que arreglar es el dato de origen.
  const r = puedeConfirmarse(resuelta({ confirmadoEn: null, multiplicadorConfirmado: null }), ABIERTA);
  assert.equal(r.ok, false);
  assert.equal(r.motivo, MOTIVO_NO_CONFIRMABLE.ESTADO_NO_CONFIRMABLE);
});

test("con la confirmación vencida vuelve a entrar por la puerta de siempre", () => {
  // Revinculada después de confirmar: la fila ya volvió sola a la cola, así que
  // su estado real es FACTOR_DUDOSO y no necesita esta puerta.
  const r = puedeConfirmarse(
    resuelta({ vinculadoEn: "2026-08-09T04:00:00Z" }),
    ABIERTA
  );
  assert.equal(r.ok, false);
  assert.equal(r.motivo, MOTIVO_NO_CONFIRMABLE.ESTADO_NO_CONFIRMABLE);
...
test("una excluida no se corrige aunque esté confirmada", () => {
  const r = puedeConfirmarse(resuelta({ excluidaManual: true }), ABIERTA);
  assert.equal(r.ok, false);
  assert.equal(r.motivo, MOTIVO_NO_CONFIRMABLE.EXCLUIDA);
});

test("con la importación cerrada no se cambia ninguna lectura", () => {
  const r = puedeConfirmarse(resuelta(), { estado: "APLICADA" });
  assert.equal(r.ok, false);
  assert.equal(r.motivo, MOTIVO_NO_CONFIRMABLE.IMPORTACION_CERRADA);
});

test("cambiar la lectura devuelve el multiplicador nuevo, no el guardado", () => {
  const r = resultadoConfirmacion({
    fila: resuelta(), base: cajaTraviata(), clave: "PACK_16",
    cantidadPresentacion: null, recargoPct: R, rango: RANGO,
  });
  assert.equal(r.ok, true);
  assert.equal(r.multiplicador, 16);
