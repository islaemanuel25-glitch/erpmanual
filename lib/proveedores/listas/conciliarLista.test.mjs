...
test("62. dos lecturas creíbles: AMBIGUA, y el motor no elige", () => {
  // El precio es por unidad y el producto agrupa 2: $1.050 y $2.100 son los dos
  // posibles contra un costo de $900. Ninguno es absurdo, así que decide una
  // persona.
  const r = una({
    fila: linea({ unidadProveedor: "UN", unidadesPorBulto: 2 }),
    productos: [prod({ unidadMedida: "pack", factorPack: 2, precioCostoActual: 900 })],
  });
  assert.equal(r.estado, ESTADO_LINEA.LISTO_PARA_ACTUALIZAR, "el estado sigue siendo aplicable");
  assert.equal(r.resultadoInterpretacion, "AMBIGUA", "pero nadie eligió la lectura");
});
...
test("66. el rango entra por contexto, y una cabecera rota no rompe el motor", () => {
  // Hoy el veredicto NO depende del rango: lo que descarta una lectura es que
  // cambie el ORDEN DE MAGNITUD del costo, y eso es un tope fijo. El rango viaja
  // igual porque es el parámetro de `recomendarHipotesis` y porque de él salen
  // los estados de variación que se muestran. Si algún día la recomendación pasa
  // a mirarlo, este candado se pone rojo y hay que decidir qué rango manda —el
  // congelado de la fila o el de la cabecera—, que es justo lo que resuelve
  // `rangoDeLaFila`.
  const conRango = { ...CTX, cabecera: { aumentoEsperadoMinPct: 90, aumentoEsperadoMaxPct: 95 } };
  const apretado = una({ productos: [prod({ precioCostoActual: 1000 })], contexto: conRango });
  const sinCabecera = una({ productos: [prod({ precioCostoActual: 1000 })] });
  assert.equal(apretado.resultadoInterpretacion, "RECOMENDADA");
  assert.equal(sinCabecera.resultadoInterpretacion, apretado.resultadoInterpretacion);

  // Sin cabecera y con una cabecera a medias tiene que caer al default del
  // sistema sin explotar: es el camino real de toda importación de hoy.
  const aMedias = una({
    productos: [prod({ precioCostoActual: 1000 })],
    contexto: { ...CTX, cabecera: { aumentoEsperadoMinPct: 10, aumentoEsperadoMaxPct: null } },
  });
  assert.equal(aMedias.resultadoInterpretacion, "RECOMENDADA");
});
