...
test("9. sugerencia única por código de barras", () => {
  ...
  assert.equal(r.estado, ESTADO_LINEA.NO_MACHEADO);
  assert.ok(r.sugerencia);
  assert.equal(r.sugerencia.motivo, MOTIVO_CONCILIACION.SUGERENCIA_CODIGO_BARRAS);
  assert.equal(r.sugerencia.producto.productoBaseId, 500);
  assert.equal(r.sugerencia.confirmada, false);
});

test("10. el código de barras NO machea automáticamente ni deja aplicar", () => {
  const productos = [prod({ productoBaseId: 500, codigosBarra: ["7793360103018"] })];
  const r = una({
    fila: linea({ codigoNormalizado: "SINVINCULO", codigoBarraProveedor: "7793360103018" }),
    productos, codigos: [],
  });
  assert.equal(r.producto, null, "la sugerencia no puede convertirse en macheo");
  assert.equal(r.seleccionable, false);
  assert.equal(r.seleccionadaPorDefecto, false);
  assert.equal(r.costoMaestroPropuesto, null);
});

test("11. código de barras ambiguo: no sugiere nada", () => {
  const productos = [
  ...
  assert.equal(r.sugerencia, null);
  assert.equal(r.estado, ESTADO_LINEA.NO_MACHEADO);
});

test("12. no existe macheo por nombre", () => {
  const productos = [prod({ productoBaseId: 500, nombre: "KETCHUP" })];
  const r = una({
    fila: linea({ codigoNormalizado: "SINVINCULO", descripcionProveedor: "KETCHUP" }),
    productos, codigos: [],
  });
  assert.equal(r.producto, null);
  assert.equal(r.sugerencia, null);
...
test("51. el resumen cuenta sugerencias, variaciones altas y selección", () => {
  ...
  assert.equal(r.resumen.sugerenciasCodigoBarras, 1);
  assert.equal(r.resumen.variacionAlta, 1); // 500 → 1050
  assert.equal(r.resumen.seleccionables, 1);
  assert.equal(r.resumen.seleccionadas, 1);
});
