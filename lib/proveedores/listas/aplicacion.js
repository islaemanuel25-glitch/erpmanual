...
import { basePrecioDeFila } from "./confirmarPresentacion.js";
...
export function revalidarFila({ fila, base, contexto = {}, config = {}, recargoPct } = {}) {
  ...
  const confirmada = Number.isInteger(mult) && mult >= 1;
  // Multiplicar solo se admite cuando el archivo lo demuestra: precio por unidad
  // y producto que agrupa. Con display o bulto, cualquier multiplicador que no
  // sea 1 es una confirmación inválida.
  const puedeMultiplicar =
    basePrecioDeFila(fila) === "UNIDAD" && requiereConversionABulto(base);
