...
import { costoDeRelacion, basePrecioDeFila } from "./confirmarPresentacion.js";
...
export const TEXTO_OMISION = {
  NO_SELECCIONADA: "No estaba seleccionada.",
  ESTADO_NO_APLICABLE: "La fila ya no está lista para actualizar.",
  YA_APLICADA: "Ya se había aplicado.",
  PRODUCTO_INEXISTENTE: "El producto ya no existe.",
  PRODUCTO_ES_COMBO: "Es un combo: no tiene costo propio que actualizar.",
  PROPIEDAD_PERDIDA: "Esta ubicación ya no puede editar el costo de este producto.",
  CONFIGURACION_CAMBIO:
    "Cambió la presentación del producto (unidad, factor o modo de compra) desde la conciliación.",
  COSTO_CAMBIO_DESDE_CONCILIACION:
    "El costo del producto cambió desde la conciliación: la propuesta quedó vieja.",
  PROPUESTA_DIFERENTE:
    "Recalculado con los datos actuales, el costo propuesto ya no coincide.",
  UNIDAD_INCOMPATIBLE: "El producto se mide por kilo y la lista informa por unidad.",
  FACTOR_FALTANTE: "El producto no tiene un factor de bulto válido.",
  PRECIO_NO_CREIBLE: "El precio de la lista es cero o demasiado bajo para ser real.",
  ERROR_DE_CALCULO: "No se pudo calcular el costo con los datos actuales.",
  SIN_CAMBIO: "El costo ya era el propuesto.",
};
...
export function revalidarFila({ fila, base, contexto = {}, config = {}, recargoPct } = {}) {
  ...
  // ── Presentación confirmada a mano ────────────────────────────────────────
  //
  ...
  //
  // La cantidad contenida (`unidadesConfirmadas`) solo entra en la cuenta con la
  // relación POR_UNIDAD_SUELTA, que es la única que el archivo demuestra al
  // decir que el precio es por unidad. Con display o bulto no multiplica nada.
  //
  ...
  // corriendo igual.
  const costoConfirmado = fila.baseConfirmada
    ? costoDeRelacion({
        precioConIva: precioLista,
        recargoPct,
        relacion: fila.baseConfirmada,
        unidades: fila.unidadesConfirmadas,
      })
    : null;

  // Una fila confirmada FALLA CERRADA. Si lo confirmado no da un costo —cantidad
  // inválida, base que ya no se puede leer, relación que no corresponde— la fila
  // no se aplica. Dejarla caer al resolvedor sería peor que no confirmar nada:
  // aplicaría un costo que NO es el que la persona aprobó.
  if (fila.baseConfirmada && (costoConfirmado === null || basePrecioDeFila(fila) === null)) {
    return no(MOTIVO_OMISION.ERROR_DE_CALCULO);
  }
