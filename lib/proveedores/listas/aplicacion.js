...
import { factorConfirmadoValido, costoConFactor } from "./confirmarArmado.js";
...
export function revalidarFila({ fila, base, contexto = {}, config = {}, recargoPct } = {}) {
  ...
  //
  // Cuando una persona confirmó el armado, ESA es la respuesta. Volver a
  // preguntarle al resolvedor devolvería el mismo "no puedo deducirlo" que dejó
  // la fila trabada, y la confirmación no serviría de nada.
  //
  ...
  // corriendo igual. Lo único que aporta la confirmación es el factor.
  const confirmado = fila.factorConfirmado;
  const costoConfirmado = factorConfirmadoValido(confirmado)
    ? costoConFactor({ precioConIva: precioLista, recargoPct, factor: confirmado, base })
    : null;
