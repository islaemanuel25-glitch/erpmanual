...
//   · FIXTURE REAL — los Excel de Arcor de verdad. NO viven en el repositorio: son
//     archivos de un proveedor, con sus precios y sus nombres, y este repositorio
//     es público. Sin ellos, esas pruebas se SALTEAN con un aviso; no se dan por
//     buenas en silencio.
//
// NO HAY NADA QUE CONFIGURAR PARA CORRERLAS. Salen solos de la carpeta hermana
// `erpazul-fixtures-dev`, al lado del repo, y el resolutor está en
// `scripts/test/fixturesExternos.mjs`.
//
// Hasta el 2026-08-17 la ruta se pasaba por las variables ARCOR_FIXTURE y
// ARCOR_FIXTURE_B, y por eso estos 21 candados NO CORRIERON NUNCA: había que
// acordarse de exportarlas antes de `npm test`, y nadie se acordaba. Un candado
// que depende de que alguien se acuerde es un candado apagado la mayor parte del
// tiempo. Volver a leerlos de una variable pone rojo a
// `scripts/test/fixtureConfiguradoNoSeSaltea.test.mjs`, que vigila justamente eso.

import { test } from "node:test";
import assert from "node:assert/strict";
import fs from "node:fs";

import { rutaFixture, motivoFaltante } from "../../../../scripts/test/fixturesExternos.mjs";
...
// LOS DOS EXCEL REALES SALEN DE LA CARPETA HERMANA, no de una variable.
//
// Vivían detrás de `ARCOR_FIXTURE` y `ARCOR_FIXTURE_B` hasta el 2026-08-17, y por
// eso no corrían nunca: había que acordarse de exportarlas antes de `npm test`.
// Ahora se resuelven solos si la carpeta `erpazul-fixtures-dev` está al lado del
// repo. Sin carpeta, se saltean igual que antes y con el motivo a la vista.
//
// El detalle de cómo se elige cada archivo —y por qué NO se usa el parser para
// clasificarlos, que sería circular— está en `scripts/test/fixturesExternos.mjs`.
const RUTA_FIXTURE = rutaFixture("A") ?? "";
const HAY_FIXTURE = RUTA_FIXTURE !== "" && fs.existsSync(RUTA_FIXTURE);
const saltar = HAY_FIXTURE ? false : motivoFaltante("A");

// Segundo fixture real: la exportación con códigos de barras (formato B).
const RUTA_FIXTURE_B = rutaFixture("B") ?? "";
const HAY_FIXTURE_B = RUTA_FIXTURE_B !== "" && fs.existsSync(RUTA_FIXTURE_B);
const saltarB = HAY_FIXTURE_B ? false : motivoFaltante("B");
