...
export const PREGUNTA = {
  ...
  PRODUCTO: "PRODUCTO",
  /** Tarjetas = FILAS del archivo candidatas. La tercera forma. */
  FILA: "FILA",
  /** Tarjetas = qué hacer con un producto que la lista no trajo. */
  DESTINO: "DESTINO",
};

/**
 * Contra qué se compara arriba del panel.
 *
 * Las dos primeras formas comparan el archivo contra el ERP. La tercera no puede:
 * el producto no tiene fila, así que arriba va lo que hay en el sistema y nada
 * más. Es un dato de la forma, no del contenido, y por eso viaja al lado de la
 * pregunta.
 */
export const ORIGEN_PANEL = {
...
//
// Las dos primeras formas viven de una comparación: el archivo dice esto, el ERP
// dice aquello. Un producto que la lista no trajo no tiene contra qué comparar,
// así que arriba va lo que hay en el sistema —producto, sku, código de barras,
// costo actual y cuándo se tocó por última vez— y abajo las tarjetas de siempre.
//
// Lo que cambia es QUÉ se elige, que es lo único que cambia entre formas:
//
//   SIN_CODIGO  las tarjetas son FILAS del archivo que no machearon con nada.
//               Elegir una es decir "este producto es esta fila", y de ahí sale
//               el vínculo que va a faltar en todas las listas siguientes.
//   NO_TRAIDO   las tarjetas son destinos: dejarlo como está, marcarlo como
//               discontinuado, o revisar el código. Ninguna toca un costo.
//
// NO se toca `formaDelPanel`. Las dos preguntas que están en producción quedan
// exactamente como estaban: esto es una función aparte porque la entrada es otra
// —un producto, no una fila— y meterlas juntas obligaría a que la de siempre
// entienda de productos para nada.

/** Qué se puede hacer con un producto que la lista no trajo. */
export const DESTINO = {
  DEJAR_COMO_ESTA: "DEJAR_COMO_ESTA",
  DISCONTINUADO: "DISCONTINUADO",
  REVISAR_CODIGO: "REVISAR_CODIGO",
};

export const TEXTO_DESTINO = {
  DEJAR_COMO_ESTA: "Dejarlo como está",
  DISCONTINUADO: "Marcarlo como discontinuado",
  REVISAR_CODIGO: "Revisar el código",
};

export const DETALLE_DESTINO = {
  DEJAR_COMO_ESTA:
    "Sigue vigente y el proveedor simplemente no lo informó este mes. No se toca nada.",
  DISCONTINUADO:
    "El proveedor dejó de venderlo. Queda registrado para no volver a buscarlo en cada lista.",
  REVISAR_CODIGO:
    "Puede que el proveedor le haya cambiado el código. Hay que mirar el código guardado contra el archivo.",
};

/**
 * La forma del panel de un PRODUCTO, no de una fila.
 *
 * @param grupo      uno de GRUPO_PRODUCTO
 * @param candidatos cuántas filas candidatas hay, en el caso sin código
 *
 * @returns {{ pregunta, origen, enCola, error, paso }}
 */
export function formaDelPanelDeProducto({ grupo = null, candidatos = 0 } = {}) {
  const base = { origen: ORIGEN_PANEL.SISTEMA, enCola: false, error: false, paso: null };

  if (grupo === "SIN_CODIGO") {
    // Sin candidatos la pregunta no se puede hacer: no hay tarjetas. Se muestra
    // igual, con la explicación, en vez de esconder el producto.
    return { ...base, pregunta: candidatos > 0 ? PREGUNTA.FILA : null };
  }
  if (grupo === "NO_TRAIDO") {
    // Los tres destinos existen siempre: no dependen de ningún dato.
    return { ...base, pregunta: PREGUNTA.DESTINO };
  }

  // Los demás grupos SÍ tienen fila, así que su panel es el de siempre y lo
  // decide `formaDelPanel`. Devolver null acá es decir "esto no me toca".
  return { ...base, pregunta: null, origen: ORIGEN_PANEL.COMPARACION };
}
