...
/** Los tres estados en los que hay comparación para mirar pero nada que elegir. */
...
export function formaDelPanel({
  ...
  const hayQueElegir =
    ...
    // producir un `resultado`. Sin esto caía en la rama sana de acá abajo y
    // desaparecía de la cola: quedaba un error que nadie podía resolver porque
    // nadie lo veía. Lo encontró el candado "el error no bloquea la cola".
    error;
 ...
 */
export function tonoDeFila({ estado = null, aplicada = false } = {}) {
  if (aplicada) return "apagado";
  switch (estado) {
...
/** Las filas de la cola, en el orden en que conviene resolverlas. */
export function esDeLaCola(fila) {
  return formaDelPanel(fila).enCola;
}
