...
test("3. nombre flojo y precio coherente: SE MUESTRA sin sugerir", () => {
  // "Flojo" es por debajo del umbral del motor pero POR ENCIMA del piso: el
  // mismo producto en otro tamaño comparte dos de tres palabras. Ése es el caso
  // que hay que seguir viendo — puede ser el producto con otra presentación.
  const c = uno(producto(), fila({ descripcionProveedor: "KETCHUP DOYPACK 500G" }));
  assert.equal(c.length, 1);
  assert.equal(c[0].nombreAlto, false, "0,5 no llega al umbral del motor");
  assert.ok(c[0].parecido >= 0.3 && c[0].parecido < 0.6, `parecido fuera del tramo: ${c[0].parecido}`);
  assert.equal(c[0].precioCoherente, true);
  assert.equal(c[0].sugerido, false, "que el precio cierre no identifica el producto");
});

test("3b. EL PISO: sin ningún parecido no entra, aunque el precio cierre", () => {
  // Antes entraba. De 626 filas sin machear, el precio solo arrastraba 276
  // tarjetas de ruido sobre 59 productos —casi todos llegaban al tope de cinco—
  // porque caer en la banda del 10 al 20 % lo cumple muchísima gente. El precio
  // confirma, no identifica.
  const c = uno(producto(), fila({ descripcionProveedor: "ARVEJAS EN LATA X 300" }));
  assert.equal(c.length, 0, "sin una palabra en común no es candidato de nada");
  assert.equal(PISO_PARECIDO, 0.3);
  assert.ok(PISO_PARECIDO < MIN_PARECIDO, "el piso deja pasar el tramo flojo, no lo tapa");
});

test("4. ninguna de las dos: NO APARECE", () => {
  // De 626 filas, la mayoría cae acá. Mostrarlas sería llenar el panel de ruido.
