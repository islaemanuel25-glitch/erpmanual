...
const gitGrepArchivos = (patron, ruta) => {
  try {
    // `--untracked`: sin eso el censo no ve un archivo nuevo sin commitear, y
    // se queda verde sobre un consumidor que acaba de aparecer. Agujero del
    // 2026-09-14, encontrado en `lib/layout/accionDePagina.test.mjs`.
    return execFileSync("git", ["grep", "--untracked", "-l", patron, "--", ruta], {
      cwd: raiz,
      encoding: "utf8",
    }).trim();
  } catch (e) {
...
test("28. el mapper es puro: sin imports de Prisma ni Next", () => {
  const src = readFileSync(new URL("./mapearVentaATransferencia.js", import.meta.url), "utf8");
  // ── DE "CERO IMPORTS" A "SOLO IMPORTS PUROS" ────────────────────────────
  //
  // Esto exigía `imports === []`. Era una forma barata de garantizar pureza
  // cuando el mapper no necesitaba nada, y dejó de alcanzar el 2026-09-09: para
  // congelar en qué presentación salió la mercadería, el mapper pasó a leer las
  // líneas comerciales con un helper.
  //
  // Lo que el candado defiende NO cambió —que acá no entren Prisma, Next, ni
  // nada que haga E/S— y ahora se afirma de verdad, en vez de por proxy: la
  // lista de imports permitidos es explícita, y de cada uno se comprueba que él
  // TAMBIÉN sea puro. Antes, un import nuevo era rojo aunque fuera inocente; hoy
  // uno que arrastre Prisma es rojo aunque esté en la lista.
  //
  // El 2026-09-09, segunda tanda: se sumó `presentacionEnvio`, que es de donde
  // salen los dos helpers con los que el mapper comprueba que el snapshot
  // describa la MISMA cantidad física que la línea va a mover. Escribir esa
  // cuenta acá habría sido una tercera copia de la aritmética que el pack
  // incompleto ya obligó a centralizar.
  const PERMITIDOS = ["./snapshotDePresentacion.js", "@/lib/transferencias/presentacionEnvio"];
  const imports = [...src.matchAll(/^import[\s\S]*?from\s+["']([^"']+)["']/gm)].map((m) => m[1]);
  assert.deepEqual(
    imports,
    PERMITIDOS,
    "el mapper solo puede importar módulos puros, y cada uno se declara acá"
  );

  const IMPUREZAS = ["@prisma", "prisma", "next/", "NextResponse", "$transaction", "await "];
  const sinComentarios = (t) => t.replace(/\/\/.*$/gm, "").replace(/\/\*[\s\S]*?\*\//g, "");

  const codigo = sinComentarios(src);
  for (const prohibido of IMPUREZAS) {
    assert.equal(codigo.includes(prohibido), false, `no debe aparecer "${prohibido}"`);
  }

  // Y la pureza es TRANSITIVA: un helper puro que mañana importe Prisma
  // ensuciaría el mapper sin que este archivo cambiara una línea.
  // El alias `@/` apunta a la raíz del repo; un `new URL` no lo resuelve solo.
  const rutaDe = (rel) =>
    rel.startsWith("@/")
      ? new URL(`../../${rel.slice(2)}${rel.endsWith(".js") ? "" : ".js"}`, import.meta.url)
      : new URL(rel, import.meta.url);

  for (const rel of PERMITIDOS) {
    const dep = sinComentarios(readFileSync(rutaDe(rel), "utf8"));
    for (const prohibido of IMPUREZAS) {
      assert.equal(
        dep.includes(prohibido),
        false,
        `${rel} dejó de ser puro: aparece "${prohibido}"`
      );
    }
  }
});
