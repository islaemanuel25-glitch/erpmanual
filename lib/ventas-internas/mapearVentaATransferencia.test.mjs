...
test("29. ningún llamador desde app/ (etapa no activada)", () => {
  const hits = gitGrepArchivos("mapearVentaATransferencia", "app/");
  assert.equal(hits, "", `no debe haber llamadores en app/, encontrados: ${hits}`);
});

test("30. SOLO_TRANSITO sigue sin uso desde app/", () => {
  const hits = gitGrepArchivos("SOLO_TRANSITO", "app/");
  assert.equal(hits, "");
});

test("31. Transferencia.ventaId sigue sin escrituras en quien crea transferencias", () => {
  for (const archivo of [
    ...
    assert.equal(/ventaId/.test(src), false, `${archivo} no debe mencionar ventaId`);
  }
});
...
test("34. BLOQUEO CONOCIDO: TransferenciaDetalle.cantidad sigue en Decimal(12,2)", () => {
  const schema = readFileSync(new URL("../../prisma/schema.prisma", import.meta.url), "utf8");
  const detalle = schema.match(/model TransferenciaDetalle \{[\s\S]*?\n\}/)[0];

  // Este test documenta el bloqueo. Si algún día se amplía la precisión a (12,3),
  // ESTE assert falla: es la señal de que el bloqueo se levantó y hay que revisar
  // la nota de la etapa 3A, no de que se rompió algo.
  assert.match(detalle, /\n\s+cantidad\s+Decimal\s+@db\.Decimal\(12,\s*2\)/,
    "si cambió a (12,3), revisar la advertencia de precisión de la etapa 3A");

  // El mapper preserva 1.925 exacto…
  const r = mapearVentaATransferencia({
    consumoFisicoConsolidado: [{ productoLocalId: 31, productoBaseId: 310, cantidad: 1.925 }],
  });
  assert.equal(r.items[0].cantidad, 1.925);
  // …pero numeric(12,2) de PostgreSQL redondearía media unidad hacia arriba.
  // La pérdida ocurriría al PERSISTIR, no acá: el mapper no la esconde.
  assert.equal(Number(r.items[0].cantidad.toFixed(2)), 1.93);
  assert.notEqual(Number(r.items[0].cantidad.toFixed(2)), r.items[0].cantidad);

  // StockLocal.cantidad también es (12,2): el techo real de precisión física del
  // sistema son 2 decimales, no 3. Es preexistente y ajeno a esta etapa.
  const stock = schema.match(/model StockLocal \{[\s\S]*?\n\}/)[0];
  assert.match(stock, /\n\s+cantidad\s+Decimal\s+@db\.Decimal\(12,\s*2\)/);
});
