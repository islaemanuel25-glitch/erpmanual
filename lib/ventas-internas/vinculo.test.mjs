...
test("el historial activo es la baseline y nada más", () => {
  const dirs = fs
    .readdirSync(DIR_MIG, { withFileTypes: true })
    .filter((e) => e.isDirectory())
    .map((e) => e.name)
    .sort();
  assert.deepEqual(dirs, ["000000000000_squashed_migrations"],
    `el directorio activo debería tener solo la baseline; tiene: ${dirs.join(", ")}`);
});
