...
test("la migración va después de la última migración con fecha real", () => {
  ...
  const hoyTs =
    ...
    `${String(hoy.getMonth() + 1).padStart(2, "0")}` +
    `${String(hoy.getDate()).padStart(2, "0")}`;
  const realesAnteriores = dirs.filter(
    (d) => d !== mia && d.slice(0, 8) <= hoyTs
  );
  const ultimaReal = realesAnteriores[realesAnteriores.length - 1];
...
test("el helper no está conectado a ningún flujo todavía", () => {
  const usos = [];
  const walk = (dir) => {
  ...
  assert.deepEqual(usos, [], `el helper no debería tener llamadores todavía: ${usos.join(", ")}`);
});
