...
test("5. no hay ninguna reducción de precisión ni de escala", () => {
  const sinComentarios = sqlMigracion.replace(/^--.*$/gm, "");
  const alters = [...sinComentarios.matchAll(
    /ALTER TABLE "(\w+)" ALTER COLUMN "(\w+)" TYPE DECIMAL\((\d+),(\d+)\)/g
  )];
  assert.equal(alters.length, 6);
  for (const [, tabla, col, p, s] of alters) {
    // Todas ganan el tercer decimal; ninguna baja de 12 dígitos.
    assert.equal(s, "3", `${tabla}.${col}: la escala siempre sube a 3`);
    assert.ok(Number(p) >= 12, `${tabla}.${col}: la precisión nunca baja de 12`);
    const esperada = tabla === "AuditoriaStock" ? "14" : "12";
    assert.equal(p, esperada, `${tabla}.${col} debe ir a DECIMAL(${esperada},3)`);
  }
});

test("10. los límites preventivos del SQL distinguen (12,3) de (14,3)", () => {
  // Los comentarios llevan las queries de verificación previas al deploy. Si el
  // límite fuera uno solo, el chequeo de AuditoriaStock daría un falso positivo y
  // abortaría un deploy sano —o peor, dejaría pasar uno que va a fallar—.
  assert.match(sqlMigracion, /abs\("cantidadAnterior"\) >= 100000000000/);
  assert.match(sqlMigracion, /abs\("cantidadNueva"\)\s+>= 100000000000/);
  assert.match(sqlMigracion, /abs\(cantidad\) >= 1000000000 OR abs\("enTransito"\) >= 1000000000/);
  assert.match(sqlMigracion, /abs\(cantidad\) >= 1000000000 OR abs\(coalesce\(recibido,0\)\) >= 1000000000/);
  // Y los dos límites son efectivamente distintos.
