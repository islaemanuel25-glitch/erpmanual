...
test("8. la baseline es la única migración del directorio activo", () => {
  // El candado viejo comprobaba que ESTA migración se ordenara después de la de
  // vínculos. Con el historial saneado no hay orden que comprobar: hay una sola.
  // Lo que sí sigue importando —y es lo que se afirma— es que nadie deje una
  // migración suelta al lado de la baseline creyendo que se aplica después,
  // porque `000000000000_` las ordena a TODAS después de ella.
  const dirs = readdirSync(join(RAIZ, "prisma", "migrations"), { withFileTypes: true })
    .filter((d) => d.isDirectory())
    .map((d) => d.name)
    .sort();
  assert.deepEqual(dirs, ["000000000000_squashed_migrations"], `hay: ${dirs.join(", ")}`);
});
