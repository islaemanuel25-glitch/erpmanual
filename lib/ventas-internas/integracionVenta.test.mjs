...
// ── EL CONTRATO SE DIO VUELTA A PROPÓSITO EL 2026-08-20 ─────────────────────
//
// El candado 39 afirmaba que cancelar una transferencia con `ventaId` se
// RECHAZABA. Ese bloqueo era correcto para lo que la ruta sabía hacer entonces:
// devolvía `cantidad` al origen, y con una venta de por medio eso habría
// inventado mercadería que la venta sigue cobrando.
//
// Pero tapaba el síntoma. Lo que faltaba era revertir las dos cosas juntas, y eso
// es lo que la ruta hace ahora: la transferencia con su política —`SOLO_TRANSITO`,
// solo libera el tránsito— y la venta con el motor de reversión, en la misma
// transacción. Ya no hay nada que prohibir.
//
// `bloqueoCancelacion` se retira junto con el candado: quedó sin llamador y
// mantenerla sería dejar una prohibición escrita que ya no rige, esperando a que
// alguien la vuelva a enchufar.
test("39. cancelar una transferencia con venta AHORA revierte las dos, no rechaza", () => {
  // La ruta dejó de rechazar por tener venta...
  assert.ok(
    !/bloqueoCancelacion/.test(RUTA_CANCELAR),
    "volvió el rechazo por tener venta: ahora la reversión es completa"
  );
  // ...y en cambio revierte la venta con el motor compartido.
  assert.match(RUTA_CANCELAR, /revertirVenta/,
    "la ruta no revierte la venta vinculada: cancelar dejaría la venta cobrando mercadería que volvió");

  // Y sigue habiendo un rechazo ANTES de la transacción — el del predicado— así
  // que ningún caso inválido llega a tocar stock. Es la propiedad que el candado
  // viejo protegía y no se pierde.
  const iVeredicto = RUTA_CANCELAR.indexOf("puedeCancelarTransferencia(");
  const iTx = RUTA_CANCELAR.indexOf("prisma.$transaction");
  const iStock = RUTA_CANCELAR.indexOf("stockLocal.updateMany");
  assert.ok(iVeredicto > 0 && iVeredicto < iTx && iTx < iStock,
    "la validación tiene que ocurrir antes de abrir la transacción");
});

// ── ESTE CANDADO QUEDÓ EN VERDE Y SU TÍTULO ERA FALSO ───────────────────────
//
// Decía "en cualquier estado" y probaba tres: Enviada, Recibiendo y Recibida. El
// 2026-08-20 se agregó la excepción de "Cancelada" —sin ella la operación de
// desvincular no puede escribir el cliente nuevo— y este candado NO se puso rojo,
// porque el estado nuevo no estaba en su lista.
//
// O sea: siguió pasando mientras afirmaba algo que había dejado de ser cierto, y
// cualquiera que lo leyera creería que Cancelada bloquea. Es la lección de releer
// los candados que quedan en VERDE, y por eso el título ahora nombra la excepción
// en vez de decir "cualquiera".
test("40. corregir una venta con transferencia VIVA se bloquea; Cancelada no", () => {
  assert.equal(bloqueoCorreccion({ transferencia: null }), null);
  assert.equal(bloqueoCorreccion({}), null);
  // La excepción, acá y no solo en el candado 9 del otro archivo: quien venga a
  // leer el contrato del bloqueo lo lee en el mismo lugar que las tres de abajo.
  assert.equal(
    bloqueoCorreccion({ transferencia: { id: 9, estado: "Cancelada" } }),
    null,
    "una transferencia cancelada no tiene stock en juego: bloquear por ella es bloquear por un rastro"
  );
...
test("42. la recepción sigue sin tocar deuda ni MovimientoCuenta", () => {
  // ── DOS COSAS CAMBIARON ACÁ EL 2026-09-11, Y CONVIENE SABER CUÁLES ────────
  //
  // 1. EL CANDADO MIRABA LA PROSA. Leía el archivo crudo y buscaba subcadenas,
  //    así que se puso ROJO por un COMENTARIO que dice "no crea MovimientoCuenta".
  //    Es el tercer caso del mismo patrón anotado en CLAUDE.md, y el arreglo es
  //    el de siempre: sacar los comentarios antes de mirar. Acá fue un falso
  //    positivo —molesta y se ve—, pero el mismo defecto produce falsos verdes.
  //
  // 2. `"venta."` DEJÓ DE SER LA AFIRMACIÓN CORRECTA. Confirmar una recepción
  //    ahora SÍ corrige su venta vinculada: el stock corregido con dinero
  //    original era el defecto, no la garantía. Lo que sigue prohibido —y es lo
  //    que de verdad protege este candado— es que la recepción escriba la venta
  //    POR SU CUENTA en vez de por el aplicador, y que toque deuda.
  //
  // Lo que NO se aflojó: cuenta corriente y fiado siguen intocables. Una venta
  // interna con remito no cuenta para el arqueo y nadie decidió que una
  // diferencia de recepción genere deuda entre el depósito y el local.
  const recepcion = leer("app", "api", "transferencias", "confirmar-recepcion", "route.js")
    .replace(/\/\*[\s\S]*?\*\//g, "")
    .replace(/(^|[^:])\/\/.*$/gm, "$1");

  for (const prohibido of ["movimientoCuenta", "MovimientoCuenta", "esFiado", "cuentaCorriente"]) {
    assert.equal(recepcion.includes(prohibido), false,
      `la recepción no debe mencionar "${prohibido}"`);
  }
  for (const escrituraDirecta of ["tx.venta.", "tx.ventaDetalle.", "tx.ventaPago.", "tx.ventaCorreccion."]) {
    assert.equal(recepcion.includes(escrituraDirecta), false,
      `la recepción escribe ${escrituraDirecta} por su cuenta en vez de por el aplicador`);
  }
});
...
test("la transacción de venta declara 30 s de ejecución y 10 s de espera", () => {
  // Incidente 2026-08-31: una venta interna de 151 líneas alcanzó 137 antes de
  // que Prisma la revirtiera a los 5071 ms. Sin estas opciones vuelve a regir el
  // default implícito de 5 s y el defecto reaparece según la carga de PostgreSQL.
  const iTx = RUTA_CREAR.indexOf("prisma.$transaction");
  const iDespues = RUTA_CREAR.indexOf("const venta = txResult.venta");
  assert.ok(iTx > 0 && iDespues > iTx, "no se pudo aislar la transacción de crear venta");

  const bloque = RUTA_CREAR.slice(iTx, iDespues);
  assert.match(
    bloque,
    /\},\s*\{\s*maxWait:\s*10_000,\s*timeout:\s*30_000,?\s*\}\s*\);/,
    "la transacción vuelve al límite implícito de 5 s"
  );
  assert.equal((bloque.match(/maxWait:/g) || []).length, 1, "maxWait se declara una sola vez");
  assert.equal((bloque.match(/timeout:/g) || []).length, 1, "timeout se declara una sola vez");
});
...
test("36-38. cuenta corriente y pagos siguen igual", () => {
...
// ── EL TRAMO DE REPORTES SE SACÓ DE ARRIBA, Y VALE CONTAR POR QUÉ ───────────
//
// El candado anterior terminaba con `assert.equal(listado.includes("transferencia"), false)`
// bajo el comentario "Reportes lee Venta sin filtrar por transferencia". Eso era
// cierto cuando se escribió, en la etapa en que la venta interna todavía no
// existía como concepto.
//
// Después llegó `whereVentaComercial` y el listado SÍ empezó a filtrar por
// transferencia — pero el candado siguió en verde, porque el filtro vive dentro
// del helper y la palabra no aparecía en el archivo. O sea: **estuvo pasando
// mientras afirmaba lo contrario de lo que pasaba.** Lo destapó recién el
// 2026-08-20, cuando el listado tuvo que seleccionar la relación para poder
// marcar las internas y la palabra apareció literalmente.
//
// Se reemplaza por lo que hoy es verdad y hay que defender.
test("36b. el listado SÍ filtra las internas, y solo las muestra si se piden", () => {
  const listado = leer("app", "api", "reportes-ventas", "listado", "route.js");
  assert.match(listado, /whereVentaComercial/,
    "el listado dejó de filtrar: las operaciones internas se contarían como ventas");
  assert.match(listado, /incluirInternas/,
    "el listado no puede mostrar las internas ni pidiéndoselo, y ahí vive el botón de anular");
  assert.doesNotMatch(listado, /incluirInternas:\s*true[,\s}]/,
    "el listado pasó a mostrarlas SIEMPRE en vez de solo cuando se piden");
});
