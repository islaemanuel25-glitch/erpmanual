...
import { snapshotsPorProductoLocal } from "./snapshotDePresentacion.js";
import {
  descriptorDeEnvio,
...
/**
 * CUÁNTAS UNIDADES FÍSICAS AFIRMA UN SNAPSHOT DE PRESENTACIÓN.
 *
 * No es una cuenta nueva: es la de la recepción, leída por los mismos dos
 * helpers que usan la pantalla y el servidor. Escribir acá
 * `cantidadPresentada × factorPresentacion + sueltasEnviadas` habría sido una
 * tercera copia de la aritmética que el pack incompleto ya obligó a centralizar.
 */
function fisicasDelSnapshot(presentacion) {
  return unidadesFisicasDelDescriptor(descriptorDeEnvio(presentacion));
}
...
export function mapearVentaATransferencia({
  ...
  // ── LA PRESENTACIÓN, RESCATADA DE LAS LÍNEAS COMERCIALES ────────────────
  //
  // `lineasComerciales` ya llegaba a esta función y solo se usaba para contar lo
  // omitido. Ahí está `modoVentaLinea` —que distingue el pack de la unidad
  // suelta— y `baseStock`, con la unidad de medida, el factor y el peso. Es la
  // ÚNICA fuente que todavía sabe cómo se contó, porque `consumoFisicoConsolidado`
  // ya viene convertido a unidades de stock.
  //
  // No cambia ninguna cantidad física: se agrega en paralelo.
  const presentaciones = snapshotsPorProductoLocal(lineasComerciales, { esDeposito });
  ...
  const items = consolidado.map((c) => {
    ...
    // El snapshot de presentación viaja aparte del de ProductoLocal: son dos
    // cosas distintas —uno congela CÓMO SE CONTÓ, el otro trae nombre y precios
    // para crear el producto en el destino—.
    //
    // ── Y SOLO SI DESCRIBE LA MISMA MERCADERÍA QUE SE VA A MOVER ─────────
    //
    // El snapshot se arma con las LÍNEAS COMERCIALES; la cantidad física sale
    // del CONSUMO CONSOLIDADO. Casi siempre son el mismo hecho contado dos
    // veces, pero no siempre, y el caso que los separa es común:
    //
    //     1 Coca directa  +  1 combo que lleva 2 Coca  →  consolidado 3
    //
    // El combo no tiene línea comercial propia —viaja expandido en sus
    // componentes— así que el snapshot solo ve la línea directa y diría
    // "1 UNIDAD" sobre una línea que físicamente mueve 3. Esa línea después se
    // recibe en la escala del snapshot: el operador contaría 1 contra un envío
    // de 3, y la recepción informaría un faltante de 2 que nunca faltó.
    //
    // Se compara con los helpers canónicos —los mismos que usa la recepción para
    // llegar a unidades físicas— y en milésimas enteras, que es la única forma
    // de que 3,25 kg contra 3,25 kg dé exacto. Si no coincide NO se guarda un
    // snapshot falso: la línea queda sin registrar, que es la verdad, y la
    // recepción la reconstruye del catálogo como cualquier histórico.
    const presentacion = presentaciones.get(c.productoLocalId);
    if (presentacion && aMilesimas(fisicasDelSnapshot(presentacion)) === c.milesimas) {
