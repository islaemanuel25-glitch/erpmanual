...
test("el helper de activación de venta interna sigue sin llamadores", () => {
  const usos = [];
  const walk = (dir) => {
  ...
  assert.deepEqual(usos, [], `vinculo.js no debería tener llamadores: ${usos.join(", ")}`);
});

test("Transferencia.ventaId sigue sin escrituras", () => {
  const usos = [];
  const walk = (dir) => {
    for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
      const p = path.join(dir, e.name);
  ...
  assert.deepEqual(usos, [], `nadie debería escribir Transferencia.ventaId: ${usos.join(", ")}`);
});
