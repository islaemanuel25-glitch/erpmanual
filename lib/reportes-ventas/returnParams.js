...
/** URL interna de la página de CORRECCIÓN de una venta, con el contexto de retorno. */
export function buildCorregirUrl(ventaId, ctx) {
  const id = Number(ventaId);
  if (!Number.isInteger(id) || id <= 0) return BASE; // fail-safe
  const qs = serializeReturnParams(ctx || {});
  return `${BASE}/${id}/corregir${qs ? `?${qs}` : ""}`;
}
