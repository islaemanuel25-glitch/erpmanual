...
test("5f. EL LISTADO COMERCIAL NO MEZCLA OPERACIONES INTERNAS", () => {
  // ── ESTE CANDADO AFIRMABA LO CONTRARIO HACE UNAS HORAS ────────────────────
  //
  // El 2026-08-20, para poder llegar al botón de anular una venta interna, se
  // agregó un `?incluirInternas=1` al listado con un interruptor en la pantalla,
  // y este candado exigía que existieran. Ese mismo día se descartó la decisión
  // de producto: **una transferencia mal enviada se corrige desde
  // Transferencias**, que es el documento que el local destino recibe y revisa.
  //
  // Ventas deja de ser un camino para eso, así que el listado comercial vuelve a
  // no mezclar operaciones que no son ventas. El candado se invierte: lo que hay
  // que impedir ahora es que el interruptor vuelva.
  //
  // La capacidad del helper —`incluirInternas`— se conserva porque el historial
  // del cliente la usa desde antes y es legítima ahí. Lo que no vuelve es el
  // listado comercial usándola.
  const listado = leerSinComentarios("app/api/reportes-ventas/listado/route.js");
  assert.ok(
    !/incluirInternas/.test(listado),
    "el listado comercial volvió a poder mezclar operaciones internas"
  );

  const pantalla = leerSinComentarios("app/modulos/reportes-ventas/page.jsx");
  assert.ok(!/incluirInternas/.test(pantalla), "volvió el interruptor a la pantalla de Ventas");
  assert.ok(!/verInternas/.test(pantalla), "volvió el estado del interruptor");
});

// ── LOS CANDADOS DE LA ANULACIÓN, MUDADOS ACÁ ───────────────────────────────
//
// Vivían en `lib/pos-ventas/anularVenta.test.mjs`, junto a un predicado que se
// retiró con el flujo de Ventas. Prueban el FILTRO, que se queda, así que se
// mudan al archivo del filtro. Es la misma lección de siempre: un candado vive
// con la pieza que prueba.

test("6. una venta anulada NO cuenta como comercial", () => {
  assert.deepEqual(soloNoAnulada(), { anuladaEn: null });
  assert.deepEqual(soloComercial(), { transferencia: { is: null }, anuladaEn: null });
});

test("6b. las internas anuladas tampoco cuentan como internas", () => {
  // Si faltara, las dos mitades no sumarían el total.
  assert.deepEqual(soloInterna(), { transferencia: { isNot: null }, anuladaEn: null });
});

test("6c. estaAnulada deriva de la fecha, no de un flag", () => {
...
test("6d. NO existe un booleano `anulada` al lado de la fecha", () => {
...
test("5g. y las dos opciones son independientes: una no arrastra a la otra", () => {
  // Pedir internas no muestra las anuladas, y pedir anuladas no muestra las
  // internas. Es el mismo error que ya se cometió una vez con el `where` del
  // llamador, y por eso se comprueban las cuatro combinaciones.
  assert.deepEqual(whereVentaComercial({}, {}), { transferencia: { is: null }, anuladaEn: null });
  assert.deepEqual(whereVentaComercial({}, { incluirInternas: true }), { anuladaEn: null });
  assert.deepEqual(whereVentaComercial({}, { incluirAnuladas: true }), { transferencia: { is: null } });
  assert.deepEqual(whereVentaComercial({}, { incluirInternas: true, incluirAnuladas: true }), {});
});
