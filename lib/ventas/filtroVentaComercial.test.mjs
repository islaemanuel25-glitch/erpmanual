...
test("5f. SE PUEDE LLEGAR A UNA VENTA INTERNA DESDE EL LISTADO", () => {
  // ── EL DEFECTO QUE ESTE CANDADO ATAJA ─────────────────────────────────────
  //
  // El 2026-08-20 se desplegó el botón de anular en el detalle de la venta y
  // Emanuel no pudo usarlo: la venta que quería anular tenía remito, así que el
  // listado la escondía. Medido ese día sobre el 19/08 en el depósito, mostraba
  // 5 de 12. El botón estaba en una pantalla a la que no se llegaba.
  //
  // El botón vive en el detalle de una venta. Si el único listado que lleva ahí
  // no puede mostrar las internas, hay ventas anulables inalcanzables. Eso es lo
  // que se afirma acá, y son las dos mitades: que el helper sepa incluirlas y
  // que la ruta del listado efectivamente lo pida.
  assert.deepEqual(whereVentaComercial({ localId: 1 }, { incluirInternas: true }), {
    localId: 1,
    anuladaEn: null,
  }, "con incluirInternas no debe quedar ninguna condición sobre `transferencia`");

  const src = leerSinComentarios("app/api/reportes-ventas/listado/route.js");
  assert.match(src, /incluirInternas/,
    "el listado no acepta incluirInternas: las ventas con remito quedan inalcanzables");
  assert.match(src, /searchParams\.get\("incluirInternas"\)/,
    "el listado no lee el parámetro de la query");

  const pantalla = leerSinComentarios("app/modulos/reportes-ventas/page.jsx");
  assert.match(pantalla, /incluirInternas/,
    "la pantalla nunca manda el parámetro: el interruptor no existe o no llegó a la ruta");
});
