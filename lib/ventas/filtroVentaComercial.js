 ...
 * 17 lugares.
 *
 * ── `incluirInternas`: LA MISMA IDEA PARA LA OTRA CONDICIÓN ────────────────
 *
 * El listado de ventas también necesita poder mostrarlas. Se agregó el
 * 2026-08-20, y lo destapó un defecto: se desplegó el botón de anular en el
 * detalle de la venta y no se llegaba a la venta que había que anular, porque
 * tenía remito y el listado la escondía. Medido ese día sobre el 19/08 en el
 * depósito, el listado mostraba 5 de 12.
 *
 * El mecanismo ya existía en `/api/clientes/[id]/ventas` como `?incluirInternas=1`;
 * acá pasa a estar en el helper para que las dos rutas usen la misma decisión y
 * no dos parecidas.
 *
 * LAS DOS OPCIONES SON INDEPENDIENTES. Pedir internas no muestra las anuladas de
 * arrastre y viceversa: cada condición se decide sola. Es la misma regla que ya
 * gobierna el `where` del llamador, y está por el mismo motivo — la primera vez
 * que se mezclaron, pisar una se llevó la otra sin que nada avisara.
 *
 * @param {object} [whereBase={}]
 * @param {object} [opciones]
 * @param {boolean} [opciones.incluirAnuladas=false] mostrar también las anuladas.
 * @param {boolean} [opciones.incluirInternas=false] mostrar también las que tienen remito.
 * @returns {object} nuevo objeto; NO muta el original.
 */
export function whereVentaComercial(
  whereBase = {},
  { incluirAnuladas = false, incluirInternas = false } = {}
) {
  ...
  if (!incluirInternas && !tiene("transferencia")) compuesto.transferencia = { is: null };
