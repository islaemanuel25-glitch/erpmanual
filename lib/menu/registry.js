...
import { PERMISOS_CONFIG_POS } from "@/lib/config/acceso";
...
export const MENU_CONFIG = [
  ...
  // ----------------------------------------------------------
  {
    ...
    items: [
      ...
      // ── EL CORTE DE SEMANA ENTRA AL MENÚ, Y NO ES UN ATAJO MÁS ────────
      //
      // La lista de trabajo ya avisa cuando FALTA configurar una relación, y
      // ese aviso lleva acá. Pero una pantalla a la que solo se llega cuando
      // algo está mal no existe el día que hay que CAMBIAR un corte que ya
      // está bien — que es exactamente cuando se la busca: un local se muda de
      // día de pedido y hay que correrle la semana.
      //
      // Va con `transferencias.crear`, el permiso del depósito, porque el corte
      // es un acuerdo del que despacha con cada local. Quien solo tiene
      // `transferencias.ver` no ve el ítem, y si igual llega por el atajo la
      // pantalla se LEE: la fila no ofrece "Cambiar". Ver
      // `components/transferencias/corteDeSemana.js`.
      {
        label: "Corte de semana",
        href: "/modulos/transferencias/corte-de-semana",
  ...
  // ----------------------------------------------------------
  {
    ...
    requiredAnyPerms: [
      ...
      // Sin esto, quien solo administra medios de cobro no veía ni el grupo.
    ...
    items: [
      ...
      {
        // Antes decía "Configuración POS Ventas" y llevaba directo a los tres
        // toggles de reglas. Ahora esa ruta es la portada del módulo y adentro
        // están Cobros, Reglas de venta, Integraciones y Apariencia.
        label: "Configuración POS",
        href: "/modulos/configuracion/pos-ventas",
        // Adentro hay dos cosas con permisos distintos —reglas de venta y medios
        // de cobro—, así que alcanza con tener cualquiera de los dos. La lista
        // vive en `lib/config/acceso.js` porque la landing de tarjetas usa la
        // misma, y estaban escritas por separado.
        requiredAnyPerms: PERMISOS_CONFIG_POS,
        icon: ShoppingCart,
        type: "config",
        scope: "config",
        requiredModule: "configuracion",
      ...
      // LA ENTRADA DE "RECARGOS POR MEDIO DE PAGO" SE FUE DE ACÁ.
      //
      // El recargo ahora se edita adentro de cada medio, en Configuración POS →
      // Cobros, junto con su nombre, su orden y su comisión, y se guarda en la
      // misma transacción. Dejar las dos entradas serían dos caminos al mismo
      // número, y eso termina con alguien editando una pantalla y mirando la
      // otra.
      //
      // La RUTA sigue viva y redirige: puede estar guardada como favorito.
      // Ver `app/modulos/configuracion/recargos-pago/page.jsx`.
      {
