 ...
 *        - `scope === "config"` SIN señal RBAC y no admin -> "config-admin-only".
 *          (con permiso/requiredAnyPerms/requiredAllPerms lo decide el bloque G)
...
export function canAccessMenuItem(user, grupoConfig, item, registry) {
  ...
  // scope:"config" YA NO es admin-only por sí mismo. La configuración de un local
  // (apariencia, stock, POS, ticket, alertas…) la gobiernan los permisos
  // config_local.* vía RBAC (bloque G), de modo que DUEÑO_LOCAL/ENCARGADO ven
  // solo lo que su rol permite. Los items que deben seguir siendo exclusivos de
  // admin llevan adminOnly:true (cortado arriba). Red de seguridad: un item de
  // config SIN ninguna señal RBAC (permiso/requiredAnyPerms/requiredAllPerms)
  // se mantiene admin-only para no exponer configuración sin gating.
  if (safeItem.scope === "config" && !esAdmin) {
    const tieneGateRbac =
      !!readItemPermission(safeItem) ||
