...
test("EL DETALLE DEL PEDIDO NO CUENTA CONTRA EL CATÁLOGO: es la comparación de otra pantalla", () => {
  ...
  // de cuando se arma el pedido, puesta en la pantalla de cuando llega. **Ése es
  // el motivo, y se sostiene solo**: el contador sobra ahí se encienda o no.
  //
  // Se midió además que no se encendía —los 13 pedidos ENVIADO de erpazul_dev, uno
  // por uno, sin el elemento en el DOM—, pero OJO CON EL PORQUÉ, que se escribió
  // mal la primera vez: NO es que el módulo reescriba el costo maestro. Ninguna
  // ruta de pedido lo escribe desde `ed52991`; solo `recibir` propaga. Coinciden
  // porque LA LÍNEA NACE CLONANDO el costo del catálogo al agregar el producto:
  // es una fotocopia, no un vínculo. Así que el contador SÍ puede encenderse
  // cuando los dos se separan —al editar la línea, al editar el producto, o al
  // aplicar una lista de proveedor—, y ese cero es un hecho de esos datos y no
  // una ley.
  const src = fs.readFileSync(
