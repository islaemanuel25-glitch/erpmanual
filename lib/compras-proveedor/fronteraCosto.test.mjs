...
// ── Los umbrales por proveedor ─────────────────────────────────────────────

test("un proveedor SIN umbrales propios cae al default global", () => {
  for (const prov of [null, undefined, {}, { umbralRevisarPct: null, umbralSospechaBajaPct: null }]) {
    const u = umbralesDelProveedor(prov);
    assert.equal(u.revisarPct, UMBRALES_COSTO_DEFAULT.revisarPct, JSON.stringify(prov));
    assert.equal(u.sospechaBajaPct, UMBRALES_COSTO_DEFAULT.sospechaBajaPct, JSON.stringify(prov));
  }
});

test("un proveedor CON los suyos los usa, y puede tener uno solo", () => {
  const u = umbralesDelProveedor({ umbralRevisarPct: 8, umbralSospechaBajaPct: 25 });
  assert.equal(u.revisarPct, 8);
  assert.equal(u.sospechaBajaPct, 25);

  // Uno propio y el otro no: cada uno resuelve por su lado.
  const mixto = umbralesDelProveedor({ umbralRevisarPct: 8, umbralSospechaBajaPct: null });
  assert.equal(mixto.revisarPct, 8);
  assert.equal(mixto.sospechaBajaPct, UMBRALES_COSTO_DEFAULT.sospechaBajaPct);
});

test("EL DEFAULT NO SE COPIA EN LA FILA: null y 5 son cosas distintas", () => {
  // Si la migración hubiera llenado las 43 filas con el default, cambiar el
  // global no movería a nadie. Este candado protege esa diferencia.
  const sinPropio = umbralesDelProveedor({ umbralRevisarPct: null });
  const conPropioIgual = umbralesDelProveedor({ umbralRevisarPct: 5 });
  assert.equal(sinPropio.revisarPct, conPropioIgual.revisarPct, "hoy dan lo mismo…");
  // …pero el proveedor sin propio sigue el global si el global cambia.
  const otroGlobal = clasificarDiferenciaCosto({
    costoAnterior: 100, costoNuevo: 108,
    umbrales: umbralesDelProveedor({ umbralRevisarPct: 10 }),
  });
  assert.equal(otroGlobal.clase, CLASE_DIFERENCIA.SIN_AVISO, "con umbral propio 10, un 8 % no molesta");
});

test("un decimal de Prisma llega como objeto y se resuelve igual", () => {
  // Prisma devuelve Decimal, no Number. Si esto se rompiera, el proveedor
  // caería al default en silencio y sus umbrales no valdrían nada.
  const comoPrisma = { toString: () => "7.5", valueOf: () => 7.5 };
  assert.equal(umbralesDelProveedor({ umbralRevisarPct: comoPrisma }).revisarPct, 7.5);
});
