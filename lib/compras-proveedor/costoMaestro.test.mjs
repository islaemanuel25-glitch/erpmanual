...
//
// ── QUIÉN LLAMA A ESTO: UNA SOLA RUTA, Y ANTES ERAN TRES ────────────────────
//
// **Lo llama `compras-proveedor/recibir/[id]` y nadie más**, y ni siquiera
// siempre: la llamada está adentro de un `if (escribeCosto)`.
//
// Este encabezado decía hasta el 2026-08-17 que lo llamaban TRES rutas —crear,
// editar-item/[id] y recibir/[id]—. Era cierto hasta el 2026-08-10, cuando
// `ed52991` sacó la propagación de las rutas de pedido: un pedido registra lo que
// se PIDIÓ, y el costo real recién se conoce cuando llega la mercadería. Los
// candados de este mismo archivo ya afirmaban lo nuevo —que crear y editar-item
// NO llaman— mientras el encabezado seguía diciendo lo viejo.
//
// **Y por eso se corrige acá primero:** una prosa desactualizada arriba de todo
// hace más daño que en cualquier otro lado, porque es lo primero que se lee y
// contradice a los candados que están tres pantallas más abajo. Quien confíe en
// ella saca la conclusión contraria a la que saca leyendo el código.
//
// Que hoy sea una sola ruta está afirmado por candados, y por dos caminos
// distintos: `retornoPedido.test.mjs` lo comprueba POR NOMBRE —ninguna ruta de
// pedido llama al helper— y también POR EFECTO —ninguna hace `productoBase.update`—,
// enumerando las rutas de forma recursiva.
//
// ── LO QUE NO SE VERIFICÓ AL CORREGIR ESTO, Y QUEDA DICHO ──────────────────
//
// La frase original decía además que éste es "uno de los DOS caminos que escriben
// el costo maestro en producción", siendo el otro el aplicador de listas.
// **Ese conteo no se comprobó** en la revisión del 2026-08-17, que miró quién
// llama a este helper y no cuántos caminos hay en total. Hay al menos un tercero
// a la vista —`app/api/productos/editar/[id]`, que escribe por
// `propagarCostoALocales`— así que "dos" es dudoso y por eso se sacó el número en
// vez de reemplazarlo por otro inventado. Cuánto son de verdad es un relevamiento
// propio. El "~787 tests" del aplicador tampoco se recontó.
//
