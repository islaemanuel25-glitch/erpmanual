...
test("FORMA: crear y editar-item llaman SIN transacción — está así hoy", async () => {
  // Este candado NO afirma que esté bien. Afirma qué hace hoy, para que el día
  // que alguien lo envuelva en una transacción el cambio sea deliberado y se vea.
  //
  // Consecuencia, ejercitada arriba en los candados de fallo a mitad de camino:
  // si la propagación se corta, la base queda con el costo nuevo y las
  // ubicaciones con el viejo, y eso PERSISTE.
  for (const rel of ["crear/route.js", "editar-item/[id]/route.js"]) {
    const src = leerRuta(rel);
    assert.match(
      src,
      /actualizarCostoRealProducto\(\s*prisma\s*,/,
      `${rel} cambió de forma de llamar al costo maestro`
    );
    assert.doesNotMatch(
      src,
      /prisma\.\$transaction/,
      `${rel} ahora usa transacción: es una mejora, pero hay que actualizar este candado y ` +
        `los de fallo a mitad de camino, que documentan el comportamiento sin ella`
    );
  }
});

test("FORMA: agregar-item NO propaga el costo maestro, editar-item SÍ", async () => {
  // La asimetría que apareció relevando: agregar una línea a un pedido escribe
  // el costo en el DETALLE y no lo propaga; editar esa misma línea después sí lo
  // propaga. El mismo hecho económico —lo que se está pagando por el producto—
  // mueve el costo maestro o no según con qué botón se cargó.
  //
  // Queda fijado para que el día que se empareje, se vea.
  ...
  assert.match(editar, /actualizarCostoRealProducto/);
});
