...
export function consolidarLineasImportadas({ lineas = [], productosPorId } = {}) {
  ...
  for (const linea of lineas) {
    ...
    const item = {
      ...
      // ── EL IMPORTE DEL PAPEL VIAJA, PARA QUE EL SERVIDOR PUEDA MIRARLO ───
      //
      // Sale de `coherencia.subtotal`, que ya vale `null` cuando el documento no
      // trae la columna: así el servidor no puede confundir "el papel no lo dice"
      // con "el papel dice cero". La ruta lo vuelve a verificar con SU
      // `factor_pack`, no con el que mande el navegador.
      subtotalPapel: linea?.coherencia?.subtotal ?? null,
    };
    ...
    // Los importes del papel se SUMAN, y basta con que a uno le falte para que
    // el consolidado quede sin importe verificable. Sumar los que hay y
    // comparar contra el total daría de menos y frenaría una línea sana.
    const subtotalPapel =
      anterior.subtotalPapel === null || item.subtotalPapel === null
        ? null
        : anterior.subtotalPapel + item.subtotalPapel;
