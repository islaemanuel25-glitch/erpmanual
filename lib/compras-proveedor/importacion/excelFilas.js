  ...
  for (let i = 0; i < limiteCabecera; i++) {
    ...
    if (lineas.length) {
      return {
        ok: true,
        documento: {
          // ── LA TABLA CRUDA VIAJA, Y NO ES OPCIONAL ───────────────────────
          //
          // Las líneas de arriba son UNA interpretación: la que sale de
          // `identificarColumnas` y de descartar los renglones sin cantidad.
          // Sin conservar las celdas, una receta que diga "la primera columna
          // es la cantidad enviada, y si está vacía no se envió" no tendría de
          // dónde sacar ni las columnas ni los renglones descartados.
          //
          // Es el mismo archivo ya abierto: no cuesta una lectura más.
          crudo: crudoDesdeFilas({ origen: ORIGEN_CRUDO.EXCEL, filas, filaEncabezado: i }),
          numeroPedido: null,
          fecha: null,
          hayColumnaSubtotal,
