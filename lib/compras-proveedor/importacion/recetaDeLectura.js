...
/**
 * ¿ESTA RECETA NECESITA LA TABLA CRUDA PARA CUMPLIR LO QUE PROMETE?
 *
 * Sí cuando dice qué columna es cuál, o cuándo un renglón cuenta como enviado.
 * Las dos cosas se aplican sobre filas y celdas: sin la tabla no hay dónde
 * aplicarlas, y una receta que mapea columnas sobre líneas ya interpretadas no
 * corrige nada — solo parece que sí.
 *
 * NO la necesita una receta que solo aporta escalas —en qué unidad viene la
 * cantidad, si el precio es por bulto—: eso se aplica sobre las líneas ya
 * leídas, y ahí "solo escalas" es la verdad y no una degradación.
 *
 * La distinción existe para que la pantalla pueda separar los dos casos. Antes
 * los trataba igual y por eso un remapeo imposible se informaba como si fuera
 * una elección.
 */
export function recetaNecesitaTablaCruda(receta) {
  const r = receta || {};
  const columnas = Object.values(r.columnas ?? {}).filter(Boolean).length;
  return columnas > 0 || (r.enviado?.criterio ?? null) !== null;
}
