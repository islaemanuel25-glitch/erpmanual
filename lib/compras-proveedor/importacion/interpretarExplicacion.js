...
export async function interpretarExplicacion({
  ...
  const respuesta = await pedirJson({
    ...
    // ── NO SE REINTENTA SOLO. NUNCA. ───────────────────────────────────
    //
    // Lo tuvo un rato, para el 503 transitorio, y se sacó el mismo día. Con
    // VEINTE consultas por día un reintento automático gasta una que nadie
    // pidió, y puede ser justo la que falte a la tarde.
    //
    // Si falla, la pantalla ofrece "Reintentar" y aclara que usa otra consulta.
    // La decisión de gastar es de quien está delante, no del código.
    env,
    fetchImpl,
    contador,
