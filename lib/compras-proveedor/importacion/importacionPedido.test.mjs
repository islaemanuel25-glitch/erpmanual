...
test("PANTALLA. la respuesta del servidor manda sobre el costo que se ve", () => {
  // ── EL DEFECTO ────────────────────────────────────────────────────────────
  //
  // La reconciliación hacía `anterior?.precioCosto ?? detalle.precioCosto`: el
  // costo VIEJO le ganaba al que el servidor acababa de corregir. O sea que el
  // arreglo del servidor se aplicaba en la base y la pantalla seguía mostrando
  // 2.520 sobre una línea que ya valía 120.
  const producto = { precio_costo: 2100 };
  assert.equal(
    costoVisibleDeDetalle({
      detalle: { precioCosto: 120 },
      anterior: { precioCosto: 2520 },
      producto,
    }),
    120,
    "el costo anterior le ganó al que devolvió el servidor"
  );

  // Sin respuesta del servidor, el anterior es el respaldo.
  assert.equal(
    costoVisibleDeDetalle({ detalle: { precioCosto: null }, anterior: { precioCosto: 2520 }, producto }),
    2520
  );
  // Y sin ninguno de los dos, el catálogo.
  assert.equal(
    costoVisibleDeDetalle({ detalle: { precioCosto: null }, anterior: null, producto }),
    2100
  );
  // CERO ES UN NÚMERO. Con `||` esto caía al catálogo y mostraba 2.100 sobre una
  // línea que el servidor dejó en cero.
  assert.equal(
    costoVisibleDeDetalle({ detalle: { precioCosto: 0 }, anterior: { precioCosto: 2520 }, producto }),
    0,
    "un cero del servidor se descartó como si fuera 'sin dato'"
  );
});

test("PANTALLA. la reconciliación usa esa pieza y no decide por su cuenta", () => {
  const pagina = leerSinComentarios("app/modulos/compras-proveedor/nueva/page.jsx");
  assert.match(pagina, /costoVisibleDeDetalle/, "la pantalla dejó de usar la pieza que decide el costo visible");
  assert.doesNotMatch(
    pagina,
    /anterior\?\.precioCosto\s*\?\?/,
    "volvió a priorizar el costo anterior sobre el del servidor"
  );
});
