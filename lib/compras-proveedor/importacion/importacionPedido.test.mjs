...
test("cada elección manual deja un alias permanente, incluso sin código", () => {
  ...
  // ── EL CONTRATO CAMBIÓ A PROPÓSITO, DOS VECES ──────────────────────────
  //
  // 1. UN RENGLÓN CON CÓDIGO Y DESCRIPCIÓN DEJA DOS MEMORIAS, no una. Antes se
  //    guardaba solo el código, así que el proveedor nunca aprendía CÓMO LLAMA
  //    al producto y el próximo documento que viniera sin código volvía a
  //    preguntar lo mismo. Por eso el primer alias produce dos filas y el
  //    segundo —que no trae código— una: tres en total.
  //
  // 2. LA PROCEDENCIA YA NO ES UNA CONSTANTE. Antes decía siempre
  //    VINCULACION_MANUAL, incluso para lo que el motor dedujo solo. Acá nadie
  //    marcó `productoElegidoAMano`, así que son deducciones y se guardan como
  //    tales — que es lo que permite revocarlas sin tocar las humanas.
  assert.equal(aliases.length, 3);
  const claves = aliases.map((a) => a.codigoInterno);
  assert.ok(claves.includes("001234"), JSON.stringify(claves));
  assert.equal(claves.filter((c) => c.startsWith("TXT:")).length, 2, JSON.stringify(claves));
  assert.ok(aliases.every((a) => a.productoBaseId === 70));
  assert.ok(
    aliases.every((a) => a.origenAlta === "APLICACION_AUTOMATICA"),
    "una deducción se guardó como si la hubiera elegido una persona"
  );
  assert.ok(aliases.every((a) => a.confirmadaEn === null));
});

test("un renglón que una persona ELIGIÓ sí queda confirmado, y solo ese", () => {
  const cuando = new Date("2026-08-27T10:00:00.000Z");
  const aliases = aliasesDeImportacion({
    grupoId: 3,
    proveedorId: 9,
    productosPorLocal: new Map([[7, { baseId: 70 }], [8, { baseId: 80 }]]),
    confirmadaPorUsuarioId: 12,
    confirmadaEn: cuando,
    items: [
      { productoLocalId: 7, aliases: [{ codigoProveedor: "111", descripcionProveedor: "ELEGIDO", productoElegidoAMano: true }] },
      { productoLocalId: 8, aliases: [{ codigoProveedor: "222", descripcionProveedor: "DEDUCIDO" }] },
    ],
  });
  const elegido = aliases.filter((a) => a.productoBaseId === 70);
  const deducido = aliases.filter((a) => a.productoBaseId === 80);
  assert.ok(elegido.length && deducido.length);
  assert.ok(elegido.every((a) => a.confirmadaPorUsuarioId === 12 && a.confirmadaEn === cuando));
  assert.ok(
    deducido.every((a) => a.confirmadaPorUsuarioId === null),
    "guardar el documento confirmó un renglón que nadie tocó"
  );
});
