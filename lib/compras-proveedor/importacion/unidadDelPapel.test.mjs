...
// ── LA EVIDENCIA DEL PRECIO ───────────────────────────────────────────────

test("EVIDENCIA. el precio pegado al unitario sugiere UNIDAD", () => {
  const e = evidenciaPorPrecio({ precioPapel: 5050, costoUnidadErp: 5050, costoBultoErp: 50500 });
...
test("EVIDENCIA. el precio pegado al del bulto sugiere BULTO", () => {
  const e = evidenciaPorPrecio({ precioPapel: 50500, costoUnidadErp: 5050, costoBultoErp: 50500 });
...
test("EVIDENCIA. si las dos referencias están igual de lejos, NO se sugiere", () => {
  ...
  const e = evidenciaPorPrecio({ precioPapel: 5050, costoUnidadErp: 5050, costoBultoErp: 5050 });
...
test("EVIDENCIA. sin precio o sin costos vigentes no hay evidencia", () => {
  assert.equal(evidenciaPorPrecio({ precioPapel: null, costoUnidadErp: 5050 }), null);
  assert.equal(evidenciaPorPrecio({ precioPapel: 5050, costoUnidadErp: null, costoBultoErp: null }), null);
  assert.equal(evidenciaPorPrecio({ precioPapel: 0, costoUnidadErp: 5050 }), null);
});
...
test("EL FACTOR PERMANENTE NO ES UNA RAZÓN DE PRECIOS", () => {
  ...
  assert.equal(typeof e.unidad, "string");
});
