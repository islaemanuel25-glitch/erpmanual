 ...
 */
export function toleranciaDeRedondeo(cantidad) {
  const c = num(cantidad);
  if (c === null || c <= 0) return TOLERANCIA_CENTAVOS;
...
export function verificarImporteDeLinea({
  ...
  const toleranciaCentavos = toleranciaDeRedondeo(cantidad);
 ...
 * impreso— evaluadas con el precio IMPRESO, que es un dato independiente del
 * subtotal. Solo sobreviven las combinaciones en las que los dos números del
 * papel dicen lo mismo. Sobre el caso real quedan exactamente dos:
 *
...
export function representacionesQueCierran({
  ...
  const precio = num(precioImpresoPapel);
  const subtotal = num(subtotalOriginalPapel);
  ...
  for (const lectura of ["UNIDAD", "BULTO"]) {
    ...
    for (const unidad of ["UNIDAD", "BULTO"]) {
      ...
      if (Math.abs(aCentavos(importe) - aCentavos(subtotal)) > toleranciaDeRedondeo(cantidadFinal)) {
        continue;
      }
