...
function conPrecios(base, { producto, facturaPor, hayColumnaSubtotal, papelManual = null }) {
  ...
  const coherencia = verificarImporteDeLinea({
    ...
    haySubtotalImpreso: hayColumnaSubtotal === true,
    // Cuántas unidades sueltas hay detrás: es la cantidad sobre la que se hizo
    // la división del importe, y por lo tanto la que fija cuánto redondeo es
    // legítimo. Con un pack, la cantidad del pedido es más chica y usarla
    // bloqueaba facturas sanas por centavos.
    unidadesDelPapel: base.cantidadPapel,
  });
  ...
  return {
    ...
    representacionesValidas: coherencia.bloquea
      ? representacionesQueCierran({
          ...
          // La bonificación viaja: sin ella, un renglón con descuento legítimo
          // se quedaría sin ninguna representación que ofrecer.
          bonificacionPct: base.bonificacionPct,
          subtotalOriginalPapel: base.subtotalPapelOriginal,
          haySubtotalImpreso: hayColumnaSubtotal === true,
          unidadesPorBultoErp: factor,
          facturaPor,
