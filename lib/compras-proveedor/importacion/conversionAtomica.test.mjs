...
// ── EL MONTAJE CAMBIÓ, LA REGLA NO ────────────────────────────────────────
//
// Antes estos dos casos ponían `cantidadPedido: 47` dejando la base en 50. Eso
// ya no puede ocurrir: la conversión deriva de `cantidadBaseUnidades`, así que
// lo que tiene que no dar entero es LA BASE. El montaje viejo describía un
// estado —la cantidad mostrada y la base en desacuerdo— que era justamente el
// que producía las multiplicaciones de más.
//
// Lo que el candado afirma sigue siendo lo mismo: lo que no da entero no se
// redondea solo, porque 47 unidades no son ni 4 ni 5 bultos y elegir uno cambia
// lo que se le pide al proveedor.

test("47 UNIDADES A BULTOS DE 10 exige confirmación y NO redondea sola", () => {
  const l = { ...preparar(null), cantidadBaseUnidades: 47, cantidadPedido: 47, unidadPedido: "UNIDAD" };
