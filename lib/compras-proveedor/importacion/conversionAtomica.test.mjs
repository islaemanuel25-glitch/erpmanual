...
test("SI LA RECETA DICE QUE LA CANTIDAD VIENE EN BULTOS, se respeta", () => {
  // El mismo renglón, con la receta declarando que el precio es por bulto.
  ...
  assert.equal(l.unidadPedido, "BULTO");
  assert.equal(l.cantidadPedido, 50);
  // Con el precio declarado por bulto, 50 × 3.360 sigue siendo el subtotal del
  // papel: la escala es coherente aunque el número total sea otro.
