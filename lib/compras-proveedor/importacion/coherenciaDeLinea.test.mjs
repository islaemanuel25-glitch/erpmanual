...
test("EL PRECIO SE ANCLA A LA CANTIDAD DEL PAPEL, NO SE REPARTE POR REPRESENTACIÓN", () => {
  // ── EL DEFECTO QUE ESTE CANDADO ATAJA ───────────────────────────────────
  //
  // La primera versión repartía el subtotal entre la cantidad de CADA
  // representación —`subtotal ÷ cantidadFinal`— y así cerraba siempre, para
  // cualquier lectura. Ofrecía como corrección la misma lectura equivocada con
  // el precio ajustado para taparla.
  //
  // Ahora el precio efectivo se calcula UNA vez, contra la cantidad que trae el
  // papel, y esa cifra se escala por el factor del bulto. Lo que cambia entre
  // lecturas es la CANTIDAD; el precio no se recalcula para hacerla cerrar.
  //
  // Papel: 10 renglones, precio de lista $100, 10 % de bonificación, importe
  // $900. El producto viene en bultos de 10.
  const opciones = representacionesQueCierran({
    cantidadPapel: 10,
  ...
  // NINGUNA opción sale de leer "10 bultos". Con el reparto por representación,
  // esa lectura habría cerrado igual: 10 bultos a $900 ÷ 10 = $90 cada uno da
  // $900. Es exactamente la corrección que tapa el error en vez de arreglarlo.
  assert.ok(opciones.length >= 1, JSON.stringify(opciones));
  assert.ok(opciones.every((o) => o.lectura === "UNIDAD"), JSON.stringify(opciones));
  // Y el precio ofrecido es el EFECTIVO por unidad del papel, no el de lista.
  assert.equal(opciones.find((o) => o.unidad === "UNIDAD").precio, 90);
});

test("UN PAPEL QUE SE CONTRADICE NO SE ARREGLA CON UNA LECTURA", () => {
  // 10 renglones con importe $505.000. El precio efectivo es $50.500 por unidad
  // —el módulo prefiere el importe impreso sobre la columna de precio, que es el
  // arreglo de la bonificación— así que la lectura en unidades cierra.
  //
  // Lo que NO cierra es leerlo como 10 bultos: eso da $5.050.000. La lectura
  // sigue siendo lo que se discrimina, que es el punto de la función.
  const opciones = representacionesQueCierran({
    cantidadPapel: 10,
  ...
  assert.ok(opciones.every((o) => o.lectura === "UNIDAD"), JSON.stringify(opciones));
  assert.equal(opciones.find((o) => o.unidad === "UNIDAD").precio, 50500);
});
...
test("sin subtotal no se ofrece ninguna representación", () => {
  // Sin importe del renglón no hay contra qué comprobar, así que no se puede
  // afirmar que ninguna representación sea correcta.
  assert.deepEqual(
    representacionesQueCierran({ cantidadPapel: 10, precioImpresoPapel: 10, subtotalOriginalPapel: null }),
    []
  );
  // Y sin cantidad tampoco: no hay con qué armar ninguna escala.
  assert.deepEqual(
    representacionesQueCierran({ cantidadPapel: null, precioImpresoPapel: 10, subtotalOriginalPapel: 100 }),
    []
  );
});

test("SIN PRECIO IMPRESO PERO CON SUBTOTAL, la representación sale igual", () => {
  // Una planilla puede traer solo cantidad e importe. El precio efectivo sale de
  // dividir, que es el primer escalón de la prioridad, y la escala se sigue
  // pudiendo comprobar. Antes esto devolvía lista vacía porque exigía el precio
  // impreso, y una planilla así se quedaba sin ninguna salida que ofrecer.
  const opciones = representacionesQueCierran({
    cantidadPapel: 10,
    precioImpresoPapel: null,
    subtotalOriginalPapel: 900,
    haySubtotalImpreso: true,
    unidadesPorBultoErp: 10,
    facturaPor: "UNIDAD",
