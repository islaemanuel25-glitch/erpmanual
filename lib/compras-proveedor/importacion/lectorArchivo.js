...
import { ORIGEN_CRUDO, crudoDesdeFilas, crudoUtilizable } from "./documentoCrudo.js";
...
export async function leerArchivoDePedido({
  ...
  if (MIMES_VISUALES.has(mime) || ["jpg", "jpeg", "png", "webp", "heic", "heif", "pdf"].includes(extension)) {
    ...
    return leerVisual({ bytes, mime: mimeSeguro, env, fetchImpl, ahora, crearSenal, contador });
  }
  return fallo("ARCHIVO_NO_SOPORTADO", "El formato no es compatible. Usá foto, PDF, XLSX o XLS.");
}

/**
 * VOLVER A TRANSCRIBIR LA TABLA, Y NADA MÁS.
 *
 * ── POR QUÉ EXISTE ────────────────────────────────────────────────────────
 *
 * El 2026-08-27, con una foto real, la pantalla mostró "Solo escalas: no hay
 * tabla cruda". El modelo había leído los renglones pero NO había transcripto
 * la tabla, así que la explicación no tenía sobre qué aplicarse: no podía
 * corregir una columna ni recuperar un renglón descartado. La pantalla lo
 * informaba y seguía igual, que es la parte mala — degradaba en silencio a algo
 * que no es la funcionalidad prometida.
 *
 * El Excel no tiene este problema: ahí la tabla cruda sale del archivo y no de
 * un modelo. Es exclusivo del camino de foto y PDF.
 *
 * ── POR QUÉ ES UN PEDIDO APARTE Y NO OTRA LECTURA COMPLETA ────────────────
 *
 * Porque pide MENOS. La lectura completa le pide al modelo transcribir la tabla
 * *además* de interpretar los renglones, contestar tres preguntas de encabezado
 * y sacar el total; con un papel de treinta renglones eso compite por el
 * presupuesto de salida y la transcripción es lo primero que se cae. Acá lo
 * único que se pide es la tabla, con el MISMO texto de instrucción que ya usa la
 * lectura completa —no una copia con otras palabras, que es lo que haría que un
 * día digan cosas distintas—.
 *
 * NO reinterpreta ni toca las líneas ya leídas: devuelve la tabla y se va. Quien
 * decide qué hacer con ella es la receta.
 */
export async function transcribirTablaDelArchivo({
  archivo,
  contador = null,
  env = process.env,
  fetchImpl = globalThis.fetch,
  ...
  if (!archivo || typeof archivo.arrayBuffer !== "function") {
    return fallo("ARCHIVO_REQUERIDO", "Elegí de nuevo la foto o el PDF para volver a transcribirlo.");
  }
  if (Number(archivo.size) > LIMITE_BYTES) {
    return fallo("ARCHIVO_GRANDE", "El archivo supera el máximo de 15 MB.");
  }
  ...
  // Un Excel no pasa por acá: su tabla cruda es determinista y ya la tiene.
  if (MIMES_EXCEL.has(mime) || ["xlsx", "xls"].includes(extension)) {
    return fallo(
      "TRANSCRIPCION_INNECESARIA",
      "Un Excel ya trae sus filas y columnas: no hace falta transcribirlo."
    );
  }
  if (!MIMES_VISUALES.has(mime) && !["jpg", "jpeg", "png", "webp", "heic", "heif", "pdf"].includes(extension)) {
    return fallo("ARCHIVO_NO_SOPORTADO", "El formato no es compatible. Usá foto, PDF, XLSX o XLS.");
  ...
  const respuesta = await pedirJson({
    instrucciones: instruccionesDeTranscripcion(),
    esquema: esquemaDeTabla(),
    adjuntos: [{ mime: mime || mimePorExtension(extension), bytes }],
  ...
  const crudo = crudoVisual(respuesta.datos?.tabla);
  if (!crudoUtilizable(crudo)) {
    // Y ACÁ NO SE FINGE. Que el segundo pedido tampoco haya traído la tabla es
    // un resultado, no un motivo para seguir como si nada: lo que sigue después
    // de esto es pedirle a alguien que elija otra foto, no aplicar media receta.
    return fallo(
      "SIN_TABLA",
      "No se pudo transcribir la tabla de esta foto. Probá con una imagen más nítida, " +
        "derecha y con la tabla entera dentro del cuadro."
    );
  }
  return { ok: true, crudo };
}

/** Solo la tabla. Sin líneas, sin totales, sin preguntas de encabezado. */
function esquemaDeTabla() {
  return {
    type: "OBJECT",
    properties: {
      tabla: {
...
/**
 * El texto del pedido de transcripción.
 *
 * Reusa `TRANSCRIPCION_DE_LA_TABLA` en vez de escribir la regla otra vez: el día
 * que se corrija algo ahí —qué se hace con una celda vacía, qué pasa con los
 * renglones de pie— tiene que corregirse para los dos caminos a la vez.
 */
function instruccionesDeTranscripcion() {
  return [
    "Este documento es un remito, una factura o una planilla de pedido a un proveedor.",
    "Tu única tarea es TRANSCRIBIR la tabla. No interpretes, no calcules y no decidas qué columna es cuál.",
    TRANSCRIPCION_DE_LA_TABLA,
    "Si la hoja está girada, enderezala mentalmente antes de transcribir. Si la tabla no tiene bordes, guiate por la alineación de las columnas.",
  ].join("\n\n");
}
 ...
 */
async function leerVisual({ bytes, mime, env, fetchImpl, ahora = Date.now, crearSenal, contador = null }) {
  if (!env.GEMINI_API_KEY) {
    return fallo("LECTOR_NO_CONFIGURADO", "La lectura de fotos y PDF no está configurada. Excel sí puede importarse.");
  }
  ...
  const primero = await unaLectura({
    bytes, mime, env, fetchImpl, crearSenal, contador,
...
async function unaLectura({ bytes, mime, env, fetchImpl, instrucciones, timeoutMs, crearSenal, contador = null }) {
