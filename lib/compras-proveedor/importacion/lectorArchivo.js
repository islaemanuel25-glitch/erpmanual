...
export async function leerArchivoDePedido({
  ...
  if (MIMES_VISUALES.has(mime) || ["jpg", "jpeg", "png", "webp", "heic", "heif", "pdf"].includes(extension)) {
    ...
    return leerVisual({ bytes, mime: mimeSeguro, env, fetchImpl, ahora });
  }
 ...
 */
async function leerVisual({ bytes, mime, env, fetchImpl, ahora = Date.now }) {
...
async function unaLectura({ bytes, mime, env, fetchImpl, instrucciones, timeoutMs }) {
  ...
  try {
    respuesta = await fetchImpl(`${BASE_GEMINI}/${modelo}:generateContent`, {
      ...
      signal: AbortSignal.timeout(Math.max(1_000, Number(timeoutMs) || TIMEOUT_LECTURA_MS)),
    });
  } catch (error) {
    return fallo(
