...
// ── ACÁ VIVÍA "el botón de editar producto está en el DETALLE del pedido" ───
//
// SE BORRÓ EL 2026-08-17 porque el botón no está en el detalle, y hacía tiempo
// que no estaba. El candado no mentía por mal escrito: afirmaba sobre dos
// archivos que existían, y ninguno de los dos se dibujaba.
//
// Miraba `components/compras-proveedor/TablaDetallePedido.jsx` —borrado en esa
// tanda por inalcanzable— y las tres piezas que la página ponía para
// alimentarlo: `ORIGENES.PEDIDO_DETALLE`, el `pedidoId` de vuelta y el permiso.
// Esas tres se borraron con él, porque su único consumidor era esa tabla.
//
// LO QUE ESTE CANDADO SE LLEVA CONSIGO, dicho entero: desde el detalle de un
// pedido ENVIADO o RECIBIDO no hay forma de ir a editar un producto. Se buscó
// alternativa en todo el repo y no hay ninguna — `ListaConciliacion` y
// `PanelComprobantes` no tienen lápiz, y `ORIGENES.PEDIDO_DETALLE` no lo produce
// ya nadie en la aplicación.
//
// La mitad del módulo que arma ese link —`linkEditarProducto` con
// `PEDIDO_DETALLE`, y su rama en `urlRetornoPedido`— NO se tocó, y sus candados
// unitarios de más arriba siguen ejerciéndola. Es código correcto que dejó de
// tener quien lo llame; el día que el lápiz vuelva, vuelve sin reescribirlo.
//
// Sigue vivo y en verde el candado de `/nueva` de más arriba —"el botón de
// editar producto mira el permiso antes de aparecer"—, que mira `CarritoPedido`
// y `nueva/page.jsx`. Ése sí está en una pantalla que se dibuja, y es el único
// lugar donde hoy se puede saltar a editar un producto desde una compra.
