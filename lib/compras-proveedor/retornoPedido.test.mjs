...
test("FORMA: el botón de editar producto está en el DETALLE del pedido", () => {
  ...
  assert.match(src, /ORIGENES\.PEDIDO_DETALLE/, "el detalle no usa el origen que le corresponde");
  assert.match(src, /pedidoId: pedido\?\.id/, "el link de vuelta no lleva el pedido");
  assert.match(
    src,
    /puedeEditarProductoP && base\?\.id \?/,
    "el botón del detalle no mira el permiso"
  );
  assert.match(src, /permisosP\.includes\("productos\.editar"\)/);
});
