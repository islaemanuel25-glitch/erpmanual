...
// ── EL PASE AL RESPALDO SE PIDE, DESDE EL 2026-08-27 ──────────────────────
//
// Pasar al respaldo es una SEGUNDA consulta, a otro modelo. Con veinte por día
// eso es medio presupuesto de lectura gastado sin que nadie lo pida, y encima
// se dispara justo cuando el titular se quedó sin cuota. El mecanismo sigue
// entero y estos candados lo siguen probando; lo que cambió es el default.
//
// El candado de que el default es NO PASAR está más abajo, y es el que hay que
// mirar si alguien vuelve a encenderlo.
const leer = (titular, respaldo) =>
  leerConCadena({
    cadena: armarCadena({ titular, respaldo, env: {} }),
    archivo: {},
    receta: {},
    env: { IA_RESPALDO_AUTOMATICO: "1" },
  });

// ── Cuándo se pasa y cuándo no ─────────────────────────────────────────────

test("POR DEFECTO NO SE PASA AL RESPALDO: gastaría una consulta más", async () => {
  // El candado que fija el cambio del 2026-08-27. Sin él, el default podría
  // volver a encenderse y nadie se enteraría hasta ver la cuota en cero a media
  // tarde — que es exactamente lo que pasó.
  const r = await leerConCadena({
    cadena: armarCadena({ titular: "t-sin-cuota", respaldo: "r-ok", env: {} }),
    archivo: {},
    receta: {},
    env: {},
  });
  assert.equal(r.usoRespaldo, false, "pasó al respaldo sin que nadie lo habilitara");
  assert.equal(r.ok, false, "y devuelve el fallo del titular, sin taparlo");
  assert.equal(r.intentos.length, 1, "hubo más de una llamada");
});
