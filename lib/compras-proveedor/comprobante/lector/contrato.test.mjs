...
test("NINGÚN CAMPO DERIVABLE ES OBLIGATORIO — el repaso completo del esquema", () => {
  // El criterio, afinado midiendo: lo peligroso no es que un campo sea
  // obligatorio, es que su valor SE PUEDA DERIVAR de los otros. Medido sobre el
  // mismo papel y el mismo modelo, el contraste es limpio:
  //
  //   · identidad (tipo, punto de venta, número, fecha): obligatorios, NO
  //     derivables de nada → volvieron en null las cinco veces que se midió.
  //   · total: obligatorio y derivable (la suma de las líneas) → volvió con la
  //     suma exacta las tres veces, y cerró con diferencia cero.
  //
  // Un campo derivable obligatorio no es un campo mal puesto: es una orden de
  // calcular justo el número contra el que se iba a comparar.
  const e = esquemaDeSalida(RECETA_DAS);

  // Los tres de la línea se despejan uno del otro: cantidad × unitario = subtotal.
  assert.deepEqual(e.properties.lineas.items.required, ["descripcion"]);

  // La identidad no se deriva, pero exigirla es presión sin ganancia.
  assert.equal(e.properties.identidad.required, undefined);

  // El pie tampoco.
  assert.equal(e.properties.pie.required, undefined);

  // Lo que SÍ sigue siendo obligatorio, y por qué está bien que lo sea:
  //   · `lineas` y `pie` son contenedores: pueden venir vacíos.
  //   · `lineasEnElPapel` y `hayTotalImpreso` son las preguntas aparte. Las dos
  //     respuestas posibles del booleano llevan a estados que NO proponen costos
  //     —SIN_TOTAL si dice que no, MAL_LEIDO si dice que sí y no lo trajo—, así
  //     que una adivinada no abre ninguna puerta.
  assert.deepEqual(e.required, [
    "identidad", "lineas", "pie", "lineasEnElPapel", "hayTotalImpreso",
  ]);
});

test("el prompt prohíbe completar el importe por renglón multiplicando", () => {
  const t = instruccionesDesdeReceta(RECETA_DAS);
  assert.match(t, /Si NO lo imprime, dejá el campo afuera/i);
  assert.match(t, /No lo completes multiplicando/i);
  // Y explica el porqué, que es lo que lo hace aplicable a un caso no nombrado.
  assert.match(t, /compara un número contra sí mismo/i);
});
