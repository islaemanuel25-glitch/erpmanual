...
// cantidad de líneas—, o sea la forma exacta del agujero del total. No se sacó:
// se dejó con su defensa en el prompt y con los dos números guardados separados,
// para poder auditarlo con datos dentro de veinte facturas. Si nunca difieren,
// el prompt no está funcionando.
...
test("cuando difieren, quedan los dos y se puede ver la diferencia", () => {
  const r = pasarPorLaPuerta({
    lectura: lecturaDeMauro({ pie: { neto: 0, total: 0 }, lineasEnElPapel: 31 }),
    receta: RECETA_MAURO,
    recetaVersion: 1,
  });
...
test("si el lector no informó el conteo, eso queda en null y no en la cantidad de líneas", () => {
  ...
  const r = pasarPorLaPuerta({
    lectura: normalizarLectura({
      identidad: {}, lineas: LINEAS_MAURO, pie: { neto: 0, total: 0 },
      hayTotalImpreso: false,
