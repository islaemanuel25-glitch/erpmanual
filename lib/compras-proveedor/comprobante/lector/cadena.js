...
/**
 * ── EL PASE AL RESPALDO ESTÁ APAGADO POR DEFECTO DESDE EL 2026-08-27 ───────
 *
 * Pasar al respaldo es hacer UNA SEGUNDA CONSULTA, a otro modelo. Mientras la
 * cuota no importaba, era una red de contención barata. Con VEINTE consultas
 * por día es la mitad del presupuesto de una lectura gastada sin que nadie lo
 * pida — y el pase se dispara justamente cuando el titular se quedó sin cuota,
 * que es cuando menos conviene gastar.
 *
 * No se borró el mecanismo: está probado y el día que la cuota deje de ser el
 * problema alcanza con poner `IA_RESPALDO_AUTOMATICO=1`. Lo que cambió es el
 * default, que es lo que decide qué pasa cuando nadie configuró nada.
 */
export function respaldoAutomaticoHabilitado(env = process.env) {
  return String(env?.IA_RESPALDO_AUTOMATICO ?? "") === "1";
}
 ...
 */
export async function leerConCadena({ cadena, archivos, receta, proveedorNombre = null, env = process.env } = {}) {
  ...
  if (!titular?.ok) {
    ...
    return {
      ...
      usoRespaldo: false,
      porQuePaso: null,
      // NO HUBO LLAMADA: falta configuración, no se gastó cuota.
      intentos: [],
    };
  }
  ...
  if (primero.ok) {
    return {
      ...primero, usoRespaldo: false, porQuePaso: null, lector: titular.lector.nombre,
      intentos: [{ lector: titular.lector.nombre, ok: true, motivo: null }],
    };
  }

  // El pase gasta una consulta más. Apagado por defecto: ver arriba.
  if (!respaldoAutomaticoHabilitado(env) || !correspondePasarAlRespaldo(primero.motivo)) {
    return {
      ...primero, usoRespaldo: false, porQuePaso: null, lector: titular.lector.nombre,
      intentos: [{ lector: titular.lector.nombre, ok: false, motivo: primero.motivo }],
    };
  }
  ...
  return {
    ...
    // LAS DOS LLAMADAS, y las dos gastaron cuota. La del titular gastó aunque
    // haya fallado: el 429 se cuenta igual que un acierto contra el tope diario,
    // y contar solo la que salió bien mostraría más lecturas disponibles de las
    // que hay — justo el número que no se puede equivocar.
    intentos: [
      { lector: titular.lector.nombre, ok: false, motivo: primero.motivo },
      { lector: respaldo.lector.nombre, ok: segundo.ok === true, motivo: segundo.ok ? null : segundo.motivo },
    ],
  };
}
