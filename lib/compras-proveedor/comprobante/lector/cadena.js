 ...
 */
export async function leerConCadena({ cadena, archivo, receta, proveedorNombre = null } = {}) {
  ...
  const primero = await titular.lector.leer({ archivo, receta, proveedorNombre });
  if (primero.ok) {
  ...
  const segundo = await respaldo.lector.leer({ archivo, receta, proveedorNombre });
  return {
    ...segundo,
    usoRespaldo: true,
