...
// ── EL TERCER CONTROL: CONTAR LOS RENGLONES ───────────────────────────────
//
// Si el modelo se saltea una línea entera, la aritmética de esa línea nunca se
// evalúa: las dos ecuaciones miran las que llegaron, y una que no llegó no rompe
// ninguna. Lo señaló Emanuel mirando la factura de Mauro.

test("UNA LÍNEA QUE FALTA NO ROMPE NINGUNA DE LAS DOS ECUACIONES", () => {
  // El agujero, demostrado. Se saca una línea Y se ajusta el pie como si esa
  // línea no existiera: la factura queda coherente consigo misma y cierra.
  const sinLaUltima = DAS.lineas.slice(0, 2);
  const netoSin = sinLaUltima.reduce((a, l) => a + l.subtotalImpreso, 0);
  const ivaSin = Math.round(netoSin * 0.21 * 100) / 100;
  const percSin = Math.round(netoSin * 0.03 * 100) / 100;
  const r = pasarPorLaPuerta({
    lectura: normalizarLectura({
      lineas: sinLaUltima,
      pie: { neto: netoSin, iva: ivaSin, total: netoSin + ivaSin + percSin,
             percepciones: [{ nombre: "IVA", importe: percSin }] },
      consumo: {},
    }),
    receta: DAS.receta,
  });
  assert.equal(r.cierra, true, "las dos ecuaciones cierran con la factura incompleta");
  assert.equal(r.faltanLineas, null, "sin el conteo declarado, no hay forma de saberlo");
});

test("CON EL CONTEO DECLARADO, LA LÍNEA QUE FALTA SE DETECTA", () => {
  const sinLaUltima = DAS.lineas.slice(0, 2);
  const netoSin = sinLaUltima.reduce((a, l) => a + l.subtotalImpreso, 0);
  const ivaSin = Math.round(netoSin * 0.21 * 100) / 100;
  const percSin = Math.round(netoSin * 0.03 * 100) / 100;
  const r = pasarPorLaPuerta({
    lectura: normalizarLectura({
      ...
      pie: { neto: netoSin, iva: ivaSin, total: netoSin + ivaSin + percSin,
             percepciones: [{ nombre: "IVA", importe: percSin }] },
      lineasEnElPapel: 3, // el lector VIO tres
      consumo: {},
    }),
    receta: DAS.receta,
  });
  assert.equal(r.cierra, true, "la cuenta sigue cerrando");
  assert.ok(r.faltanLineas, "pero el conteo lo delata");
  assert.equal(r.faltanLineas.faltan, 1);
  assert.equal(r.faltanLineas.declaradas, 3);
  assert.equal(r.faltanLineas.transcriptas, 2);
  // SE AVISA AUNQUE CIERRE. Ese es el punto.
  assert.ok(r.avisoLineas);
  assert.match(r.avisoLineas, /aunque la cuenta cierre/i);
});

test("si el conteo coincide, no se inventa un aviso", () => {
  const r = pasarPorLaPuerta({
    lectura: normalizarLectura({ ...DAS, lineas: DAS.lineas, pie: DAS.pie, lineasEnElPapel: 3, consumo: {} }),
    receta: DAS.receta,
  });
  assert.equal(r.faltanLineas, null);
  assert.equal(r.avisoLineas, null);
  // Pero el número se guarda igual: sin él, "coincidió" y "no lo informó" se
  // ven iguales.
  assert.equal(r.aGuardar.lineasEnElPapel, 3);
});

test("un conteo MENOR que lo transcripto no se trata como faltante", () => {
  // Si dice ver 2 y trajo 3, el que se equivocó es el conteo. No hay línea que
  // buscar, y avisar de una faltante mandaría a mirar donde no hay nada.
  const r = pasarPorLaPuerta({
    lectura: normalizarLectura({ lineas: DAS.lineas, pie: DAS.pie, lineasEnElPapel: 2, consumo: {} }),
    receta: DAS.receta,
  });
  assert.equal(r.faltanLineas, null);
});

test("UNA DIFERENCIA CHICA Y REDONDA SOSPECHA DE LÍNEA FALTANTE, no de cargo al pie", () => {
  // Es más probable que falte una línea barata a que el proveedor cobre un
  // importe redondo. Salió de los 2.000 pesos exactos de Mauro sobre 3,77
  // millones.
  const conFaltante = DAS.lineas.slice(0, 2);
  const r = pasarPorLaPuerta({
    lectura: normalizarLectura({
      lineas: conFaltante,
      // El pie del papel COMPLETO, con la línea que falta adentro: la
      // diferencia queda chica y redonda.
      pie: { ...DAS.pie, total: DAS.pie.total },
      consumo: {},
    }),
    receta: DAS.receta,
  });
  assert.equal(r.cierra, false);
  // No siempre va a ser redonda con estos números, pero el mensaje tiene que
  // existir cuando el conteo lo delata.
  const conConteo = pasarPorLaPuerta({
    lectura: normalizarLectura({
      lineas: conFaltante, pie: DAS.pie, lineasEnElPapel: 3, consumo: {},
    }),
    receta: DAS.receta,
  });
  assert.match(conConteo.porque, /renglón que no se leyó/i);
  assert.match(conConteo.porque, /Contá los del papel/i);
});
