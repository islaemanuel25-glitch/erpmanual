...
export function horaLocalDeReposicion(ahora, huso = HUSO_POR_DEFECTO, husoLocal = "America/Argentina/Buenos_Aires") {
...
export function cuotaDelDia({ llamadas = [], modelos = [], ahora = new Date(), huso = HUSO_POR_DEFECTO, limite = LIMITE_POR_MODELO_POR_DIA } = {}) {
  ...
  const porModelo = modelos.filter(Boolean).map((m) => {
    ...
    return { modelo: m, usadas, quedan: Math.max(0, limite - usadas), limite };
  });
