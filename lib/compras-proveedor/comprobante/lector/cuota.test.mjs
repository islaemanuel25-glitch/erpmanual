...
test("una llamada FALLIDA gasta cuota igual que una buena", () => {
  ...
  // Los motivos son de falla PERO NO de cuota agotada: ese apaga el modelo
  // entero y taparía lo que este candado quiere ver, que es la resta.
  const c = cuotaDelDia({
    llamadas: [
      llamada(TITULAR, "2026-08-12T18:00:00Z", false, "TARDO_DEMASIADO"),
...
test("si el proveedor ya dijo que se agotó, quedan CERO aunque la cuenta diga otra cosa", () => {
  // El caso que lo motivó: la tabla arrancó vacía con el titular ya agotado por
  // las pruebas del día. Sin esto la pantalla decía "quedan 40" con la mitad en
  // cero. Y vuelve a pasar cada vez que algo consuma cuota con la misma clave
  // por fuera de esta aplicación.
  const ahora = new Date("2026-08-12T20:00:00Z");
  const c = cuotaDelDia({
    llamadas: [llamada(TITULAR, "2026-08-12T19:00:00Z", false, "CUOTA_AGOTADA")],
    modelos: MODELOS,
    ahora,
  });
  assert.equal(c.porModelo[0].usadas, 1, "por nuestra cuenta fue una sola");
  assert.equal(c.porModelo[0].quedan, 0, "pero él dijo que no hay más");
  assert.equal(c.porModelo[0].loDijoElProveedor, true);
  assert.equal(c.porModelo[1].quedan, 20, "el respaldo no se contagia");
  assert.equal(c.quedan, 20);
});

test("un agotado de AYER no apaga el modelo de hoy", () => {
  // 2026-08-11 a las 19:00 UTC es antes del corte del 12: quedó del día anterior.
  const c = cuotaDelDia({
    llamadas: [llamada(TITULAR, "2026-08-11T19:00:00Z", false, "CUOTA_AGOTADA")],
    modelos: [TITULAR],
    ahora: new Date("2026-08-12T20:00:00Z"),
  });
  assert.equal(c.porModelo[0].quedan, 20);
  assert.equal(c.porModelo[0].loDijoElProveedor, false);
});
