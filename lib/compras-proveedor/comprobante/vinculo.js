...
//
// En recepción SOLO se vincula automáticamente por código exacto o alias
// exacto. El importador de borradores puede habilitar, de forma explícita, la
// escalera por terminación única: ese flujo muestra el origen en pantalla y
// aprende el vínculo confirmado para el proveedor.
//
...
export const ORIGEN_VINCULO = Object.freeze({
  ...
  CODIGO_APROXIMADO: "CODIGO_APROXIMADO",
...
export const TEXTO_ORIGEN = Object.freeze({
  ...
  [ORIGEN_VINCULO.CODIGO_APROXIMADO]: "Vinculado por la terminación del código del proveedor",
...
export function buscarCandidatos({
  ...
  universoProveedor = [],
  catalogo = [],
  permitirCodigoAproximado = false,
} = {}) {
  ...
  if (codigo) {
    if (permitirCodigoAproximado) {
      const porEscalera = macheePorCodigo(indexarCodigosProveedor(vinculos), {
        codigoCrudo: linea?.codigoProveedor,
        codigoNormalizado: codigo,
      });
      if (porEscalera.candidatos.length === 1) {
        const origen = porEscalera.tipo === TIPO_COINCIDENCIA.CODIGO_INTERNO
          ? ORIGEN_VINCULO.CODIGO_PROVEEDOR
          : ORIGEN_VINCULO.CODIGO_APROXIMADO;
        return armar(origen, [candidatoDeVinculo(porEscalera.candidatos[0])], {
          // Solo el importador de borradores habilita esta escalera. Recepción
          // no la pide y conserva su contrato: allí solo código y alias exactos
          // vinculan sin preguntar.
          automatico: true,
          codigo,
          texto,
          sufijoDigitos: esCoincidenciaPorSufijo(porEscalera.tipo)
            ? porEscalera.sufijoDigitos
            : null,
        });
      }
      if (porEscalera.tipo === TIPO_COINCIDENCIA.AMBIGUA) {
        return armar(
          ORIGEN_VINCULO.SIN_CANDIDATOS,
          porEscalera.candidatos.map(candidatoDeVinculo),
          {
            codigo,
            texto,
            problema: `El código ${codigo} coincide con más de un producto. Elegí cuál corresponde.`,
          }
        );
      }
    }
...
function armar(origen, candidatos, extra = {}) {
  const { automatico = false, ...datosExtra } = extra;
  const auto = (automatico || esAutomatico(origen)) && candidatos.length === 1;
  return {
    origen,
    texto: TEXTO_ORIGEN[origen],
    candidatos,
    // Solo uno y con una política automática explícita. Recepción conserva el
    // contrato estricto; el importador puede sumar la terminación única.
