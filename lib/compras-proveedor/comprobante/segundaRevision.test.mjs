...
test("la segunda revisión la hacen dueño, encargado y administrador", () => {
  for (const rol of ["DUENO", "encargado", "Admin"]) {
    assert.equal(puedeHacerSegundaRevision(rol), true, rol);
  }
  for (const rol of ["CAJERO", "REPOSITOR", "", null, undefined]) {
    assert.equal(puedeHacerSegundaRevision(rol), false, String(rol));
  }
});
