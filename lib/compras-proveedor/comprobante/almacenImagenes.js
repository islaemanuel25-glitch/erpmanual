...
//
// DÓNDE VIVEN LAS FOTOS DE LOS COMPROBANTES, Y CÓMO SE COMPRUEBA QUE EL LUGAR EXISTE.
//
...
//    intenta subir un comprobante un sábado a la mañana.
...
export const NOMBRE_CENTINELA = ".volumen-comprobantes";

export const MOTIVO_ALMACEN = Object.freeze({
  SIN_RUTA:
    "No está configurada la ruta del volumen de comprobantes. Sin eso no se sabe dónde escribir, " +
  ...
  SIN_ESPACIO:
    "No hay espacio en el volumen de comprobantes: la foto NO se guardó. No se borra nada viejo " +
    "para hacer lugar — las fotos que ya están cumplen sus siete días. Descargá el paquete y " +
    "avisá para ampliar el volumen.",
});

/**
 * ¿Este error de escritura es que el volumen se llenó?
 *
 * SI SE LLENA, SE RECHAZA LA SUBIDA. Nunca se borra lo más viejo para hacer
 * lugar: la ventana promete siete días, y adelantarse la rompe EN SILENCIO —
 * el que subió una foto ayer creería tener seis días y no los tendría, sin que
 * nada se lo diga. Rechazar también molesta, pero molesta de frente y en el
 * momento, que es cuando todavía se puede hacer algo.
 *
 * `EDQUOT` entra igual que `ENOSPC`: para el que sube son lo mismo —no hay
 * lugar— y distinguirlos solo serviría para dar dos mensajes donde hace falta
 * uno claro.
 */
export function esFaltaDeEspacio(error) {
  const codigo = error && typeof error === "object" ? error.code : null;
  return codigo === "ENOSPC" || codigo === "EDQUOT";
}
 ...
 *
 * Se arma con el proveedor, el punto de venta y el número para que un comprobante
 ...
 */
export function nombreDeArchivo({ comprobanteId, proveedorNombre, puntoVenta, numero, orden = 1, extension = "jpg" } = {}) {
  const limpio = (s) =>
    String(s ?? "")
      .normalize("NFD")
      .replace(/[̀-ͯ]/g, "")
  ...
  const partes = [
    ...
    // EL ORDEN VA EN EL NOMBRE. Sin esto, las dos fotos de una factura de dos
    // páginas se escriben en el mismo archivo y la segunda pisa a la primera:
    // se perdería media factura sin ningún error.
    "p" + String(Number(orden) || 1),
  ];
