...
export function resumenEnCriollo(fila) {
  ...
  if (r.percepciones.length) {
    // El plural de "percepción" PIERDE EL ACENTO: percepciones, no percepciónes.
    // Salió agregándole "es" a la palabra en singular, que en castellano no
    // funciona con las terminadas en -ción. Se escribe cada forma entera.
    const n = r.percepciones.length;
    partes.push(`${n} ${n === 1 ? "percepción" : "percepciones"}`);
  }
