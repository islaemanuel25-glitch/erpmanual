...
const MONTADO = { ruta: "/vol/facturas", existeDirectorio: true, existeCentinela: true, esEscribible: true };
...
test("el centinela tiene nombre fijo y empieza con punto", () => {
  ...
  // factura, y fijo para que el que aprovisiona y el que verifica busquen lo
  // mismo.
  assert.equal(NOMBRE_CENTINELA, ".volumen-facturas");
});

// ── El nombre del archivo ──────────────────────────────────────────────────

test("EL NOMBRE ATA EL ARCHIVO AL COMPROBANTE", () => {
  // Para poder encontrar una factura suelta sin abrir el paquete entero.
  ...
  assert.equal(n, "Dyssa_0011_00794708_42.jpg");
});
