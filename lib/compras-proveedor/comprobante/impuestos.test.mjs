...
test("el final de una línea con impuesto interno lo incluye", () => {
  ...
  const final = finalUnitarioSinPercepcionesCentavos({ netoUnitario: 2580.57, internoUnitario: 535.47, receta: DYSSA.receta });
...
test("una línea sin impuesto interno solo suma IVA", () => {
  const final = finalUnitarioSinPercepcionesCentavos({ netoUnitario: 1279.37, receta: DYSSA.receta });
...
test("LAS PERCEPCIONES ENTRAN EN EL COSTO UNITARIO", () => {
  // Decisión de negocio de Emanuel del 2026-08-11, con su motivo y lo que
  // resigna escritos en el módulo. Este candado la fija: si alguien las sacara
  // "porque son recuperables", se pone rojo y tiene que leer el porqué.
  const r = verificarComprobante(DYSSA);
  const l1 = r.lineas[0];
  assert.ok(l1.percepcionUnitariaCentavos > 0, "la línea tiene que recibir su parte");
  assert.equal(
    l1.finalUnitarioCentavos,
    l1.finalSinPercepcionCentavos + l1.percepcionUnitariaCentavos,
    "el final tiene que incluirla"
  );
});

test("se reparten EN PROPORCIÓN AL NETO, y la suma cierra exacta", () => {
  // Es el único reparto que hace que las líneas sumen el pie. El resto de
  // redondeo va a la línea de mayor neto: si se dejara caer, el comprobante no
  // cerraría por un defecto nuestro y no del proveedor.
  for (const f of [DYSSA, DAS]) {
    const r = verificarComprobante(f);
    const sumaLineas = r.lineas.reduce((a, l) => a + l.percepcionLineaCentavos, 0);
    assert.equal(sumaLineas, r.percepcionesCentavos, `${f.nombre}: el reparto no puede perder ni inventar centavos`);
  }
});

test("una línea más grande recibe más percepción que una chica", () => {
  const r = verificarComprobante(DYSSA);
  const porNeto = [...r.lineas].sort((a, b) => b.subtotalCentavos - a.subtotalCentavos);
  assert.ok(
    porNeto[0].percepcionLineaCentavos > porNeto[porNeto.length - 1].percepcionLineaCentavos,
    "el reparto tiene que seguir al neto"
  );
});

test("sin percepciones en la receta, el costo no las inventa", () => {
  const sinPerc = verificarComprobante({ ...DYSSA, receta: { ...DYSSA.receta, percepciones: [] } });
  assert.equal(sinPerc.lineas[0].percepcionUnitariaCentavos, 0);
  assert.equal(sinPerc.lineas[0].finalUnitarioCentavos, sinPerc.lineas[0].finalSinPercepcionCentavos);
});

// ── LA CONSECUENCIA OPERATIVA, que Emanuel señaló ──────────────────────────

test("UNA FACTURA SIN PERCEPCIONES DESPUÉS DE UNA CON PERCEPCIONES: cuánto cae", () => {
  // La consecuencia operativa de meter las percepciones en el costo: el mismo
  // producto, al mismo precio de lista, comprado una vez con percepciones y otra
  // sin ellas, muestra una BAJA que no es ni error de lectura ni precio nuevo.
  //
  // ── LO QUE SE MIDIÓ, Y CORRIGE LO QUE PARECÍA ──────────────────────────
  //
  // Las dos percepciones de DYSSA suman 6 % DEL NETO, pero la comparación de
  // costos ocurre sobre el FINAL, que ya trae IVA e impuesto interno adentro.
  // El 6 % se diluye dos veces: contra el IVA y contra sí mismo.
  //
  //     caída = p / (1 + alícuota + p)  →  6 / 127 = 4,72 %
  //
  // Medido en las siete líneas de DYSSA: entre 4,06 % y 4,72 %. Las que tienen
  // impuesto interno caen menos, porque el interno diluye todavía más.
  //
  // O sea: con estos números NO llega al umbral de 5 % y NO marca. Pero por
  // poco, y esa cercanía es el punto: basta con que las percepciones pasen de
  // ~6,4 %, o que el producto tenga IVA al 10,5 % en vez de 21, para cruzarlo.
  //
  // Cuando eso pase, se resuelve con el umbral POR PROVEEDOR, que ya existe: a
  // un proveedor que a veces percibe y a veces no, se le sube el suyo.
  const conPerc = verificarComprobante(DYSSA);
  const sinPerc = verificarComprobante({ ...DYSSA, receta: { ...DYSSA.receta, percepciones: [] } });

  const caidas = conPerc.lineas.map((l, i) => {
    const c = l.finalUnitarioCentavos;
    const s = sinPerc.lineas[i].finalUnitarioCentavos;
    return ((s - c) / c) * 100;
  });

  for (const [i, caida] of caidas.entries()) {
    assert.ok(caida < 0, `L${i + 1}: sin percepciones el costo tiene que bajar`);
    assert.ok(Math.abs(caida) > 4, `L${i + 1}: la caída (${caida.toFixed(2)} %) tiene que ser sustancial`);
    assert.ok(
      Math.abs(caida) < 15,
      `L${i + 1}: NO puede llegar al umbral de sospecha de 15 %, o se confundiría con una mala lectura`
    );
  }

  // La línea sin impuesto interno es la que más cae, y da la fórmula exacta.
  const sinInterno = conPerc.lineas.findIndex((l) => l.internoUnitarioCentavos === 0);
  assert.equal(
    Number(Math.abs(caidas[sinInterno]).toFixed(2)),
    Number((6 / 1.27).toFixed(2)),
    "sin impuesto interno la caída tiene que dar p/(1+alícuota+p)"
  );
});

test("EL PUNTO DESDE EL QUE LA CAÍDA SÍ CRUZA EL UMBRAL", () => {
  // Deja registrado dónde está el borde, para que no haya que redescubrirlo.
  // Con IVA al 21 %, las percepciones tienen que pasar de ~6,37 % para que la
  // caída llegue al 5 %. Con 6 % justo, no llega — por eso hoy no marca.
  const caidaCon = (pct, alicuota = 21) => {
    const f = { ...DYSSA, receta: { ...DYSSA.receta, alicuotaIvaPct: alicuota, tieneImpuestoInterno: false, percepciones: [{ nombre: "X", pct }] } };
    const con = verificarComprobante(f);
    const sin = verificarComprobante({ ...f, receta: { ...f.receta, percepciones: [] } });
    const c = con.lineas[1].finalUnitarioCentavos, s = sin.lineas[1].finalUnitarioCentavos;
    return Math.abs(((s - c) / c) * 100);
  };
  assert.ok(caidaCon(6) < 5, `con 6 % no cruza (${caidaCon(6).toFixed(2)} %)`);
  assert.ok(caidaCon(7) > 5, `con 7 % sí cruza (${caidaCon(7).toFixed(2)} %)`);
  // Y con IVA al 10,5 %, el mismo 6 % ya cruza: la alícuota diluye menos.
  assert.ok(caidaCon(6, 10.5) > 5, `con IVA 10,5 % y 6 % de percepciones sí cruza (${caidaCon(6, 10.5).toFixed(2)} %)`);
});
