...
export function correspondeBorrar(comprobante, ahora = new Date()) {
  const c = comprobante || {};
  // UN COMPROBANTE TIENE VARIAS FOTOS. Se mira si le queda alguna guardada, no
  // una sola ubicación: una factura de dos páginas con la primera ya borrada
  // sigue teniendo qué borrar.
  if (!tieneFotos(c)) return false; // no hay nada que borrar
...
export function descargables(comprobantes) {
  return (Array.isArray(comprobantes) ? comprobantes : []).filter(
    (c) => c && tieneFotos(c) && !c.imagenBorradaEn
  );
}

/**
 * ¿A este comprobante le queda alguna foto guardada?
 *
 * Acepta las dos formas a propósito: la lista de `archivos` —que es como viene
 * de la base desde que un comprobante puede tener varias fotos— y la
 * `archivoUbicacion` suelta, para no romper a quien todavía arme el objeto a
 * mano. Lo que decide es que HAYA alguna, no cuántas.
 */
export function tieneFotos(comprobante) {
  const c = comprobante || {};
  if (Array.isArray(c.archivos)) return c.archivos.some((a) => a && a.ubicacion);
  return !!c.archivoUbicacion;
}
