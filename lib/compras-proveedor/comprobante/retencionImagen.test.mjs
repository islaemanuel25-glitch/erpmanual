...
// ── Un comprobante con VARIAS fotos ────────────────────────────────────────

test("UN COMPROBANTE DE VARIAS FOTOS SE TRATA COMO UNO SOLO", () => {
  // La ventana es del PAPEL, no de cada foto: las dos páginas de la factura de
  // DAS vencen juntas porque son el mismo comprobante.
  const dosPaginas = {
    venceEn: calcularVencimiento(hace(10)),
    archivos: [{ ubicacion: "/vol/comprobantes/a_p1.jpg" }, { ubicacion: "/vol/comprobantes/a_p2.jpg" }],
  };
  assert.equal(correspondeBorrar(dosPaginas, HOY), true);
  assert.equal(descargables([dosPaginas]).length, 1, "es UN comprobante, no dos");
});

test("con una página ya borrada todavía queda qué borrar", () => {
  const aMedias = {
    venceEn: calcularVencimiento(hace(10)),
    archivos: [{ ubicacion: null }, { ubicacion: "/vol/comprobantes/a_p2.jpg" }],
  };
  assert.equal(correspondeBorrar(aMedias, HOY), true);
});

test("sin ninguna foto guardada no hay nada que borrar", () => {
  const sinNada = { venceEn: calcularVencimiento(hace(10)), archivos: [{ ubicacion: null }] };
  assert.equal(correspondeBorrar(sinNada, HOY), false);
  assert.equal(correspondeBorrar({ venceEn: calcularVencimiento(hace(10)), archivos: [] }, HOY), false);
});

test("el aviso cuenta COMPROBANTES, no fotos", () => {
  // Decir "4 comprobantes por vencer" cuando son dos facturas de dos páginas
  // haría que nadie entendiera qué está por perder.
  const dos = [
    { venceEn: calcularVencimiento(hace(5)), archivos: [{ ubicacion: "a" }, { ubicacion: "b" }] },
    { venceEn: calcularVencimiento(hace(6)), archivos: [{ ubicacion: "c" }] },
  ];
  const a = avisoDePaquete(dos, HOY);
  assert.equal(a.cantidad, 2);
  assert.match(a.titulo, /2 comprobantes/);
});
