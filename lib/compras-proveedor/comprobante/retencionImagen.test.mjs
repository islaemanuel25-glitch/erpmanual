...
test("el aviso lo ven el dueño y el administrador, nadie más", () => {
  assert.equal(puedeVerAvisoDePaquete("DUENO"), true);
  assert.equal(puedeVerAvisoDePaquete("admin"), true);
  for (const rol of ["CAJERO", "REPOSITOR", "ENCARGADO", "", null, undefined]) {
    assert.equal(puedeVerAvisoDePaquete(rol), false, String(rol));
  }
});
