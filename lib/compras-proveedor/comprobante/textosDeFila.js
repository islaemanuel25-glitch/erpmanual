...
/**
 * QUÉ DICE LA FILA CERRADA EN LA COLUMNA DEL PRODUCTO. Dos renglones.
 *
 * ── EL NOMBRE NUNCA ES UN ESTADO ───────────────────────────────────────────
 *
 * Ponía "Sin vincular" arriba, en el lugar del nombre, y con la factura de
 * Mauro eso daba quince filas seguidas diciendo exactamente lo mismo: no había
 * forma de distinguir una de otra sin abrirlas. El estado ya se ve en el tono
 * de la fila; el lugar del nombre es para nombrar.
 *
 * Así que arriba va siempre una IDENTIDAD: el nombre del ERP si la línea está
 * vinculada, y si no, el texto del papel — que cuando no hay producto es lo
 * único que identifica a esa línea.
 *
 * Y abajo va el contexto, que es el mismo par que en la grilla de listas: el
 * del ERP arriba y el del archivo abajo. Sin vincular no hay texto del archivo
 * que poner abajo, porque ya subió arriba, así que abajo va el candidato en
 * forma de PREGUNTA — "¿Chester 20 convertible?" —, que es lo que la fila
 * espera que alguien conteste.
 */
export function textoDelProducto(f) {
  const vinculada = estadoDeLaFila(f).codigo !== ESTADO_FILA.SIN_VINCULAR;
  const papel = f?.textoCrudo || null;

  if (vinculada) {
    return { arriba: f?.producto || papel || "Sin nombre", abajo: papel, esPregunta: false };
  }

  const candidato = f?.candidatos?.[0]?.nombre || null;
  return {
    arriba: papel || "Sin texto",
    abajo: candidato ? `\u00bf${candidato}?` : null,
    esPregunta: candidato != null,
  };
}

/**
 * LOS DOS LADOS QUE SE COMPARAN, cada uno con su cantidad y su precio.
 *
 * ── LO QUE SE COMPARA TIENE QUE ESTAR AL LADO ──────────────────────────────
 *
 * La fila cerrada mostraba UN número —la cantidad de la factura— sin nada
 * contra qué compararlo, y esa lista existe justamente para comparar. Es el
 * equivalente exacto de "costo actual" contra "costo nuevo" en la grilla de
 * listas: dos columnas, pegadas.
 *
 * ── EL GUION ACÁ SÍ VA ─────────────────────────────────────────────────────
 *
 * Más arriba, en `numerosDeLaFila`, el pedazo que no existe NO se arma: "Pedido
 * —" suelto en un renglón de prosa se leía como una afirmación sobre el pedido.
 * En una columna es al revés — el encabezado dice "Pedido", así que el guion
 * tiene nombre y significa "de este lado no hay". Un hueco en blanco se leería
 * como que el dato no se cargó.
 *
 * ── EL ÁMBAR ES SOLO DEL PRECIO DE LA FACTURA ──────────────────────────────
 *
 * Y solo cuando hay los dos números para comparar. Con uno solo no se sabe si
 * difiere: pintarlo sería afirmar una diferencia que nadie midió.
 */
export function ladosDeLaFila(f) {
  const cat = f?.costoCatalogo;
  const fac = f?.costoFactura;
  const difiere = !ausente(cat) && !ausente(fac) && Number(cat) !== Number(fac);

  return {
    pedido: { cantidad: limpio(f?.cantidadPedida), precio: importe(cat) },
    factura: { cantidad: limpio(f?.cantidad), precio: importe(fac) },
    difiere,
  };
}
