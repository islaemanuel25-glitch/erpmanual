...
// desproporción no la puede decidir el módulo de facturas por su cuenta.
//
...
import { evaluarAlmacen, nombreDeArchivo, NOMBRE_CENTINELA } from "./almacenImagenes.js";

/** El nombre de la variable, una sola vez, para que nadie la escriba a mano. */
export const VARIABLE_RUTA = "FACTURAS_VOLUMEN_PATH";
 ...
 */
export async function inspeccionarAlmacen({ ruta = process.env[VARIABLE_RUTA] } = {}) {
  ...
  // el motivo.
  const existeCentinela = existeDirectorio ? await existe(join(limpia, NOMBRE_CENTINELA)) : false;
...
export async function verificarAlArrancar() {
  ...
  if (veredicto.ok) {
    console.log(`[facturas] almacén de imágenes verificado en ${veredicto.ruta}`);
    return veredicto;
  }
  ...
  console.error(
    `[facturas] ALMACÉN DE IMÁGENES NO DISPONIBLE — no se van a poder subir fotos de comprobantes.\n` +
      `[facturas] ${veredicto.motivo}\n` +
      `[facturas] Ruta configurada (${VARIABLE_RUTA}): ${veredicto.ruta ?? "(vacía)"}\n` +
      `[facturas] El procedimiento para dejarlo andando está en docs/RUNBOOK-VOLUMEN-FACTURAS.md.\n` +
      `[facturas] El resto de la aplicación sigue funcionando normalmente.`,
  );
  return veredicto;
}
