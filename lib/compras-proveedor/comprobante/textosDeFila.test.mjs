...
test("un precio que solo se informa NO cuenta como pendiente", () => {
  ...
  assert.equal(textoDePendiente(f).texto, "Listo");
});
...
// ── LA ÚLTIMA COLUMNA DICE QUÉ FALTA, NO CÓMO ESTÁ ─────────────────────────

test("la columna de decisión NO repite lo que ya dice la de producto", () => {
  // La de producto muestra "Sin vincular" cuando no hay producto, igual que en
  // la grilla de listas. Si esta dijera lo mismo, la fila lo afirmaría dos
  // veces — que es exactamente lo que este módulo vino a sacar.
  const t = textoDePendiente({});
  assert.equal(t.texto, "Producto");
  assert.notEqual(t.texto, "Sin vincular");
  // Una palabra, no una frase: el encabezado ya dice "Falta", y cada letra de
  // más en esta columna se la saca al nombre del producto.
  assert.ok(t.texto.split(" ").length === 1, t.texto);
});

test("cada pendiente se nombra por lo que hay que hacer", () => {
  assert.equal(textoDePendiente({ productoLocalId: 5, unidad: { requiereDecision: true } }).texto, "Unidad");
  assert.equal(
    textoDePendiente({ productoLocalId: 5, precio: { decision: { ofreceAceptar: true } } }).texto,
    "Precio"
  );
});

test("la resuelta dice «Listo», en una palabra y en verde", () => {
  const t = textoDePendiente({ productoLocalId: 5 });
  assert.equal(t.texto, "Listo");
