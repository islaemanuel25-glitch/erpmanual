...
/**
 * Cuántas líneas tienen precio distinto del catálogo.
 *
 * Vive ACÁ y no en cada pantalla a propósito. Son dos pantallas —el carrito del
 * pedido nuevo y la grilla del detalle— con formas de ítem distintas, y contar
 * en cada una habría significado escribir el criterio dos veces. El día que una
 * cambie el umbral o la conversión, se despegan y muestran números distintos
 * para el mismo pedido.
 *
 * Cada pantalla mapea sus ítems a las cuatro entradas de `compararCostoLinea`;
 * la decisión de qué cuenta como diferencia se toma una sola vez.
 *
 * @param {Array<{precioLinea, unidad, costoCatalogo, base}>} entradas
 * @returns {number}
 */
export function contarLineasConAviso(entradas) {
  if (!Array.isArray(entradas)) return 0;
  let n = 0;
  for (const e of entradas) {
    if (compararCostoLinea(e || {}).hayDiferencia) n += 1;
  }
  return n;
}

/**
 * El texto del contador. En singular y plural, porque "1 líneas" se lee mal en
 * una pantalla que alguien mira todos los días.
 *
 * Devuelve "" cuando no hay ninguna: el contador NO se muestra en cero. Un
 * cartel que dice "0 líneas con precio distinto" ocupa lugar para decir que no
 * pasa nada.
 */
export function textoContadorAvisos(n) {
  const c = Number(n) || 0;
  if (c <= 0) return "";
  return c === 1
    ? "1 línea tiene un precio distinto al del catálogo"
    : `${c} líneas tienen un precio distinto al del catálogo`;
}
