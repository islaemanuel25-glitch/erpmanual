...
test("F10. LA FOTO NO SE GUARDA EN NINGUNA CACHÉ COMPARTIDA", () => {
  // ── EL DEFECTO QUE ESTO IMPIDE, Y OCURRIÓ ───────────────────────────────
  //
  // La ruta comprobaba `productos.ver` y respondía `Cache-Control: public`.
  // Cloudflare guardó la respuesta y a partir de ahí la sirvió desde el borde a
  // peticiones SIN sesión, que ya no llegaban al servidor. Medido en producción
  // el 2026-08-23: 200 sin credenciales, `cf-cache-status: HIT`, `Age: 11572`.
  //
  // El chequeo de permiso funcionaba. Lo que falló fue que dejó de correr, y por
  // eso este candado no mira el permiso —de eso se ocupa `permisoEnCadaGet`—
  // sino lo que hace que el permiso siga siendo el que decide.
  const ruta = leer("app/api/productos/foto/[archivo]/route.js");

  // `leer` saca los comentarios ANTES de mirar. Sin eso este candado daría rojo
  // por el párrafo de arriba de la propia ruta, que explica el defecto y por lo
  // tanto contiene la palabra prohibida. Es el mismo tropiezo que este repo ya
  // tuvo tres veces, dos como falso rojo y una como falso VERDE.
  assert.doesNotMatch(
    ruta,
    /["']Cache-Control["']\s*:\s*["'][^"']*\bpublic\b/,
    "la foto volvió a declararse cacheable por una caché compartida: eso deja que " +
      "el CDN la sirva sin pasar por el chequeo de permiso"
  );

  // Y lo que SÍ tiene que decir. Las dos mitades, porque hacen cosas distintas:
  // `private` guarda la foto en el navegador y se la prohíbe a las compartidas;
  // la de Cloudflare es la única que sobrevive a una Cache Rule del panel que
  // ignore lo que manda el origen.
  assert.match(ruta, /["']Cache-Control["']\s*:\s*["']private,\s*max-age=31536000,\s*immutable["']/);
  assert.match(ruta, /["']Cloudflare-CDN-Cache-Control["']\s*:\s*["']no-store["']/);

  // EL BENEFICIO NO SE PIERDE: sigue siendo un año e inmutable. Si alguien
  // "arreglara" esto poniendo `no-store` a secas, el celular volvería a bajar
  // las 25 fotos en cada visita y nadie se enteraría hasta ver los datos.
  assert.doesNotMatch(
    ruta,
    /["']Cache-Control["']\s*:\s*["'][^"']*no-store/,
    "no-store en Cache-Control apaga también la caché del navegador: en un celular " +
      "eso son 25 fotos bajadas de nuevo en cada visita"
  );
});
