...
/** El cuerpo real de una de las dos funciones de la pantalla. */
function cuerpoDe(nombre) {
  const m = PANTALLA.match(new RegExp(`const ${nombre} = async \\(\\) => \\{[\\s\\S]*?\\n  \\};`));
  assert.ok(m, `no se encontró ${nombre} en la pantalla`);
  return m[0];
}

/** El bloque `catch` de ese cuerpo, que es donde vivía el defecto. */
function catchDe(cuerpo, nombre) {
  const m = cuerpo.match(/catch \(err\) \{([\s\S]*?)\n    \}/);
  assert.ok(m, `${nombre} no tiene un bloque catch reconocible`);
  return m[1];
}

/** La línea que apaga un indicador de carga, con su guardia si la tiene. */
function lineaQueApaga(cuerpo, setter) {
  const linea = cuerpo.split("\n").find((l) => l.includes(`${setter}(false)`) && !l.includes("//"));
  assert.ok(linea, `no se encontró dónde se apaga ${setter}`);
  return linea;
}

const VIGENCIA = /vigente\(\)|miToken === token\w+Ref\.current/;

test("OC10. TODOS LOS CAMINOS QUE TOCAN ESTADO COMPARTIDO COMPRUEBAN EL TOKEN", () => {
  // ── POR QUÉ ESTE CANDADO SE REESCRIBIÓ, Y ES LO QUE LO DELATÓ ───────────
  //
  // La primera versión contaba que existieran los tokens y sus dos `return`.
  // Los cuatro existían, así que pasaba en VERDE — y el defecto grave estaba
  // adentro: el guardia cubría solo el camino exitoso. El `catch` de
  // `fetchControles` hacía `setControles([])` sin mirar el token, así que un
  // pedido viejo que fallaba tarde borraba los contadores YA PINTADOS de la
  // ubicación nueva.
  //
  // O sea que el candado no estaba débil: estaba mirando el lugar equivocado.
  // Ahora se enumeran los caminos TERMINALES sobre el texto real de las dos
  // funciones, y cada uno tiene que comprobar vigencia.
  //
  // Se lee el bloque real y no una copia: si mañana alguien saca el guardia del
  // `catch`, esto se pone rojo aunque el del camino feliz siga puesto.
  for (const nombre of ["fetchProductos", "fetchControles"]) {
    const cuerpo = cuerpoDe(nombre);

    // 1. El token se toma al salir, y hay una forma de preguntar por su vigencia.
    assert.match(cuerpo, /const miToken = \+\+token\w+Ref\.current/, `${nombre} no toma un token`);
    assert.match(cuerpo, /const vigente = \(\) =>/, `${nombre} no tiene con qué preguntar si sigue vigente`);

    // 2. El camino exitoso.
    assert.match(cuerpo, /if \(!vigente\(\)\) return;/, `${nombre}: el camino exitoso no comprueba vigencia`);

    // 3. LA EXCEPCIÓN. Es donde estaba el defecto grave.
    const bloque = catchDe(cuerpo, nombre);
    assert.match(
      bloque,
      VIGENCIA,
      `${nombre}: el catch toca estado compartido sin comprobar el token. ` +
        "Un pedido viejo que falla tarde puede borrar los datos del pedido nuevo."
    );
  }

  // 4. EL INDICADOR DE CARGA. Solo lo puede apagar el pedido vigente: si lo
  //    apaga uno viejo, la pantalla dice "listo" con el pedido nuevo en vuelo.
  assert.match(
    lineaQueApaga(cuerpoDe("fetchProductos"), "setLoading"),
    VIGENCIA,
    "un pedido viejo puede apagar el spinner del listado vigente"
  );
  assert.match(
    lineaQueApaga(cuerpoDe("fetchControles"), "setCargandoControles"),
    VIGENCIA,
    "un pedido viejo puede apagar el indicador de carga de los controles vigentes"
  );
});

test("OC15. CARRERA DE CONTROLES: el pedido de 7 que falla no borra lo de 9", () => {
  // ── LA CARRERA, TAL COMO OCURRE ─────────────────────────────────────────
  //
  //   1. sale A por el local 7;
  //   2. se cambia de ubicación y sale B por el local 9;
  //   3. B contesta bien y pinta los contadores de 9;
  //   4. A falla DESPUÉS —se cortó la red, o el cuerpo vino roto—;
  //   5. A no puede tocar nada de B.
  //
  // Se ejerce contra `haceElUltimoPedido`, que es LA MISMA función que la
  // pantalla llama en su `vigente()`. No es una copia equivalente: si se cambia
  // la función, este candado cambia con ella.
  const tokenRef = { current: 0 };

  const tokenDeA = ++tokenRef.current;   // sale A (local 7)
  const tokenDeB = ++tokenRef.current;   // sale B (local 9)

  // B contesta bien: es el vigente y puede pintar.
  assert.equal(haceElUltimoPedido(tokenRef, tokenDeB), true, "B no pudo pintar sus propios contadores");

  // A falla después. NO es vigente: no puede borrar, ni vaciar, ni apagar nada.
  assert.equal(
    haceElUltimoPedido(tokenRef, tokenDeA),
    false,
    "el pedido viejo se creyó vigente y puede borrar los contadores del local nuevo"
  );

  // Y que B siga siendo vigente después de que A terminó: un pedido que muere
  // no puede cederle la vigencia a nadie.
  assert.equal(haceElUltimoPedido(tokenRef, tokenDeB), true, "A al morir le sacó la vigencia a B");
});

test("OC16. CARRERA DEL LISTADO: el pedido viejo que falla no apaga ni desmarca", () => {
  //   1. sale A;  2. sale B;  3. A falla después.
  //
  // A no puede: borrar la referencia de B —que habilitaría un TERCER pedido
  // duplicado—, ni apagar el spinner de B, ni tocar totales o paginación.
  const tokenRef = { current: 0 };
  const tokenDeA = ++tokenRef.current;
  const tokenDeB = ++tokenRef.current;

  assert.equal(haceElUltimoPedido(tokenRef, tokenDeA), false, "el listado viejo se creyó vigente");
  assert.equal(haceElUltimoPedido(tokenRef, tokenDeB), true);

  // El caso de UN solo pedido no se rompe: sin carrera, el pedido es vigente y
  // tiene que poder cerrar su propio spinner. Un guardia que dijera `false` acá
  // dejaría la pantalla cargando para siempre, que es peor que el defecto.
  const solo = { current: 0 };
  const suToken = ++solo.current;
  assert.equal(haceElUltimoPedido(solo, suToken), true, "un pedido sin carrera no se pudo cerrar");

  // Y no explota si le pasan cualquier cosa.
  assert.equal(haceElUltimoPedido(null, 1), false);
  assert.equal(haceElUltimoPedido(undefined, 1), false);
});
