...
/**
 * ── EL MARCADOR DE ORIGEN, Y POR QUÉ ES EXPLÍCITO ─────────────────────────
 *
 * El editor de combos tiene DOS comportamientos de salida y necesita saber de
 * cuál viene:
 *
 *   · desde una card del celular → vuelve al listado EXACTO, con su página, su
 *     búsqueda, sus filtros y su orden, para que la restauración funcione;
 *   · desde la tabla de escritorio → vuelve a donde volvía antes de `81541c37`:
 *     guardar a `?tipo=combos`, cancelar y atrás a `/modulos/productos` pelado.
 *
 * ── POR QUÉ NO SE DEDUCE DE "TIENE QUERY" ────────────────────────────────
 *
 * Porque un listado móvil sin ningún filtro puesto tampoco tiene query, y sí
 * necesita restauración. Deducirlo así lo mandaría por el camino de escritorio
 * justo en el caso más común — entrar al catálogo y editar el primero que se ve.
 *
 * Tampoco se deduce del ancho de la pantalla: eso supondría que "angosto" y "hay
 * cards" son lo mismo, y quién está visible lo decide el CSS.
 *
 * Se marca a propósito, y solo desde la card.
 *
 * ── Y NO VUELVE AL LISTADO ───────────────────────────────────────────────
 *
 * El marcador viaja al editor y se saca al armar la URL de vuelta. Si volviera,
 * quedaría pegado en la barra de direcciones y —peor— entraría en la comparación
 * que decide si el estado de retorno es de este listado: la URL guardada no lo
 * tiene, así que nunca coincidirían y la restauración no ocurriría nunca.
 */
export const PARAM_ORIGEN = "origen";
export const ORIGEN_MOVIL = "movil";

/** ¿La URL del editor dice que se vino de una card del celular? */
export function vinoDeUnaCardMovil(qs) {
  if (qs === null || qs === undefined) return false;
  const p = new URLSearchParams(typeof qs === "string" ? qs : String(qs));
  return p.get(PARAM_ORIGEN) === ORIGEN_MOVIL;
}

/** La query del listado con el marcador puesto, para la URL del editor. */
export function conMarcaDeOrigenMovil(qs) {
  const p = new URLSearchParams(qs || "");
  p.set(PARAM_ORIGEN, ORIGEN_MOVIL);
  return p.toString();
}

/**
 * La URL del listado a la que volver, SIN el marcador.
 *
 * Se saca acá y no en cada salida del editor: con tres salidas escribiéndolo por
 * su cuenta, la que se olvide deja el marcador pegado y rompe la comparación del
 * estado de retorno sin que nada avise.
 */
export function urlDeListadoSinMarca(qs, base = "/modulos/productos") {
  const p = new URLSearchParams(qs || "");
  p.delete(PARAM_ORIGEN);
  const limpio = p.toString();
...
/**
 * ── ¿ESTE ESTADO ES DE LA PANTALLA QUE SE ESTÁ MIRANDO? ────────────────────
 *
 * La pregunta que faltaba, y su ausencia era un defecto: el estado guardaba la
 * URL y nadie la comparaba. Solo se validaban la forma y el vencimiento.
 *
 * El caso que rompía:
 *
 *   · se abre Editar desde la página 2 con filtros puestos;
 *   · se abandona el editor sin volver por el camino normal —el menú, un enlace,
 *     otra pestaña—, así que el estado queda sin consumir;
 *   · dentro de la media hora se entra a Productos por OTRA URL;
 *   · y ese estado viejo movía el scroll y marcaba un producto que no tenía nada
 *     que ver con lo que la persona estaba mirando.
 *
 * ── SE COMPARA LA URL CANÓNICA ENTERA, NO EL `pathname` ───────────────────
 *
 * Todo lo que distingue un listado de otro viaja en la query: la página, la
 * búsqueda, los filtros, el orden y su dirección, el control de mantenimiento y
 * las presentaciones. Comparar solo la ruta las daría todas por iguales, que es
 * exactamente el caso que hay que atajar.
 *
 * ── Y SE COMPARA EL CONJUNTO DE PARÁMETROS, NO LA CADENA ──────────────────
 *
 * Porque la pantalla reconstruye su URL desde el estado, así que el orden de los
 * parámetros no tiene por qué ser el que alguien tipeó. Entrar por `?q=a&page=2`
 * y que la pantalla escriba `?page=2&q=a` es el MISMO listado; compararlas como
 * cadenas daría "distinto" sobre algo idéntico y la restauración no ocurriría
 * nunca en ese camino.
 *
 * Los dos lados se normalizan igual, así que la función es simétrica.
 */
export function mismaUrlDeListado(a, b) {
  if (typeof a !== "string" || typeof b !== "string") return false;
  const partir = (u) => {
    const [ruta, qs = ""] = u.split("?");
    // Se ordena por clave Y por valor: dos parámetros repetidos con los mismos
    // valores en distinto orden siguen siendo el mismo listado.
    const pares = [...new URLSearchParams(qs).entries()].sort((x, y) =>
      x[0] === y[0] ? String(x[1]).localeCompare(String(y[1])) : x[0].localeCompare(y[0])
    );
    return `${ruta}?${JSON.stringify(pares)}`;
  };
  return partir(a) === partir(b);
}

/**
 * ¿Se puede restaurar este estado en ESTA pantalla?
 *
 * Junta las dos preguntas —vigente y de este listado— para que ningún llamador
 * pueda hacer una y olvidarse de la otra. Es exactamente lo que pasó:
 * `esEstadoVigente` existía y se llamaba, y la comparación de URL no existía.
 */
export function sePuedeRestaurarAca({ estado, urlActual, ahora, vencimientoMs }) {
  if (!esEstadoVigente(estado, ahora, vencimientoMs)) return false;
  return mismaUrlDeListado(estado.url, urlActual);
}
