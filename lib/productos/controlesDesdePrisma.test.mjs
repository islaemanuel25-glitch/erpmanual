...
test("G13. LAS DOS CONSULTAS USAN EL MISMO TECHO Y EL MISMO ORDEN", () => {
  ...
  for (const ruta of LAS_DOS_RUTAS) {
    ...
    assert.match(
      fuente,
      /\.\.\.opcionesDelTecho\(\)/,
      `${ruta} dejó de usar el techo y el orden compartidos`
    );
    // Y NADIE los reescribe al lado: es la contraprueba de lo anterior. Usar la
    // función y además escribir un `take` propio pasaría lo de arriba en verde
...
test("G14. y el flag de truncado sale de la misma función en las dos", () => {
  ...
  // separen: una podría quedar en `>=` y la otra en `>`.
  for (const ruta of LAS_DOS_RUTAS) {
    const fuente = sinComentarios(leerRuta(ruta));
    assert.match(fuente, /seTrunco\(/, `${ruta} dejó de usar seTrunco`);
    assert.match(fuente, /dentroDelTecho\(/, `${ruta} dejó de usar dentroDelTecho`);
    assert.equal(
      /\.length\s*>\s*\w*MAX_CONTROL/.test(fuente),
      false,
