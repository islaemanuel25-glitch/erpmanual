...
test("G14-bis. UN DORSO DE SOLA IDENTIFICACIÓN NO PROMETE UNA REFERENCIA", () => {
  // Salió en una captura: un producto suelto con los códigos prendidos tiene
  // dorso —la identificación es contenido del dorso— y el botón decía "Ver
  // referencia" sobre una cara que son dos códigos. Prometía algo que esa cara no
  // tiene.
  //
  // `null` es "no hay referencia": el dorso existe por la identificación.
  assert.equal(nombreDelDorso(null), "códigos");
  // Con referencia de escala, el nombre sale de la presentación.
  assert.equal(nombreDelDorso({ presentacion: "PACK X 24" }), "pack");
  assert.equal(nombreDelDorso({ presentacion: "UNIDAD" }), "unidad");
  // Y kilo y pieza, que tienen una LÍNEA de referencia y no una segunda escala,
  // se siguen llamando referencia.
  assert.equal(nombreDelDorso({ presentacion: null, detalle: "$130,00 cada 100 g" }), "referencia");
});
