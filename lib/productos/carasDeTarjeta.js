...
/**
 * ¿Este producto tiene una conversión unidad ↔ pack/cajón?
 *
 * ── POR QUÉ NO ALCANZA CON PREGUNTAR SI HAY DORSO ─────────────────────────
 *
 * Porque un dorso puede ser dos cosas distintas y solo una de ellas es una
 * conversión. El de pack trae una SEGUNDA ESCALA: su propio importe y su propio
 * rótulo, y la tarjeta puede alternar el bloque del precio entre las dos. El de
 * kilo y pieza fija trae una LÍNEA DE TEXTO —"1 pieza = 6 kg · $1.000,00 por
 * kilo"—; alternar hacia eso no sería cambiar el importe, sería cambiar de qué
 * habla el bloque.
 *
 * La diferencia se lee en `presentacion`: una escala con nombre la tiene, la
 * línea de referencia no.
 *
 * ── POR QUÉ VIVE ACÁ Y NO EN EL COMPONENTE ────────────────────────────────
 *
 * Porque la usan los dos: la tarjeta para decidir si dibuja el control, y
 * `scripts/sonda-tarjeta-producto.mjs` para decidir si tiene que estar. La sonda
 * corre en node pelado y no puede importar un componente de React, así que si el
 * predicado viviera allá tendría que reescribirlo — y sería la copia número
 * cinco de una condición que este repo ya tuvo divergida en cuatro lugares.
 */
export function hayEquivalenciaDeBulto(caras) {
  return !!caras?.dorso && !!caras.dorso.presentacion;
}
