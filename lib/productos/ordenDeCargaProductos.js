...
/**
 * ¿ESTE PEDIDO SIGUE SIENDO EL ÚLTIMO QUE SALIÓ?
 *
 * ── EL DEFECTO QUE ESTO IMPIDE, Y QUE UNA AUDITORÍA ENCONTRÓ ────────────────
 *
 * Cambiar de ubicación deja el pedido anterior en el aire. La primera versión de
 * este arreglo comprobaba el token SOLO en el camino exitoso, y con eso no
 * alcanza: el `catch` seguía tocando estado compartido sin mirar nada.
 *
 * La carrera concreta, con los contadores:
 *
 *   1. sale A por el local 7;
 *   2. se cambia de ubicación y sale B por el local 9;
 *   3. B contesta bien y pinta los contadores de 9;
 *   4. A falla después —se cortó la red— y entra a su `catch`;
 *   5. ese `catch` hacía `setControles([])` y `controlesRef.current = null`.
 *
 * Resultado: los contadores CORRECTOS del local 9 quedaban en cero, y la marca
 * de "ya se pidieron" borrada. Sin un solo error a la vista, y con el candado
 * viejo en verde, porque contaba que los guardias existieran y no dónde estaban.
 *
 * ── LA REGLA ────────────────────────────────────────────────────────────────
 *
 * Un pedido viejo puede TERMINAR —no se lo cancela— pero no puede tocar nada
 * compartido: ni pintar, ni borrar, ni apagar el indicador de carga del pedido
 * nuevo, ni desmarcar la referencia que evita duplicados. Todos los caminos
 * terminales —éxito, `ok:false`, excepción y cierre de la carga— preguntan por
 * acá antes de escribir.
 *
 * Falla CERRADA a propósito, al revés que la puerta: ante la duda —un ref que no
 * llegó— la respuesta es "no soy el vigente", que no toca nada. Lo peor que
 * puede pasar es no apagar un spinner; lo contrario sería borrar datos buenos.
 *
 * @param {{current: number}|null} refDelToken  el contador compartido
 * @param {number} miToken  el que tomó este pedido al salir
 */
export function haceElUltimoPedido(refDelToken, miToken) {
  if (!refDelToken || typeof refDelToken !== "object") return false;
  return refDelToken.current === miToken;
}
