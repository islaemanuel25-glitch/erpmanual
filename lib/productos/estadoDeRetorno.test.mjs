...
test("R41. el contenedor de escritorio se resuelve COMO ANTES, con sus límites", () => {
  // No se reusa `contenedorDeScrollDe`: aquella saltea el candidato sin
  // sobrante, y con una tabla corta caería al `<main>` y restauraría el scroll
  // de la PÁGINA — algo que escritorio nunca hizo. "Exactamente como antes"
  // incluye sus límites.
  const fuente = sinComentarios(leer("lib/productos/retornoEscritorio.js"));
  assert.match(fuente, /getElementById\("productos-scroll"\)/);
  assert.match(fuente, /querySelector\("main"\)/);
  assert.doesNotMatch(fuente, /contenedorDeScrollDe/, "reusó la resolución nueva, que se comporta distinto");

  // Y el doc de mentira contesta lo mismo que contestaba la pantalla.
  const doc = {
    getElementById: (id) => (id === "productos-scroll" ? { nombre: "tabla" } : null),
    querySelector: () => ({ nombre: "main" }),
  };
  assert.equal(contenedorDeLaTabla(doc).nombre, "tabla");
  const sinTabla = { getElementById: () => null, querySelector: () => ({ nombre: "main" }) };
  assert.equal(contenedorDeLaTabla(sinTabla).nombre, "main");
