...
test("R34. SOLO SE GUARDA SI HAY UNA CARD MÓVIL VISIBLE, y no por ancho de pantalla", () => {
  ...
  assert.match(bloque, /offsetParent !== null/, "no exige que la card esté visible");
  assert.match(bloque, /if \(!card\) return;/, "sin card visible tendría que no guardar nada");
  // Y la salida temprana va ANTES de escribir: si estuviera después, el estado
  // ya se habría guardado.
  assert.ok(
  ...
  // Nada de decidir por el ancho.
  assert.doesNotMatch(bloque, /matchMedia|innerWidth|md:hidden/, "decide por el ancho de pantalla");
});
