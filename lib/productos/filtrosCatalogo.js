...
/**
 * ── QUÉ ENTRA AL HISTORIAL DEL NAVEGADOR, Y QUÉ NO ─────────────────────────
 *
 * Tocar una card es una acción deshacible: quien la toca espera que Atrás la
 * deshaga. Escribir en el buscador NO lo es — con una entrada por tecla, volver
 * atrás desde "aceite" son seis toques de Atrás para llegar a donde se estaba.
 *
 * Por eso la decisión NO es "qué manejador corrió" sino "¿cambió la selección de
 * cards?". Escrita así:
 *
 *   · sirve para los tres manejadores y para cualquiera que aparezca mañana;
 *   · es pura y se puede probar sin navegador;
 *   · y resuelve solo el caso mixto —escribir en el buscador con una card puesta
 *     la apaga, o sea que la selección SÍ cambió y esa transición merece su
 *     entrada—. Las teclas siguientes ya no cambian la selección y reemplazan.
 *
 * El resto —página, orden, filtros— sigue reemplazando, que es lo que esta
 * pantalla ya hacía y nadie pidió cambiar.
 */
export function laSeleccionDeCardsCambio(anterior, actual) {
  if (!anterior || !actual) return false;
  if ((anterior.control ?? null) !== (actual.control ?? null)) return true;
  const a = anterior.presentaciones ?? presentacionesNeutras();
  const b = actual.presentaciones ?? presentacionesNeutras();
  return (a.venta ?? null) !== (b.venta ?? null) || (a.compra ?? null) !== (b.compra ?? null);
}
