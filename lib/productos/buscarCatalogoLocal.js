...
export function mapProductoParaRecepcion(item) {
  return {
    ...
    // LA POLÍTICA DE SALIDA DEL DEPÓSITO, que es lo que decide en qué escala se
    // puede recibir un producto que se agrega durante la recepción. Sin ella la
    // pantalla ofrece "PACK x6" sobre algo que el depósito solo despacha suelto
    // y el servidor rechaza el alta con UNIDAD_CONTRADICE_CATALOGO.
    modoEnvio: item.modoEnvio ?? null,
    // El tercero del predicado, para los productos no migrados. Sin él la
    // pantalla y el servidor resuelven distinto la MISMA ficha.
    pesoEsFijo: item.pesoEsFijo ?? null,
...
export function mapItemCatalogo(productoLocal) {
  ...
  return {
    ...
    // La política de salida depósito → local. Ver `proyectarItem`.
    modoEnvio: base?.modo_envio ?? null,
    // El fallback de los productos NO migrados. `esFiambreFijo` lo consulta
    // cuando `modoVentaDeposito` está vacío, así que sin él la pantalla lee como
    // kilo un producto que el servidor va a congelar como PIEZA — y entonces el
    // rótulo del catálogo y lo que queda guardado dicen cosas distintas.
