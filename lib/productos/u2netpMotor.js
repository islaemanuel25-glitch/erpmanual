...
async function crearSesion({ alAvanzar }) {
  ...
  // pantalla de productos y la carga inicial paga 400 KB de JS aunque nadie
  ...
  // El binario del runtime se lo damos NOSOTROS, ya bajado y guardado. Sin
  // esto, ORT lo iría a buscar solo a una ruta relativa que en Next no existe, y
  // el error que da —"no available backend found"— no dice nada de rutas.
  ort.env.wasm.wasmBinary = await recursos.traerConCache(recursos.RUTA_WASM, alAvanzar);

  const modelo = await recursos.traerConCache(recursos.RUTA_MODELO, alAvanzar);
