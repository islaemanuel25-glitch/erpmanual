 ...
 * exige.
 *
 * ── Y DESDE EL 2026-09-11 LA LISTA SE ACHICÓ, QUE ES MEJOR ────────────────
 *
 * `alternarControl`, `alternarPresentacion` y `aplicarFiltros` dejaron de
 * escribir filtros por su cuenta: los tres pasan por `fijarBusquedaConfirmada`,
 * la puerta única del buscador, que además de fijarlos cancela la búsqueda
 * diferida que estuviera en cola y repone el borrador del campo del celular.
 *
 * Es una decisión y no un ajuste, y la razón es la que este candado siempre
 * defendió: las tres cosas van juntas o no sirven. Con un `setFiltros` suelto,
 * tocar una card quedaba deshecho 250 ms más tarde por un temporizador que nadie
 * apagó, y el campo seguía mostrando una búsqueda que el listado ya no aplicaba.
 * Menos puertas es más fácil de custodiar que más manejadores sancionados.
 *
 * ── Y POR ESO AHORA SON DOS LISTAS Y NO UNA ──────────────────────────────
 *
 * Los filtros pasaron a tener una puerta única; las PRESENTACIONES no, y siguen
 * apagándose desde los tres manejadores de siempre, que es legítimo. Una sola
 * lista obligaba a elegir entre dejar entrar escrituras de filtros donde ya no
 * corresponden, o marcar como sueltas unas escrituras de presentaciones que
 * están bien. Separarlas afirma más, no menos.
 */
const MANEJADORES_DE_FILTROS = [
  "const fijarBusquedaConfirmada",
  "const aplicarEstadoDeLaUrl",
];

const MANEJADORES_DE_PRESENTACIONES = [
...
test("G6. LA PANTALLA LIMPIA CON ESTA FUNCIÓN, y no con un objeto escrito al lado", () => {
  ...
  // ── EL SETTER CAMBIÓ, LO QUE SE AFIRMA NO ──────────────────────────────
  //
  // Acá decía `setFiltros(filtrosNeutros())`. Ahora limpia por la puerta única
  // del buscador, que además cancela la búsqueda diferida y repone el borrador
  // — sin eso, tocar una card quedaba deshecho 250 ms después.
  //
  // Lo que este candado defiende es lo mismo de siempre: que la pantalla limpie
  // con LA FUNCIÓN del dominio y no con un objeto escrito al lado, que es como
  // el default se define en dos lugares y se separa.
  assert.match(bloque, /filtrosNeutros\(\)/, "dejó de limpiar con la función");
  assert.match(
    bloque,
    /fijarBusquedaConfirmada\(/,
    "volvió a escribir filtros por su cuenta: no cancela lo pendiente ni repone el campo"
  );
...
test("G11-pantalla. LA PANTALLA NORMALIZA AL ENTRAR y apaga el control al filtrar", () => {
  ...
  const rangos = MANEJADORES_DE_FILTROS.map((ancla) => ({ ancla, ...cuerpoDe(fuente, ancla) }));
  ...
  assert.deepEqual(
    sueltas,
    [],
    `hay ${sueltas.length} llamada(s) a setFiltros fuera de ${MANEJADORES_DE_FILTROS.join(", ")}`
  );
...
test("G17-pantalla. la pantalla apaga cada bloque desde el otro", () => {
  ...
  // Y nadie escribe presentaciones fuera de los cuatro, igual que con los
  // filtros — que ahora tienen su propia lista, más corta. Ver el comentario de
  // las dos listas arriba.
  const rangos = MANEJADORES_DE_PRESENTACIONES.map((a) => cuerpoDe(fuente, a));
...
test("G20-pantalla. EL ESTADO SIGUE A LA URL, o Atrás cambiaría el enlace y no la pantalla", () => {
  ...
  // Y hay algo que lo llama cuando el navegador navega.
  //
  // ── EL DISPARADOR CAMBIÓ, Y EL CANDADO SIGUE AL CÓDIGO ──────────────────
  //
  // Antes acá decía `aplicarEstadoDeLaUrl(searchParams)`: un efecto que
  // reaccionaba a CUALQUIER cambio de la URL y decidía si aplicarlo comparando
  // contra la última que esta pantalla había escrito. Eso es exactamente lo que
  // produjo el "quiquilmes" del buscador móvil — escribiendo rápido hay varias
  // navegaciones propias en vuelo, y una intermedia se leía como externa y
  // pisaba el texto que la persona estaba escribiendo.
  //
  // Lo que este candado defiende NO cambió: Atrás tiene que cambiar la pantalla
  // y no solo el enlace. Lo que cambió es de dónde sale esa señal: `popstate`,
  // que el navegador dispara SOLO al recorrer el historial y nunca por un
  // `pushState`/`replaceState` programático. Dejarlo apuntando a `searchParams`
  // lo habría dejado en rojo defendiendo una arquitectura que ya no está.
  assert.match(
    fuente,
    /addEventListener\("popstate"/,
    "nadie escucha el historial: Atrás cambiaría el enlace y no la pantalla"
  );
  assert.match(
    fuente,
  ...
  assert.match(fuente, /removeEventListener\("popstate"/, "el oyente del historial no se limpia");

  // Y LA HIDRATACIÓN NO VUELVE A DECIDIRSE COMPARANDO CADENAS.
  assert.ok(
    !/aplicarEstadoDeLaUrl\(searchParams\)/.test(fuente),
    "volvió a hidratar el estado desde searchParams: ahí vive la carrera del buscador"
  );

  // EL CORTE DEL BUCLE sigue existiendo, con otro papel: la pantalla no reescribe
  // la dirección que la barra ya muestra. Sin eso, volver de un Atrás dejaría una
  // entrada de historial nueva por cada Atrás.
  assert.match(fuente, /ultimaUrlEscritaRef/, "se perdió el corte que evita reescribir la misma URL");
});
