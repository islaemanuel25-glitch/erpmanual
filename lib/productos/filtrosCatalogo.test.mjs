...
test("G11-pantalla. LA PANTALLA NORMALIZA AL ENTRAR y apaga el control al filtrar", () => {
  ...
  const MANEJADORES = ["const alternarControl", "const alternarPresentacion", "const aplicarFiltros"];
  const rangos = MANEJADORES.map((ancla) => {
    const desde = fuente.indexOf(ancla);
    assert.notEqual(desde, -1, `se fue \`${ancla}\`: reanclar este candado, no borrarlo`);
    const hasta = fuente.indexOf("\n  };", desde);
    assert.notEqual(hasta, -1, `no se encontró el fin de \`${ancla}\``);
    return { ancla, desde, hasta };
  });
  ...
  // Y las tres siguen existiendo de verdad: sin esto, borrar los manejadores
  // dejaría la lista de arriba en cero llamadas sueltas y el candado en verde.
  for (const r of rangos) {
    assert.ok(
      fuente.slice(r.desde, r.hasta).includes("setFiltros("),
...
test("G17-pantalla. la pantalla apaga cada bloque desde el otro", () => {
  ...
  // Y nadie escribe presentaciones fuera de esos tres, igual que con los filtros.
  const rangos = ["const alternarControl", "const alternarPresentacion", "const aplicarFiltros"].map(
    (a) => {
      const desde = fuente.indexOf(a);
      return { desde, hasta: fuente.indexOf("\n  };", desde) };
    }
  );
  const sueltas = [...fuente.matchAll(/setPresentaciones\(/g)]
    .map((m) => m.index)
    .filter((pos) => !rangos.some((r) => pos > r.desde && pos < r.hasta));
  assert.deepEqual(sueltas, [], "hay escrituras de presentaciones fuera de los tres manejadores");
});
