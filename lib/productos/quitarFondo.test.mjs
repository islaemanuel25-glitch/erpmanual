...
//
// ── EL MOTOR CAMBIÓ, Y ESTE ARCHIVO SE RELEYÓ ENTERO ──────────────────────
//
// El motor principal pasó a ser u2netp y el de bordes quedó de respaldo. Eso
// movió código a `motorBordes.js` y a `confianzaDelRecorte.js`, así que se
// releyeron los diecisiete candados preguntando qué afirma cada uno HOY, no si
// pasa. Es la regla del repo y acá se cobró:
//
//   · Q10 buscaba `cola = new Int32Array` en `quitarFondo.js`. El patrón se mudó
//     entero a `motorBordes.js`. El candado se puso ROJO y avisó solo.
//   · **Q1 se habría quedado en VERDE afirmando menos.** Prohibía la palabra
//     jpeg en `quitarFondo.js`, y ahora hay DOS archivos de motor más donde
//     nadie miraba. Un motor nuevo podía escribir jpeg y el candado no se
//     enteraba. Ese es el que hay que ver releyendo, porque no se queja.
//   · Q6, Q7, Q8, Q12, Q13 y Q14 siguen midiendo el motor por BORDES, que ahora
//     es el respaldo. Siguen valiendo —el respaldo tiene que seguir andando— pero
//     ya no describen lo que ve la persona. Lo que hace u2netp con esos mismos
//     casos lo mide la sonda, porque necesita WebAssembly.
...
import { distanciaDeColor, quitarFondoPorBordes } from "@/lib/productos/motorBordes";
import { tipoConAlfa } from "@/lib/productos/quitarFondo";
...
const MOTOR_BORDES = leer("lib/productos/motorBordes.js");
const MOTOR_U2NETP = leer("lib/productos/u2netpMotor.js");
...
/** Todo lo que participa del recorte. La lista crece con cada motor nuevo. */
const TODOS_LOS_MOTORES = { QUITAR, MOTOR_BORDES, MOTOR_U2NETP, RECURSOS };
...
test("Q1. TRANSPARENCIA NUNCA EN JPEG — la regla que no se negocia", () => {
  ...
  // ── Y EN NINGÚN ARCHIVO DEL RECORTE PUEDE APARECER JPEG ─────────────────
  //
  // Antes esto miraba UN archivo. Al partir el motor en varios, mirar solo
  // `quitarFondo.js` habría dejado dos archivos nuevos sin vigilar, y el candado
  // habría seguido en verde afirmando menos de lo que dice su título. Se mira la
  // lista entera, y agregar un motor obliga a agregarlo a la lista.
  for (const [nombre, fuente] of Object.entries(TODOS_LOS_MOTORES)) {
    assert.doesNotMatch(fuente, /jpe?g/i, `${nombre} nombra jpeg en algún lado`);
  }
  assert.ok(
    Object.keys(TODOS_LOS_MOTORES).length >= 4,
    "la lista de archivos del recorte se achicó: alguno dejó de estar vigilado"
  );
});
...
test("Q10. UNA FOTO GRANDE NO DESBORDA LA PILA", () => {
  ...
  // El patrón se mudó con el motor: vive en `motorBordes.js`. Buscarlo en
  // `quitarFondo.js` seguiría pasando el día que alguien lo vuelva recursivo.
  assert.match(MOTOR_BORDES, /cola = new Int32Array/, "el relleno dejó de ser iterativo");
});
...
test("Q11. EL MOTOR ENTRA POR UNA COSTURA, no está cosido a la pantalla", () => {
  // ── ESTE CANDADO YA SE COBRÓ SU PRECIO, Y A FAVOR ───────────────────────
  //
  // Se escribió cuando el motor era el de bordes, apostando a que cambiarlo
  // fuera cambiar un archivo. El cambio ocurrió —entró u2netp, con dependencia,
  // modelo de 18 MB y WebAssembly— y `CargarFotoProducto.jsx` NO se tocó.
  ...
  // Y TAMPOCO CONOCE AL MOTOR NUEVO. Es la mitad que hubo que agregar: sin esto,
  // la próxima vez alguien importa `onnxruntime-web` desde la pantalla para
  // mostrar el avance de la descarga y la costura se rompe sin que nada avise.
  assert.doesNotMatch(
    PANTALLA,
    /onnxruntime|u2netp|InferenceSession|modelos\//i,
    "la pantalla se enteró de qué motor hay adentro"
  );
  assert.ok(
...
test("Q17. LA CASCADA: u2netp primero, bordes SOLO como respaldo", () => {
  // ── LO QUE ESTO IMPIDE, Y ES LO QUE PIDIÓ EMANUEL ───────────────────────
  //
  // Que el motor por bordes vuelva a ser el camino principal por descuido. Se
  // aprobó dejarlo como respaldo técnico y nada más: si un día alguien invierte
  // el orden "porque es más rápido", las fotos vuelven a salir mal y nadie lo
  // nota, porque las dos ramas devuelven una imagen.
  const cascada = QUITAR.match(/async function recortar\([\s\S]*?\n\}/);
  assert.ok(cascada, "no se encontró la cascada de motores");
  const cuerpo = cascada[0];

  // u2netp va en el `try`, o sea PRIMERO; bordes en el `catch`, o sea solo
  // después de que el otro falló. El orden en el texto lo dice.
  const iU2 = cuerpo.indexOf("mascaraDeProducto");
  const iBordes = cuerpo.indexOf("quitarFondoPorBordes");
  assert.ok(iU2 >= 0, "la cascada no llama al motor principal");
  assert.ok(iBordes >= 0, "la cascada perdió el respaldo");
  assert.ok(iU2 < iBordes, "el motor por bordes volvió a correr primero");
  assert.match(cuerpo, /catch[\s\S]*quitarFondoPorBordes/, "el respaldo dejó de estar en el catch");

  // Y SE DICE CUÁL CORRIÓ. Sin esto, un respaldo permanente —el modelo que nunca
  // carga en cierto teléfono— se ve igual que el motor bueno andando.
  assert.match(cuerpo, /motor: "u2netp"/);
  assert.match(cuerpo, /motor: "bordes"/);
  assert.match(cuerpo, /motivoDelRespaldo/);
});

test("Q18. EL RESPALDO NO ARRANCA SOBRE UN RECORTE A MEDIO HACER", () => {
  // ── EL DEFECTO QUE ESTO IMPIDE ──────────────────────────────────────────
  //
  // Los dos motores escriben sobre el MISMO arreglo de píxeles. u2netp puede
  // fallar DESPUÉS de haber compuesto alfa —la falla no es siempre al inferir—,
  // y en ese caso el motor por bordes empezaría a trabajar sobre una imagen que
  // ya tiene medio fondo borrado. El resultado sería plausible y equivocado.
  //
  // No se puede ejercer sin navegador, así que se afirma que la restitución del
  // alfa está en el camino del respaldo y antes de llamarlo.
  const cascada = QUITAR.match(/async function recortar\([\s\S]*?\n\}/)[0];
  const iRestituye = cascada.indexOf("px[i] = 255");
  const iBordes = cascada.indexOf("quitarFondoPorBordes");
  assert.ok(iRestituye >= 0, "el respaldo dejó de limpiar el alfa antes de correr");
  assert.ok(iRestituye < iBordes, "limpia el alfa DESPUÉS de recortar, que no sirve de nada");
});
