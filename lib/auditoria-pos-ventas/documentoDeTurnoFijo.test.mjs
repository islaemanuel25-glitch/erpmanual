...
export function claseDelRecuadroDeCaja(fuente) {
  ...
  // El ancla lleva el `||` porque la versión corta aparece DOS veces: la condición
  // que monta el recuadro y la del campo de adentro. Anclado en la corta, un
  // reordenamiento la haría agarrar la de adentro y devolver el className de un
  // div interno —que no tiene color— así que el candado pasaría en verde sin
  // haber mirado el recuadro. Lo destapó la contraprueba del candado hermano.
...
test("el ancla del recuadro es ÚNICA en el archivo", () => {
  const veces = sinComentarios(FUENTE).split(ANCLA_RECUADRO).length - 1;
