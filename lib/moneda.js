 ...
 * tarjeta: son 142 en producción y se veían iguales. La franja de escala es el
 * lugar natural para decirlo, porque ya contesta "qué es lo que estás comprando"
 * y está justo debajo del precio.
 *
 * Va como PREFIJO y no como texto aparte, para que se componga con lo que la
 * escala ya diga en vez de reemplazarlo:
 *
 *   · combo suelto   — `Combo · se vende por unidad`
 *   · combo por kilo — `Combo · se vende por kilo · $129,80 cada 100 g`
 *   · combo de pack  — `Combo · 1 pack = 6 un · $583,17 por unidad`
 *
 * **Medido: los 142 combos de producción son todos de unidad y ninguno tiene
 * factor mayor a 1**, así que hoy solo ocurre el primero. Los otros dos se
 * soportan igual porque nada impide cargarlos mañana, pero no se probaron contra
 * datos reales y eso está dicho — no se fabricó una fila para que la captura
 * saliera linda.
 *
 * @param {object} p
...
export function lineaDeEquivalencia({
  ...
  const conPrefijo = (frase) =>
    esCombo ? `Combo · ${frase.charAt(0).toLowerCase()}${frase.slice(1)}` : frase;
  ...
  if (!Number.isFinite(f) || f <= 1) return conPrefijo("Se vende por unidad");

  // ── DE ACÁ PARA ABAJO ES EL CASO DE BULTO, Y ES EL ÚNICO QUE LAS DOS ──────
  // ── PREFERENCIAS TOCAN ───────────────────────────────────────────────────
  //
  // Y ese recorte importa: el interruptor se llama "ocultar la equivalencia de
  // BULTO", así que no puede tocar al 40 % de las filas que no son de bulto —
  // ahí esta línea es lo único que dice la escala, y sacarla les quitaría
  // información en vez de simplificarles la vista. Por eso las dos ramas de
  // arriba —kilo y suelto— salen antes de mirar las preferencias.

  // OCULTAR LA EQUIVALENCIA no deja el renglón vacío: lo reemplaza por la escala
  // dicha con palabras, que es lo que hacen los otros dos casos. Un renglón que
  // desaparece dejaría esa tarjeta más baja que las vecinas y perdería el dato.
  if (ocultarEquivalencia) {
    return conPrefijo(mostrarUnitario ? "Se vende por unidad" : "Se vende por bulto");
  }
  ...
  if (mostrarUnitario) return conPrefijo(`1 pack = ${f} un`);
