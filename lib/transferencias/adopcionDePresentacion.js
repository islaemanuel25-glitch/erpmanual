...
/**
 * LA EQUIVALENCIA, PARA MOSTRARLA ANTES DE DECIDIR.
 *
 * "5 CAJÓN x8", o "5 CAJÓN x8 + 2 unidades sueltas" cuando la cantidad no cae
 * redonda. El operador tiene que poder VER en qué se va a convertir su línea
 * antes de tocar el botón: adoptar sin saber el resultado es firmar en blanco.
 *
 * Es la MISMA conversión que persiste el servidor —`conversionParaAdoptar`— y
 * el MISMO formateador que usa la auditoría —`rotuloConSueltas`—. Acá no se
 * multiplica ni se formatea nada por segunda vez: si esta cuenta pudiera
 * separarse de la del servidor, la pantalla prometería un número y la base
 * guardaría otro.
 *
 * Devuelve `ok: false` con su motivo cuando no se puede representar exacto, y
 * ahí la pantalla NO ofrece adoptar: se queda con la lectura histórica.
 */
export function equivalenciaParaAdoptar({ fisicasM, presentacion, factor = null, pesoPiezaKg = null } = {}) {
  const conv = conversionParaAdoptar({ fisicasM, presentacion, factor, pesoPiezaKg });
  if (!conv.ok) return conv;
  return {
    ok: true,
    conversion: conv,
    rotulo: rotuloConSueltas({
      presentacion,
      cantidad: conv.cantidadPresentada,
      sueltas: conv.sueltasEnviadas,
...
/**
 * ¿ESTA LÍNEA ESTÁ EN MODO DECISIÓN DE ADOPCIÓN HISTÓRICA?
 *
 * ── UNA SOLA CONDICIÓN, TRES CONSUMIDORES ───────────────────────────────
 *
 * Lo preguntan tres lugares y tienen que contestar lo mismo:
 *
 *   · la ficha, para decidir QUÉ dibuja —la decisión o el conteo—;
 *   · la hoja móvil, para su subtítulo "Transferencia histórica";
 *   · el ofrecimiento en sí.
 *
 * Escrita tres veces, el día que una cambie la hoja diría "Transferencia
 * histórica" sobre una ficha normal, o al revés. Por eso vive acá y es pura.
 *
 * Son cinco preguntas y las cinco tienen que darse:
 *
 *   1. se puede recibir — con la transferencia cerrada no hay nada que contar;
 *   2. la línea es una histórica de verdad —`admiteAdopcion`—;
 *   3. el DTO trae la presentación actual del catálogo;
 *   4. esa presentación dice algo DISTINTO de lo que se está mostrando;
 *   5. la conversión es EXACTA. Si no se puede representar sin redondear, no se
 *      ofrece nada y se sigue viendo el hecho histórico.
 *
 * @param {object} linea      La línea, con la forma del DTO de `detalle`.
 * @param {object} opciones
 * @param {boolean} opciones.puedeRecibir
 * @param {number|null} opciones.fisicasM Físicas en milésimas. Se pasa porque la
 *   calcula el consumidor con `enviadoFisicoM`, que ya tiene: pedírsela evita
 *   que este módulo dependa de `controlFisico` y se arme un ciclo.
 *
 * @returns {{activo:boolean, equivalencia:object|null}}
 */
export function modoAdopcionHistorica(linea = {}, { puedeRecibir = false, fisicasM = null } = {}) {
  const inerte = { activo: false, equivalencia: null };

  if (!puedeRecibir) return inerte;
  if (!admiteAdopcion(linea).ok) return inerte;

  const actual = linea.presentacionActual;
  if (!actual) return inerte;
  if (!hayPresentacionDistinta(actual, descriptorDeEnvio(linea))) return inerte;
  if (fisicasM === null) return inerte;

  const equivalencia = equivalenciaParaAdoptar({
    fisicasM,
    presentacion: actual.presentacion,
    factor: actual.factor,
    pesoPiezaKg: actual.pesoPiezaKg,
  });
  if (!equivalencia.ok) return inerte;

  return { activo: true, equivalencia };
}
