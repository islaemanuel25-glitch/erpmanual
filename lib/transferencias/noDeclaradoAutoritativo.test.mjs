...
test("6. el servidor rechaza una unidad que contradiga al catálogo del origen", () => {
  ...
  // el pedido no puede influir en cómo sale el producto.
  //
  // Y ES LA DE SALIDA DEPÓSITO → LOCAL. Un producto que se incorpora durante la
  // recepción se recibe en la escala en que el depósito lo despacha, no en la
  // que se lo compra al proveedor.
  assert.match(src, /const presentacion = presentacionDeSalida\(\{/);
  assert.match(src, /modoEnvio: producto\.base\?\.modo_envio/);
  assert.ok(
    !/modoCompraProveedor/.test(src.replace(/\/\/[^\n]*/g, "")),
...
test("6a. el catálogo del origen manda los tres campos del predicado de pieza", () => {
  ...
  //
  // `modoCompraProveedor` SALIÓ de la lista y `modoEnvio` entró. No es un
  // cambio de nombre: el primero describe la relación proveedor→depósito y no
  // decide nada acá; el segundo es la política de salida del depósito, y sin él
  // la pantalla ofrece "PACK x6" sobre un producto que solo sale suelto y el
  // servidor rechaza el alta con UNIDAD_CONTRADICE_CATALOGO.
  //
  // El nombre en la base no siempre es el del DTO: la columna de la política de
  // salida es `modo_envio`, en snake, y por eso cada campo trae el suyo. Buscar
  // los dos con el mismo nombre dejaba este candado en verde sin mirar nada.
  const src = codigoDe("lib/productos/buscarCatalogoLocal.js");
  const CAMPOS = [
    ["modoVentaDeposito", "modoVentaDeposito"],
    ["modoEnvio", "modo_envio"],
    ["pesoReferenciaKg", "pesoReferenciaKg"],
    ["pesoEsFijo", "pesoEsFijo"],
  ];
  for (const [campo, enLaBase] of CAMPOS) {
    assert.ok(
      new RegExp(`${campo}:\\s*item\\.${campo}`).test(src),
      `la proyección de recepción no manda ${campo}`
    );
    assert.ok(
      new RegExp(`${campo}:[\\s\\S]{0,40}base\\??\\.${enLaBase}`).test(src),
      `el mapper del catálogo no lee ${enLaBase} de la base`
    );
  }
});
