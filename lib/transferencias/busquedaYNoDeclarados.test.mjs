...
// ═══════════════════════════════════════════════════════════════════════════
// LO VISUAL, CONTRA EL DISEÑO APROBADO
// ═══════════════════════════════════════════════════════════════════════════

test("3c. «No declarado» NO usa el gris de «Pendiente»", () => {
  const noDeclarado = renderToStaticMarkup(
    React.createElement(FilaProducto, { d: agregada(), activa: false, onElegir: () => {} })
  );
  const pendiente = renderToStaticMarkup(
    React.createElement(FilaProducto, { d: linea(), activa: false, onElegir: () => {} })
  );

  // El de la derecha del titulo: el estado.
  const tono = (html, texto) => {
    const m = html.match(new RegExp('<span class="([^"]*)"[^>]*>' + texto));
    return m ? m[1] : null;
  };

  const tonoNoDeclarado = tono(noDeclarado, "No declarado");
  const tonoPendiente = tono(pendiente, "Pendiente de revisar");

  assert.ok(tonoNoDeclarado, "no se encontro el estado del no declarado");
  assert.ok(tonoPendiente, "no se encontro el estado del pendiente");
  assert.notEqual(
    tonoNoDeclarado,
    tonoPendiente,
    "un producto que llego sin estar en el remito se ve igual que uno que nadie conto"
  );

  // El pendiente sigue en muted; el no declarado usa SU token semantico.
  assert.match(tonoPendiente, /sunmi-text-muted/);
  // ── Y ES WARNING, NO LINK ─────────────────────────────────────────
  //
  // Estaba en `sunmi-text-link`, el azul de los enlaces: nadie navega a ningún
  // lado desde ahí, y competía con los links de verdad. Un producto que llegó
  // sin estar en el remito es una inconsistencia física —la misma familia que
  // el sobrante— y comparte su token.
  assert.match(tonoNoDeclarado, /sunmi-text-warning/);
  assert.ok(
    !/sunmi-text-link/.test(tonoNoDeclarado),
    "«No declarado» volvió a pintarse como un enlace"
  );

  // Y de tokens del tema, no de colores inventados.
  for (const html of [noDeclarado, pendiente]) {
    assert.ok(!/#[0-9a-fA-F]{3,8}\b/.test(html), "aparecio un color hexadecimal");
    assert.ok(
      !/\b(?:text|bg|border)-(?:green|orange|amber|red|blue)-\d{2,3}\b/.test(html),
      "aparecio la paleta cruda de Tailwind"
    );
  }
});

test("7b. el selector de categoria dice el mismo universo que el tab «Todos»", () => {
  const src = codigoDe(MOVIL);

  // Decia `totalRemito` mientras el tab decia `totalFisico`: con 52 originales y
  // 1 agregado la pantalla mostraba "Todos 53" y "Todas · 52" a cinco pixeles.
  assert.match(src, /Todas · \{resumen\?\.totalFisico/);
  assert.ok(
    !/Todas · \{resumen\?\.totalRemito \?\? 0\}/.test(src),
    "el selector volvio a contar solo el remito"
  );
});

test("8c. el progreso del remito NO se contagia del universo fisico", () => {
  const originales = Array.from({ length: 52 }, (_, i) =>
    linea({
      id: i + 1,
      ...(i < 3
  ...
  const r = resumenDeRecepcion([...originales, agregada()]);

  // Los cuatro numeros del encabezado, tal como el diseño los pide.
  assert.equal(r.revisados, 3);
  assert.equal(r.totalRemito, 52);
  assert.equal(r.pendientes, 49);
  assert.equal(r.totalFisico, 53);

  // El movil los reparte asi: los tres tabs del trabajo cuentan el remito y
  // "Todos" cuenta lo fisico.
  const src = codigoDe(MOVIL);
  assert.match(src, /\[FILTRO\.PENDIENTES\]: \(r\) => r\.pendientes/);
  assert.match(src, /\[FILTRO\.REVISADOS\]: \(r\) => r\.revisados/);
  assert.match(src, /\[FILTRO\.TODOS\]: \(r\) => r\.totalFisico/);
...
test("9. el camino de excepcion va DESPUES de los filtros y la categoria", () => {
  // El orden aprobado: resumen → buscador → filtros → categoria → aviso → CTA.
  // Estaba entre el buscador y los filtros, y empujaba el trabajo normal hacia
  // abajo cada vez que una busqueda no encontraba algo.
  for (const [rel, marcaFiltros] of [
    [MOVIL, "SunmiFiltroEstado"],
    [WORKSPACE, "SunmiSelectorUnidad"],
  ]) {
    const src = codigoDe(rel);
    const filtros = src.indexOf(marcaFiltros);
    const categoria = src.lastIndexOf("Categor");
    const aviso = src.indexOf("noFigura && puedeRecibir");

    assert.ok(filtros > 0 && categoria > 0 && aviso > 0, `${rel}: falta alguna de las tres piezas`);
    assert.ok(filtros < aviso, `${rel}: el aviso sigue antes de los filtros`);
    assert.ok(categoria < aviso, `${rel}: el aviso sigue antes de la categoria`);
  }
});
