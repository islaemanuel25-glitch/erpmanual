...
test("21j. LAS SUPERFICIES PASAN EL ORIGEN, o el arreglo no corre nunca", () => {
  ...
  // Y ADENTRO DE `agregadosPeriodo`, UNA POR UNA. Buscar el nombre en el archivo
  // no alcanza: tiene TRES lugares que valorizan y basta con que uno lo pase para
  // que el candado de arriba dé verde. Se escapó exactamente así —
  // `agruparPorDestino`, que es el que contesta cuánto se le mandó a cada local,
  // quedó sin el origen y la búsqueda por nombre no lo vio.
  const agregados = leerSinComentarios("lib/transferencias/agregadosPeriodo.js");
  const llamadas = agregados.match(/importeDeDetalleCentavos\(|importeDeLinea\(/g) ?? [];
  const conOrigen = agregados.match(/importeDe(?:DetalleCentavos|Linea)\([^)]*origenEsDeposito/gs) ?? [];
  // Las dos declaraciones `export function` también matchean el primer patrón.
  assert.equal(
    llamadas.length - 2,
    conOrigen.length,
    `hay ${llamadas.length - 2} llamadas que valorizan y solo ${conOrigen.length} pasan el origen`
  );
});
