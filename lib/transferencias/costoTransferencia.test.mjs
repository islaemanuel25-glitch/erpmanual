...
// ── QUÉ FORMA DE LÍNEA DESCRIBEN 13, 14 Y 15, Y POR QUÉ SIGUEN VALIENDO ───
//
// Los tres usan filas SIN SNAPSHOT: no traen `presentacionEnvio` ni
// `factorPresentacion`. En esa forma `recibido` está en la misma escala que
// `cantidad` —las dos físicas para UNIDAD, las dos en bultos para BULTO— y el
// cálculo de siempre es el correcto. Son 1.665 líneas de producción y no se
// tocaron.
//
// Se revisaron al arreglar la #198 y NO hizo falta reescribirlos: lo que
// afirman sigue siendo verdad para la forma que describen. Lo que se agregó es
// lo que les faltaba decir —que esa forma es una de dos— y eso vive en
// `valorizacionDelRemito.test.mjs`, donde está el caso con snapshot: ahí
// `cantidad` viene en unidades físicas y `recibido` en packs, en la misma fila,
// y multiplicar uno por el costo del otro era el defecto.
//
// Si algún día un candado de acá recibe una fila CON snapshot, hay que mirarlo
// de nuevo: el supuesto implícito deja de valer.
...
test("18d. agregadosPeriodo no reimplementa la fórmula: la importa", () => {
  ...
  // El 2026-09-11 la primitiva cambió, y lo que este candado afirma no: que el
  // reporte NO tenga su propia fórmula. Antes pedía `valorizarDetalle`, que
  // valoriza lo RECIBIDO; ahora el reporte contesta por el importe ENVIADO y la
  // primitiva que corresponde es `valorizarLineaDelRemito`. Es la misma
  // exigencia con otro nombre, no un candado aflojado — y sigue prohibido
  // reimplementar la división acá.
  assert.ok(
    /valorizarLineaDelRemito/.test(src),
    "agregadosPeriodo debe usar valorizarLineaDelRemito"
  );

  // ── AHORA SON DOS NÚMEROS, Y CADA UNO CON SU PRIMITIVA ────────────────
  //
  // Este candado prohibía `valorizarDetalle` en TODO el archivo, y el motivo era
  // bueno: que el importe del reporte no cambiara mientras alguien cuenta.
  //
  // El 2026-09-13 el módulo ganó un segundo número que no existía —`importeRecibido*`,
  // lo que el local PAGA— y ese sí se valoriza sobre lo recibido, porque cada
  // local paga lo que llegó y no lo que salió. Los dos conviven: el del remito es
  // el documento de lo despachado y no se toca.
  //
  // Prohibir el símbolo dejaba de defender el concepto y pasaba a impedir el
  // segundo. Así que ahora se afirma lo que de verdad importa: **cuál función usa
  // cada uno**, y que ninguno cruce a la primitiva del otro.
  const enviado = src.slice(src.indexOf("export function importeEnviadoDeLinea"));
  const cuerpoEnviado = enviado.slice(0, enviado.indexOf("export function importeRecibidoDeLinea"));
  assert.ok(
    /valorizarLineaDelRemito/.test(cuerpoEnviado),
    "el importe del REMITO dejó de salir de su primitiva"
  );
  assert.ok(
    !/valorizarDetalle/.test(cuerpoEnviado),
    "el importe del REMITO pasó a valorizar lo recibido: cambiaría al contar"
  );

  const recibido = src.slice(src.indexOf("export function importeRecibidoDeLinea"));
  assert.ok(
    /valorizarDetalle/.test(recibido) && /cantidadModo: "VALORIZAR"/.test(recibido),
    "lo que el local paga dejó de valorizar lo RECIBIDO"
  );
  assert.ok(
    !/valorizarLineaDelRemito/.test(recibido.slice(0, recibido.indexOf("export function importeRecibidoDeDetalleCentavos"))),
    "lo que el local paga pasó a cobrar el remito en vez de lo que llegó"
  );
  assert.ok(
    !/precioCosto\s*\/\s*factor|\/\s*factor_pack/.test(src),
...
test("21k. las funciones que valorizan EXIGEN el origen — no tienen default", () => {
  ...
  const casos = [
    ["importeEnviadoDeLinea", () => importeEnviadoDeLinea({ cantidad: 1, precioCosto: 100 })],
