 ...
 * @param {number}   [opts.ventaId]           Venta que originó el envío (venta interna).
 *   Excluyente con posTransferenciaId: una transferencia no puede tener dos
 *   documentos de origen. Los dos en null se aceptan (no hay evidencia de que el
 *   flujo actual dependa de rechazarlo, y romperlo sería un cambio no pedido).
...
export async function crearTransferencia({
  ...
  // Una transferencia tiene UN documento de origen: la POS manual o la venta.
  if (posTransferenciaId != null && ventaId != null) {
    throw error(
      "ORIGEN_DOCUMENTAL_AMBIGUO",
      "Una transferencia no puede originarse en una POS y en una venta a la vez"
    );
  }
  ...
  for (const item of items) {
    ...
    // ── EL SNAPSHOT DE PRESENTACIÓN, SI EL LLAMADOR LO TRAE ─────────────────
    //
    // Congela CÓMO se contó lo que salió: la presentación, cuántos bultos, las
    // sueltas, el factor y —para PIEZA— el peso de referencia. Sin esto, leer
    // una transferencia vieja significa preguntarle al catálogo de hoy, y editar
    // un producto reescribe la historia de un remito que ya salió.
    //
    // Es OPCIONAL a propósito: un llamador que todavía no lo arme sigue creando
    // transferencias válidas, con el snapshot en null. Es lo mismo que les pasa a
    // las líneas anteriores a la migración, y se lee igual.
    //
    // La cantidad FÍSICA no se toca: sigue saliendo de `item.cantidad`, que es la
    // que mantiene paridad con el descuento de stock del origen.
    const p = item.presentacion || null;

    // Datos congelados del detalle.
    detallesTransferencia.push({
      ...
      ...(p
        ? {
            presentacionEnvio: p.presentacionEnvio,
            cantidadPresentada: p.cantidadPresentada,
            sueltasEnviadas: p.sueltasEnviadas ?? 0,
            factorPresentacion: p.factorPresentacion ?? null,
            pesoPiezaKg: p.pesoPiezaKg ?? null,
          }
        : {}),
    });
