...
// ── 5. Documento de origen: POS manual vs venta interna (etapa 4) ─────────────

test("venta interna: persiste ventaId y deja posTransferenciaId en null", async () => {
  const tx = fakeTx();
  const { transferencia } = await crearTransferencia({
    ...BASE_ARGS,
    ...
    politicaStockOrigen: SOLO_TRANSITO,
    tx,
    items: [item({ cantidad: 36, unidadEnviada: "UNIDAD", factorPackItem: 1 })],
  });
  const data = tx.calls.transferenciaCreate[0].data;
  assert.equal(data.ventaId, 555);
  assert.equal(data.posTransferenciaId, null);
  assert.equal(data.estado, "Enviada");
  assert.ok(data.fechaEnvio instanceof Date);
  assert.equal(transferencia.ventaId, 555);
});

test("flujo manual: conserva posTransferenciaId y ventaId queda null", async () => {
  const tx = fakeTx();
  await crearTransferencia({
    ...BASE_ARGS, // posTransferenciaId: 42
    tx,
    items: [item()],
  });
  const data = tx.calls.transferenciaCreate[0].data;
  assert.equal(data.posTransferenciaId, 42);
  assert.equal(data.ventaId, null, "por defecto no hay venta de origen");
});

test("rechaza los dos documentos de origen a la vez", async () => {
  const tx = fakeTx();
  await assert.rejects(
    () => crearTransferencia({
      ...BASE_ARGS, // posTransferenciaId: 42
    ...
    (e) => {
      assert.equal(e.code, "ORIGEN_DOCUMENTAL_AMBIGUO");
      return true;
    }
  );
...
test("los dos en null se siguen aceptando (compatibilidad, sin evidencia de romperlo)", async () => {
  const tx = fakeTx();
  const { transferencia } = await crearTransferencia({
    origenId: 1, destinoId: 2, creadoPorId: 7, tx, items: [item()],
  });
  assert.equal(transferencia.posTransferenciaId, null);
  assert.equal(transferencia.ventaId, null);
});

test("venta interna con SOLO_TRANSITO: NO descuenta cantidad, solo enTransito", async () => {
  const tx = fakeTx();
  await crearTransferencia({
    ...BASE_ARGS,
    posTransferenciaId: null,
    ...
    politicaStockOrigen: SOLO_TRANSITO,
    tx,
    items: [item({ cantidad: 1.925, unidadEnviada: "UNIDAD", factorPackItem: 1 })],
  });
  const mov = tx.calls.stockLocalUpsert[0];
  assert.deepEqual(mov.update, { enTransito: { increment: 1.925 } });
  assert.equal("cantidad" in mov.update, false, "el descuento ya lo hizo la venta");
  // Y apunta al ProductoLocal del ORIGEN.
  assert.equal(mov.where.localId_productoId.localId, 1);
  assert.equal(mov.where.localId_productoId.productoId, 11);
});
