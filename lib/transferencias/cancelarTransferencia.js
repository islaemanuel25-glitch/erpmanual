...
export const MOTIVO_MIN_CANCELACION = 10;
 ...
 * @param {object|null} args.turnoAbierto   turno operativo del local de la VENTA, si hay venta
...
export function puedeCancelarTransferencia({
  ...
  if (typeof motivo !== "string" || motivo.trim().length < MOTIVO_MIN_CANCELACION) {
    return no(
      CODIGOS_CANCELAR.MOTIVO_AUSENTE,
      `Escribí por qué se cancela, con al menos ${MOTIVO_MIN_CANCELACION} caracteres. Una transferencia ` +
        `cancelada sin motivo es indistinguible dentro de un mes de un error de datos.`
    );
  }

  // Con venta vinculada hay que revertirla también, y para eso hace falta un
  // turno abierto donde imputar la diferencia de caja. Sin venta, no aplica.
  const venta = transferencia.venta || null;
  if (venta) {
    if (venta.anuladaEn != null) {
      return no(
        CODIGOS_CANCELAR.ESTADO_INVALIDO,
        `La venta #${venta.numero ?? venta.id} que originó este remito ya está anulada.`
      );
    }
    if (!turnoAbierto || !idValido(turnoAbierto.id)) {
      return no(
        CODIGOS_CANCELAR.SIN_TURNO_DESTINO,
        "Esta transferencia vino de una venta, y para revertirla hace falta una caja abierta " +
          "en el local que vendió. Abrí la caja y volvé a intentar."
      );
    }
  }
...
export function resumenDeLaCancelacion(transferencia) {
  ...
  if (venta) {
    efectos.push(
      `Se anula la venta #${venta.numero ?? venta.id}, que conserva su ticket y queda marcada.`,
