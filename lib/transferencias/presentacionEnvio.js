...
export function descriptorDeEnvio(linea = {}) {
  ...
  // ── LAS DOS FORMAS DE LA MISMA LÍNEA, LEÍDAS POR UNA SOLA PUERTA ────────
  //
  // Esta línea llega de dos lados y con dos formas. El DTO de
  // `/api/transferencias/detalle` trae el catálogo PLANO —`unidadMedida`,
  // `factorPack`—; una fila de Prisma lo trae COLGANDO de `producto.base`, y en
  // snake. Hasta el 2026-09-13 este módulo solo entendía la primera, así que una
  // fila de Prisma caía en la reconstrucción sin catálogo y salía en UNIDAD con
  // factor 1 — o sea, la escala equivocada, en silencio.
  //
  // Eso es lo que empujó a los agregados del período a escribir la cuenta a mano:
  // llamaron a la puerta canónica, les devolvió algo inservible, y la resolvieron
  // aparte. El pozo era del módulo, no de quien lo usó. Ver `INC-0009`.
  const deCatalogo = linea.producto?.base || {};
  const { presentacion, factor, pesoPiezaKg } = presentacionDeProducto({
    unidadMedida: linea.unidadMedida ?? deCatalogo.unidad_medida,
    factorPack: linea.factorPack ?? deCatalogo.factor_pack,
    modoVentaDeposito: linea.modoVentaDeposito ?? deCatalogo.modoVentaDeposito,
    pesoReferenciaKg: linea.pesoReferenciaKg ?? deCatalogo.pesoReferenciaKg,
    modoCompraProveedor: linea.modoCompraProveedor ?? deCatalogo.modoCompraProveedor,
    pesoEsFijo: linea.pesoEsFijo ?? deCatalogo.pesoEsFijo,
...
/**
 * ════════════════════════════════════════════════════════════════════════════
 * CUÁNTO SE CONTÓ, LEÍDO DE CUALQUIERA DE LAS DOS FORMAS — O UN ERROR RUIDOSO
 * ════════════════════════════════════════════════════════════════════════════
 *
 * El mismo hecho tiene dos nombres según de dónde venga la línea:
 *
 *     DTO de `/api/transferencias/detalle`  →  `cantidadRecibida`
 *     fila de Prisma                        →  `recibido`
 *
 * Y hasta el 2026-09-13 las funciones canónicas leían solo el primero. Pasarles
 * una fila de Prisma devolvía **null**, que en este dominio significa "todavía no
 * se contó" — una respuesta perfectamente válida y completamente falsa. Medido:
 * `fisicasRecibidasDe` sobre la fila del detalle 6843 devolvía null teniendo 4
 * cajones guardados.
 *
 * **Ese silencio es el pozo.** Quien escribió los agregados del período llamó a la
 * puerta canónica, recibió null, y terminó sumando `d.recibido` a mano contra
 * `d.cantidad` — dos escalas distintas. El defecto no fue descuido: fue que la
 * función correcta contestaba mal sin avisar.
 *
 * Así que esta puerta hace las dos cosas:
 *
 *   · acepta las DOS formas, y por eso hay una sola puerta y no dos;
 *   · y si le llega un objeto que no trae NINGUNO de los dos campos, **tira un
 *     error con el nombre de los dos**, en vez de devolver null. Un objeto que no
 *     puede contestar la pregunta no es una línea sin contar: es el objeto
 *     equivocado, y eso tiene que doler donde ocurre.
 *
 * `null` queda reservado para su único significado: el campo está y vale `null`.
 *
 * @returns {{cantidad: any, sueltas: any}|null} los dos números CRUDOS, en la
 *   escala de la presentación. No se convierten acá: `milesimasFisicas` sabe leer
 *   `Decimal`, string y número, y convertir dos veces es donde se pierden las
 *   milésimas.
 */
export function recibidoDeLinea(d = {}) {
  const esDto = Object.prototype.hasOwnProperty.call(d, "cantidadRecibida");
  const esFila = Object.prototype.hasOwnProperty.call(d, "recibido");

  if (!esDto && !esFila) {
    throw new Error(
      "recibidoDeLinea: la línea no trae `cantidadRecibida` (DTO) ni `recibido` " +
        "(fila de Prisma). Sin uno de los dos no se puede saber cuánto llegó, y " +
        "contestar `null` diría «no se contó», que es otra cosa. " +
        "Si estás sumando en el servidor, la fila tiene que venir con `recibido`; " +
        "si es una pantalla, con `cantidadRecibida`."
    );
  }

  const cruda = esDto ? d.cantidadRecibida : d.recibido;
  if (cruda === null || cruda === undefined) return null;
  return { cantidad: cruda, sueltas: d.recibidoUnidadesSueltas ?? 0 };
}
