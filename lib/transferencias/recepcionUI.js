 ...
 * ambiguo: en KG y en PIEZA la diferencia vive en esa escala, en los agrupados
 * son unidades y escribirlo es ruido.
 *
 * @param {object} a
...
export function resultadoDeConteo({ recibidas, enviadas, envio } = {}) {
  ...
  const cabeza = `${fmtCant(recibidas)} de ${fmtCant(enviadas)}`;
  if (delta === 0) return `${cabeza} · sin diferencia`;

  const u = unidadDeDiferencia(envio || {});
  const sufijo = u === "unidades" ? "" : ` ${u}`;
