 ...
 *
 * La decisión no se toma acá: sale de `presentacionDeSalida`, el único lugar
 * del repo donde se decide qué es un pack, un cajón, un kilo o una pieza.
 */
export function presentacionDeProductoNuevo(producto = {}) {
  // Es la MISMA función que usa `linea-recepcion` para decidir del lado del
  // servidor. Si acá se preguntara otra cosa, la pantalla ofrecería una escala
  // y el alta se rechazaría con UNIDAD_CONTRADICE_CATALOGO.
  return presentacionDeSalida({
    unidadMedida: producto?.unidadMedida,
    factorPack: producto?.factorPack,
    modoEnvio: producto?.modoEnvio,
...
export function validarLineaNueva({
  ...
  // Una línea que no informa NADA no tiene sentido: sería agregar un producto
  // para decir que llegaron cero.
  if (cantidad <= 0 && sueltas <= 0) {
    return {
      ok: false,
      error: "CANTIDAD_INVALIDA",
