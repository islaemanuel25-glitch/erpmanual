...
// De físicas a la escala en la que se cuenta: completos y resto, nunca una
// fracción de bulto. Es la MISMA conversión que persiste el servidor al adoptar
// —se escribió para eso— y por eso se reusa en vez de dividir acá al lado.
import { conversionParaAdoptar } from "./adopcionDePresentacion.js";
...
/**
 * EL RESULTADO DEL CONTEO, EN UNA LÍNEA: "47 de 48 · falta 1".
 *
 * ── POR QUÉ VIVE ACÁ Y NO EN CADA PANTALLA ──────────────────────────────
 *
 * Lo dicen DOS superficies: la tarjeta del teléfono, en su aviso de diferencia,
 * y el panel de corrección, en su bloque de ingreso físico. Con una copia en
 * cada una, el día que cambie el vocabulario —o el singular, o la unidad— una
 * diría "falta 1" y la otra "faltan 1" sobre la misma línea y en la misma
 * pantalla. Es la regla 1.
 *
 * ── LAS TRES DECISIONES QUE ENCAPSULA ───────────────────────────────────
 *
 * El SENTIDO se dice al derecho: faltar y sobrar son dos hechos distintos y
 * "−4" obliga a interpretarlo. El SINGULAR se respeta —"falta 1", no
 * "faltan 1"—, que es el caso más frecuente de todos: una unidad rota adentro
 * de un bulto. Y la UNIDAD se nombra solo cuando decir el número pelado sería
 * ambiguo.
 *
 * ── Y LA CABEZA HABLA EN LA ESCALA EN LA QUE SE CUENTA ──────────────────
 *
 * Éste es el arreglo del V25 y vale la pena entender el defecto: la línea se
 * despachaba en PACK x6, el operador contaba PACKS —el campo dice "PACK x6
 * completos"— y el mensaje le contestaba "15 de 6 · sobran 9". Los tres números
 * estaban en unidades físicas y ninguno coincidía con lo que la persona tenía
 * en la mano. Para verificarlo había que multiplicar por 6 de cabeza.
 *
 * Ahora dice "2 PACK x6 + 3 de 1 PACK x6 · sobran 9 unidades": la cabeza en la
 * escala del conteo, el resto en el formato corto —"+ 3" y no "+ 3 unidades
 * sueltas", que en un renglón de aviso no entra—, y la diferencia en unidades
 * físicas CON la palabra escrita. Ese "unidades" no es redundante acá: con la
 * cabeza en packs, un "9" pelado se leería como nueve packs.
 *
 * Solo cambia para los AGRUPADOS. En UNIDAD, KG y PIEZA la escala de conteo ya
 * es la física, así que "47 de 48 · falta 1" habla en la escala correcta desde
 * siempre y se deja igual. Y si la conversión no es exacta —una física
 * fraccionaria dentro de un bulto— se cae al formato viejo en vez de inventar
 * una fracción de pack.
 *
 * @param {object} a
 * @param {number} a.recibidas  físicas contadas
 * @param {number} a.enviadas   físicas del remito
 * @param {object} a.envio      el descriptor, para saber en qué escala mide
 * @returns {string|null} `null` si falta alguno de los dos números
 */
export function resultadoDeConteo({ recibidas, enviadas, envio } = {}) {
  if (recibidas == null || enviadas == null) return null;
  const delta = Number(recibidas) - Number(enviadas);
  const enEscala = agrupa(envio?.presentacion);
  const cabeza = enEscala
    ? cabezaEnEscala(recibidas, enviadas, envio)
    : `${fmtCant(recibidas)} de ${fmtCant(enviadas)}`;
  if (delta === 0) return `${cabeza} · sin diferencia`;
  ...
  // Con la cabeza en packs la unidad SIEMPRE se nombra. Sin agrupar se mantiene
  // la regla vieja: "unidades" a secas era ruido cuando los tres números ya
  // estaban en esa escala.
  const sufijo = !enEscala && u === "unidades" ? "" : ` ${u}`;
  const verbo =
    delta < 0 ? (abs === 1 ? "falta" : "faltan") : abs === 1 ? "sobra" : "sobran";
  return `${cabeza} · ${verbo} ${fmtCant(abs)}${sufijo}`;
}

/**
 * "2 PACK x6 + 3 de 1 PACK x6". Devuelve `null` si alguno de los dos lados no se
 * puede representar en la escala sin redondear, y ahí manda el formato viejo.
 */
function enLaEscala(fisicas, envio) {
  const conv = conversionParaAdoptar({
    // La conversión trabaja en milésimas enteras, como el resto del dominio.
    fisicasM: Math.round(Number(fisicas) * 1000),
    presentacion: envio?.presentacion,
    factor: envio?.factor,
    pesoPiezaKg: envio?.pesoPiezaKg ?? null,
  });
  if (!conv.ok) return null;
  const base = rotuloDeEnvio({
    presentacion: envio?.presentacion,
    cantidad: conv.cantidadPresentada,
    factor: conv.factorPresentacion,
  });
  return conv.sueltasEnviadas > 0 ? `${base} + ${fmtCant(conv.sueltasEnviadas)}` : base;
}

function cabezaEnEscala(recibidas, enviadas, envio) {
  const r = enLaEscala(recibidas, envio);
  const e = enLaEscala(enviadas, envio);
  if (r === null || e === null) return `${fmtCant(recibidas)} de ${fmtCant(enviadas)}`;
  return `${r} de ${e}`;
}
