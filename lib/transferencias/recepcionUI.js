...
/**
 * CUÁL DE LOS DOS COSTOS SE MUESTRA AL LADO DEL ENVIADO.
 *
 * El endpoint manda los dos ya calculados: `precioCosto` es el de la
 * presentación que el renglón rotula —el de un pack— y `costoUnitarioFisico` el
 * de una unidad de stock. Ninguno se divide en la pantalla.
 *
 * La elección sigue a la presentación que se está MOSTRANDO: si la línea salió
 * sin ningún bulto entero, lo que se rotula es "1 UNIDAD" y el precio de un pack
 * al lado sería el de algo que no vino. En el resto de los casos no cambia nada.
 *
 * Vive acá, con las otras decisiones de esta pantalla, porque la necesitan las
 * DOS superficies del teléfono —la tarjeta y el panel— y una copia en cada una
 * se separa el día que una cambie.
 */
export function costoMostradoDe(d = {}, envio = null) {
  const e = envio || descriptorDeEnvio(d);
  const elegido = seCuentaPorUnidad(e) ? d.costoUnitarioFisico : d.precioCosto;
  return elegido == null ? null : elegido;
}
...
export function fisicasRecibidasDe(d = {}, { cantidad, sueltas } = {}) {
  ...
  // Con la cantidad pasada a mano manda el llamador: es el caso del operador
  // tipeando, donde lo persistido todavía no es lo que hay en pantalla. Sin ella,
  // sale de `recibidoDeLinea`, que entiende las dos formas de la línea y TIRA un
  // error si le llega un objeto que no trae ninguna — ver ahí por qué.
  const contado = cantidad === undefined ? recibidoDeLinea(d) : { cantidad, sueltas: d.recibidoUnidadesSueltas };
  if (contado === null) return null;

  const cant = contado.cantidad;
  if (cant === null || cant === undefined) return null;
  return unidadesFisicasDe({
    cantidad: cant,
    sueltas: sueltas === undefined ? contado.sueltas : sueltas,
    unidad: e.unidad,
    factorPack: e.factorPack,
  });
}
...
function filaDeServidor(d) {
  return {
    ...
    //
    // Decía `d.cantidadEnviada`, que es la cantidad FÍSICA. Y este campo se
    // persiste en la escala de la PRESENTACIÓN: `guardar-recepcion` lo lee con
    // `escalaDeRecepcion` y multiplica por el factor congelado.
    //
    // O sea que sobre una línea de 6 PACK x24 sin contar, el editor de escritorio
    // proponía 144 y un "Guardar cambios" —que manda `editItems` ENTERO, no solo
    // las líneas tocadas— habría persistido 144 PACKS: 3.456 unidades al
    // confirmar, donde salieron 144.
    //
    // Medido el 2026-09-13: **cero líneas** en producción tienen esa firma
    // —`recibido` igual a la cantidad física sobre una línea cuya presentada es
    // otra—, así que nunca se ejerció. Era una trampa latente, no un dato roto.
    // Las 54 que sí tienen conteo lo tienen en la escala correcta, porque las
    // guardó el panel del teléfono, que ya proponía bien desde el V25.
    //
    // El descriptor contesta en la misma escala en la que está escrito el campo.
    // Es exactamente el arreglo que `FichaProductoRecepcion.inicial()` ya tenía, y
    // está acá porque es la misma pregunta. Ver `INC-0009`.
    recibido:
