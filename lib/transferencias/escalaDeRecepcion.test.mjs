...
test("15. la pantalla y el servidor comparten la MISMA escala", () => {
  ...
  // Se admite que el import traiga hermanos —desde el 2026-09-13 entra también
  // `recibidoDeLinea`, la puerta que lee las dos formas de la línea— pero
  // `escalaDeEnvio` tiene que seguir saliendo de ahí y no de una copia local.
  assert.match(src, /import \{[^}]*\bescalaDeEnvio\b[^}]*\} from "\.\/presentacionEnvio\.js"/);
  assert.ok(
