...
// ═══════════════════════════════════════════════════════════════════════════
// 4-5. LA UNIDAD NO SE ADIVINA — Y AHORA TAMPOCO SE PREGUNTA
// ═══════════════════════════════════════════════════════════════════════════
//
// ── QUÉ AFIRMA ESTE BLOQUE HOY, Y QUÉ DEJÓ DE AFIRMAR ────────────────────
//
// Lo que defendía era correcto y sigue vigente: **una unidad supuesta no se
// detecta nunca más.** Una línea agregada descuenta del origen y no hay ningún
// envío contra el cual contrastar la diferencia, así que con factor 20 suponer
// son 57 unidades que no aparecen en ningún lado.
//
// Lo que estaba mal era el remedio. La pantalla pedía la respuesta al operador
// —"¿UNIDAD o BULTO?"— cuando el catálogo del ORIGEN ya la sabe: un producto con
// `unidad_medida = "cajon"` y factor 8 es un CAJÓN x8. Preguntarla no es una
// defensa, es delegar en una persona una traducción que el dominio tiene
// resuelta, y darle la oportunidad de equivocarla.
//
// Así que el candado cambia de objeto: ya no exige que la unidad arranque en
// null a la espera de un toque, exige que **salga del catálogo y nunca de un
// default silencioso**. La prohibición del default se queda intacta, porque es
// la mitad que sigue protegiendo lo mismo.

test("4. la unidad de un producto extra sale del CATÁLOGO, no de un default", () => {
  const src = leerSinComentarios(AGREGAR);

  // La prohibición de siempre, sin aflojar: nada de suponer una unidad.
  assert.ok(
  ...
  // Y lo nuevo: se DERIVA, y no hay ningún camino para tocarla.
  assert.match(src, /unidadDeProductoNuevo\(producto\)/);
  assert.ok(!/setUnidad\(/.test(src), "la unidad volvió a ser algo que se elige a mano");

  // La derivación contesta las cinco presentaciones. Cambiar de producto la
  // cambia sola, así que arrastrar "BULTO" de un x6 a un x24 ya no es posible.
  assert.equal(unidadDeProductoNuevo({ unidadMedida: "pack", factorPack: 6 }), "BULTO");
  assert.equal(unidadDeProductoNuevo({ unidadMedida: "cajon", factorPack: 8 }), "BULTO");
  assert.equal(unidadDeProductoNuevo({ unidadMedida: "unidad", factorPack: 1 }), "UNIDAD");
  assert.equal(unidadDeProductoNuevo({ unidadMedida: "kg" }), "UNIDAD");
  assert.equal(
    unidadDeProductoNuevo({
  ...
  // Un producto que agrupa pero sin factor conocido NO se cuenta en bultos:
  // multiplicar por un factor inventado es exactamente el daño que este bloque
  // existe para impedir.
  assert.equal(unidadDeProductoNuevo({ unidadMedida: "pack", factorPack: 1 }), "UNIDAD");
});
...
test("14. confirmar sigue BLOQUEADO con cambios sin guardar", () => {
  ...
  // ── EL BLOQUEO SIGUE, PERO SOLO PARA EL EDITOR POR LOTES ────────────────
  //
  // Esto exigía `if (dirty)`. El 2026-09-09 pasó a `dirtyEfectivo`, que es el
  // mismo `dirty` apagado cuando lo que está montado es el control físico: ahí
  // no hay editor por lotes ni botón de guardar, así que el aviso mandaba al
  // operador a apretar algo que no existe. El bloqueo no se aflojó —el editor
  // histórico lo conserva entero— y acá se afirman las dos mitades.
  assert.match(
    confirmar.slice(0, 700),
    /if \(dirtyEfectivo\) \{[\s\S]{0,300}return;/,
    "se perdió el bloqueo por cambios sin guardar"
  );
  assert.match(
    pagina,
    /const dirtyEfectivo = modo === MODO_RECEPCION\.EDITOR_LOTES && dirty/,
    "el dirty legacy volvió a poder gobernar el puesto de control físico"
  );
...
test("15. un estado no editable no muestra Agregar ni Quitar", () => {
  ...
  // ── LA REGLA SE MUDÓ DE FORMA, NO DE FONDO ─────────────────────────────
  //
  // Antes la página le pasaba —o no— los handlers a la tabla, y este candado
  // exigía esos dos ternarios. El 2026-09-08 la recepción pasó a ser un puesto
  // de trabajo aparte: quien puede recibir ve `WorkspaceRecepcion` y todos los
  // demás siguen viendo el detalle histórico, que no tiene ni Agregar ni Quitar.
  //
  // O sea que la regla ahora es MÁS fuerte: no es que los botones no reciban
  // handler, es que el componente que los contiene no se monta. Se reescribe
  // afirmando eso, y se le agrega que el detalle histórico siga sin poder editar.
  assert.match(
    pagina,
    /\{puedeRecibir \? \(\s*<WorkspaceRecepcion/,
    "el puesto de trabajo tiene que estar atado a puedeRecibir"
  );
  // El cálculo de `puedeRecibir` se mudó arriba de los handlers —`cargar()`
  // necesita el modo—, y en la mudanza dejó de ser cuatro líneas con variables
  // intermedias. Los estados que permiten recibir son los mismos dos, que es lo
  // único que este candado afirma.
  assert.match(
    pagina,
  ...
  assert.match(
    ...
    "recibir dejó de exigir ser el local destino"
  );

  // El detalle histórico —el que ven el origen, un admin o una transferencia
  // Recibida— no recibe ningún handler de escritura.
  const historico = pagina.slice(pagina.indexOf("<TablaDetalleTransferencia"));
  const bloque = historico.slice(0, historico.indexOf("/>") + 2);
  assert.ok(
    !/onAgregarProducto|onQuitarLinea/.test(bloque),
    "el detalle histórico volvió a ofrecer acciones de escritura"
  );

  // Y la tabla sigue sin dibujar nada si no le pasaron el handler: es la segunda
  // mitad de la garantía, por si mañana alguien se los vuelve a pasar.
  const tabla = leerSinComentarios(TABLA);
  ...
  // Y el workspace solo entrega las acciones cuando se puede recibir.
  const ws = leerSinComentarios("components/transferencias/WorkspaceRecepcion.jsx");
  assert.match(ws, /\{puedeRecibir && \(/, "el workspace ofrece agregar sin permiso");
});
...
test("17. recibido 0 sigue siendo 0 y no vuelve a ser lo enviado", () => {
  ...
  //
  // ── Y EL 2026-09-13 CAMBIÓ LO QUE PROPONE, NO CÓMO LO DECIDE ──────────
  //
  // El `== null` es lo que este candado defiende y no se movió. Lo que cambió es
  // la rama del `null`: proponía `d.cantidadEnviada`, que es la cantidad FÍSICA,
  // sobre un campo que se persiste en la escala de la PRESENTACIÓN. Con 6 PACK
  // x24 proponía 144 y un guardado habría escrito 144 packs. Ver `INC-0009`.
  const lib = leerSinComentarios("lib/transferencias/recepcionUI.js");
  assert.match(
    lib,
    /d\.cantidadRecibida == null \?\s*descriptorDeEnvio\(d\)\.cantidad : d\.cantidadRecibida/,
    "volvió el fallback por truthiness: un 0 guardado reaparecería como el total enviado"
  );
  assert.ok(
    !/== null \? d\.cantidadEnviada/.test(lib),
    "volvió a proponer la cantidad física en un campo que se guarda en la presentación"
  );
  ...
  // Los dos caminos siguen existiendo, pero la página ya no elige entre ellos:
  // desde el 2026-09-09 le pasa el MODO a `siguienteEdicion`, que es quien
  // decide. Ver el bloque del dirty fantasma en `recepcionUI`.
  assert.ok(
    pagina.includes("siguienteEdicion"),
    "la página tiene que delegar en el helper que decide el camino"
  );
  assert.ok(
    lib.includes("construirEditItems") && lib.includes("reconciliarEditItems"),
    "los dos caminos tienen que seguir viviendo en el helper"
  );
...
test("una sola pieza para el teléfono y el escritorio: no hay dos modales", () => {
  ...
  // El kit, no piezas nuevas. `SunmiSelectorUnidad` salió de la lista porque el
  // control que dibujaba se fue de esta pantalla: la presentación la contesta el
  // catálogo del origen y ya no hay nada que elegir. Lo que el candado afirma
  // —que acá no nazca una pieza al lado del kit— no cambió, y el kit sigue
  // poniendo el modal, los botones, los campos y el buscador por voz.
  for (const pieza of ["SunmiModalLayout", "SunmiButton", "SunmiInput", "SunmiCampoBusquedaVoz"]) {
    assert.ok(src.includes(pieza), `${pieza} tendría que reusarse`);
  }
  // Y nada de elementos nativos sueltos, que es la otra mitad de la misma regla.
  for (const crudo of ["<button", "<input", "<select"]) {
    assert.ok(!src.includes(crudo), `apareció un ${crudo}> nativo en vez de la pieza del kit`);
  }
});
...
test("la página usa los dos modos, y cada uno donde corresponde", () => {
  ...
  // Y el dirty del modo preservado se RECALCULA, no se fuerza. La decisión se
  // mudó de la página a `siguienteEdicion` el 2026-09-09, así que se afirma
  // donde vive: dejarla mirando la página la habría dejado en verde afirmando
  // nada. Lo que se defiende es lo mismo — que preservar no encienda el aviso de
  // gusto cuando ya no queda ninguna edición.
  assert.match(
    lib,
    /dirty: conservar \? hayEdicionPendiente\(/,
...
test("cambiar de producto limpia lo contado, y la unidad cambia sola", () => {
  const src = leerSinComentarios(AGREGAR);
  ...
  // La unidad ya no se reinicia porque ya no es estado: se deriva del producto,
  // así que cambiar de producto la cambia por construcción. Arrastrar "BULTO"
  // de un x6 a un x24 dejó de ser posible, que era lo que el candado defendía.
  assert.ok(!/setUnidad\(/.test(elegir), "la unidad volvió a ser estado que hay que reiniciar");
  assert.match(
    elegir,
    /setCantidad\(""\)/,
    "la cantidad sobrevive al cambio de producto: '2' en bultos de 6 no es '2' en bultos de 24"
  );
  assert.match(elegir, /setSueltas\(""\)/, "las unidades sueltas sobreviven al cambio de producto");
  assert.match(elegir, /setMensaje\(""\)/, "queda un mensaje del producto anterior");
});

// ── CUÁNTAS LÍNEAS SE CORRIGIERON ────────────────────────────────────────
//
// Lo pide la barra de cierre: "Total · 2 corregidos". Va como función pura y no
// como un `filter` suelto en el JSX porque tiene TRES exclusiones que no son
// obvias, y cada una es un defecto si se olvida.
//
// Se mide en FÍSICO por la misma razón de siempre: 6 packs contra 5 packs + 6
// sueltas es la misma mercadería, y contarlo como corrección diría que alguien
// tocó algo que no tocó.

const lineaPack = (extra = {}) => ({
  id: 1,
  cantidadEnviada: 144,
  cantidadRecibida: null,
...
test("CORREGIDAS · cuenta las revisadas cuyo conteo se separó de lo enviado", () => {
  const items = [
    lineaPack({ id: 1, revisadoEnRecepcion: true, cantidadRecibida: 6 }), // coincide
    lineaPack({ id: 2, revisadoEnRecepcion: true, cantidadRecibida: 4 }), // corregida
    lineaPack({ id: 3, revisadoEnRecepcion: true, cantidadRecibida: 9 }), // corregida
  ];
  assert.equal(contarCorregidas(items), 2);
});

test("CORREGIDAS · una línea SIN revisar todavía no cuenta", () => {
  // Tener una diferencia cargada no es lo mismo que haberla cerrado. La barra
  // dice cuántas quedaron corregidas, no cuántas están a medias.
  const items = [lineaPack({ cantidadRecibida: 4 })];
  assert.equal(contarCorregidas(items), 0);
});

test("CORREGIDAS · un NO DECLARADO no es una corrección", () => {
  // No venía en el remito: no hay nada que se haya corregido. Si contara, la
  // barra diría que se tocaron líneas del documento que nadie tocó.
  const items = [
    lineaPack({
      agregadoEnRecepcion: true,
...
test("CORREGIDAS · el mismo total con otro desglose NO es una corrección", () => {
  // 5 packs de 24 más 24 sueltas son 144, igual que 6 packs. Cambió cómo se
  // contó, no cuánto llegó. Medirlo en la PRESENTACIÓN diría 5 ≠ 6 y contaría
  // una corrección que no existe — es el mismo error que ya rompió esta
  // pantalla midiendo diferencias fuera de físico.
  const items = [
    lineaPack({ revisadoEnRecepcion: true, cantidadRecibida: 5, recibidoUnidadesSueltas: 24 }),
  ];
  assert.equal(contarCorregidas(items), 0);
});

test("CORREGIDAS · y las sueltas SÍ cuentan cuando cambian el total", () => {
  // 6 packs más 1 suelta son 145 contra 144: es una corrección aunque el número
  // de packs sea el mismo.
  const items = [
    lineaPack({ revisadoEnRecepcion: true, cantidadRecibida: 6, recibidoUnidadesSueltas: 1 }),
  ];
  assert.equal(contarCorregidas(items), 1);
});

test("CORREGIDAS · sin items devuelve 0 y no explota", () => {
  assert.equal(contarCorregidas([]), 0);
  assert.equal(contarCorregidas(), 0);
  assert.equal(contarCorregidas(null), 0);
});
