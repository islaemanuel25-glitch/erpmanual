...
test("la ruta no lee presentación, factor ni peso del cuerpo del pedido", () => {
  ...
  // `contadoEn`: el pedido no puede influir en cómo sale el producto.
  //
  // Y ES LA DE SALIDA DEPÓSITO → LOCAL. Lo que se le ofrece adoptar al operador
  // es la presentación en que el depósito despacha HOY hacia su local, con
  // `modo_envio` adentro; nunca la que el depósito le compra al proveedor.
  assert.match(src, /const actual = presentacionDeSalida\(\{/);
  assert.match(src, /modoEnvio: base\?\.modo_envio/);
  assert.ok(
    !/modoCompraProveedor:/.test(src),
    "la relación proveedor→depósito volvió a decidir qué se propone adoptar"
  );
...
test("10. la histórica muestra el remito original, la presentación de hoy y la equivalencia", () => {
  ...
  // Llega dentro de `modoAdopcionHistorica`, que la calcula una sola vez con la
  // MISMA `conversionParaAdoptar` que persiste el servidor.
  assert.match(src, /const equivalencia = modo\.equivalencia/);
  assert.ok(
    !/Math\.floor\(/.test(src),
...
test("11b. y no se ofrece adoptar cuando la conversión no es exacta", () => {
  const src = codigoDe(FICHA);
  // La exactitud es una de las cinco preguntas de `modoAdopcionHistorica`: si la
  // conversión no da exacta, `activo` es false y no se dibuja nada.
  assert.match(src, /const puedeAdoptar = modo\.activo/);
  const dominio = codigoDe("lib/transferencias/adopcionDePresentacion.js");
  assert.match(dominio, /if \(!equivalencia\.ok\) return inerte/);
});

// ═══════════════════════════════════════════════════════════════════════════
// LA HOJA MÓVIL ES UNA VISTA DEDICADA A DECIDIR
// ═══════════════════════════════════════════════════════════════════════════
//
// Mientras haya que decidir en qué presentación se va a contar, no se muestran
// la categoría, el estado, el "Enviado", el "Recibido" ni el input. Pedirle a
// alguien que cuente en una escala y a la vez ofrecerle cambiarla es pedirle dos
// cosas juntas, y la de abajo decide en qué unidad significa el número de arriba.
...
test("3. en la hoja, una histórica adoptable NO muestra el conteo antes de decidir", () => {
  const html = pintarHoja({ id: 7, ...dtoHistorico({ cantidadEnviada: 40 }) });

  // Lo que NO puede estar: el input de cantidad ni los rótulos del conteo.
  assert.ok(!/<input/.test(html), "aparece el input de cantidad antes de decidir la escala");
  assert.ok(!html.includes("Recibido"), "aparece el bloque de recepción");
  assert.ok(!html.includes("Enviado"), "aparece el bloque de envío");
  assert.ok(!html.includes("Marcar revisado"), "aparece el botón de cerrar el control");
});

test("4-8. y muestra exactamente lo que el diseño pide para decidir", () => {
  const html = pintarHoja({ id: 7, ...dtoHistorico({ cantidadEnviada: 40 }) });

  assert.ok(html.includes(ROTULO_HISTORICA), "falta el rótulo de transferencia histórica");
  assert.ok(html.includes("no registró cómo salió del depósito"), "falta el hecho");
  assert.ok(html.includes("Remito original: 40 UNIDAD"), "falta el remito original");
  assert.ok(html.includes("Presentación actual del depósito"), "falta la presentación de hoy");
  assert.ok(html.includes("CAJÓN x8"), "falta el nombre de la presentación actual");
  assert.ok(
    html.includes("Equivale a 5 CAJÓN x8 para 40 unidades"),
    "falta la equivalencia canónica"
  );
  assert.ok(
    html.includes("Usar CAJÓN x8 para esta recepción"),
    "el CTA no nombra la presentación de verdad"
  );
  assert.ok(html.includes("No cambia el remito original"), "falta la ayuda");
});

test("9-11. DESPUÉS de adoptar, la misma línea muestra la ficha normal y propone 5", () => {
  const antes = { id: 7, ...dtoHistorico({ cantidadEnviada: 40 }) };
  const despues = { id: 7, ...dtoAdoptado({ cantidadEnviada: 40, cantidadPresentada: 5 }) };

  // La vista de decisión desaparece…
  const html = pintarHoja(despues);
  assert.ok(!html.includes(ROTULO_HISTORICA), "sigue en modo decisión después de adoptar");
  assert.ok(
    !html.includes("Usar CAJÓN x8 para esta recepción"),
    "sigue ofreciendo adoptar algo que ya se adoptó"
  );

  // …y aparece el conteo, con el campo YA en la escala nueva.
  assert.ok(html.includes("Recibido"), "no apareció la ficha normal");
  ...
  // Y el modo lo decide la MISMA derivación, no una condición escrita al lado.
  const fis = (l) => Number(l.cantidadEnviada) * 1000;
  assert.equal(modoAdopcionHistorica(antes, { puedeRecibir: true, fisicasM: fis(antes) }).activo, true);
  assert.equal(modoAdopcionHistorica(despues, { puedeRecibir: true, fisicasM: fis(despues) }).activo, false);
});
...
test("la condición del modo se escribe UNA vez, en el dominio", () => {
  const ficha = codigoDe(FICHA);

  assert.match(ficha, /const modo = modoAdopcionHistorica\(d, \{ puedeRecibir, fisicasM \}\)/);
  assert.match(ficha, /const soloDecision = enHoja && puedeAdoptar/);

  // La ficha no vuelve a armar las cinco preguntas a mano.
  assert.ok(
    !/hayPresentacionDistinta\(/.test(ficha),
    "la ficha volvió a escribir la condición por su cuenta"
  );

  // ── LA SEGUNDA SUPERFICIE NO EXISTE, Y CONVIENE SABER POR QUÉ ────────
  //
  // El pedido era usar la misma derivación también para el `subtitle` de
  // `SunmiModalLayout`. No se puede: el kit NO reenvía `subtitle`, y es a
  // propósito — encenderlo prendería solos los seis subtítulos muertos de otros
  // modales, y dos de ellos repiten su propio título. Está escrito al lado de
  // la línea y anotado en el roadmap.
  //
  // Pasarle la prop igual habría dejado código que parece cableado y no dibuja
  // nada. El rótulo se dibuja adentro del bloque, que en la hoja queda justo
  // debajo del nombre — que es donde el diseño lo pide.
  const kit = codigoDe("components/sunmi/SunmiModalLayout.jsx");
  assert.ok(
    !/subtitle=\{subtitle\}/.test(kit),
    "el kit empezó a reenviar subtitle: revisar si el rótulo tiene que mudarse al encabezado"
  );
  assert.match(ficha, /\{ROTULO_HISTORICA\}/);

  // En escritorio la composición no cambia: el bloque sigue conviviendo con el
  // conteo. `soloDecision` exige `enHoja`.
  assert.match(ficha, /enHoja && puedeAdoptar/);
});
