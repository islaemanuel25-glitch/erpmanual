...
test("15. la migración es ADITIVA y no rellena ninguna de las 6388 históricas", () => {
  ...
  // ── EL CONTEO SUBE A 11, Y SE ACTUALIZA A PROPÓSITO ──────────────────
  //
  // Este número existe para que ninguna migración entre sin que se note, y acaba
  // de hacer exactamente eso: el 2026-09-13 entró
  // `20260913120000_acuerdo_deposito_local`, de OTRA tanda —el corte de período
  // por relación depósito–local, que hasta ahora estaba escrito en el código— y
  // está autorizada.
  //
  // Lo que este candado sigue afirmando es lo suyo: que LA MIGRACIÓN DE ESTA
  // TANDA, la de la presentación adoptada, siga siendo aditiva. Por eso se nombra
  // explícitamente en vez de mirar "la última": ahora la última es otra.
  const dirs = fs
    .readdirSync(path.join(RAIZ, "prisma/migrations"))
    .filter((x) => /^\d/.test(x))
    .sort();
  // 12 desde el 2026-09-15: la migración de datos del arreglo de escala de
  // ofertas. Ver el comentario de su propio `migration.sql`.
  assert.equal(dirs.length, 12);
  assert.ok(
    dirs.includes("20260910120000_presentacion_adoptada_en_recepcion"),
    "desapareció la migración que este candado defiende"
  );
});
