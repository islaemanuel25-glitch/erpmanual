 ...
 */
export function bloquesPorLocal({ transferencias = [], acuerdos = [], unidad = UNIDADES.SEMANA, hoy } = {}) {
  ...
  for (const t of transferencias) {
    ...
    if (!porLocal.has(localId)) {
      ...
      porLocal.set(localId, {
        ...
        rango: rangoDelPeriodo({ unidad, diaDeCorte, hoy }),
        transferencias: [],
        aPagarCentavos: 0,
        sinRecibir: 0,
      });
 ...
 */
export function cuentaDelLocal({ transferencias = [], acuerdos = [], localId, unidad = UNIDADES.SEMANA, hoy } = {}) {
  const { diaDeCorte, sinConfigurar } = acuerdoDeLocal(acuerdos, localId);
  const rango = rangoDelPeriodo({ unidad, diaDeCorte, hoy });
