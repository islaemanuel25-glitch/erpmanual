...
export function bloquesPorLocal({
  ...
  const abrirBloque = (localId, nombre, deLaLista = false) => {
  ...
  for (const l of locales || []) {
    ...
    abrirBloque(l.id, l.nombre, true);
  }
  ...
  return [...porLocal.values()]
    // Un bloque vacío se muestra SI el llamador dijo que ese local existe. El
    // que quedó vacío sin estar en la lista se descarta: nació de una
    // transferencia que cae fuera de su período, y mostrarlo sería inventar un
    // local que nadie nombró.
    .filter((b) => b.deLaLista || b.transferencias.length > 0)
