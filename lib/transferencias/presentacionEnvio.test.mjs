...
test("4. snapshot KG: 3,250 KG, y la diferencia se dice en KG", () => {
  ...
  // ── TRES DECIMALES, CON LOS CEROS A LA DERECHA ─────────────────────────
  //
  // El nombre de este candado decía "3,250 KG" desde el primer día y la
  // afirmación decía "3,25". La balanza pesa en gramos, así que el tercer decimal
  // es un dato de la báscula: `0,730` no es lo mismo que `0,73` redondeado.
  //
  // Y no era un problema de la base —`cantidad` es `Decimal(12,3)`, con 96 líneas
  // de producción con tercer decimal distinto de cero—: era el
  // `minimumFractionDigits: 0` del formateador, que se comía los ceros de la
  // derecha. Ver `decimalesDeCantidad`.
  assert.equal(rotuloDeEnvio(d), "3,250 KG");
