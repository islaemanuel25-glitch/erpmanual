...
/** Dónde vive hoy la hidratación de `recibido`. Ver el test 21b. */
const HIDRATACION = "lib/transferencias/recepcionUI.js";
// El guard de estado y de alcance se mudó acá: lo comparten las cuatro rutas de
// recepción. Los invariantes de las dos rutas NUEVAS viven en
// `recepcionDiferencias.test.mjs`, que es donde está su contrato.
const SERVIDOR = "lib/transferencias/recepcionServidor.js";
...
// ── EL CONTRATO CAMBIÓ EL 2026-09-08, Y ESTOS DOS LO DICEN ──────────────────
//
// Hasta acá `recibido > enviado` se rechazaba con
// `CANTIDAD_RECIBIDA_SUPERA_ENVIADA`, y estos dos tests defendían ese tope.
//
// El tope no era una regla de negocio: era la consecuencia de que la aritmética
// posterior solo supiera SUMAR al origen. Recibir de más es un hecho físico —los
// bultos traían más— y el sistema tiene que poder representarlo. Ahora la
// aritmética tiene signo y el tope se fue.
//
// Se reescriben sabiendo qué se cambia, no se borran: lo que antes probaba que
// se rechazaba, ahora prueba que se acepta CONSERVANDO los tres números. Lo que
// NO cambió es lo que estos tests defendían de verdad: que no se recorte ni se
// redondee en silencio, y que el enviado no se reescriba nunca.

test("7. recibido > enviado ahora es VÁLIDO y conserva los tres números", () => {
  const r = resolverRecibido({ recibido: 15, enviada: 10 });
  assert.equal(r.ok, true);
  assert.equal(r.enviada, 10, "el enviado NO se reescribe: es el remito");
  assert.equal(r.recibida, 15);
  assert.equal(r.hayDiferencia, true);
});

test("7b. sigue sin recortarse al máximo: 100 sobre 20 son 100, no 20", () => {
  const r = resolverRecibido({ recibido: 100, enviada: 20 });
  assert.equal(r.ok, true);
  assert.equal(r.recibida, 100, "no se recorta");
  assert.equal(r.enviada, 20, "y el enviado queda intacto");
});

test("7d. el código del tope viejo ya no existe en el dominio", () => {
  assert.equal(
    ERRORES_RECEPCION.RECIBIDA_SUPERA_ENVIADA,
    undefined,
    "dejarlo declarado invitaría a volver a usarlo"
  );
});
...
test("13d. la validación corre ANTES de abrir la transacción (no se toca stock)", () => {
  const src = leerSinComentarios(CONFIRMAR);
  // El nombre cambió el 2026-09-08: la validación detalle por detalle se agrupó
  // en `planificarRecepcion` para poder llamarla DOS veces —afuera, para
  // contestar rápido; adentro del lock, como la que manda—. Lo que este candado
  // afirma no cambió: que haya una que corra antes de abrir la transacción.
  const posValidacion = src.indexOf("planificarRecepcion(");
  ...
  assert.ok(
    ...
    "validar solo después de abrir la transacción dejaría el estado en Confirmando"
  );

  // Y la de adentro también tiene que estar: la de afuera lee sin lock, así que
  // por sí sola no es una garantía de nada.
  assert.ok(
    src.indexOf("planificarRecepcion(", posTransaccion) > posTransaccion,
    "falta la revalidación autoritativa adentro del lock"
  );
});
...
test("17. la doble confirmación sigue bloqueada", () => {
  const src = leerSinComentarios(CONFIRMAR);
  // El guard de estado se mudó al módulo compartido —lo usan las cuatro rutas de
  // recepción— y la barrera atómica se quedó donde tiene que estar: adentro de
  // la transacción. Las dos siguen siendo obligatorias.
  assert.ok(src.includes("estadoAdmiteRecepcion"), "sigue el guard de estado");

  // La barrera atómica también se mudó al módulo compartido el 2026-09-08: era un
  // `updateMany` escrito acá con su `ALREADY_CONFIRMED`, y ahora es
  // `reclamarOFallar`, que usan las cuatro escrituras de recepción. Sigue siendo
  // el mismo `updateMany` condicionado al estado; lo que cambió es dónde vive.
  assert.ok(src.includes("reclamarOFallar"), "sigue la barrera atómica");

  const guard = leerSinComentarios(SERVIDOR);
  assert.ok(guard.includes('"Recibida"'), "el módulo compartido conserva el caso confirmada");
  assert.ok(/ESTADOS_EDITABLES\s*=\s*\["Enviada",\s*"Recibiendo"\]/.test(guard));
  assert.ok(
    /updateMany\([\s\S]{0,120}estado:\s*\{ in: ESTADOS_EDITABLES \}/.test(guard),
    "la barrera dejó de ser un updateMany condicionado al estado"
  );
});
...
test("21b. la pantalla usa `== null`, no truthiness", () => {
  // ── EL TERNARIO SE MUDÓ, Y ESTE CANDADO SE MUDÓ CON ÉL ──────────────────
  //
  // Vivía en la página. El 2026-09-08 pasó a `filaDeServidor`, en
  // `lib/transferencias/recepcionUI.js`, porque `editItems` se construye ahora
  // por DOS caminos —reemplazar todo, o reconciliar conservando lo que el
  // operador tiene escrito sin guardar— y con una copia del ternario en cada
  // uno, el día que alguien toque una la otra queda atrás.
  //
  // Se reescribe apuntando al lugar nuevo en vez de aflojarlo: dejarlo mirando
  // la página lo habría dejado en VERDE afirmando nada, que es el patrón que
  // `CLAUDE.md` anota como "el candado que mira el lugar equivocado".
  const src = leerSinComentarios(HIDRATACION);
  assert.ok(
  ...
  // El `== null` explícito es lo que se defiende acá. La rama del null pasó de
  // `d.cantidadEnviada` —la física— a la cantidad del descriptor el 2026-09-13,
  // porque este campo se guarda en la escala de la presentación. `INC-0009`.
  assert.ok(
    /cantidadRecibida\s*==\s*null\s*\?\s*descriptorDeEnvio\(d\)\.cantidad\s*:\s*d\.cantidadRecibida/.test(
      src
    ),
    "debe distinguir explícitamente null de 0"
  );

  // Y la pantalla no volvió a armarlo por su cuenta al lado del helper.
  const pantalla = leerSinComentarios(PANTALLA);
  assert.ok(
    !/cantidadRecibida\s*==\s*null\s*\?/.test(pantalla),
    "la página tiene una segunda copia del ternario: son dos verdades sobre lo mismo"
  );
  // ── LA PÁGINA DEJÓ DE LLAMAR AL CONSTRUCTOR, Y ES UN PASO MÁS LEJOS ─────
  //
  // Esto exigía `construirEditItems` en la página. El 2026-09-09 la página dejó
  // de elegir entre construir y reconciliar: le pasa el MODO a `siguienteEdicion`
  // y esa función decide. El motivo está en el bloque del dirty fantasma de
  // `recepcionUI`.
  //
  // Lo que el candado defiende no cambió —que la pantalla no arme `editItems`
  // por su cuenta— y se afirma sobre la forma nueva, que además es más fuerte:
  // la página ya no puede ni siquiera elegir mal el camino.
  assert.ok(
    pantalla.includes("siguienteEdicion"),
    "la página tiene que delegar la construcción de editItems en el helper compartido"
  );
  assert.ok(
    !/construirEditItems|reconciliarEditItems/.test(pantalla),
    "la página volvió a elegir el camino por su cuenta: esa decisión es de siguienteEdicion"
  );
});
...
test("21c. la API de detalle preserva el null en vez de colapsarlo a 0", () => {
  ...
  // ── EL ACUMULADO CAMBIÓ DE ESCALA, NO DE REGLA ─────────────────────────
  //
  // Esto exigía el literal `itemsRecibidos += cantidadRecibida ?? 0`. El
  // 2026-09-08 el acumulado pasó a sumar milésimas FÍSICAS, porque sumar la
  // cantidad en la presentación mentía con packs incompletos: "6 packs + 1
  // suelta" contra 6 enviados daba diferencia cero con 37 unidades contra 36.
  //
  // Lo que este candado defiende sigue igual y se afirma sobre la forma nueva:
  // que `null` no se confunda con 0. Una línea sin recepción cargada no suma
  // nada; una con 0 recibido suma 0 y eso es un dato.
  assert.ok(
    /itemsRecibidos \+= recFisM == null \? 0 : desdeMilesimas\(recFisM\)/.test(src),
    "el acumulado tiene que distinguir 'no cargado' de 'no llegó nada'"
  );
  assert.ok(
    /const recFisM =\s*cantidadRecibida == null\s*\?\s*null/.test(src),
    "el null se está colapsando antes de acumular"
  );
});
...
test("20d. el faltante SÍ se devuelve automáticamente al origen (regla vigente)", () => {
  ...
  const src = leerSinComentarios(CONFIRMAR);
  assert.ok(
    /cantidad:\s*\{\s*increment:\s*ajusteOrigenUnidades\s*\}/.test(src),
    "la diferencia tiene que ajustar origen.cantidad"
  );
  assert.ok(
...
test("22.4b. la resta se hace en milésimas, no en punto flotante", () => {
  ...
  for (const [env, rec, esperado] of [
    ...
    const dev = calcularAjusteOrigenUnidades({
      enviada: env,
      recibida: rec,
      unidad: "UNIDAD",
      factorPack: 1,
...
test("22.5. recibido > enviado → el ajuste del origen sale NEGATIVO", () => {
  const plan = planDe({ cantidad: 20, recibido: 21, motivoPrincipal: "Sobrante" });
  assert.equal(plan.ok, true);
  assert.equal(plan.ajusteOrigenUnidades, -1, "el origen pierde la unidad de más");
  assert.equal(plan.devolucionUnidades, 0, "no vuelve nada");
  assert.equal(plan.excedenteUnidades, 1);
  assert.equal(
    calcularAjusteOrigenUnidades({ enviada: 20, recibida: 21, unidad: "UNIDAD", factorPack: 1 }),
    -1,
    "el helper puro ahora devuelve la diferencia con signo"
  );
});
...
test("22.6. recibido negativo → bloqueado", () => {
  ...
  assert.equal(
    calcularAjusteOrigenUnidades({ enviada: 20, recibida: -1, unidad: "UNIDAD", factorPack: 1 }),
    null
  );
});
...
test("22.7. NaN, Infinity, strings, boolean, array y objeto → bloqueados", () => {
  for (const malo of [NaN, Infinity, -Infinity, "abc", "1e3", "1.9251", true, false, [], {}, "  "]) {
    ...
    assert.equal(plan.devolucionUnidades, undefined);
    assert.equal(
      calcularAjusteOrigenUnidades({ enviada: 20, recibida: malo, unidad: "UNIDAD", factorPack: 1 }),
      null,
...
test("22.13. SOLO_TRANSITO: mismo neto, sin devolver dos veces", () => {
  ...
  assert.equal(
    (src.match(/increment:\s*ajusteOrigenUnidades/g) || []).length,
    2,
    "el ajuste se escribe en las dos ramas del literal —con y sin tránsito— y en ninguna más"
  );
});
...
test("22.14. cantidad y enTransito se mueven en la MISMA escritura atómica", () => {
  const src = leerSinComentarios(CONFIRMAR);
  assert.ok(
    /\{\s*cantidad:\s*\{\s*increment:\s*ajusteOrigenUnidades\s*\},\s*enTransito:\s*\{\s*decrement:\s*enviadaUnidades\s*\}\s*\}/.test(
      src
    ),
    "partirlo en dos updates deja el stock del origen a medio corregir"
  );
  assert.ok(
    /datos:\s*datosOrigen/.test(src),
    "la guardia recibe el literal ya armado, no un objeto mutado por partes"
  );
  assert.equal(
  ...
  // `stockLocal.updateMany` estaba PROHIBIDO acá, con el motivo correcto: no
  // devuelve la fila, así que no se puede auditar el antes/después. El 2026-09-08
  // se admitió a propósito, porque es lo único que hace atómico el chequeo de
  // stock negativo —la condición viaja dentro de la sentencia—. La objeción se
  // resolvió releyendo la fila después, no ignorándola: eso es lo que se exige.
  const guardia = src.slice(
    src.indexOf("async function descontarConGuardia"),
    src.indexOf("export async function POST")
  );
  assert.ok(guardia.includes("stockLocal.updateMany"), "la guardia perdió el update condicional");
  assert.ok(
    /stockLocal\.findUnique\(\{ where: donde \}\)/.test(guardia),
    "un updateMany sin relectura deja la auditoría sin el valor nuevo"
  );
  assert.ok(
    !src.slice(src.indexOf("export async function POST")).includes("stockLocal.updateMany"),
    "el handler escribe stock con updateMany por fuera de la guardia"
  );
});
...
test("22.16. la auditoría se crea solo cuando hubo movimiento", () => {
  const src = leerSinComentarios(CONFIRMAR);
  const posGuard = src.indexOf("if (accion)");
  const posAud = src.indexOf("auditoriaStock.create");
  assert.ok(posGuard > 0, "debe existir el guard explícito");
  assert.ok(posGuard < posAud, "una recepción exacta no genera auditoría");
  // Y el guard es una decisión del dominio, no un `> 0` escrito en la ruta: así
  // no puede quedar una acción sin cubrir cuando se agregue la próxima.
  assert.ok(src.includes("accionAuditoriaDe(plan)"));
});

test("22.16b. la auditoría registra transferencia, detalle, enviado, recibido y movimiento", () => {
  const src = leerSinComentarios(CONFIRMAR);
  assert.ok(/accion,/.test(src), "la acción sale del dominio");
  for (const campo of [
    ...
    "cantidadAnterior:",
    "cantidadNueva:",
    // El vínculo con la transferencia ahora es ESTRUCTURAL y no solo texto.
    "transferenciaId,",
    "transferenciaDetalleId: d.id",
  ]) {
    assert.ok(src.includes(campo), `falta ${campo} en la auditoría`);
  }
  for (const dato of ["transferencia #", "detalle #", "enviado ", "recibido "]) {
    assert.ok(src.includes(dato), `el motivo debe incluir "${dato}"`);
  }
});

test("22.16c. las TRES acciones existen y el histórico conserva su nombre", () => {
  assert.equal(ACCIONES_RECEPCION.FALTANTE, "DIFERENCIA_RECEPCION_TRANSFERENCIA");
  assert.equal(ACCIONES_RECEPCION.EXCEDENTE, "EXCEDENTE_RECEPCION_TRANSFERENCIA");
  assert.equal(ACCIONES_RECEPCION.AGREGADO, "AGREGADO_RECEPCION_TRANSFERENCIA");
  assert.equal(
    new Set(Object.values(ACCIONES_RECEPCION)).size,
    3,
    "tres acciones distintas: un reporte que agrupe no puede sumar faltantes con sobrantes"
  );
});
...
test("22.17. sigue habiendo una sola transacción (sin mutaciones parciales)", () => {
  ...
  // Las escrituras se miden por CLIENTE y no por posición en el archivo. Desde
  // que el descuento del origen vive en `descontarConGuardia` —declarada arriba
  // del handler— comparar índices diría que está "fuera de la transacción"
  // cuando lo que la determina es con qué cliente escribe. Todo lo que escriba
  // con `prisma.` en vez de `tx.` sí está afuera de verdad.
  for (const escritura of [
    "stockLocal.upsert",
    "stockLocal.update(",
    "stockLocal.updateMany",
    "auditoriaStock.create",
    "transferenciaDetalle.update",
  ]) {
    assert.ok(src.includes(escritura), `desapareció ${escritura}`);
    assert.ok(
      !src.includes(`prisma.${escritura}`),
      `${escritura} se ejecuta con el cliente de fuera de la transacción`
    );
    assert.ok(src.includes(`tx.${escritura}`), `${escritura} no se ejecuta con el cliente transaccional`);
  }
});
...
test("22.18. la barrera de estado sigue siendo la PRIMERA escritura", () => {
  ...
  const posBarrera = tx.indexOf("reclamarOFallar");
  assert.ok(posBarrera >= 0, "la transacción no toma la barrera");
  for (const escritura of [
    "stockLocal.upsert",
    "descontarConGuardia(tx",
    "auditoriaStock.create",
    "transferenciaDetalle.update",
  ]) {
    const pos = tx.indexOf(escritura);
    assert.ok(pos > 0, `desapareció ${escritura} de la transacción`);
    assert.ok(posBarrera < pos, `la barrera debe ir antes de ${escritura}`);
  }
});
...
test("22.19. toda la validación ocurre antes de abrir la transacción", () => {
  ...
  for (const validacion of [
    // `validarDetalleRecepcion` se agrupó en `planificarRecepcion`: la que corre
    // acá es la prevalidación amable, que sigue teniendo que ir antes.
    "planificarRecepcion(",
    "Number(session.id",
    "getGrupoIdDeLocal",
...
test("22.24. la confirmación duplicada no puede devolver dos veces", () => {
  ...
  assert.ok(src.includes("estadoAdmiteRecepcion"));

  // La barrera se mudó a `reclamarOFallar`, en el módulo compartido. Que corte
  // antes de mutar se mide DENTRO del cuerpo de la transacción: el descuento del
  // origen ahora se llama desde una función declarada más arriba en el archivo.
  const tx = src.slice(src.indexOf("prisma.$transaction"));
  const posLock = tx.indexOf("reclamarOFallar");
  assert.ok(posLock >= 0, "la transacción no toma la barrera");
  for (const mutacion of ["descontarConGuardia(tx", "stockLocal.upsert", "auditoriaStock.create"]) {
    const pos = tx.indexOf(mutacion);
    assert.ok(pos > 0, `desapareció ${mutacion}`);
    assert.ok(posLock < pos, `la barrera debe cortar antes de ${mutacion}`);
  }

  const guard = leerSinComentarios(SERVIDOR);
  assert.ok(/estado:\s*\{ in: ESTADOS_EDITABLES \}/.test(guard));
});
...
test("22.26. los abortos se lanzan (no se responde desde adentro de la transacción)", () => {
  const src = leerSinComentarios(CONFIRMAR);
  // La clase se mudó al módulo compartido el 2026-09-08: las cuatro escrituras de
  // recepción abortan igual, y con dos copias una se habría quedado atrás.
  const guard = leerSinComentarios(SERVIDOR);
  assert.ok(guard.includes("class ErrorRecepcion"), "error tipado para abortar la transacción");
  assert.ok(
    !src.includes("class ErrorRecepcion"),
    "la ruta tiene su propia copia de la clase en vez de usar la compartida"
  );
