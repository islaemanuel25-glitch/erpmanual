...
//
// Tenía default `false` y se me escapó DOS VECES: primero `agruparPorDestino`,
// después `importeDeDetalle` y `resumenPorEstado`. Las dos veces el resultado fue
// el mismo —un importe de plata mal calculado en una pantalla— y las dos veces
// pasó en silencio, porque un default hace que olvidarse se vea igual que
// decidir.
//
// La segunda vez llegó a producción: la lista de transferencias mostraba la #97
// en 144.086,40 y el detalle en 155.486,40, once mil cuatrocientos de diferencia
// en el mismo documento. Lo encontró Emanuel con dos capturas, no un candado.
//
// Ahora falta el default y falta a propósito: quien agregue un llamador se entera
// al correr los tests. Decidido con Emanuel: "prefiero que rompa en los tests a
// un importe equivocado en una lista de plata".
function exigirOrigen(opciones, quien) {
  if (!opciones || typeof opciones.origenEsDeposito !== "boolean") {
    throw new Error(
      `${quien}: falta origenEsDeposito. Sin saber de dónde sale la mercadería no se ` +
        ...
        `piezas y su costo, por kilo. Pasá { origenEsDeposito: t.origen?.es_deposito === true }.`
    );
  }
  ...
  for (const t of transferencias) {
    for (const d of t.detalle || []) {
      ...
      fila.importeCentavos += aCentavos(
        importeDeLinea(d, { origenEsDeposito: t.origen?.es_deposito === true })
      );
    }
  }
