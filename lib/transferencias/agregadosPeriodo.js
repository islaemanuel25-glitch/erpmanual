...
// Las dos puertas canónicas del hecho, en MILÉSIMAS: son las que leen la escala
// del remito y entienden la fila de Prisma que este módulo recibe.
import { enviadoFisicoM, recibidoFisicoM } from "./controlFisico.js";
// La que fija el signo de la diferencia para todo el repo: recibida − enviada.
import { diferenciaDeLinea } from "./recepcionUI.js";
import {
  valorizarLineaDelRemito,
  // La que contesta cuánto vale lo RECIBIDO, que es otra pregunta y otra función.
  valorizarDetalle,
  exigirOrigen,
  origenEsDepositoDe,
...
/**
 * EL IMPORTE **ENVIADO** DE UNA LÍNEA: cuánto valorizó el origen al despachar.
 *
 * ── POR QUÉ ENVIADO Y NO RECIBIDO ──────────────────────────────────────────
 *
 * Este módulo contesta **cuánto se transfirió en el período**. Ese número no
 * puede moverse porque el destino haya empezado a recibir, haya recibido de
 * menos, de más, o haya cargado sueltas: un reporte histórico cuyo importe
 * cambia después no es un reporte, es una foto que se mueve. Vale para el total
 * global, el desglose por estado, el agrupado por destino y los rankings por
 * importe — todos salen de acá.
 *
 * Hasta el 2026-09-11 llamaba a `valorizarDetalle` en su modo default, que
 * valoriza lo RECIBIDO cuando hay recepción cargada. O sea que el importe del
 * período efectivamente cambiaba mientras alguien contaba, y con la #198 —snapshot
 * PACK x24— además lo hacía con las escalas mezcladas: 1.312,50 sobre 31.500.
 *
 * Se renombró de `importeDeLinea` a esto: el nombre viejo no decía cuál de los
 * dos importes era, y cuál es fue exactamente la ambigüedad que costó el defecto.
 *
 * ── Y LOS CAMPOS DEL SNAPSHOT VIAJAN, AUNQUE HOY NO CAMBIEN EL NÚMERO ──────
 *
 * Esto hay que decirlo con precisión, porque la contraprueba lo corrigió: sin
 * el snapshot, `valorizarLineaDelRemito` reconstruye la presentación desde los
 * campos crudos y para el importe ENVIADO **da lo mismo**. La #198 sale 31.500
 * de las dos formas: `cantidad` ya está en unidades físicas —144— y el costo se
 * normaliza a esa misma escala. Se ejerció rompiéndolo a propósito y ningún
 * número se movió.
 *
 * Donde el snapshot SÍ decide es en la otra cantidad, la recibida: `recibido`
 * está en packs y sin el descriptor registrado se lee como unidades. Ése es el
 * defecto de la #198 y vive en `valorizarDetalle`, no acá.
 *
 * Entonces por qué se pasa igual: para que el reporte parta del descriptor
 * REGISTRADO y no de una reconstrucción que hoy coincide. Coincidir no es lo
 * mismo que ser la misma fuente, y "una escala" fue justamente la decisión.
 * Lo vigilan un candado de wiring en `agregadosPeriodo.test.mjs` y el del
 * `select` en `formaDelSelect.test.mjs`; ninguno de los dos promete un número
 * distinto, prometen que la fuente es una sola.
 *
 * Las cantidades recibidas y las diferencias siguen existiendo aparte: esto
 * cambia de qué importe habla el reporte, no borra información.
 */
...
export function importeEnviadoDeLinea(d = {}, opciones) {
  const origenEsDeposito = exigirOrigen(opciones, "importeEnviadoDeLinea");
  const precioCosto =
    d.precioCosto ?? d.producto?.precio_costo ?? d.producto?.base?.precio_costo ?? 0;

  const { subtotal } = valorizarLineaDelRemito(
    {
      cantidad: d.cantidad,
      unidadEnviada: d.unidadEnviada,
      precioCosto,
      // El snapshot, que es lo que fija en qué presentación salió.
...
/**
 * Importe ENVIADO de un remito completo, en centavos enteros.
 *
...
export function importeDeDetalleCentavos(detalle = [], opciones) {
  ...
  return detalle.reduce(
    (acc, d) => acc + aCentavos(importeEnviadoDeLinea(d, { origenEsDeposito })),
    0
  );
}

/** Importe ENVIADO de un remito completo, en pesos. Redondeado una sola vez. */
...
/**
 * ════════════════════════════════════════════════════════════════════════════
 * LO QUE EL LOCAL PAGA: EL IMPORTE DE LO RECIBIDO
 * ════════════════════════════════════════════════════════════════════════════
 *
 * ── SON DOS NÚMEROS Y CONVIVEN ─────────────────────────────────────────
 *
 * `importeEnviadoDeLinea` contesta cuánto SALIÓ del depósito y es el valor del
 * documento: no se mueve mientras alguien cuenta, y por eso sirve para controlar.
 * Éste contesta otra cosa — **cuánto vale lo que LLEGÓ** — y es la plata que el
 * local le debe al depósito, porque cada local paga lo recibido y no lo enviado.
 *
 * El del remito no se reemplaza ni se toca. El día que los dos difieran, esa
 * diferencia es justamente el dato: mercadería que salió y no entró.
 *
 * ── NO HAY CUENTA NUEVA ACÁ ────────────────────────────────────────────
 *
 * Sale de `valorizarDetalle` en su modo `VALORIZAR`, que es el que ya usa el acta
 * de recepción en PDF —hoy su único consumidor— y que lee el snapshot para saber
 * en qué escala está `recibido`. Sin recepción cargada cae a lo enviado, que es
 * su contrato y el correcto: una transferencia despachada y sin contar se debe
 * entera hasta que alguien diga otra cosa.
 *
 * Y se suma en CENTAVOS ENTEROS, como su hermano: acumular pesos en punto
 * flotante sobre las transferencias de una semana deja residuos en un número que
 * alguien va a cobrar.
 */
export function importeRecibidoDeLinea(d = {}, opciones) {
  const origenEsDeposito = exigirOrigen(opciones, "importeRecibidoDeLinea");
  const precioCosto =
    d.precioCosto ?? d.producto?.precio_costo ?? d.producto?.base?.precio_costo ?? 0;
  ...
  const { subtotal } = valorizarDetalle(
    {
      cantidad: d.cantidad,
      recibido: d.recibido,
      // Las sueltas son parte de lo recibido: sin ellas, "1 pack + 100 sueltas"
      // se cobra como 1 pack. Es el mismo arreglo que ya tiene el acta.
      recibidoUnidadesSueltas: d.recibidoUnidadesSueltas,
      unidadEnviada: d.unidadEnviada,
      precioCosto,
      // El snapshot, que es lo que dice en qué escala está `recibido`.
      presentacionEnvio: d.presentacionEnvio,
      cantidadPresentada: d.cantidadPresentada,
      factorPresentacion: d.factorPresentacion,
      sueltasEnviadas: d.sueltasEnviadas,
...
/** Lo que se paga por un remito completo, en centavos enteros. */
export function importeRecibidoDeDetalleCentavos(detalle = [], opciones) {
  const origenEsDeposito = exigirOrigen(opciones, "importeRecibidoDeDetalleCentavos");
  return detalle.reduce(
    (acc, d) => acc + aCentavos(importeRecibidoDeLinea(d, { origenEsDeposito })),
    0
  );
}

/** Lo mismo en pesos, redondeado una sola vez al final. */
export function importeRecibidoDeDetalle(detalle = [], opciones) {
  const origenEsDeposito = exigirOrigen(opciones, "importeRecibidoDeDetalle");
  return desdeCentavos(importeRecibidoDeDetalleCentavos(detalle, { origenEsDeposito }));
}
...
export function cantidadesDeDetalle(detalle = []) {
  ...
  // ── LAS DOS SALEN DE LA ESCALA DEL REMITO, Y ANTES NO ───────────────────
  //
  // Decía `aMilesimas(d.cantidad)` contra `aMilesimas(d.recibido)`. `cantidad` es
  // física y `recibido` está en la escala de la PRESENTACIÓN, así que esto sumaba
  // dos escalas distintas y además ignoraba las sueltas. Medido sobre la
  // transferencia #204: devolvía **40 enviadas y 5 recibidas** cuando la verdad es
  // 40 y 40 — los cuatro cajones y el pack llegaron completos.
  //
  // De acá salían CINCO superficies: el listado de transferencias en las dos
  // vistas, la tarjeta, la fila y el reporte por destino, que imprimía
  // "env 40 · rec 5".
  //
  // Se suma en MILÉSIMAS enteras y por eso se usan `enviadoFisicoM` y
  // `recibidoFisicoM` y no las de unidades: acumular en punto flotante sobre
  // cientos de líneas es donde aparecen los residuos binarios. Y las dos entienden
  // la fila de Prisma, que es lo que este módulo recibe. Ver `INC-0009`.
  for (const d of detalle) {
    const envM = enviadoFisicoM(d);
    if (envM !== null) enviadaM += envM;

    const recM = recibidoFisicoM(d);
...
export function productosMasTransferidos(transferencias = [], limite = MAX_PRODUCTOS) {
  ...
  for (const t of transferencias) {
    for (const d of t.detalle || []) {
      ...
      // La misma corrección que `cantidadesDeDetalle`, y por el mismo motivo:
      // `cantidad` es física y `recibido` está en la escala de la presentación.
      // Medido sobre el detalle 6843: esta función informaba 32 enviadas, **4**
      // recibidas y una diferencia de **28** sobre una línea que llegó completa.
      const envM = enviadoFisicoM(d);
      ...
      fila.importeCentavos += aCentavos(
        importeEnviadoDeLinea(d, {
          origenEsDeposito: origenEsDepositoDe(t, "productosMasTransferidos"),
        })
      );
    }
  }
  ...
  return [...porProducto.values()]
    .map((f) => ({
      ...
      //
      // ── Y EL SIGNO ES EL DEL RESTO DEL REPO: RECIBIDA − ENVIADA ──────────
      //
      // Era `enviadaM − recibidaM`, o sea al revés que `diferenciaDeLinea` y que
      // todo lo que se dibuja en el detalle. Con eso un faltante salía POSITIVO
      // acá y NEGATIVO en la ficha de la misma línea, y había que saber en qué
      // pantalla estabas para leer el número. Negativo es falta, en las dos.
      //
      // Y la resta se PIDE, no se escribe: `diferenciaDeLinea` es la que fija el
      // signo para todo el repo, y sobre milésimas enteras hace exactamente la
      // misma cuenta. Con la resta escrita acá, este archivo ya tuvo el signo al
      // revés durante meses sin que nada lo notara.
      diferencia:
        f.recibidaM === null
          ? null
          : desdeMilesimas(diferenciaDeLinea({ enviada: f.enviadaM, recibida: f.recibidaM })),
