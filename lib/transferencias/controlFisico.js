...
import { agrupa, descriptorDeEnvio } from "./presentacionEnvio.js";
...
function escalaDe(d) {
  ...
  const agrupada = agrupa(envio.presentacion) && envio.factor > 1;
  return {
    unidad: agrupada ? "BULTO" : "UNIDAD",
    factorPack: agrupada ? envio.factor : 1,
    envio,
  };
}
...
export function enviadoFisicoM(d = {}) {
  const { unidad, factorPack, envio } = escalaDe(d);
  return milesimasFisicas({
    ...
    sueltas: unidad === "BULTO" ? envio.sueltas : 0,
    unidad,
    factorPack,
  });
}
...
export function recibidoFisicoM(d = {}) {
  ...
  const { unidad, factorPack } = escalaDe(d);
  return milesimasFisicas({
