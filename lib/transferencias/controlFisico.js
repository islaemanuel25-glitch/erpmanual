...
import { escalaDeEnvio, recibidoDeLinea } from "./presentacionEnvio.js";
...
export function recibidoFisicoM(d = {}) {
  // ── LEE LAS DOS FORMAS, Y GRITA SI NO ES NINGUNA ──────────────────────
  //
  // Decía `d.cantidadRecibida`, el nombre del DTO. Con una fila de Prisma —donde
  // el campo se llama `recibido`— devolvía null, que acá significa "no se contó".
  // Eso es lo que dejó a los agregados del período sumando la cuenta a mano.
  // `recibidoDeLinea` entiende las dos y tira un error cuando le llega un objeto
  // que no puede contestar la pregunta. Ver `INC-0009`.
  const contado = recibidoDeLinea(d);
  if (contado === null) return null;
  const { unidad, factorPack } = escalaDeEnvio(d);
  return milesimasFisicas({
    cantidad: contado.cantidad,
    sueltas: unidad === "BULTO" ? contado.sueltas : 0,
    unidad,
    factorPack,
  });
}
