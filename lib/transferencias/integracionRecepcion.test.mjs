...
test("2c. y la categoría SIGUE filtrando en los tabs del remito", () => {
  ...
  // Y los agregados nunca entran en los tabs del remito, con chip o sin chip.
  for (const categoriaId of [null, "4"]) {
    assert.ok(
      !productosVisibles(items, { filtro: FILTRO.TODOS, categoriaId }).some((d) => d.id === 2),
      "un no declarado se coló en los tabs del remito"
    );
  }
});
...
test("3h. LA CONEXIÓN REAL: el workspace aplica la cascada y no busca por su cuenta", () => {
  ...
  // El aviso de "no figura" solo se enciende en la rama que corresponde.
  const bloqueResolver = ws.slice(ws.indexOf("const resolver ="), ws.indexOf("const alEscanear"));
  assert.match(bloqueResolver, /if \(r\.tipo === RESOLUCION\.NO_FIGURA\) \{[\s\S]{0,200}setAviso\(MENSAJE_NO_FIGURA\)/);
  assert.ok(
    !/RESOLUCION\.LISTA[\s\S]{0,120}setAviso\(MENSAJE_NO_FIGURA\)/.test(bloqueResolver),
    "con varios resultados se sigue avisando que no figura"
  );
});
