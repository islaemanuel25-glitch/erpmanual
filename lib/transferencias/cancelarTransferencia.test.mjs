...
test("12. NINGUNA ruta de transferencias borra una línea de detalle", () => {
  ...
  // ── LA ÚNICA EXCEPCIÓN, Y POR QUÉ NO AFLOJA LA REGLA ──────────────────────
  //
  // Lo que esta regla defiende es que una línea DEL REMITO no desaparezca: si se
  // enviaron 10 y llegaron 0, tiene que quedar el 0 y su motivo, no el silencio.
  //
  // Desde el 2026-09-08 existe otra clase de línea: la que se AGREGA durante la
  // recepción porque llegó algo que el remito no mencionaba. Esa línea nunca
  // estuvo en el remito, nunca movió stock, y si se agregó por error tiene que
  // poder sacarse. Borrarla no esconde nada — al revés: dejarla obligaría a
  // registrar que llegó algo que no llegó.
  //
  // Así que la excepción no se declara con un salteo, que sería aflojar: para
  // esa ruta se exige algo MÁS FUERTE que la prohibición —que el borrado esté
  // guardado por `agregadoEnRecepcion` y que una línea del remito lo rechace—.
  const RUTA_LINEA = "app/api/transferencias/linea-recepcion/route.js";

  for (const rel of rutas) {
    const src = leer(rel);
    if (rel === RUTA_LINEA) continue;
    assert.ok(
  ...
  assert.ok(rutas.includes(RUTA_LINEA), "la ruta de líneas de recepción tiene que estar enumerada");
  const linea = leer(RUTA_LINEA);
  assert.ok(
    /if\s*\(!detalle\.agregadoEnRecepcion\)/.test(linea),
    "el borrado tiene que estar guardado por la marca de línea agregada"
  );
  assert.ok(
    linea.includes("LINEA_DEL_REMITO_NO_SE_BORRA"),
    "y una línea del remito tiene que rechazarse con su código"
  );
  const posGuard = linea.indexOf("!detalle.agregadoEnRecepcion");
  const posDelete = linea.indexOf("transferenciaDetalle.delete");
  assert.ok(posGuard > 0 && posDelete > 0 && posGuard < posDelete, "el guard va ANTES del borrado");
  assert.ok(
    !/deleteMany/.test(linea),
    "un deleteMany podría llevarse líneas del remito de arrastre"
  );
});
