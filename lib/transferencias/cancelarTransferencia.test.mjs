...
test("3c. el motivo es obligatorio", () => {
  for (const motivo of ["", "   ", "error", null, 7]) {
    assert.equal(
  ...
  const justo = "x".repeat(MOTIVO_MIN_CANCELACION);
  assert.equal(
    puedeCancelarTransferencia({ transferencia: T97, localId: CASIANO, motivo: justo, turnoAbierto: TURNO }).puede,
    true
  );
});
...
test("4. el resumen dice la verdad según el remito tenga venta o no", () => {
  ...
  assert.ok(conVenta.efectos.some((e) => /anula la venta/i.test(e)));
  ...
  assert.equal(manual.revierteVenta, false);
  assert.ok(!manual.efectos.some((e) => /anula la venta/i.test(e)));
