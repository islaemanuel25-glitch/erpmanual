...
const PROHIBIDO = Object.freeze([
  ...
  {
    // ── Y LA PLATA, CON UNA RED MÁS ANGOSTA QUE LAS CANTIDADES ─────────
    //
    // Se agregó el 2026-09-13, cuando apareció el segundo número del hecho: lo que
    // el local PAGA es el importe de lo RECIBIDO, y hasta acá el candado solo
    // cubría las cantidades. Sin esto, una suma de plata mal escrita se iba a
    // encontrar de casualidad, que es como se encontraron las tres anteriores.
    //
    // La red es más angosta a propósito y no por prudencia: medio repo multiplica
    // un precio por una cantidad con toda razón —el carrito del POS, los combos,
    // el pedido a proveedor, el valorizado de stock—. Lo que delata ESTE hecho es
    // multiplicar por lo enviado o lo recibido de una transferencia, que es
    // justamente lo que `valorizarDetalle` hace con el snapshot adentro.
    // `cantidad * precioCosto` no cae acá; `recibido * precioCosto` sí.
    nombre: "valorizar a mano lo enviado o lo recibido",
    patron:
      /(recibid|enviad)[A-Za-z]*\s*\*\s*[A-Za-z_.?]*(precioCosto|costoUnitario|costoPresentacion)|(precioCosto|costoUnitario|costoPresentacion)\s*\*\s*[A-Za-z_.?(]*(recibid|enviad)/i,
    porque:
      "lo que se cobra sale de `valorizarDetalle` en modo VALORIZAR, que lee el snapshot; multiplicarlo a mano se saltea la escala",
  },
  {
...
test("LOS PATRONES DETECTAN LOS DEFECTOS QUE YA OCURRIERON, Y LOS DE LA PLATA", () => {
  ...
  const DEFECTOS = [
    ...
    {
      caso: "LA PLATA · valorizar a mano lo que el local paga, sin el snapshot",
      codigo: "const aPagar = d.recibido * d.precioCosto;",
    },
    {
      caso: "LA PLATA · la misma cuenta con los términos al revés",
      codigo: "acc += costoUnitario * cantidadRecibida;",
    },
  ];
...
test("y NO se frena sobre las cuentas de otros hechos", () => {
  ...
  const LEGITIMOS = [
    ...
    // Y las de PLATA de otros hechos, que son las que obligan a la red angosta.
