...
let seq = 0;
// `origenEsDeposito` es un parámetro del fixture y no un default de la función,
// y la diferencia importa: acá es un dato de prueba ELEGIDO y visible en la
// firma; allá era un silencio que hacía que olvidarse se viera igual que decidir.
// Se agregó el 2026-08-20, cuando `origenEsDepositoDe` pasó a exigir la columna:
// estos fixtures traían el origen sin `es_deposito` y por eso valían para probar
// la agrupación pero no para notar que una ruta no lo estaba pidiendo.
const tr = ({
  id = ++seq,
  estado = "Enviada",
  ...
  origen: { id: 99, nombre: origenNombre, es_deposito: origenEsDeposito },
  fechaEnvio,
...
test("4f. los totales del agrupado cierran con el período: documentos vs operativo", () => {
  // ── ESTE CANDADO CAMBIÓ DE CONTRATO EL 2026-08-20, Y LO DETECTÓ SOLO ──────
  //
  // Afirmaba `items === 60`, o sea una línea por transferencia contando TODAS.
  // Al separar documento de movimiento se puso rojo, y tenía razón en avisar:
  // los ítems pasaron a ser operativos.
  //
  // El fixture tiene 60 transferencias con los cuatro estados rotando, así que
  // 15 son Canceladas. Los dos números que quedan son distintos a propósito y
  // ésa es toda la corrección:
  //
  //   · transferencias = 60 → DOCUMENTOS. Tiene que dar lo mismo que el resumen
  //     general de la pantalla; si diera 45, la pestaña Por destino mostraría
  //     menos remitos que el encabezado y ése era el defecto.
  //   · items = 45 → MOVIMIENTO. Las 15 canceladas no movieron mercadería.
  const grupos = agruparPorDestino(periodoGrande);
  const t = totalesDeDestinos(grupos);
  assert.equal(t.destinos, 3);
  assert.equal(t.transferencias, 60, "los documentos tienen que estar todos");
  assert.equal(t.canceladas, 15);
  assert.equal(t.items, 45, "los ítems son operativos: las canceladas no aportan líneas");

  // Y las filas del sublistado están completas: se puede abrir cada documento.
  const filas = grupos.reduce((a, g) => a + g.transferencias.length, 0);
  assert.equal(filas, 60, "el sublistado perdió documentos");
  const canceladasEnFilas = grupos
    .flatMap((g) => g.transferencias)
    .filter((f) => f.esOperativa === false).length;
  assert.equal(canceladasEnFilas, 15, "las canceladas no viajan marcadas");
});
...
test("6. el techo es 5000 y los dos endpoints piden UNA fila de más", () => {
  ...
  for (const rel of [
    ...
    // El +1 es la sonda: sin ella no se puede distinguir "exactamente 5000" de
    // "más de 5000", porque las dos consultas devuelven 5000 filas.
    assert.ok(
      /take:\s*MAX_TRANSFERENCIAS\s*\+\s*1/.test(src),
      `${rel} debe pedir MAX_TRANSFERENCIAS + 1`
    );
    assert.ok(
      !/take:\s*MAX_TRANSFERENCIAS\s*[,\n}]/.test(src),
      `${rel} todavía pide exactamente MAX_TRANSFERENCIAS`
    );
    assert.ok(/truncado/.test(src), `${rel} no informa truncado`);
  }
});

test("6b. los dos endpoints delegan el recorte en el helper compartido", () => {
  for (const rel of [
    "app/api/transferencias/listar/route.js",
    "app/api/transferencias/por-destino/route.js",
  ]) {
    const src = sinComentarios(rel);
    assert.ok(/recortarPeriodo\(/.test(src), `${rel} no usa recortarPeriodo`);
    // Y no vuelve a decidirlo por su cuenta con el comparador viejo.
    assert.ok(
      !/length\s*>=\s*MAX_TRANSFERENCIAS/.test(src),
      `${rel} conserva la comparación >= que confunde el borde`
    );
  }
});

test("6d. BORDE: exactamente MAX no es truncado", () => {
  const exacto = Array.from({ length: 5 }, (_, i) => tr({ id: 900 + i }));
  const { truncado, periodo } = recortarPeriodo(exacto, 5);
  assert.equal(truncado, false, "un período que entra completo no puede marcarse truncado");
  assert.equal(periodo.length, 5, "no debe recortarse nada");
  assert.deepEqual(periodo, exacto);
});

test("6e. BORDE: MAX + 1 sí es truncado, y la fila sonda se descarta", () => {
  const conSonda = Array.from({ length: 6 }, (_, i) => tr({ id: 910 + i }));
  const { truncado, periodo } = recortarPeriodo(conSonda, 5);
  assert.equal(truncado, true);
  assert.equal(periodo.length, 5, "la fila extra no puede entrar en los agregados");
  assert.deepEqual(periodo, conSonda.slice(0, 5));
  assert.ok(!periodo.includes(conSonda[5]), "la sonda quedó adentro");
});

test("6f. BORDE: por debajo del techo nunca es truncado", () => {
  for (const n of [0, 1, 4]) {
    const { truncado, periodo } = recortarPeriodo(Array.from({ length: n }, () => tr({})), 5);
    assert.equal(truncado, false, `${n} filas no deberían truncar`);
    assert.equal(periodo.length, n);
  }
});

test("6g. la fila sonda NO altera ningún agregado", () => {
  // Seis transferencias con techo 5: la sexta es la sonda y no debe sumar.
  const seis = [
    ...Array.from({ length: 5 }, (_, i) => tr({ id: 920 + i, estado: "Enviada", detalle: [linea(1, 1, null, 100)] })),
    tr({ id: 999, estado: "Cancelada", detalle: [linea(2, 100, null, 999)] }),
  ];
  const { truncado, periodo } = recortarPeriodo(seis, 5);
  assert.equal(truncado, true);

  const filas = resumenPorEstado(periodo);
  assert.equal(filas.length, 1, "la Cancelada de la sonda no puede aparecer");
  assert.equal(filas[0].estado, "Enviada");
  assert.equal(filas[0].cantidadTransferencias, 5);
  assert.equal(filas[0].importeTotal, 500);

  const prods = productosMasTransferidos(periodo);
  assert.equal(prods.length, 1, "el producto de la sonda no puede aparecer");
  assert.equal(prods[0].productoId, 1);

  const grupos = agruparPorDestino(periodo);
  assert.equal(grupos.reduce((a, g) => a + g.cantidadTransferencias, 0), 5);
});

test("6h. el recorte usa el techo real por defecto", () => {
  const justo = Array.from({ length: MAX_TRANSFERENCIAS }, (_, i) => ({ id: i }));
  assert.equal(recortarPeriodo(justo).truncado, false);
  assert.equal(recortarPeriodo([...justo, { id: -1 }]).truncado, true);
  assert.equal(recortarPeriodo([...justo, { id: -1 }]).periodo.length, MAX_TRANSFERENCIAS);
});
...
test("8. recepción real de cero → 0, y la diferencia se calcula", () => {
  ...
  // ── EL SIGNO SE UNIFICÓ EL 2026-09-13: RECIBIDA − ENVIADA ─────────────
  //
  // Esperaba 10. Esta función restaba `enviada − recibida`, o sea al revés que
  // `diferenciaDeLinea` y que todo lo que se dibuja en el detalle: un faltante
  // salía POSITIVO en el reporte y NEGATIVO en la ficha de la misma línea. Lo que
  // el candado defiende —que un 0 contado SÍ produce diferencia, contra un null
  // que no— no cambió.
...
test("8b. una sola línea con recepción alcanza para que el producto deje de ser null", () => {
  ...
  assert.equal(p.diferencia, -10, "negativo es falta, igual que en la ficha");
});
...
test("9b. la diferencia se calcula en milésimas enteras, no restando floats", () => {
  ...
  // −0,305 y no 0,305: el signo se unificó en `recibida − enviada`. Lo que el
  // candado defiende es que no aparezca un residuo binario, y ese sigue igual.
...
test("9d. importes: mil líneas de 0.1 dan 100.00 exacto", () => {
  ...
  // `origenEsDeposito` pasó a ser OBLIGATORIO: sin él la función lanza. No es un
  // ajuste del candado para que pase — es el contrato nuevo, y este test lo
  // ejerce igual que antes. Ver el porqué en `exigirOrigen`.
  assert.equal(importeDeDetalle(detalle, { origenEsDeposito: false }), 100);
});
...
test("9f. el importe del desglose por estado coincide con el del período", () => {
  ...
  const directo = desdeCentavos(
    periodoGrande.reduce(
      (a, t) => a + aCentavos(importeDeDetalle(t.detalle, { origenEsDeposito: t.origen?.es_deposito === true })),
      0
    )
  );
...
//
// ── QUÉ AFIRMA HOY, Y POR QUÉ SE REESCRIBIÓ ─────────────────────────────────
//
// Buscaba la palabra "transferencias" en todo el archivo, y con eso alcanzaba
// mientras la pantalla de Ventas no tuviera nada que decir sobre ellas. El
// 2026-08-20 sí lo tuvo: el listado ganó un interruptor para mostrar las ventas
// internas —las que generan un remito— y su texto de ayuda las nombra.
//
// Ese texto es legítimo y necesario: sin él, una venta que falta del listado no
// tiene explicación en pantalla. Lo que este candado quiere impedir es el
// ACOPLAMIENTO —que la pantalla de Ventas importe código del módulo de
// Transferencias o consulte sus endpoints—, no que la palabra aparezca. Eso es
// lo que se afirma ahora.
test("R3b. los archivos de Ventas no IMPORTAN ni consultan Transferencias", () => {
  for (const rel of [
    ...
    const src = sinComentarios(rel);
    assert.ok(
      !/from\s+["'][^"']*transferencias/i.test(src),
      `${rel} importa código del módulo de Transferencias`
    );
    assert.ok(
      !/["'`]\/api\/transferencias/i.test(src),
      `${rel} consulta un endpoint de Transferencias`
    );
    assert.ok(
      !/<Reporte(Transferencias|TransferenciasPorDestino)/i.test(src),
      `${rel} monta un componente del reporte de Transferencias`
    );
  }
});
...
// ════════════════════════════════════════════════════════════════════════════
// EL IMPORTE DEL PERÍODO ES EL **ENVIADO**, Y NO SE MUEVE MIENTRAS SE CUENTA
// ════════════════════════════════════════════════════════════════════════════
//
// Este módulo contesta cuánto se transfirió en el período. Hasta el 2026-09-11
// contestaba otra cosa sin decirlo: valorizaba lo RECIBIDO en cuanto alguien
// empezaba a contar, así que el total de agosto cambiaba en septiembre, cuando
// un local terminaba de recibir un remito viejo. Un reporte histórico cuyo
// número se mueve no es un reporte.
//
// Y con la #198 —snapshot PACK x24— además lo movía con las escalas cruzadas:
// leía los 6 packs recibidos como 6 unidades sueltas y daba 1.312,50 sobre un
// remito de 31.500.
//
// Los candados de abajo ejercen las CUATRO superficies del reporte sobre la
// misma línea, en cinco estados de recepción distintos. Lo que afirman es que
// el número no se mueve — no que valga 31.500 en un caso suelto.

/** La fila 6698 de la #198, tal como está en producción. */
...
const IMPORTE_ENVIADO_198 = 31500;

/** Los cinco estados por los que pasa una recepción. El importe no los mira. */
const ESTADOS_DE_RECEPCION = [
  ["sin contar", { recibido: null, recibidoUnidadesSueltas: null }],
  ["contado completo", { recibido: 6 }],
  ["faltan 2 packs", { recibido: 4 }],
  ["sobran 3 packs", { recibido: 9 }],
  ["5 packs y 7 sueltas", { recibido: 5, recibidoUnidadesSueltas: 7 }],
];

const remito198 = (parche = {}) => ({
  id: 198,
  estado: "Enviada",
  tieneDiferencias: false,
  destinoId: 3,
  ...
  fechaEnvio: "2026-09-05T12:00:00.000Z",
  createdAt: "2026-09-05T12:00:00.000Z",
  detalle: [{ ...LINEA_198, ...parche }],
});

test("E1. el importe del remito es el ENVIADO en los cinco estados de recepción", () => {
  for (const [caso, parche] of ESTADOS_DE_RECEPCION) {
    const t = remito198(parche);
    assert.equal(
      importeDeDetalle(t.detalle, { origenEsDeposito: true }),
      IMPORTE_ENVIADO_198,
      `el importe se movió al contar (${caso})`
    );
  }
});

test("E2. las CUATRO superficies del reporte dan lo mismo, y siguen dando lo mismo al contar", () => {
  // Es la enumeración de la regla 10: el arreglo anterior de este módulo dejó
  // cuatro vistas afuera porque nadie las había listado. Se listan.
  for (const [caso, parche] of ESTADOS_DE_RECEPCION) {
    const t = remito198(parche);

    // 1 · El total global del período.
    assert.equal(
      desdeCentavos(aCentavos(importeDeDetalle(t.detalle, { origenEsDeposito: true }))),
      IMPORTE_ENVIADO_198,
      `1 · total global (${caso})`
    );

    // 2 · El desglose por estado.
    const [estado] = resumenPorEstado([t]);
    assert.equal(estado.importeTotal, IMPORTE_ENVIADO_198, `2 · por estado (${caso})`);

    // 3 · El agrupado por destino: el total del grupo, la fila de adentro y el
    //     total de la vista. Los tres salen del mismo cálculo.
    const [grupo] = agruparPorDestino([t]);
    assert.equal(grupo.importeTotal, IMPORTE_ENVIADO_198, `3 · grupo por destino (${caso})`);
    assert.equal(
      grupo.transferencias[0].totalCosto,
      IMPORTE_ENVIADO_198,
      `3b · fila dentro del grupo (${caso})`
    );
    assert.equal(
      totalesDeDestinos([grupo]).importeTotal,
      IMPORTE_ENVIADO_198,
      `3c · totales de la vista (${caso})`
    );

    // 4 · Productos más transferidos.
    const [producto] = productosMasTransferidos([t]);
    assert.equal(
      producto.importeTransferido,
      IMPORTE_ENVIADO_198,
      `4 · productos más transferidos (${caso})`
    );
  }
});

test("E3. y las CANTIDADES recibidas sí siguen cambiando: no se borró información", () => {
  // Cambiar de qué importe habla el reporte no es dejar de contestar cuánto
  // llegó. Si esto se pusiera verde con las cantidades congeladas, el arreglo
  // habría tapado el otro concepto en vez de separarlo.
  const recibidas = ESTADOS_DE_RECEPCION.map(([, parche]) => {
    const [fila] = productosMasTransferidos([remito198(parche)]);
    return fila.cantidadRecibida;
  });
  // ── ESTE CANDADO CONGELABA EL DEFECTO, Y AHORA LO DELATA ───────────────
  //
  // Esperaba `[null, 6, 4, 9, 5]`: los PACKS, tal como están guardados en la
  // columna. Pero la línea del 198 salió como **6 PACK x24**, y el reporte informa
  // junto a `cantidadEnviada`, que son 144 unidades FÍSICAS. Con los packs crudos
  // el renglón decía "enviada 144 · recibida 6" sobre un remito que llegó
  // completo, y este candado lo afirmaba.
  //
  // Ahora las dos columnas hablan la misma escala: 6 packs son 144, 4 son 96, 9
  // son 216, y "5 packs y 7 sueltas" son 127 — las sueltas también entran, que es
  // lo que la escala cruda perdía. Ver `INC-0009`.
  assert.deepEqual(recibidas, [null, 144, 96, 216, 127]);
});

test("E4. el número viejo era 1.312,50 y su causa era leer packs como unidades", () => {
  // Se reconstruye por su definición, para que la constante no quede vieja en
  // silencio: 6 packs leídos como 6 unidades, al costo por unidad.
  const costoPorUnidad = IMPORTE_ENVIADO_198 / 144; // 218,75
  assert.equal(6 * costoPorUnidad, 1312.5);
  // Y el bueno es el mismo costo por las unidades físicas que de verdad salieron.
  assert.equal(144 * costoPorUnidad, IMPORTE_ENVIADO_198);
});

// ── EL CASO #191: LOS DOS IMPORTES, CON NOMBRE Y APELLIDO ───────────────────
//
// Un PACK x12 cuyo pack cuesta 21.600. Se envió 1 pack; el destino contó 1 pack
// y además 100 unidades sueltas. Los dos números son correctos y contestan
// preguntas distintas:
//
//   ENVIADO  (reporte del período, detalle de recepción): 12 × 1.800 =  21.600
//   RECIBIDO (acta de recepción):              (12 + 100) × 1.800 = 201.600
//
// Que difieran ONCE veces no es un defecto: es un remito con una diferencia
// enorme, y el acta existe justamente para dejarla escrita. El defecto sería
// que el reporte del período se mueva a 201.600 y el total de agosto cambie.

// `precioCosto` va POR BULTO: es como lo guarda el catálogo y como lo copia
// `crearTransferencia`. Los 1.800 por unidad los deriva la fórmula.
const LINEA_191 = Object.freeze({
  cantidad: 12,
  unidadEnviada: "UNIDAD",
  precioCosto: 21600,
  recibido: 1,
  recibidoUnidadesSueltas: 100,
  presentacionEnvio: "PACK",
  cantidadPresentada: 1,
  factorPresentacion: 12,
  sueltasEnviadas: 0,
  productoId: 5001,
  producto: {
    precio_costo: null,
    nombre: "Producto de la #191",
    base: {
      precio_costo: 21600,
      nombre: "Producto de la #191",
      unidad_medida: "pack",
...
test("E5. #191: el reporte del período dice 21.600 aunque se hayan contado 100 sueltas de más", () => {
  assert.equal(importeDeDetalle([LINEA_191], { origenEsDeposito: true }), 21600);
});

test("E6. el reporte le pasa la línea COMPLETA: lee la presentación registrada, no la reconstruye", () => {
  // ── LO QUE ESTE CANDADO AFIRMA, Y LO QUE NO ───────────────────────────────
  //
  // NO afirma un número. Se ejerció: sacándole el snapshot a esta llamada, los
  // cinco candados de arriba siguen en verde y la #198 sigue dando 31.500. Para
  // el importe ENVIADO la reconstrucción coincide, porque `cantidad` ya está en
  // unidades físicas y el costo se normaliza a esa misma escala.
  //
  // Lo que afirma es la decisión: que haya UNA fuente de la presentación. Hoy
  // la reconstrucción da lo mismo; el día que deje de darlo —una presentación
  // nueva, un producto al que le cambian `factor_pack` después del despacho—
  // el reporte tiene que estar leyendo el snapshot y no adivinando. Que las dos
  // coincidan no las convierte en la misma fuente.
  //
  // Se escribe como candado de código y no de número a propósito: un candado
  // que afirmara un número acá estaría verde por la coincidencia, no por lo que
  // dice defender. Es el caso del `total` derivable de CLAUDE.md.
  const src = sinComentarios("lib/transferencias/agregadosPeriodo.js");
  const i = src.indexOf("valorizarLineaDelRemito(");
  assert.ok(i > -1, "el reporte dejó de usar la primitiva canónica");
  const llamada = src.slice(i, i + 800);
  for (const campo of [
    "presentacionEnvio",
    "cantidadPresentada",
    "factorPresentacion",
    ...
    assert.match(
      llamada,
      new RegExp(`${campo}:\\s*d\\.${campo}\\b`),
      `el reporte dejó de pasar ${campo}: vuelve a reconstruir la presentación en vez de leerla`
    );
  }
});
