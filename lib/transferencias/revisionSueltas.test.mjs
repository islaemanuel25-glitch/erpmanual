...
test("3. la acción del catálogo del origen solo existe tras no encontrar", () => {
  ...
  // TODA puerta de entrada al catálogo tiene que estar condicionada al aviso de
  // "no figura", no a `puedeRecibir` a secas. Se afirma sobre CADA aparición y
  // no sobre "existe alguna guardada": la primera redacción de este candado
  // pedía una sola coincidencia, y con dos lugares en el archivo alcanzaba con
  // que uno estuviera bien para quedarse en verde con el otro suelto.
  const aperturas = [...src.matchAll(/setAgregarAbierto\(true\)/g)];
  assert.ok(aperturas.length >= 1, "no hay ninguna puerta al catálogo");
  for (const m of aperturas) {
    const antes = src.slice(Math.max(0, m.index - 200), m.index);
    assert.match(
      antes,
      /aviso === MENSAJE_NO_FIGURA && puedeRecibir && \(/,
      "hay una puerta al catálogo sin condicionar al aviso de que no figura"
    );
  }
