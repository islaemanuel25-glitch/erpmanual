...
test("6. la tabla histórica mide la diferencia en FÍSICO", () => {
  ...
  // ── LA PUERTA CANÓNICA CAMBIÓ EL 2026-09-13, Y ES MÁS FUERTE ──────────
  //
  // Este candado exigía `unidadesFisicasDe(`. Esa función es correcta y la escala
  // que se le pasaba NO lo era: la tabla le daba las columnas crudas
  // `d.unidadEnviada` y `d.factorPack`, así que con una línea consolidada a
  // físicas —`UNIDAD`, factor 1— los 4 CAJÓN x8 de `recibido` se leían como 4
  // unidades contra 32 enviadas. El candado estaba en verde sobre eso, porque
  // miraba el nombre de la función y no de dónde salía la escala.
  //
  // Ahora se exigen las dos funciones que resuelven la escala con el descriptor
  // —el snapshot cuando la línea lo tiene— y se PROHÍBE volver a la columna
  // cruda, que es la afirmación que faltaba. Ver `INC-0009`.
  assert.match(src, /fisicasEnviadasDe\(d\)/, "no usa la función canónica de lo enviado");
  assert.match(src, /fisicasRecibidasDe\(d, \{/, "no usa la función canónica de lo recibido");
  assert.ok(
    !/unidad: d\.unidadEnviada/.test(src),
    "volvió a leer la escala de la columna cruda: eso resta dos escalas distintas"
  );
  assert.ok(
    !/factorPack: d\.factorPack/.test(src),
    "volvió a leer el factor del catálogo de hoy en vez del congelado del remito"
  );
  assert.match(src, /sueltas: d\.recibidoUnidadesSueltas/, "ignora el pack incompleto");
  // La resta se PIDE desde el 2026-09-13: `diferenciaDeLinea` fija el signo para
  // el detalle, el listado y el reporte, así que el mismo hecho no puede tener dos
  // signos según la pantalla. Y se prohíbe la resta escrita a mano, que es lo que
  // este candado no podía ver cuando solo afirmaba el texto de una de las dos.
  assert.match(src, /diferenciaDeLinea\(\{ enviada: envFis, recibida: recFis \}\)/);
  assert.ok(
    !/recFis - envFis/.test(src),
    "volvió la resta escrita a mano: el signo se vuelve a poder equivocar solo"
  );
