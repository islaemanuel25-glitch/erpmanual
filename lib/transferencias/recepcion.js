...
export const ERRORES_RECEPCION = {
  ...
  // El desglose de unidades sueltas solo existe cuando la presentación AGRUPA.
  // En UNIDAD no hay bultos que completar, así que un desglose ahí es un dato
  // que no significa nada — y si se aceptara, se sumaría dos veces.
  SUELTAS_SIN_BULTO: "UNIDADES_SUELTAS_SIN_BULTO",
  SUELTAS_INVALIDAS: "UNIDADES_SUELTAS_INVALIDAS",
};
...
// ── EL PACK INCOMPLETO, Y POR QUÉ NO SE MODELA CON UN DECIMAL ─────────────
//
// Salieron 6 PACK de 6 —36 unidades— y llegaron 5 packs enteros más 5 sueltas.
// Físicamente son 35. La tentación es guardar `recibido = 5.833` y dejar que el
// factor haga el resto, y eso está MAL con exactitud aritmética:
//
//     5.833 × 6 = 34.998
//
// En un `Decimal(12,3)` eso no es 35, es 34.998. El stock del destino quedaría
// con dos milésimas de menos y el ajuste del origen con dos de más, en cada
// pack incompleto, para siempre. No es un redondeo de presentación: es el número
// que se escribe en la base.
//
// Por eso la representación es EXPLÍCITA y son dos datos:
//
//     recibido                = 5   ← packs enteros, en la unidad del envío
//     recibidoUnidadesSueltas = 5   ← unidades físicas fuera de los packs
//
// y el total sale de multiplicar UNA vez y sumar:
//
//     5 × 6 + 5 = 35     exacto, sin decimales periódicos
//
// Todo en milésimas enteras, que es la escala de `Decimal(12,3)`: 5000 × 6 +
// 5000 = 35000. Un entero por otro entero más un entero.

/**
 * MILÉSIMAS FÍSICAS de una cantidad expresada en la presentación del envío.
 *
 * ES LA ÚNICA FUENTE de "cuántas unidades son". La alimenta el preview, la
 * diferencia, el resumen, la validación, el ajuste del origen, el ingreso al
 * destino y los tests. Si cada pantalla hiciera `packs * factor + sueltas` por su
 * cuenta, la primera que se olvide de las sueltas escribe stock mal.
 *
 * @returns {number|null} milésimas enteras, o null si el dato no es representable
 */
export function milesimasFisicas({ cantidad, sueltas, unidad, factorPack } = {}) {
  const cM = aMilesimas(cantidad);
  if (cM === null || cM < 0) return null;

  const f = factorFisico(unidad, factorPack);

  const vacio = sueltas === null || sueltas === undefined || sueltas === "";
  const sM = vacio ? 0 : aMilesimas(sueltas);
  if (sM === null || sM < 0) return null;

  // Sin bulto no hay sueltas: en UNIDAD la cantidad YA está en unidades físicas,
  // así que un desglose se sumaría encima de sí mismo.
  if (f === 1 && sM !== 0) return null;
...
/** Lo mismo, en unidades. Azúcar sobre `milesimasFisicas` para los llamadores. */
export function unidadesFisicasDe(args) {
  const m = milesimasFisicas(args);
  return m === null ? null : desdeMilesimas(m);
}
...
export function calcularAjusteOrigenUnidades({
  ...
  // Las dos por el MISMO camino, `milesimasFisicas`. Antes esto era
  // `(envM - recM) × factor`, que es el mismo número mientras no haya sueltas
  // —enteros, distributiva exacta— pero no sabe representar un pack incompleto.
  //
  // Lo enviado también puede traer sueltas desde que el snapshot registra un
  // despacho mixto; el default 0 conserva el cálculo de las líneas anteriores.
  const envM = milesimasFisicas({ cantidad: enviada, sueltas: enviadaSueltas, unidad, factorPack });
  const recM = milesimasFisicas({ cantidad: recibida, sueltas: recibidaSueltas, unidad, factorPack });
  if (envM === null || recM === null) return null;

  const ajusteM = envM - recM;
...
export function validarDetalleRecepcion({
  ...
  const sueltas =
    sueltasPropuestas === undefined ? detalle.recibidoUnidadesSueltas : sueltasPropuestas;
  ...
  // ── LAS CANTIDADES FÍSICAS, POR UN SOLO CAMINO ──────────────────────────
  //
  // Lo recibido puede tener un pack incompleto, y de ahí sale la exactitud.
  // Las sueltas, normalizadas una sola vez. `null`, `undefined` y `""` son lo
  // mismo: no hay pack incompleto.
  //
  // ── Y LO ENVIADO TAMBIÉN PUEDE TENER SUELTAS ──────────────────────────
  //
  // Acá decía `sueltas: 0` con el comentario "el remito salió en packs
  // enteros", y eso dejó de ser cierto cuando el snapshot aprendió a registrar
  // un despacho mixto: el POS permite vender el mismo producto en dos modos, y
  // 4 packs de 6 más 5 sueltas se congelan como `cantidadPresentada = 4` y
  // `sueltasEnviadas = 5`. Ignorar esas 5 leería 24 unidades enviadas donde
  // salieron 29, y la línea informaría un sobrante de 5 que nunca existió.
  //
  // El default sigue siendo 0, así que una línea sin snapshot calcula
  // exactamente lo que calculaba antes.
  const sinDesglose = sueltas === null || sueltas === undefined || sueltas === "";
  const sueltasM = sinDesglose ? 0 : aMilesimas(sueltas);

  const enviadaM = milesimasFisicas({
    cantidad: res.enviada,
    sueltas: detalle.sueltasEnviadas ?? 0,
    unidad: uni.unidad,
    factorPack,
  });
  const recibidaM = milesimasFisicas({
    cantidad: res.recibida, sueltas, unidad: uni.unidad, factorPack,
  });
  if (enviadaM === null) return { ok: false, error: ERRORES_RECEPCION.ENVIADA_INVALIDA };
  if (recibidaM === null) {
    // Se distingue el caso: un desglose sobre una línea en UNIDAD no es una
    // cantidad inválida, es un dato que no puede existir. Decirlo con el mismo
    // error escondería el motivo real.
    //
    // La pregunta se hace sin volver a multiplicar: si la cantidad sola ya es
    // válida, lo único que pudo fallar es el desglose.
    const soloCantidad = milesimasFisicas({
      cantidad: res.recibida, sueltas: 0, unidad: uni.unidad, factorPack,
    });
    if (soloCantidad !== null && sueltasM !== null && sueltasM !== 0) {
      return { ok: false, error: ERRORES_RECEPCION.SUELTAS_SIN_BULTO };
    }
    return { ok: false, error: ERRORES_RECEPCION.SUELTAS_INVALIDAS };
  }

  // ── LA DIFERENCIA SE MIDE EN FÍSICO, Y ESO CAMBIÓ ───────────────────────
  //
  // `resolverRecibido` compara la cantidad en la PRESENTACIÓN: 6 contra 6 da
  // "sin diferencia". Con un pack incompleto eso miente en los dos sentidos:
  //
  //   6 packs + 1 suelta   → 6 == 6, pero físicamente son 37 contra 36
  //   5 packs + 6 sueltas  → 5 != 6, pero físicamente son 36 contra 36
  //
  // Lo que decide si hay que explicar algo, y si el stock se mueve, es el
  // número físico. Por eso el `hayDiferencia` que sale de acá lo reemplaza.
  const hayDiferencia = enviadaM !== recibidaM;
  ...
  const ajusteM = enviadaM - recibidaM;
  if (!Number.isSafeInteger(ajusteM)) {
    return { ok: false, error: ERRORES_RECEPCION.AJUSTE_INVALIDO };
  }
  const ajusteOrigenUnidades = desdeMilesimas(ajusteM);
  ...
  return {
    ...
    enviada: res.enviada,
    // Las unidades sueltas del pack incompleto, normalizadas: `0` cuando no hay
    // desglose, para que el llamador no tenga que distinguir null de vacío.
    recibidaSueltas: desdeMilesimas(sueltasM),
    hayDiferencia,
    agregadoEnRecepcion: agregado,
    ...
    // y los tres por `milesimasFisicas`: ninguna pantalla ni ruta multiplica por
    // su cuenta.
    recibidaUnidades: desdeMilesimas(recibidaM),
    enviadaUnidades: desdeMilesimas(enviadaM),
