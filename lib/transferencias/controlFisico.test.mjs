...
test("las cards y el listado NO pueden discrepar: son el mismo estado", () => {
  ...
  // ── "TODOS" ES EL TOTAL FISICO, NO EL DEL REMITO ────────────────────
  //
  // Aca decia `r.totalRemito` con el comentario "'Todos' son los del remito".
  // Ese contrato dejaba a los agregados sin NINGUN filtro donde aparecer:
  // alguien informaba un producto que llego de mas y lo veia desaparecer.
  //
  // Lo que el candado defiende no cambio —que las cards y el listado no puedan
  // discrepar— y ahora hay DOS totales con dos nombres, cada uno con su lista.
  assert.equal(cuenta(FILTRO.TODOS), r.totalFisico, "'Todos' es toda la mercaderia presente");
  assert.equal(r.totalFisico, r.totalRemito + r.noDeclarados);
  assert.ok(r.totalFisico > r.totalRemito, "el caso de prueba tiene un agregado");
});
...
test("12. el filtro No declarado devuelve SOLO los agregados", () => {
  ...
  // Y "Todos" SI lo incluye: es toda la mercaderia de la recepcion, no el
  // remito. Aca se afirmaba lo contrario —`[1]`— y ese contrato es el que dejaba
  // al agregado sin ningun filtro donde aparecer.
  assert.deepEqual(productosVisibles(items, { filtro: FILTRO.TODOS }).map((d) => d.id), [1, 2]);

  // Lo que sigue siendo cierto: NO se cuela en los tabs de trabajo del remito.
  for (const f of [FILTRO.PENDIENTES, FILTRO.REVISADOS, FILTRO.DIFERENCIAS]) {
    assert.ok(
      !productosVisibles(items, { filtro: f }).some((d) => d.id === 2),
      `un no declarado se colo en ${f}`
    );
  }
});
...
test("24-25. guardar un borrador NO marca revisado; revisar SÍ lo persiste", () => {
  ...
  // ── LA MARCA SE PERSISTE, HOY POR DOS CAMINOS SEPARADOS ────────────────
  //
  // Esto exigía un único `update` ternario: `revisadoEnRecepcion: revisado`,
  // con el autor y la fecha colgando del mismo booleano. El 2026-09-08 se
  // partió en dos caminos, porque el ternario obligaba a pasar por el
  // validador también al DESMARCAR — y validar sin cuerpo reescribía el
  // conteo con lo enviado. El candado 1 de `revisionSueltas.test.mjs` cubre
  // eso; acá se sostiene lo que este candado siempre afirmó: que marcar
  // escribe la marca con su autoría, y que desmarcar la borra entera.
  const revisar = leerSinComentarios("app/api/transferencias/revisar-producto/route.js");
  assert.match(revisar, /revisadoEnRecepcion: true/, "marcar tiene que persistir la marca");
  assert.match(revisar, /revisadoEnRecepcionPorId: usuarioId/);
  assert.match(revisar, /revisadoEnRecepcionAt: new Date\(\)/);
  assert.match(revisar, /revisadoEnRecepcion: false/, "desmarcar tiene que borrarla");
  assert.match(revisar, /revisadoEnRecepcionPorId: null/);
  assert.match(revisar, /revisadoEnRecepcionAt: null/);
...
test("26. revisar usa el MISMO mutex y valida con la MISMA función", () => {
  ...
  // ── LO QUE ESTE CANDADO AFIRMA HOY ────────────────────────────────────
  //
  // Que la cantidad enviada, la unidad y la procedencia salen de la BASE. Eso
  // no cambió; lo que cambió es DÓNDE se arma ese objeto: se mudó a
  // `detalleParaValidar`, en el dominio compartido, para que las tres rutas que
  // escriben no puedan interpretar la línea cada una a su manera.
  //
  // El candado se muda con él. Si se hubiera dejado apuntando al archivo viejo
  // seguiría en verde sin afirmar nada, que es el modo silencioso de perder un
  // candado — y en este repo ya pasó.
  assert.match(revisar, /detalleParaValidar\(d/, "revisar dejó de usar la resolución compartida");
  ...
  assert.match(servidor, /cantidad: escala\.cantidad/);
  assert.match(servidor, /unidadEnviada: escala\.unidad/);
  assert.match(servidor, /agregadoEnRecepcion: d\.agregadoEnRecepcion/);
  assert.ok(
    !/body/.test(servidor),
    "la resolución compartida no puede mirar el request: es la fuente de verdad"
  );
