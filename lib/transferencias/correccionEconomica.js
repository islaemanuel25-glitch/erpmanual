...
/**
 * La línea que apareció al abrir los bultos y no tiene precio congelado.
 *
 * FAIL CLOSED: sin precio no se confirma la recepción. Las tres salidas fáciles
 * —valorizarla en cero, dejarla fuera del importe, o buscar el precio del
 * catálogo de hoy— dejan el stock corregido con el dinero mal, que es
 * exactamente lo que este módulo existe para impedir.
 */
export const LINEA_AGREGADA_SIN_PRECIO = "LINEA_AGREGADA_SIN_PRECIO";
 ...
 */
export function lineasCorregidasDeProducto({
  detalleVenta,
  lineaAgregada,
  factor,
  recibidasFisicas,
  ...
  // ── DE DÓNDE SALE EL PRECIO: DOS FUENTES, PORQUE SON DOS COSAS ───────────
  //
  // Una línea DEL REMITO tiene su VentaDetalle, con el precio comercial que se
  // cobró. Una línea AGREGADA en recepción no lo tiene —nunca se vendió— y por
  // eso usa el `precioCosto` que `linea-recepcion` congeló al agregarla.
  //
  // En los dos casos el precio está CONGELADO y viene de afuera. Ninguno de los
  // dos consulta el catálogo: entre agregar una línea y confirmar la recepción
  // pueden pasar días, y revalorizar con el precio del día de confirmar
  // cambiaría lo que costó una mercadería que ya está en la góndola.
  const esAgregada = !!lineaAgregada;
  const vd = detalleVenta || {};

  let precioUnitarioFisico;
  let precioPresentacion;
  let costoUnitarioFisico;
  let enviadasFisicas;
  let origenDetalleId;
  let productoBaseId;
  let nombre;
  ...
  if (esAgregada) {
    const congelado = Number(lineaAgregada.precioPresentacion);
    // FAIL CLOSED. Cero tampoco es un precio: valorizar en cero dejaría el stock
    // corregido y el importe no, que es la misma excepción pero en silencio.
    if (!Number.isFinite(congelado) || congelado <= 0) {
      const e = new Error(
        `${LINEA_AGREGADA_SIN_PRECIO}: el producto ${lineaAgregada.nombre ?? lineaAgregada.productoBaseId} ` +
          "se agregó en la recepción sin un precio interno congelado, así que no se puede valorizar"
      );
      e.code = LINEA_AGREGADA_SIN_PRECIO;
      throw e;
    }
    precioPresentacion = congelado;
    // El congelado está en la escala de la PRESENTACIÓN —por cajón, por pack—,
    // igual que el `precioCosto` de una línea del remito. La unidad física sale
    // de dividir por el factor canónico, sin redondear.
    precioUnitarioFisico = congelado / f;
    costoUnitarioFisico = precioUnitarioFisico;
    // No se envió: su cantidad anterior es 0, y eso es el dato honesto.
    enviadasFisicas = 0;
    origenDetalleId = null;
    productoBaseId = lineaAgregada.productoBaseId ?? null;
    nombre = lineaAgregada.nombre ?? null;
  } else {
    const stockOriginal = Number(vd.cantidadStock);
    // Sin consumo físico congelado no hay cómo pasar de plata a unidades: un
    // combo gasta por sus componentes y un servicio no gasta nada. La auditoría
    // dio cero de los dos en ventas internas, así que no hay caso con el que
    // probar una conversión — y una conversión sin caso que la ejerza es
    // exactamente como se escribe una que nadie sabe si funciona.
    ...
    precioUnitarioFisico = Number(vd.subtotal) / stockOriginal;
    precioPresentacion = Number(vd.precio);
    costoUnitarioFisico = (Number(vd.precioCosto) || 0) / f;
  ...
  const lineaBultos = {
    ...
    precio: round2(precioPresentacion),
    precioCosto: round2(esAgregada ? precioPresentacion : vd.precioCosto),
    subtotal: dePesos(centBultos),
    ganancia: dePesos(centBultos - Math.round(stockBultos * costoUnitarioFisico * 100)),
    esSueltas: false,
    agregadaEnRecepcion: esAgregada,
  };
  ...
  return {
    ...
    enviadasFisicas,
    agregadaEnRecepcion: esAgregada,
    // Una línea agregada NUNCA es "sin cambio": no estaba en la venta, así que
    // su sola existencia mueve el importe.
    sinCambio: !esAgregada && totalCent === aCent(vd.subtotal) && fisicas === enviadasFisicas,
  };
}
...
export function planCorreccionEconomica({
  ...
  // ── LO QUE APARECIÓ AL ABRIR LOS BULTOS ───────────────────────────────────
  //
  // Una línea agregada no tiene VentaDetalle que emparejar, así que no entra por
  // el bucle de arriba: se suma acá, como líneas nuevas de la venta corregida.
  //
  // Solo entran las marcadas `agregada`. Un `productoBaseId` que no casa y que NO
  // viene marcado es un error de armado, no un producto nuevo, y agregarlo sería
  // inventar una venta.
  for (const r of recibido) {
    if (!r.agregada) continue;
    huboMatch = true;
    todasSinCambio = false;

    const res = lineasCorregidasDeProducto({
      lineaAgregada: {
        productoBaseId: r.productoBaseId,
        nombre: r.nombre ?? null,
        precioPresentacion: r.precioPresentacion,
      },
      factor: r.factor,
      recibidasFisicas: r.recibidasFisicas,
    });
    ...
    diffProductos.push({
      ...
      nombre: r.nombre ?? null,
      // Cero y no null: el remito NO la mandó, y eso es un hecho, no un dato
      // que falte.
      cantidadAntes: 0,
      cantidadDespues: res.recibidasFisicas,
      estado: "MAS",
      agregadaEnRecepcion: true,
    });
  }
  ...
  return {
    ...
    snapshotDespues: {
      ...
      detalles: lineasNuevas.map((l) => ({
        ...
        // Para que la auditoría distinga, dentro de un mes, qué llegó de más de
        // lo que se mandó y qué llegó sin estar en el remito.
        agregadaEnRecepcion: l.agregadaEnRecepcion === true,
      })),
