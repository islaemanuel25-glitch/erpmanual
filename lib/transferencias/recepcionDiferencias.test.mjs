 ...
 * Un plan validado.
 *
 * `motivoPrincipal` va por default porque una línea DEL REMITO con diferencia lo
 * exige, y lo que estos tests miden es la aritmética, no el motivo. Se puede
 * pasar `motivoPrincipal: null` para medir justamente la regla del motivo — que
 * es lo que hace la sección "el motivo se pide según la PROCEDENCIA".
 */
const plan = ({
  cantidad,
  recibido,
...
test("9. no hay doble conversión: el factor multiplica la RESTA, no cada término", () => {
  ...
  // ── LA FORMA CAMBIÓ, LA GARANTÍA NO ────────────────────────────────────
  //
  // Esto exigía el literal `(envM - recM) * factorFisico`. El 2026-09-09 el
  // cálculo pasó por `milesimasFisicas`, que multiplica cada término por el
  // factor y después resta:
  //
  //     envM × f − (recM × f + sueltas)
  //
  // Sobre ENTEROS las dos formas dan el mismo número —distributiva exacta, sin
  // punto flotante— y la nueva además sabe representar un pack incompleto, que
  // la vieja no podía. Exigir el literal viejo sería pedir que se rompa eso.
  //
  // Lo que el candado defiende sigue igual y se afirma por el camino nuevo: el
  // factor se aplica UNA vez, todo en milésimas enteras, y hay una sola función
  // que lo hace.
  const src = leerSinComentarios("lib/transferencias/recepcion.js");
  assert.ok(
    /export function milesimasFisicas/.test(src),
    "desapareció la única fuente de cantidad física"
  );
  assert.ok(
    /const total = cM \* f \+ sM/.test(src),
    "el factor tiene que multiplicar una sola vez, sobre milésimas enteras"
  );
  // Y el ajuste sale de restar dos físicas, no de una segunda multiplicación.
  assert.ok(
    /const ajusteM = enviadaM - recibidaM/.test(src),
    "el ajuste volvió a multiplicar por su cuenta"
  );
  assert.equal(
    (src.match(/\* factorFisico\(/g) || []).length,
    0,
    "quedó una multiplicación por el factor fuera de `milesimasFisicas`"
  );
...
// ── 20. QUÉ PUEDE TOCAR LA RECEPCIÓN, Y QUÉ SIGUE PROHIBIDO ────────────────
//
// ── POR QUÉ ESTE CANDADO CAMBIÓ, Y POR QUÉ NO ES AFLOJARLO ────────────────
//
// Hasta el 2026-09-11 decía "la recepción sigue sin tocar Venta, caja, pagos ni
// puntos", y era verdad: confirmar movía stock y dejaba la venta facturando lo
// enviado. Esa era la mitad que faltaba, no una garantía — la #198 terminaba con
// 156 unidades en el depósito y 144 en la factura.
//
// La decisión de negocio es que lo recibido pasa a ser la verdad económica, así
// que confirmar AHORA corrige su propia venta. Lo que sigue prohibido es todo lo
// demás, y eso es lo que este candado defiende hoy.
//
// ── Y LO QUE CASI PASA DESAPERCIBIDO ─────────────────────────────────────
//
// Al mudar las escrituras a `aplicarCorreccionEconomica.js`, la versión vieja de
// este candado quedó en VERDE sin cambiarle una letra: buscaba "venta.update" en
// el texto de la ruta, y ahí ya no está. Habría seguido en la suite afirmando
// nada, que es el caso que CLAUDE.md llama el falso verde. Por eso ahora el
// candado SIGUE LA LLAMADA: mira también el archivo que confirma invoca.

const APLICADOR = "lib/transferencias/aplicarCorreccionEconomica.js";

test("20. guardar y línea siguen sin tocar NADA económico", () => {
  // Estas dos no confirman: escriben lo que el operador va contando. Mientras la
  // recepción no esté confirmada, la plata no se mueve.
  for (const ruta of [GUARDAR, LINEA]) {
    const src = leerSinComentarios(ruta);
    for (const tabla of [
      "venta.update", "venta.create", "ventaDetalle", "ventaPago",
      "movimientoCuenta", "cajaMovimiento", "puntos", "ventaCorreccion",
    ]) {
...
test("20b. confirmar corrige la venta SOLO por el aplicador, no por su cuenta", () => {
  const src = leerSinComentarios(CONFIRMAR);
  // Se pide la LLAMADA y no el nombre suelto: `includes` se conformaría con
  // cualquier identificador que contenga la cadena —el import, un comentario, un
  // `aplicarCorreccionEconomicaDESACTIVADA`— y quedaría verde sin que la
  // corrección ocurra. Es el mismo falso verde que ya se pagó una vez.
  assert.match(
    src,
    /await aplicarCorreccionEconomica\(tx,/,
    "confirmar dejó de corregir la venta: el stock volvería a quedar corregido con dinero original"
  );
  // La ruta no escribe la venta a mano. Tener dos lugares que la corrigen es
  // tener dos criterios que un día difieren.
  for (const suelto of ["venta.update", "venta.create", "ventaDetalle", "ventaPago", "ventaCorreccion"]) {
    assert.ok(!src.includes(`tx.${suelto}`), `confirmar escribe ${suelto} por fuera del aplicador`);
  }
});

test("20c. NI CAJA NI CUENTA CORRIENTE NI PUNTOS, en ninguno de los dos archivos", () => {
  // Ésta es la parte que NO cambió y es la que más plata mueve. Una venta interna
  // con remito está excluida del arqueo —lo dice `impactoEnArqueo`—, así que
  // moverle la caja sería descuadrar un turno por una recepción. Y una deuda
  // entre el depósito y el local es un hecho nuevo que nadie decidió.
  for (const ruta of [CONFIRMAR, APLICADOR]) {
    const src = leerSinComentarios(ruta);
    for (const tabla of ["movimientoCuenta", "cajaMovimiento", "puntos", "arqueo"]) {
      assert.ok(!src.includes(tabla), `${ruta} volvió a tocar ${tabla}`);
    }
  }
});

test("20d. el aplicador NO mueve inventario: el stock lo movió la recepción", () => {
  // Es el candado del doble descuento. Una recepción de +12 unidades tiene que
  // mover el stock UNA vez; si la corrección de la venta volviera a aplicar su
  // delta, el destino quedaría con 24 de más.
  const src = leerSinComentarios(APLICADOR);
  for (const escritura of ["stockLocal", "aplicarDeltaStock", "productoLocal.update", "auditoriaStock"]) {
    assert.ok(!src.includes(escritura), `el aplicador toca ${escritura}: el stock se movería dos veces`);
  }
});

test("20e. confirmar NO exige turno abierto ni pasa por el beta de corrección", () => {
  // Medido sobre producción: 147 de las 196 ventas internas con remito tienen el
  // turno original cerrado. Reusar el flujo COMPLETA habría bloqueado tres de
  // cada cuatro recepciones, y el feature flag las habría bloqueado todas.
  for (const ruta of [CONFIRMAR, APLICADOR]) {
    const src = leerSinComentarios(ruta);
    for (const prohibido of [
      "estadoTurnoCorreccion", "TURNO_CERRADO", "enBetaCorreccionCompleta", "correccionCompletaServer",
    ]) {
      assert.ok(!src.includes(prohibido), `${ruta} arrastró ${prohibido} del flujo COMPLETA`);
    }
  }
});
...
test("agregar una línea sin unidad se rechaza; no se elige una por defecto", () => {
  ...
  // ── Y AHORA HAY UN TERCER CASO, QUE ES EL QUE FALTABA ─────────────────
  //
  // Este candado decía `src.includes("uni.unidad")` con el comentario "la línea
  // no se crea con la unidad validada". Eso se quedó corto: validar que el dato
  // del cliente sea BULTO o UNIDAD no dice nada sobre si es el CORRECTO. Un
  // cliente viejo que mande "UNIDAD" sobre un PACK x6 pasaba la validación y
  // convertía 2 bultos en 2 unidades — 10 que se le descuentan de menos al
  // origen, sin envío contra el cual contrastar.
  //
  // El producto ya se relee del catálogo del ORIGEN unas líneas antes, así que
  // el servidor TIENE la respuesta. Ahora la deriva, la compara, y la fila se
  // crea con la suya.
  assert.match(src, /const unidadAutoritativa = unidadFisicaDe\(presentacion\)/);
  assert.match(src, /uni\.unidad !== unidadAutoritativa/, "no se compara contra el catálogo");
  assert.ok(src.includes("UNIDAD_CONTRADICE_CATALOGO"), "no se distingue la unidad que contradice");
  assert.match(src, /unidadEnviada: unidadAutoritativa/, "la línea no se crea con la del catálogo");
  assert.ok(
...
// ═══════════════════════════════════════════════════════════════════════════
// EL MOTIVO SE PIDE SEGÚN LA PROCEDENCIA, NO SOLO SEGÚN HAYA DIFERENCIA
//
// ── EL DEFECTO ────────────────────────────────────────────────────────────
//
// `linea-recepcion` crea la línea agregada con `cantidad = 0`, `recibido > 0`,
// su autor y su fecha — y SIN motivo, porque no hay ninguno que sea un dato real.
// Después `validarDetalleRecepcion` la veía como una diferencia cualquiera y le
// exigía uno. O sea que el sistema creaba una línea que él mismo no podía
// confirmar: el flujo aprobado se cortaba con FALTA_MOTIVO_DIFERENCIA.
//
// ── POR QUÉ EXIMIRLA NO ES AFLOJAR ────────────────────────────────────────
//
// La línea agregada ya explica de dónde salió, y con un registro más fuerte que
// un texto elegido de una lista: el hecho, el autor, la fecha, y la auditoría
// como AGREGADO_RECEPCION_TRANSFERENCIA. Pedirle además "Sobrante" es pedir dos
// veces la misma verdad.
//
// Lo que NO se toca es la línea del REMITO: sigue exigiendo motivo, y el
// contraste está probado abajo con el par 0→2 agregada / 0→2 del remito.
// ═══════════════════════════════════════════════════════════════════════════

test("exigeMotivo separa los dos casos, y es UNA sola función", () => {
  // Línea del remito con diferencia: sí.
  assert.equal(exigeMotivo({ hayDiferencia: true, agregadoEnRecepcion: false }), true);
  assert.equal(exigeMotivo({ hayDiferencia: true }), true);
  // Sin diferencia: no hay nada que explicar.
  assert.equal(exigeMotivo({ hayDiferencia: false }), false);
  // Agregada en recepción: su procedencia ya está registrada.
  assert.equal(exigeMotivo({ hayDiferencia: true, agregadoEnRecepcion: true }), false);

  // Y `validarMotivoDiferencia` la usa, en vez de tener su propia condición.
  assert.equal(
    validarMotivoDiferencia({ hayDiferencia: true, motivoPrincipal: "" }).ok,
    false,
    "una línea del remito sin motivo tiene que seguir fallando"
  );
  assert.equal(
    validarMotivoDiferencia({ hayDiferencia: true, motivoPrincipal: "", agregadoEnRecepcion: true }).ok,
    true,
    "una línea agregada no necesita motivo genérico"
  );
});

test("FANTA: 0 → 2 BULTO x6 agregada, SIN motivo, valida y confirma", () => {
  const p = plan({
    cantidad: 0,
    recibido: 2,
    unidad: "BULTO",
  ...
  assert.equal(p.ok, true, `la línea que el propio sistema creó no valida: ${p.error}`);
  assert.notEqual(p.error, ERRORES_RECEPCION.FALTA_MOTIVO);

  // Y sigue siendo lo que era: nada de la aritmética cambió.
  assert.equal(p.agregadoEnRecepcion, true);
  assert.equal(p.recibidaUnidades, 12, "2 bultos de 6 son 12 unidades");
  assert.equal(p.enviadaUnidades, 0);
  assert.equal(p.tocaTransito, false, "una línea agregada no tiene tránsito que liberar");
  assert.equal(p.excedenteUnidades, 12, "las 12 salen del stock del origen");
  assert.equal(p.devolucionUnidades, 0);
  assert.equal(p.ajusteOrigenUnidades, -12);
  assert.equal(
    accionAuditoriaDe(p),
    ACCIONES_RECEPCION.AGREGADO,
    "la auditoría tiene que seguir distinguiéndola de un excedente"
  );
});

test("EL CONTRASTE: la misma 0 → 2 SIN la marca de agregada SÍ pide motivo", () => {
  // Esto es lo que prueba que no se eliminó la validación, sino que se distinguió
  // la procedencia. Mismos números, misma unidad, mismo factor.
  const delRemito = plan({
    cantidad: 0,
    recibido: 2,
    unidad: "BULTO",
    factorPack: 6,
  ...
  assert.equal(delRemito.ok, false, "una línea del remito sin motivo no puede pasar");
  assert.equal(delRemito.error, ERRORES_RECEPCION.FALTA_MOTIVO);

  // Y con motivo, pasa.
  const conMotivo = plan({
    cantidad: 0,
    recibido: 2,
    unidad: "BULTO",
    factorPack: 6,
...
test("10 → 15 del remito sigue exigiendo Sobrante, y el flujo entero cierra", () => {
  // Producto A: línea original con excedente. Sin motivo NO pasa.
  const sinMotivo = plan({ cantidad: 10, recibido: 15, motivoPrincipal: null });
  assert.equal(sinMotivo.ok, false);
  assert.equal(sinMotivo.error, ERRORES_RECEPCION.FALTA_MOTIVO);

  // Con Sobrante, sí.
  const A = plan({ cantidad: 10, recibido: 15, motivoPrincipal: "Sobrante" });
  assert.equal(A.ok, true);
  assert.equal(A.enviada, 10, "el remito sigue diciendo 10");
  assert.equal(A.excedenteUnidades, 5);
  assert.equal(A.tocaTransito, true, "una línea del remito sí libera su tránsito");
  assert.equal(accionAuditoriaDe(A), ACCIONES_RECEPCION.EXCEDENTE);

  // Fanta: agregada, sin motivo. Las DOS juntas validan, que es el flujo del
  // pedido: 10/15 con Sobrante + Fanta 0/2 agregada, y Confirmar puede seguir.
  const fanta = plan({
    cantidad: 0, recibido: 2, unidad: "BULTO", factorPack: 6, agregado: true, motivoPrincipal: null,
  });
  assert.equal(fanta.ok, true);
  assert.ok(
    A.ok && fanta.ok,
    "con las dos líneas válidas, confirmar-recepcion no tiene por qué frenar"
  );
});

test("una línea agregada NO se exime de nada más que del motivo genérico", () => {
  // Sigue sin poder tener envío: eso la delataría como una línea del remito mal
  // marcada, y su tránsito quedaría sin limpiar.
  const conEnvio = plan({ cantidad: 3, recibido: 6, agregado: true, motivoPrincipal: null });
  assert.equal(conEnvio.ok, false);
  assert.equal(conEnvio.error, ERRORES_RECEPCION.AGREGADA_CON_ENVIO);

  // Sigue necesitando unidad válida.
  const sinUnidad = validarDetalleRecepcion({
    detalle: { cantidad: 0, recibido: 2, unidadEnviada: null, agregadoEnRecepcion: true },
    factorPack: 6,
  });
  assert.equal(sinUnidad.ok, false);
  assert.equal(sinUnidad.error, ERRORES_RECEPCION.UNIDAD_AUSENTE);

  // Y cantidad recibida válida.
  const cantidadMala = plan({
    cantidad: 0, recibido: -1, agregado: true, motivoPrincipal: null,
  });
  assert.equal(cantidadMala.ok, false);
  assert.equal(cantidadMala.error, ERRORES_RECEPCION.RECIBIDA_INVALIDA);
});

test("no se inventa un motivo: la línea agregada sigue sin ninguno", () => {
  const p = plan({
    cantidad: 0, recibido: 2, unidad: "BULTO", factorPack: 6, agregado: true, motivoPrincipal: null,
  });
  assert.equal(p.ok, true);
  // El plan NO devuelve un motivo fabricado que la ruta pudiera escribir.
  assert.equal(p.motivoPrincipal, undefined, "se estaría inventando un motivo para la base");

  // Y la ruta que crea la línea tampoco escribe uno.
  const linea = leerSinComentarios(LINEA);
  assert.ok(
    !/motivoPrincipal:\s*"Sobrante"/.test(linea),
    "la ruta escribe un motivo falso solo para satisfacer la validación"
  );
});
