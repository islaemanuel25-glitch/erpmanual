...
import { fechaAR, horaAR, fechaHoraAR } from "@/lib/fechas/formatearFechaHora";
...
// El remito impreso es papel que viaja con la mercadería: la hora tiene que ser
// la de Argentina y en 24 horas. Del helper único. Se conserva el `null` para
// vacío, que es lo que este ticket usa para no dibujar la fila.
function fmtFecha(d) {
  if (!d) return null;
  return fechaHoraAR(d, { vacio: String(d) });
}
...
export default function imprimirTicketTransferencia(item, me, ancho = 58) {
  ...
  const fechaPrincipal = item.fechaCreada
    ...
    : new Date();
  const fechaStr = fechaAR(fechaPrincipal);
  const horaStr = horaAR(fechaPrincipal, { segundos: true });
