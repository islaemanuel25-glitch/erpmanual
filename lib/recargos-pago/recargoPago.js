 ...
 */
export function avisoPagoCombinado({
  mediosUsados,
  recargo,
  hayOfertaSoloEfectivoEnCarrito,
  // ── CÓMO SE LLAMA EL QUE GANÓ, CUANDO SE SABE ───────────────────────────
  //
  // Con modalidades, `recargo.medio` es el tipo contable y dos condiciones
  // distintas pueden compartirlo: decir "Crédito +8 %" no distingue "Crédito 1
  // pago" de "Crédito cuotas". Cuando el motor devuelve el nombre del ganador se
  // usa ése.
  //
  // Es un texto de PRESENTACIÓN y no una segunda fuente: el porcentaje sigue
  // saliendo de `recargo`, que salió del motor. Sin nombre se sigue usando la
  // etiqueta del tipo contable, exactamente como antes.
  ...
  if (recargo?.pct > 0 && recargo?.medio) {
    const nombre = etiquetaGanador || MEDIO_RECARGO_LABEL[recargo.medio] || recargo.medio;
    partes.push(`Se aplicará la condición más alta: ${nombre} +${recargo.pct} %.`);
  }
