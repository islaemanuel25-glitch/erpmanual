  ...
  for (const item of items) {
    ...
    if (item.__servicio) {
      ...
      lineas.push({
        ...
        servicio: s,
        // Snapshot de oferta congelado en el route (siempre "sin oferta" para un
        // servicio; viaja igual para que las tres ramas devuelvan la misma forma).
        oferta: item.__oferta ?? null,
      });
      continue;
    }
    ...
    if (pb.es_combo) {
      ...
      lineas.push({
        ...
        componentes,
        // Un combo SÍ puede estar ofertado explícitamente; lo que nunca pasa es
        // que la oferta de un COMPONENTE le cambie el precio — la oferta se
        // resuelve contra el ProductoLocal de la línea, que acá es el del combo.
        oferta: item.__oferta ?? null,
      });
    } else {
      ...
      lineas.push({
        ...
        // Peso cargado POR IMPORTE: el total de la línea ya está fijado por el
        // cajero y validado server-side en el route. Se arrastra para que
        // planConsumoVenta no lo re-derive del peso redondeado.
