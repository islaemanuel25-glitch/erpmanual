...
import { getDepositoIdDeGrupo } from "../visibilidad.js";
...
async function buildComboDTO(db, { grupoId, localId, comboPL, base, esDeposito, tieneStockPropio }) {
  ...
  return {
    ...
    // El estado de un combo vive SOLO en ProductoLocal.activo (no en base.activo).
    activo: comboPL.activo === true,
    esCombo: true,
    componentes,
    costoTotal,
    ganancia,
...
export async function crearCombo({ prisma, grupoId, localId, data }) {
  ...
  // El cliente se PASA: `getDepositoIdDeGrupo` sin él va a buscar el global,
  // y este servicio lo recibe inyectado — los candados le pasan un doble y se
  // pusieron los 22 en rojo pidiendo DATABASE_URL.
  const depositoLocalId = await getDepositoIdDeGrupo(Number(grupoId), prisma);

  const created = await prisma.$transaction(async (tx) => {
    await bloquearCodigosDelGrupo(tx, Number(grupoId));
    const vUnic = await validarUnicidadCodigos({
      prisma: tx,
      grupoId: Number(grupoId),
      // Un combo es un ProductoBase con `creadoEnLocalId` del local que lo tiene,
      // y ese local es el que decide dónde se puede escanear su código.
      ambitoLocalId: Number(localId),
      depositoLocalId,
      baseIdExcluir: null,
...
//   · El estado del combo vive SOLO en ProductoLocal.activo. NUNCA se toca
//     ProductoBase.activo (aunque para un combo la base sea exclusiva, se respeta
//     la invariante global "no modificar la base por estado").
//   · Guardar con activo=true (combo activo) → validación ESTRICTA de composición:
//     no se puede dejar/​volver activo un combo con componentes inválidos. Esto
//     implementa "para reactivar, exigir composición estructuralmente válida".
//   · Guardar con activo=false (desactivar) → SIEMPRE permitido, aunque la
//     composición sea inválida. Se conserva la composición y el historial: solo se
//     actualiza identidad (nombre/códigos) + activo=false; NO se recalcula precio ni
//     se reemplazan componentes.
export async function editarCombo({ prisma, grupoId, localId, productoLocalId, data }) {
  ...
  const targetActivo =
    data?.activo === undefined ? comboPL.activo === true : Boolean(data.activo);
  ...
  if (targetActivo === false) {
    // ── Desactivar (o guardar ya inactivo): SIEMPRE permitido ─────────────────
    // Conserva composición, precio e historial. Solo identidad + activo=false.
    await prisma.$transaction(async (tx) => {
      ...
      await tx.productoBase.update({
        ...
        data: {
          ...
          // NO se toca ProductoBase.activo ni el precio.
        },
      });
      await tx.productoLocal.update({
        where: { id: comboPL.id },
        data: { activo: false }, // el estado vive SOLO acá
      });
      // Composición y VentaDetalleComponente: intactos.
    });
    return getCombo({ prisma, grupoId, localId, productoLocalId: comboPL.id });
  }

  // ── Combo activo: validación ESTRICTA (implica reactivar-si-válido) ─────────
  const formacion = parseFormacionPrecio(data);
  await prisma.$transaction(async (tx) => {
    await bloquearCodigosDelGrupo(tx, Number(grupoId));
    const vUnic = await validarUnicidadCodigos({
    ...
    // Lanza si algún componente está inactivo/inexistente/de otro local/es combo →
    // no se puede dejar/​volver activo un combo roto.
    const comps = await cargarYValidarComponentes(tx, { localId, componentesInput: data?.componentes });
    const conCosto = comps.map((c) => ({
      ...c,
      costoUnitario: costoUnitarioComponente({ base: c.base, productoLocal: c.productoLocal, esDeposito }),
    ...
    await tx.productoBase.update({
      ...
      data: {
        ...
        margen: formacion.margen,
        redondeo_100: formacion.redondeo100,
        es_combo: true, // defensivo
        creadoEnLocalId: Number(localId), // defensivo
        // NO se toca ProductoBase.activo.
      },
    });
    await tx.productoLocal.update({
      where: { id: comboPL.id },
      data: {
        ...
        precio_venta: precioVenta,
        margen: formacion.margen,
        activo: true, // el estado vive SOLO acá
      },
    });
...
// ── Evaluación estructural de un combo (reutilizada por listado, PATCH estado y
// disponibilidad). Devuelve si está bloqueado estructuralmente + un motivo legible.
export async function evaluarEstructuraCombo(db, { localId, comboProductoLocalId }) {
  const filas = await db.comboComponente.findMany({
  ...
  const pls = compIds.length
    ? await db.productoLocal.findMany({
        where: { id: { in: compIds } },
        include: {
          base: { select: { activo: true, nombre: true } },
          stock: { where: { localId: Number(localId) }, select: { cantidad: true } },
        },
      })
    : [];
  ...
  const componentes = filas.map((f) => {
    ...
    return {
      ...
      tieneStockLocal,
      stock,
      nombre: pl?.base?.nombre ?? String(f.componenteProductoLocalId),
    };
  });

  const disp = disponibilidadCombo({ componentes, allowNegativeStock: false });

  let motivo = null;
  if (disp.bloqueadoEstructural) {
    if (!componentes.length) {
      motivo = "el combo no tiene componentes";
    } else {
      const TXT = {
        INACTIVO: "está desactivado",
        SIN_STOCKLOCAL: "no tiene stock configurado",
        CANTIDAD_INVALIDA: "tiene una cantidad inválida",
      };
      const m = (disp.motivos || []).find((x) => TXT[x.motivo]);
      const comp = componentes.find((c) => c.componenteProductoLocalId === m?.componenteProductoLocalId);
      motivo = comp ? `el componente "${comp.nombre}" ${TXT[m.motivo]}` : "la composición es inválida";
    }
  }
...
// Activar / desactivar un combo (acción rápida). Cambia SOLO ProductoLocal.activo.
...
// No toca ProductoBase.activo, ni composición, ni stock, ni historial.
export async function cambiarEstadoCombo({ prisma, grupoId, localId, productoLocalId, activo }) {
  if (typeof activo !== "boolean") throw errCombo("El campo 'activo' es obligatorio (booleano).", 400);
  const { productoLocal: comboPL } = await resolverComboPL(prisma, { grupoId, localId, productoLocalId });

  if (activo === true) {
    const estructura = await evaluarEstructuraCombo(prisma, { localId, comboProductoLocalId: comboPL.id });
    if (estructura.bloqueadoEstructural) {
      throw errCombo(
        `No se puede reactivar el combo: ${estructura.motivo || "la composición es estructuralmente inválida"}.`,
        409
      );
    }
  }

  await prisma.productoLocal.update({ where: { id: comboPL.id }, data: { activo } });
  return getCombo({ prisma, grupoId, localId, productoLocalId: comboPL.id });
}
