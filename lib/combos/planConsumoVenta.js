...
import { subtotalLinea } from "../pos-ventas/lineaPorImporte.js";
...
export function planConsumoVenta({ lineas = [], esDeposito = false } = {}) {
  ...
  const lineasComerciales = lineas.map((l, idx) => {
    ...
    // El dinero de la línea sale de subtotalLinea(): si nació de un IMPORTE
    // (peso cargado por precio) devuelve ese importe exacto; si nació de un peso,
    // de una unidad o de un pack, calcula precio × cantidad como siempre.
    // El COSTO se deriva siempre de la cantidad real: lo que se pagó por la
    // mercadería depende del peso despachado, no de lo que se le cobró al cliente.
