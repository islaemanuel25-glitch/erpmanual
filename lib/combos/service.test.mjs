...
function fresh() {
  const db = { _seq: { base: 200, pl: 2000, cc: 9000, stock: 8000 }, locks: [], ...clone(seedData()) };
  ...
  const client = {
    ...
    // EL DEPÓSITO DEL GRUPO. El doble lo tiene porque el servicio ahora lo
    // pregunta: la unicidad del código de barras depende de si el combo es del
    // depósito o de un local, y sin este modelo `getDepositoIdDeGrupo` se caía
    // buscando el cliente global —los 22 candados en rojo pidiendo DATABASE_URL—.
    // El depósito del seed es el local 1.
    grupoDeposito: {
      findFirst: async ({ where: { grupoId }, select }) => {
        const depo = db.locales.find((x) => x.es_deposito);
        if (!depo || grupoId !== 1) return null;
        return pick({ grupoId: 1, localId: depo.id }, select);
      },
    },
    configuracionGrupo: {
    ...
    productoLocal: {
      ...
      // Lo consulta `validarUnicidadCodigos` para ver si el código ya está puesto
      // como código propio en la ubicación. En este seed nadie tiene uno, así que
      // devuelve null — pero tiene que EXISTIR: sin el método, cualquier combo
      // con código de barras explota en vez de validarse.
      findFirst: async ({ where }) => {
        const codigos = where?.codigo_barra_propio?.in || [];
        const row = db.pls.find(
          (p) =>
            p.codigo_barra_propio &&
            codigos.includes(p.codigo_barra_propio) &&
            (where.localId === undefined || p.localId === where.localId) &&
            (where.baseId?.not === undefined || p.baseId !== where.baseId.not)
        );
        return row ? { ...row, local: { nombre: "local" }, base: { id: row.baseId, nombre: "x" } } : null;
      },
    ...
    // EL BLOQUEO NO SE TRAGA EN SILENCIO: SE ANOTA.
    //
    // Un doble que devuelve `undefined` y sigue dejaría pasar una versión sin
    // bloqueo sin que nadie se entere — que es exactamente el "candado que no
    // puede ponerse rojo". Se registra la llamada y hay un candado que la mira.
    $executeRaw: async (strings, ...valores) => {
      db.locks.push({ sql: Array.isArray(strings) ? strings.join("?") : String(strings), valores });
      return 0;
    },
...
// ── EL BLOQUEO DEL CÓDIGO DE BARRAS ──────────────────────────────────────
//
// Crear un combo reclama un código de barras igual que crear un producto, así
// que pasa por la misma función de unicidad y por el mismo bloqueo. Sin él, dos
// personas armando combos con el mismo código en el mismo grupo pasan las dos
// la validación: el índice de la base solo cubre el principal contra otro del
// mismo creador, y ni el secundario ni el código propio los mira nadie.

test("L1) crear un combo toma el bloqueo del grupo, y lo toma PRIMERO", async () => {
  const db = fresh();
  await crearCombo({
    prisma: db, grupoId: 1, localId: 2,
    data: { nombre: "Combo con código", codigo_barra: "CB-COMBO", precio_venta: 1000,
      componentes: comps([1001, 1]) },
  });
  const locks = db.__db.locks;
  assert.equal(locks.length, 1, "no se tomó el bloqueo al crear el combo");
  assert.match(locks[0].sql, /pg_advisory_xact_lock/, locks[0].sql);
  assert.deepEqual(locks[0].valores.map(Number).slice(1), [1], "el bloqueo no es por grupo");
});

test("L2) editar un combo también lo toma", async () => {
  const db = fresh();
  const combo = await crearCombo({
    prisma: db, grupoId: 1, localId: 2,
    data: { nombre: "Combo", precio_venta: 1000, componentes: comps([1001, 1]) },
  });
  db.__db.locks.length = 0;
  await editarCombo({
    prisma: db, grupoId: 1, localId: 2, productoLocalId: combo.productoLocalId,
    data: { nombre: "Combo", precio_venta: 1200, activo: true,
      componentes: comps([1001, 1]) },
  });
  assert.ok(db.__db.locks.length >= 1, "editar un combo no toma el bloqueo");
});
