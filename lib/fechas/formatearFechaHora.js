...
/**
 * "lunes 21 de septiembre" — la fecha como se dice en voz alta.
 *
 * Existe porque hay pantallas donde la fecha NO es un dato de tabla sino una
 * frase: "Termina el lunes 21 de septiembre". Ahí "21/09/2026" obliga a
 * traducir mentalmente y además no dice qué día de la semana cae, que es lo
 * único que la persona necesita para decidir si le sirve.
 *
 * SIN AÑO a propósito. Estas frases hablan siempre de los próximos días, y el
 * año en una oferta que dura una semana es ruido. Si algún día hace falta una
 * variante con año, va acá al lado y no como una opción: son dos frases
 * distintas y una opción booleana las dejaría a las dos a medio explicar.
 *
 * Va en este archivo y no en la pantalla por lo mismo que todo lo de acá: el
 * nombre del día se desplaza con la zona. Una oferta que termina a las 22:00 en
 * Argentina cae al día siguiente en UTC, así que sin `timeZone` la frase
 * diría el día equivocado justo en los vencimientos de la noche.
 */
export function fechaLargaAR(valor, { vacio = "—" } = {}) {
  const d = aFecha(valor);
  if (!d) return vacio;
  ...
  return `${g("weekday")} ${g("day")} de ${g("month")}`;
}

/**
 * "lunes 21" — la versión corta, para cuando el mes ya se dijo o no hace falta.
 */
export function diaYNumeroAR(valor, { vacio = "—" } = {}) {
  const d = aFecha(valor);
  if (!d) return vacio;
  const partes = new Intl.DateTimeFormat("es-AR", {
    ...BASE,
