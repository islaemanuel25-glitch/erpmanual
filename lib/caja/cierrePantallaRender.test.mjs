...
test("1. la pantalla previa NO corta: pide una confirmación explícita", () => {
  ...
  // El botón principal está, pero es el que PIDE confirmar.
  assert.match(html, /Iniciar cierre y liberar POS/);
  // Y el confirmatorio todavía NO aparece.
  assert.doesNotMatch(html, /Sí, cortar el turno/);
});

test("2. al pedir confirmación aparece el segundo paso, con la consecuencia escrita", () => {
  const html = render(PanelAntesDelCorte, {
    turno: { id: 188, apertura: "2026-08-04T12:00:00.000Z" },
    esperado: 127700,
    confirmando: true,
  });
  assert.match(html, /Sí, cortar el turno/);
  assert.match(html, /no admite más ventas ni movimientos/);
  assert.match(html, /turno #188/i);
});
...
test("18. el borrador se guarda y se recupera POR TOKEN", () => {
  ...
  guardarBorradorCierre(s, armarBorradorCierre({ token, desgloseContado: CONTADO, desgloseCambio: CAMBIO, observacion: "media jornada" }));
  const b = leerBorradorCierre(s, token);
  assert.equal(b.desgloseContado["20000"], 7);
  assert.equal(b.desgloseCambio["10000"], 1);
...
test("19. el borrador NO guarda ninguna cifra del servidor", () => {
  const b = armarBorradorCierre({ token: "t", desgloseContado: CONTADO, desgloseCambio: CAMBIO });
  for (const campo of ["efectivoEsperado", "efectivoEsperadoCorte", "diferencia", "totalRetiro", "retiroFinal"]) {
