...
test("1. la pantalla previa NO corta: pide una confirmación explícita", () => {
  ...
  // El botón principal está, pero es el que PIDE confirmar. El texto nombra
  // las tres cosas que van a pasar, en el orden en que pasan.
  assert.match(html, /Separar cambio, iniciar cierre y liberar POS/);
  // Y el confirmatorio todavía NO aparece.
  assert.doesNotMatch(html, /Sí, separar cambio y cortar/);
});
...
test("2. al pedir confirmación aparece el segundo paso, con la consecuencia escrita", () => {
  const html = render(PanelAntesDelCorte, {
    ...
    esperado: 127700,
    totalCambio: 30000,
    retiroEstimado: 97700,
    confirmando: true,
  });
  assert.match(html, /Sí, separar cambio y cortar/);
  assert.match(html, /no admite más ventas ni movimientos/);
  assert.match(html, /turno #188/i);
  // La consecuencia que antes no existía: el cambio queda publicado y no se
  // puede volver a elegir.
  assert.match(html, /no vas a poder cambiarlo/);
