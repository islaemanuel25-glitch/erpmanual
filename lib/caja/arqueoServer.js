  ...
  const [ventas, movimientos] = await Promise.all([
    tx.venta.findMany({
      ...
      select: {
        ...
        // No entra en el efectivo esperado —esa fórmula no mira la comisión— y
        // se trae para que quien muestre `comisionDigital` y `netoDigital` pueda
        // decir que están incompletos.
