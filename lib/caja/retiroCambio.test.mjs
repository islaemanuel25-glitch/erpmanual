...
test("10c. el borrador NO guarda estado del servidor", () => {
  const b = armarBorrador({ turnoId: 42, usuarioId: 1, desgloseContado: { 20000: 1 } });
...
test("11b. un borrador con DESGLOSE se recupera, aunque venga de otra versión", () => {
  const viejo = {
    version: 2,
    turnoId: 42,
    usuarioId: 1,
    ...
    observacion: "hola",
    conteoIniciadoEn: "2026-08-04T11:30:00.000Z",
    guardadoEn: "2026-08-04T12:00:00.000Z",
  };
  const migrado = migrarBorradorViejo(viejo, Date.parse("2026-08-04T13:00:00Z"));
  assert.equal(migrado.version, VERSION_BORRADOR);
  assert.deepEqual(migrado.desgloseContado, { 20000: 7, 10000: 1, [CLAVE_MONEDAS]: 450 });
  assert.deepEqual(migrado.desgloseCambio, { 20000: 1 }, "el cambio de la v2 sí se conserva");
  assert.equal(migrado.observacion, "hola");
  assert.equal(migrado.conteoIniciadoEn, "2026-08-04T11:30:00.000Z");
  assert.equal("montoContado" in migrado, false);
  assert.equal("modoConteo" in migrado, false);
  assert.equal(migrado.avisoCompat, undefined, "nada perdido, nada que avisar");
});

test("11d. un borrador con SOLO el total tipeado no se puede recuperar", () => {
  // Sin billetes no hay forma de reconstruir el conteo, y fabricar un desglose
  // sería inventar el tope del cambio y, con él, el importe del retiro.
  const soloTotal = {
    version: 2,
    turnoId: 42,
    usuarioId: 1,
  ...
  assert.equal(migrarBorradorViejo(soloTotal, Date.now()), null, "no se migra");

  // Y al leerlo, se descarta avisando con el texto exacto.
  const clave = claveBorrador({ turnoId: 42, usuarioId: 1 });
  const s = storageFalso({ [clave]: JSON.stringify(soloTotal) });
  const leido = leerBorrador(s, { turnoId: 42, usuarioId: 1, ahora: Date.parse("2026-08-04T13:00:00Z") });
  assert.equal(leido.descartado, true);
  assert.equal(leido.avisoCompat, AVISO_SIN_DESGLOSE);
  assert.equal(
    AVISO_SIN_DESGLOSE,
    "El borrador anterior no tenía detalle de billetes y no puede recuperarse."
  );
  assert.equal(s.getItem(clave), null, "además se borra");
});

test("11e. las versiones que se saben migrar están declaradas", () => {
  assert.deepEqual(VERSIONES_ANTERIORES, [1, 2]);
  assert.equal(VERSION_BORRADOR, 3);
  assert.ok(!VERSIONES_ANTERIORES.includes(VERSION_BORRADOR));
});
