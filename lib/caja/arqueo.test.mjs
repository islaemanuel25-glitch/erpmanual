...
test("C11c. las dos superficies llevan a la pantalla de retiro, no a un modal", () => {
  const pos = sinComentarios("app/modulos/pos-ventas/page.jsx");
  const rutas = pos.match(/\/modulos\/pos-ventas\/retiros\/nuevo/g) || [];
  assert.ok(rutas.length >= 2, `el botón y la franja tienen que navegar a la pantalla; hay ${rutas.length}`);
