...
test("C11. el POS solo dibuja con `visible`, que exige respuesta del backend", () => {
  ...
  // El retiro dejó de ser un modal y pasó a ser una pantalla propia, así que en
  // el POS quedan DOS superficies: el botón y la franja de aviso. El invariante
  // no cambió —ninguna aparece antes de que el backend resuelva, para que no
  // parpadeen—, solo desapareció la tercera.
  const usos = pos.match(/arqueo\.visible/g) || [];
  assert.ok(usos.length >= 2, `esperaba botón y franja gateados; hay ${usos.length}`);
  assert.ok(/turnoActual && arqueo\.visible && \(\s*<AvisoArqueo/.test(pos), "la franja no está gateada");
  assert.ok(/turnoActual && arqueo\.visible && \(\s*<button/.test(pos), "el botón no está gateado");
  assert.ok(!/<ModalRetiroDinero/.test(pos), "el retiro volvió a montarse como modal");
});

test("C11c. las dos superficies llevan a la pantalla de retiro, no a un modal", () => {
  const pos = sinComentarios("app/modulos/pos-ventas/page.jsx");
  // Las dos llaman al mismo abridor, que decide pestaña nueva o navegación
  // según el ancho real. Duplicar la decisión en cada superficie es la forma
  // segura de que una de las dos se quede vieja.
  const usos = pos.match(/abrirRetiro\(turnoActual\.id\)/g) || [];
  assert.ok(usos.length >= 2, `esperaba el botón y la franja; hay ${usos.length}`);
  assert.ok(/const abrirRetiro = /.test(pos), "falta el abridor único");
  assert.ok(/\/modulos\/pos-ventas\/retiros\/nuevo/.test(pos), "falta la ruta de la pantalla");
  assert.ok(!/mostrarRetiro/.test(pos), "quedó estado del modal viejo");
});

test("C11d. en escritorio el retiro abre una pestaña aparte; en móvil, no", () => {
  const pos = sinComentarios("app/modulos/pos-ventas/page.jsx");
  // El POS no puede quedar inaccesible mientras se cuenta el cajón.
  assert.ok(/window\.open\(/.test(pos), "en escritorio tiene que abrirse en otra pestaña");
  assert.ok(/nombreVentanaRetiro\(/.test(pos), "sin nombre de ventana se acumulan pestañas del mismo turno");
  assert.ok(/esEscritorio/.test(pos), "la decisión tiene que depender del ancho real");
  assert.ok(/router\.push\(ruta\)/.test(pos), "en móvil se navega en la misma pestaña");

  // Y la decisión NO se toma por user-agent: miente con tablets y táctiles.
  const hook = sinComentarios("hooks/useEsEscritorio.js");
  assert.ok(/matchMedia/.test(hook), "el hook debería usar matchMedia");
  assert.ok(!/userAgent/.test(hook), "no se decide por user-agent");
  assert.ok(/min-width: 1024px/.test(hook), "debería usar el mismo breakpoint que el CSS");
});
