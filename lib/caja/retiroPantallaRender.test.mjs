...
import { MODO_TOTAL, MODO_BILLETES, CLAVE_MONEDAS } from "@/lib/caja/conteoBilletes";
...
const TODO = () =>
  [
    ...
    render(PanelConteo, propsConteo({ modo: MODO_TOTAL, monto: "150450" })),
...
test("20b. renderizar con props vacías tampoco rompe", () => {
  ...
  assert.doesNotThrow(() => {
    render(PanelConteo, { onModo() {}, onMonto() {}, onDesglose() {} });
...
test("X1. la ayuda del conteo habla del CAMBIO, no del fondo", () => {
  const html = render(PanelConteo, {
    modo: MODO_TOTAL,
    monto: "",
    desglose: {},
    onModo() {},
  ...
  assert.match(html, /Incluí el cambio que hay en el cajón\./);
