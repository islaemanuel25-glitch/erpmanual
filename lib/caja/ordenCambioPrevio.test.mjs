...
test("33. la pantalla previa es SIMPLE: sin grilla y sin cifras del cambio", () => {
  // Volvió a ser una tarjeta con cuatro datos, el esperado, el aviso y un botón.
  // La grilla convertía en una página de dos columnas la pantalla que el cajero
  // ve todos los días.
  ...
  // Nada de la grilla ni de sus cifras.
  ...
  // Y el botón que SÍ corta vive en el modal, no acá.
  assert.doesNotMatch(html, /Separar cambio, iniciar cierre y liberar POS/);
});

test("33b. el cierre y el retiro comparten la misma pantalla previa", () => {
  // Sólo cambian los textos. Si una pidiera la grilla de entrada y la otra no,
  // el cajero tendría que acordarse de cuál es cuál.
  const cierre = render(PanelAntesDelCorte, { turno: { id: 1 }, esperado: 1000 });
  const retiro = render(PanelAntesDelCorte, {
    turno: { id: 1 }, esperado: 1000,
    titulo: "Antes de retirar",
    aviso: AVISO_ANTES_DEL_RETIRO,
    textoBoton: ACCION_ABRIR_CAMBIO_RETIRO,
  });
  assert.match(cierre, /Iniciar cierre y liberar POS/);
  assert.match(retiro, /Iniciar retiro/);
  assert.match(retiro, /La caja sigue vendiendo con el cambio que separes/);
  // El aviso del retiro NO puede decir que el POS se libera: no se libera.
  assert.doesNotMatch(retiro, /El POS quedará disponible/);
  // Los dos muestran los mismos datos y ninguno la grilla.
  for (const html of [cierre, retiro]) {
    assert.match(html, /Efectivo esperado ahora/);
    assert.doesNotMatch(html, /id="cambio-previo-/);
  }
});

test("34. el modal advierte si el cambio supera el esperado", () => {
  const html = render(ModalCambioPrevio, {
    abierto: true, esperado: 30000, desglose: { 20000: 2 },
    totalCambio: 40000, retiroEstimado: -10000,
  });
  assert.match(html, /supera el efectivo esperado/);
  assert.match(html, /sobrante/);
});

test("35. el modal trae la grilla, las tres cifras y la resta escrita", () => {
  const html = render(ModalCambioPrevio, {
    abierto: true, esperado: ESPERADO, desglose: { 10000: 3 },
    totalCambio: CAMBIO, retiroEstimado: RETIRO_ESPERADO,
  });
  assert.match(html, /Cambio que queda en la caja/);
  assert.match(html, /Separá los billetes y monedas/);
  // La grilla es de CONTEO, no de reparto: no hay pila previa contra la cual
  // topear, así que no lleva columna "Contadas".
  assert.match(html, /id="cambio-previo-10000"/);
  assert.doesNotMatch(html, /Contadas/);
  // Las siete denominaciones más monedas.
  for (const d of [20000, 10000, 2000, 1000, 500, 200, 100]) {
    assert.match(html, new RegExp(`id="cambio-previo-${d}"`));
  }
  assert.match(html, /id="cambio-previo-monedas"/);
  // Las tres cifras y la fórmula visible.
  assert.match(html, /Efectivo esperado/);
  assert.match(html, /Retiro estimado/);
  assert.match(html, /Retiro estimado = \$ 150\.000,00 − \$ 30\.000,00/);
  // Y las dos acciones.
  assert.match(html, /Cancelar/);
  assert.match(html, /Separar cambio, iniciar cierre y liberar POS/);
});

test("35b. el modal cerrado no renderiza nada", () => {
  assert.equal(render(ModalCambioPrevio, { abierto: false, esperado: 1000 }), "");
});

test("35c. el modal es un diálogo con scroll interno y acciones fijas", () => {
  const html = render(ModalCambioPrevio, { abierto: true, esperado: 1000, totalCambio: 0 });
  assert.match(html, /role="dialog"/);
  assert.match(html, /aria-modal="true"/);
  // El cuerpo scrollea solo; el pie con los botones no se va nunca.
  assert.match(html, /overflow-y-auto/);
  assert.match(html, /max-h-\[92vh\]/);
  // En escritorio queda angosto, no a pantalla completa.
  assert.match(html, /sm:max-w-lg/);

  // FONDO OPACO. `sunmi-pos-panel` es translúcido al 50 % y dejaba leer el
  // texto de la página a través del modal — visto en la captura de móvil 360.
  assert.match(html, /sunmi-surface/);
  assert.doesNotMatch(html, /sunmi-pos-panel/);
});

test("35d. el modal avisa cuando el esperado se movió mientras estaba abierto", () => {
  const html = render(ModalCambioPrevio, {
    abierto: true, esperado: 170000, totalCambio: 30000, retiroEstimado: 140000,
    revalidado: true,
  });
  assert.match(html, /El efectivo esperado cambió por una operación nueva/);
  assert.match(html, /confirmá otra vez/);
});
