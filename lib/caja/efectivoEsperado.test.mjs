...
test("E2b. ModalCierreTurno consume el esperado del backend, no lo rearma", () => {
  const src = sinComentarios("components/pos-ventas/ModalCierreTurno.jsx");
  assert.ok(
    /resumen\.efectivoEsperado/.test(src),
    "el modal no usa el esperado que devuelve el backend"
  );
  // La fórmula manual solo puede quedar como fallback de una pestaña vieja,
  // nunca como camino principal.
  const usaFallback = /resumen\.efectivoEsperado != null\s*\?/.test(src);
  assert.ok(usaFallback, "la fórmula manual no está detrás de un fallback explícito");
});
...
// ── EL EFECTIVO ESPERADO NO DEPENDE DE LA COMISIÓN ──────────────────────────
//
// Se midió al diseñar la tanda de "comisión sin configurar": la fórmula es
// `inicial + efectivo + ingresos − retiros`, y la comisión no participa. Queda
// escrito como candado porque de esa medición cuelga una decisión —que una
// comisión desconocida NO descuadre un arqueo ni le falte plata a nadie— y una
// medición que no se fija se vuelve a discutir en el peor momento.

test("una venta con comisión pendiente NO mueve el efectivo esperado", () => {
  // Efectivo $5.000 + débito $5.000 con el débito sin comisión configurada: los
  // dos tenders llegan con `comision: 0`, uno porque no cobra y el otro porque
  // todavía no se sabe. En la caja entran los $5.000 de efectivo y nada más.
  const pendiente = calcularEfectivoEsperado({
    montoInicial: 1000,
    ventas: [
      venta([
        { medio: "EFECTIVO", monto: 5000 },
        { medio: "DEBITO", monto: 5000, comision: 0, neto: 5000 },
      ]),
    ],
  });

  // La MISMA venta con el débito al 7 %: cambia el neto digital, no la caja.
  const conocida = calcularEfectivoEsperado({
    montoInicial: 1000,
    ventas: [
      venta([
        { medio: "EFECTIVO", monto: 5000 },
        { medio: "DEBITO", monto: 5000, comision: 350, neto: 4650 },
      ]),
    ],
  });

  assert.equal(pendiente.efectivoEsperado, 6000);
  assert.equal(conocida.efectivoEsperado, 6000, "la comisión no entra en el efectivo esperado");
  assert.equal(pendiente.ventasEfectivo, conocida.ventasEfectivo);

  // Y lo que sí cambia son los dos renglones informativos, que por eso hay que
  // rotular como incompletos cuando la venta está pendiente.
  assert.equal(pendiente.comisionDigital, 0);
  assert.equal(conocida.comisionDigital, 350);
});
