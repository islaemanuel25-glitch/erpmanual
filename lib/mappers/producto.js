...
export function mergeBaseLocalToUi(base = {}, local = null) {
  ...
  return {
    ...
    // ── LOS PRECIOS NO USAN `pick`, USAN EL CRITERIO COMPARTIDO ────────────
    //
    // `pick` solo mira NULO, así que un cero guardado en la fila del local
    // ganaba sobre el precio del depósito y se mostraba $0,00. El módulo de
    // stock ya caía a la base en ese caso, así que las dos pantallas mostraban
    // números distintos para el mismo producto. Ahora las dos preguntan en el
    // mismo lugar: un cero no es un precio, es que no le pusieron uno.
    //
    // EL MARGEN SIGUE CON `pick`, y no es un olvido: un margen de CERO sí es un
    // valor cargado —"vendé esto al costo"— y no una ausencia. Son 1.691 filas
    // sin porcentaje asignado y ésa es una decisión distinta, que no se toca acá.
    precioCosto: toNum(precioDeLaUbicacion(base.precio_costo, local?.precio_costo)),
    precioVenta: toNum(precioDeLaUbicacion(base.precio_venta, local?.precio_venta)),
