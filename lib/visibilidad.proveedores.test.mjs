...
/**
 * ── SE ENUMERA CON GIT, Y ADEMÁS INCLUYENDO LO SIN COMMITEAR ─────────────
 *
 * `git ls-files` a secas lista solo lo trackeado, así que una ruta nueva y
 * todavía sin commitear quedaría afuera y este candado la daría por buena en
 * verde. Se pondría rojo en la tanda siguiente, cuando ya está empujada.
 *
 * Es el mismo agujero que se encontró el 2026-09-14 en los censos de
 * `lib/layout/accionDePagina.test.mjs`, y acá lo que se enumera son rutas de
 * API: lo que se deja sin mirar es una ruta que puede estar devolviendo datos de
 * otro proveedor.
 */
function archivosDeApi() {
  const salida = execFileSync(
    "git",
    ["ls-files", "--cached", "--others", "--exclude-standard", "app/api"],
