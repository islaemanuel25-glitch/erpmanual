...
import { comisionEsExacta, TEXTO_PENDIENTE } from "./comisionPendiente";
...
${
      // Con la comisión pendiente NO se imprime "$0": ni la comisión ni el neto
      // son importes todavía. Se dice qué falta, que además es accionable para
      // quien lee el ticket en el mostrador.
      !comisionEsExacta(venta)
        ? `<div class="t-lr"><span>Comisión bancaria:</span><span>${TEXTO_PENDIENTE}</span></div>`
        : comisionV > 0
          ? `<div class="t-lr"><span>Comisión bancaria:</span><span>-$${formatPrecio(comisionV)}</span></div><div class="t-lr"><span>Neto recibido:</span><span>$${formatPrecio(netoV)}</span></div>`
          : ""
    }`;
  } else {
