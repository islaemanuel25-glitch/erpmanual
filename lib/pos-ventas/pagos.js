...
export function aplicarComisiones(pagos, comisionPctPorMedio = {}) {
  return pagos.map((p) => {
    ...
    // UN MEDIO QUE COBRA COMISIÓN Y NO ESTÁ EN EL MAPA ES "SIN CONFIGURAR".
    //
    // Antes esto era `?? 0`: un medio ausente se cobraba como 0 % y quedaba
    // registrado como si alguien hubiera decidido no cobrar comisión. Son dos
    // cosas distintas y solo una es un dato.
    //
    // El `comisionPct` en `null` es la señal, y sobrevive a la escritura porque
    // esa columna YA es nulable. Los importes que van abajo —`comision` y
    // `neto`— son placeholders estructurales: las columnas no son nulables y no
    // se cambian en esta tanda. Lo que impide que ese cero se lea como una
    // medición es `Venta.comisionPendiente`, que se deriva más abajo.
    const crudo = cobra ? comisionPctPorMedio?.[p.medio] : null;
    const sinConfigurar = cobra && crudo == null;
    const pct = cobra && !sinConfigurar ? Number(crudo) : 0;

    const comision = cobra && !sinConfigurar && pct > 0 ? round2((p.monto * pct) / 100) : 0;
    return {
      medio: p.medio,
