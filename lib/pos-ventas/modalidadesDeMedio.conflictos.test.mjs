...
test("B bis · los dos índices parciales están en la migración, con su predicado", () => {
  ...
  // ── SE QUITA COMO ÍNDICE, QUE ES LO QUE ES ─────────────────────────────
  //
  // La baseline lo crea con `CREATE UNIQUE INDEX`. La primera versión de la
  // migración usaba solo `DROP CONSTRAINT IF EXISTS`, que sobre un índice no
  // borra nada — y el `IF EXISTS` lo tapó: aplicaba sin error dejando la
  // restricción vieja viva, así que el segundo tender habría seguido siendo
  // rechazado y la tanda entera no habría funcionado.
  //
  // Lo encontró el chequeo de deriva, no un candado. Este existe para que la
  // próxima vez lo encuentre un candado.
  assert.match(
    sqlSinComentarios,
    /DROP INDEX IF EXISTS "VentaPago_ventaId_medio_key"/,
    "falta el DROP INDEX: la baseline lo crea como índice único, y un DROP CONSTRAINT no lo borra"
  );
  assert.match(
    readFileSync(join(RAIZ, "prisma", "migrations", "000000000000_squashed_migrations", "migration.sql"), "utf8"),
    /CREATE UNIQUE INDEX "VentaPago_ventaId_medio_key"/,
    "la baseline cambió cómo crea la restricción vieja: revisar si el DROP sigue siendo el correcto"
  );
});
