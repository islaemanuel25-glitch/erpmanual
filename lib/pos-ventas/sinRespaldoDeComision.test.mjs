...
function lineasDeCodigo(patron, rutas) {
  // `--untracked`: `git grep` a secas mira solo lo trackeado, así que un archivo
  // nuevo sin commitear con el patrón prohibido pasaría en verde. Agujero
  // encontrado el 2026-09-14.
  const salida = execSync(
    `git grep --untracked -nE ${JSON.stringify(patron)} -- ${rutas} || true`,
    {
      encoding: "utf8",
    }
  );
