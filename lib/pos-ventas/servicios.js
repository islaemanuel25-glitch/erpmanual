 ...
 *
 * ── LA IDENTIDAD DE UNA FILA DEJÓ DE SER SU TIPO CONTABLE ─────────────────
 *
 * Una fila puede traer `clave` —la identidad de la opción elegida: un medio, o
 * una modalidad de ese medio— y `tipoContable`. Cuando vienen, mandan ellas:
 * "Crédito 1 pago" y "Crédito cuotas" son las dos CREDITO y NO son la misma
 * fila, así que contarlas como duplicadas impediría un pago mixto que la base
 * acepta.
 *
 * Cuando no vienen —el camino de siempre— la identidad sigue siendo el `medio`
 * en minúscula y el efectivo se reconoce por esa cadena, exactamente como antes.
 * No hay dos evaluadores: hay uno, y lo que cambia es de dónde sale la identidad.
 *
 * Lo que NO se decide por el nombre es cuál fila cubre los servicios: se mira el
 * TIPO CONTABLE. Una modalidad puede llamarse como quiera.
 *
 * @param {{ filas: Array<{medio:string, monto:any, clave?:string, tipoContable?:string,
 *   medioCobroLocalId?:number|null, modalidadId?:number|null}>,
 *   total:number, minEfectivoServicios?:number }} args
 * @returns {{
 ...
 *   estado:"falta"|"exacto"|"excedente", pagos:Array<object>
 * }}
 */
export function claveDeFilaDePago(fila) {
  if (fila?.clave) return String(fila.clave);
  return String(fila?.medio || "").toLowerCase();
}

/** ¿Esta fila aporta EFECTIVO? Por tipo contable si se sabe, por el medio legacy si no. */
export function filaEsEfectivo(fila) {
  if (fila?.tipoContable) return String(fila.tipoContable).toUpperCase() === "EFECTIVO";
  return String(fila?.medio || "").toLowerCase() === "efectivo";
}

/**
 * El tender que esta fila manda al backend.
 *
 * Con identidad configurable manda IDENTIDAD y monto, y nada más: el tipo, el
 * recargo y la comisión los relee el servidor. Sin identidad —un default sin
 * fila, el modo offline— manda lo de siempre.
 */
export function tenderDeFila(fila, monto) {
  if (fila?.medioCobroLocalId != null) {
    return {
      medioCobroLocalId: fila.medioCobroLocalId,
      modalidadId: fila.modalidadId ?? null,
      monto,
    };
  }
  return { medio: String(fila?.medio || "").toLowerCase(), monto };
}
...
export function evaluarDivisionPago({ filas = [], total = 0, minEfectivoServicios = 0 }) {
  ...
  for (const f of filas) {
    ...
    pagadoCent += c;
    if (filaEsEfectivo(f)) efectivoCent += c;
    claves.push(claveDeFilaDePago(f));
  }
  const restanteCent = totalCent - pagadoCent;
  const excedenteCent = pagadoCent - totalCent;
  const hayDuplicados = new Set(claves).size !== claves.length;
  const montosValidos =
    filas.length > 0 &&
    filas.every((f) => {
  ...
  return {
    ...
    pagos: filas.map((f) => tenderDeFila(f, montoNum(f?.monto))),
  };
}
 ...
 *
 * ── LA IDENTIDAD ES OPCIONAL, Y CUANDO ESTÁ MANDA ─────────────────────────
 *
 * `identidad` es `{medioCobroLocalId, modalidadId}`: lo ÚNICO que el servidor
 * lee de un tender. Cuando viene, el cobro viaja como `pagos` con esa identidad
 * en vez de apoyarse solo en `formaPago`, y así la venta congela con qué botón y
 * con qué modalidad se cobró. Sin ella —un medio por defecto que todavía no
 * tiene fila, el modo offline— sale exactamente lo de siempre: no se inventan
 * ids.
 *
 * `identidadEfectivo` es la del medio de efectivo del local, y hace falta solo
 * para el reparto de servicios. Si el local no tiene un efectivo direccionable
 * —porque todavía es un default, o porque su botón tiene modalidades y hay que
 * elegir una— el tender de efectivo sale legacy. En ese último caso el servidor
 * va a pedir que se elija la modalidad, que es lo correcto: el camino para eso
 * es "Dividir pago", donde la modalidad se elige por fila.
 *
 * @param {{ medio:string, total:number, minEfectivoServicios?:number,
 *   identidad?:object|null, identidadEfectivo?:object|null }} args
 * @returns {{ formaPago:string, total:number, pagos?:Array<object> }}
 */
export function componerCobroSimple({
  medio,
  total,
  minEfectivoServicios = 0,
  identidad = null,
  identidadEfectivo = null,
}) {
  ...
  // Un tender: con identidad si la hay, con el medio legacy si no.
  const tender = (ident, medioLegacy, monto) =>
    ident ? { ...ident, monto } : { medio: medioLegacy, monto };

  if (m === "fiado") {
    // FIADO no es un medio de cobro configurable: no tiene fila, no tiene
    // modalidades y sigue siendo tender único por regla del sistema.
    return { formaPago: "fiado", total: tot };
  }
  if (m === "efectivo") {
    // Un solo tender efectivo por el total (cubre los servicios si los hay).
    return identidad
      ? { formaPago: "efectivo", total: tot, pagos: [tender(identidad, "efectivo", tot)] }
      : { formaPago: "efectivo", total: tot };
  }
  // Medio digital:
  if (hayServicios) {
    const resto = round2(tot - minEf);
    // Si por algún motivo no queda resto, es una venta 100% servicios → efectivo.
    if (aCentavos(resto) <= 0) {
      return identidadEfectivo
        ? { formaPago: "efectivo", total: tot, pagos: [tender(identidadEfectivo, "efectivo", tot)] }
        : { formaPago: "efectivo", total: tot };
    }
    return {
      formaPago: "mixto",
      total: tot,
      pagos: [
        tender(identidadEfectivo, "efectivo", minEf),
        tender(identidad, m, resto),
      ],
    };
  }
  return identidad
    ? { formaPago: m, total: tot, pagos: [tender(identidad, m, tot)] }
    : { formaPago: m, total: tot };
}
