...
import { comisionEsExacta, TEXTO_PENDIENTE } from "./comisionPendiente.js";
...
export default function generarTicketPDF(venta, opts = {}) {
  ...
  // ── CONDICIÓN COMERCIAL ──────────────────────────────────────────────────
  //
  // Las filas de arriba ya se imprimen al precio COBRADO, así que el ticket
  // cierra sin estos dos renglones. Están para poder LEER por qué el total no es
  // la suma de los precios de la góndola: cuánto se ahorró por ofertas y cuánto
  // se sumó por el medio de pago. El recargo nombra el medio que impuso la
  // condición —en un pago combinado no es necesariamente con el que se pagó más—
  // para que el número se pueda discutir con el papel en la mano.
  const ahorroPromocional = Number(venta.descuentoPromocional) || 0;
  if (ahorroPromocional > 0) {
    resumen.push({ clase: "linea", label: "Ahorro por ofertas", valor: `-$${formatPrecio(ahorroPromocional)}` });
  }
  const recargoPago = Number(venta.recargoPagoImporte) || 0;
  if (recargoPago > 0) {
    const medio = MEDIO_LABEL[String(venta.recargoPagoMedio || "").toUpperCase()] || venta.recargoPagoMedio || "";
    const pct = Number(venta.recargoPagoPct) || 0;
    const detalle = [medio, pct > 0 ? `${pct} %` : ""].filter(Boolean).join(" ");
    resumen.push({
      clase: "linea",
      label: detalle ? `Recargo ${detalle}` : "Recargo",
      valor: `+$${formatPrecio(recargoPago)}`,
    });
  }
  ...
  if (!comisionEsExacta(venta)) {
    // Ni "$0" ni un neto igual al total: los dos se leerían como que el
    // procesador no cobró nada. Se imprime lo que realmente pasa.
    resumen.push({ clase: "linea", label: "Comisión bancaria", valor: TEXTO_PENDIENTE });
  } else if (comision > 0) {
