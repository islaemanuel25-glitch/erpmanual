...
export async function cargarVentaOriginal(db, ventaId) {
  return db.venta.findUnique({
    ...
    include: {
      ...
      // Remito generado por esta venta (venta interna). Alimenta el bloqueo
      // temporal de bloqueoCorreccion: mientras la sincronización no exista, una
      // venta con transferencia vinculada no se puede corregir.
