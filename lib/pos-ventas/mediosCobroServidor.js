...
import { adjuntarModalidades } from "./modalidadesDeMedio.js";
...
export async function mediosDelLocal(db, { localId, grupoId }) {
  ...
  const [filas, recargos, configuracionGrupo] = await Promise.all([
    db.medioCobroLocal.findMany({
      ...
      // LAS MODALIDADES VIENEN EN LA MISMA CONSULTA, y eso es el punto.
      //
      // Un `findMany` por medio para traer sus modalidades sería N+1 contra la
      // base que atiende la caja: cuatro o cinco consultas más por cada apertura
      // del POS, por cada preview y por cada venta. Prisma resuelve el `include`
      // con un solo `IN`, así que el costo es el mismo que antes.
      include: { modalidades: { orderBy: [{ orden: "asc" }, { nombre: "asc" }] } },
    }),
  ...
  // `componerMedios` no sabe de modalidades a propósito: los dos módulos puros
  // se necesitan entre sí y un ciclo de imports en ESM es de los defectos que
  // aparecen mucho después, cuando alguien mueve una constante. Se componen por
  // separado y se pegan acá.
  return adjuntarModalidades(
    componerMedios({
      filas,
...
/**
 * RESOLVER EL MEDIO PADRE DE UNA MODALIDAD, materializando defaults si hace falta.
 *
 * Las rutas de modalidades cuelgan del medio, y ese medio se direcciona con la
 * MISMA clave de edición que usa la pantalla de Cobros: un id, o `defecto:TIPO`
 * mientras el local no configuró nada.
 *
 * Por eso reusa `resolverMedioParaEditar` en vez de buscar por id: si alguien
 * agrega una modalidad a un Mercado Pago que todavía es un default, hay que
 * materializar LOS CUATRO medios primero. Materializar solo Mercado Pago dejaría
 * al local con una sola fila y el POS se quedaría sin los otros tres botones —es
 * el mismo defecto que ya se resolvió una vez para la edición del medio, y por
 * eso no se resuelve de nuevo acá: se llama a la pieza que ya lo hace.
 */
export async function resolverMedioDeModalidad(db, { localId, clave }) {
  return resolverMedioParaEditar(db, { localId, clave });
}

/**
 * Una modalidad concreta, comprobando que cuelgue del medio pedido.
 *
 * Las dos condiciones van en el MISMO where y no en dos comprobaciones: una
 * modalidad de otro padre y una inexistente tienen que ser indistinguibles desde
 * afuera. Y el padre ya viene verificado contra el local por
 * `resolverMedioParaEditar`, así que el aislamiento entre locales queda cerrado
 * por composición y no por un segundo filtro que alguien pueda olvidar.
 */
export async function resolverModalidad(db, { medioId, modalidadId }) {
  const id = Number(modalidadId);
  if (!Number.isInteger(id) || id <= 0) return null;
  return db.medioCobroModalidadLocal.findFirst({
    where: { id, medioCobroLocalId: Number(medioId) },
  });
}
