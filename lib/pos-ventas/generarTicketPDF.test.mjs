...
function venta(n, over = {}) {
  ...
  return {
    ...
    // La comisión de una venta REAL siempre trae este booleano; el ticket lo usa
    // para no imprimir "$0" cuando el cero es estructural. Va en el fixture
    // porque `comisionEsExacta` falla cerrado a propósito: sin el campo, el
    // ticket diría "pendiente", que es lo correcto para un dato que no llegó.
...
// ── H. Venta con oferta y con recargo ────────────────────────────────────────
//
// El caso que el POS no podía imprimir bien: el carrito decía $1.000 y el
// backend cobró $900. Si el ticket se arma con el carrito, sale "9 × $1.000"
// arriba de un total de $8.100 — un papel que no cierra y que el cliente mira.
//
// La afirmación central es ARITMÉTICA y no de redacción: lo que se imprime en la
// columna de precio, multiplicado por la cantidad, tiene que dar el subtotal
// impreso; y el subtotal, con el ahorro y el recargo, tiene que dar el total.

test("oferta: la línea se imprime al precio COBRADO y el ticket cuadra", async () => {
  const conOferta = {
    localNombre: "Sucursal centro",
    numero: 900,
    fecha: new Date("2026-09-04T13:12:00.000Z"),
    vendedor: "Cajero",
    cliente: null,
    // Así lo arma el POS desde `breakdown.lineas`: `precio` YA es lo cobrado.
    items: [
      {
        nombre: "Nueve de Oro",
        cantidad: 9,
    ...
    recargoPagoPct: 0,
    recargoPagoMedio: null,
    comisionBancaria: 0,
    netoRecibido: 8100,
    formaPago: "efectivo",
    pagos: [{ medio: "EFECTIVO", monto: 8100 }],
  };
  const { traza } = await gen(conOferta, {});

  // El precio impreso es el promocional, NO el de la góndola.
  //
  // Se compara con `exactos` y no con `cuenta`: `cuenta` usa `includes`, y el
  // renglón del ahorro dice "-$900,00", que CONTIENE "$900,00". La primera
  // versión de esta afirmación contaba 2 donde hay una sola celda de precio y
  // fallaba por su propia imprecisión, no por el código. La comparación exacta
  // distingue la celda del importe negativo.
  assert.equal(exactos(traza, "$900,00").length, 1, "la línea tiene que decir el precio cobrado");
  assert.equal(cuenta(traza, "$1.000,00"), 0, "el precio normal no se imprime como si se hubiera cobrado");

  // 9 × 900 = 8.100, que es el subtotal Y el total: el papel cierra solo.
  assert.ok(exactos(traza, "$8.100,00").length >= 2, "subtotal y total tienen que dar el mismo 8.100");

  // Y se puede LEER por qué el total no es la suma de los precios de lista.
  assert.equal(cuenta(traza, "Ahorro por ofertas"), 1);
  assert.equal(exactos(traza, "-$900,00").length, 1);
});

test("recargo: el ticket nombra el medio que impuso la condición", async () => {
  const conRecargo = {
    localNombre: "Sucursal centro",
    numero: 901,
    fecha: new Date("2026-09-04T13:12:00.000Z"),
    vendedor: "Cajero",
    cliente: null,
    items: [{ nombre: "Nueve de Oro", cantidad: 9, precio: 1000, precioNormal: 1000, esServicio: false }],
    subtotal: 9000,
    descuento: 0,
    total: 9450,
    descuentoPromocional: 0,
    // La oferta era de solo efectivo y no aplicó; el débito sumó su 5 %.
    recargoPagoImporte: 450,
    recargoPagoPct: 5,
    recargoPagoMedio: "DEBITO",
    comisionBancaria: 0,
    netoRecibido: 9450,
    formaPago: "debito",
    pagos: [{ medio: "DEBITO", monto: 9450 }],
  };
  const { traza } = await gen(conRecargo, {});

  // "Recargo: $450" a secas se lee como un cargo arbitrario. Con el medio y el
  // porcentaje, el número se puede discutir con el papel en la mano.
  assert.equal(cuenta(traza, "Recargo Débito 5 %"), 1);
  assert.equal(cuenta(traza, "+$450,00"), 1);
  // 9.000 + 450 = 9.450.
  assert.equal(cuenta(traza, "$9.000,00"), 2, "subtotal y la línea de 9 × 1.000");
  assert.ok(cuenta(traza, "$9.450,00") >= 1, "el total con recargo");
});

test("sin oferta ni recargo el ticket no agrega renglones", async () => {
  // El papel de todos los días no puede crecer por una función que no aplica.
  const { traza } = await gen(venta(2));
  assert.equal(cuenta(traza, "Ahorro por ofertas"), 0);
  assert.equal(cuenta(traza, "Recargo "), 0);
});
