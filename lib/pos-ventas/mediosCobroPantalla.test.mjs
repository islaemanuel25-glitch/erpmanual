...
test("el origen de la comisión se dice con palabras, no solo con el número", () => {
  ...
  // Ya no existe el origen "default": ese respaldo era un 7 que nadie había
  // decidido. Lo que hay cuando no está configurada es la ausencia, y se dice.
  assert.equal(
    textoOrigenComision({ ...MEDIO, comisionOrigen: "sin-configurar" }),
...
test("y el resumen distingue SIN CONFIGURAR de una comisión de 0 %", () => {
  // Antes las dos decían "Sin comisión", porque un `|| 0` las juntaba. Una es
  // una decisión y la otra es que todavía nadie la cargó.
  assert.match(resumenComercial({ recargoPct: 0, comisionPct: null }), /Comisión sin configurar/);
  assert.match(resumenComercial({ recargoPct: 0, comisionPct: 0 }), /Sin comisión/);
  assert.match(resumenComercial({ recargoPct: 0, comisionPct: 5 }), /Comisión 5/);
});
...
test("'sin procesador' y 'no elegí' viajan los dos como null", () => {
  // En la base es el mismo null. La diferencia es de pantalla: un medio nuevo
  // tiene que PEDIR que se elija, y uno que ya existe sin procesador tiene que
  // mostrar "Sin procesador", que es lo que alguien decidió.
  assert.equal(cuerpoParaGuardar({ ...estadoInicialDeMedio(MEDIO), procesador: "" }).procesador, null);
  assert.equal(
    cuerpoParaGuardar({ ...estadoInicialDeMedio(MEDIO), procesador: SIN_PROCESADOR }).procesador,
    null
  );
});

test("un medio que existe sin procesador muestra 'sin procesador', no 'elegir'", () => {
  const form = estadoInicialDeMedio({ ...MEDIO, procesador: null });
  assert.equal(form.procesador, SIN_PROCESADOR);
});

test("un medio nuevo no elige procesador por su cuenta", () => {
  assert.equal(estadoInicialDeMedio(null, { ordenSugerido: 1 }).procesador, "");
});
