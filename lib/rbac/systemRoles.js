...
const ENCARGADO_PERMISOS = [
  ...
  // ── EL ENCARGADO COMPRA ──────────────────────────────────────────────
  //
  // Estaba solo en DUEÑO_LOCAL, así que un encargado podía VER y RECIBIR una
  // compra pero no armarla: veía llegar la mercadería de un pedido que no
  // podía hacer. Es un requerimiento de negocio, no una ampliación oportunista.
  //
  // Sube acá y DUEÑO_LOCAL lo hereda por el spread de abajo, donde por eso se
  // sacó su línea propia: repetido serían dos lugares que dicen lo mismo.
  "compras.crear",
  // ── DAR DE ALTA O ASOCIAR UN PROVEEDOR ───────────────────────────────
  //
  // Va en ENCARGADO, así que DUEÑO_LOCAL lo hereda por el spread de abajo y NO
  // se repite en su lista. Repetirlo daría dos lugares que dicen lo mismo, y el
  // día que haya que sacárselo a uno solo habría que acordarse de los dos.
  //
  // Es SOLO el alta y la asociación. Editar y eliminar un proveedor siguen
  // pidiendo administrador, porque sus datos son globales: ver el comentario del
  // permiso en `lib/rbac/registry.js`.
  "proveedores.crear",
  "reportes.ver",
...
const DUENO_LOCAL_PERMISOS = [
  ...ENCARGADO_PERMISOS,
  // `compras.crear` se fue a ENCARGADO y se hereda por el spread de arriba.
  "costos.ver",
  "clientes.cc.ajustar",
  "clientes.eliminar",
