...
test("la migración de rollout existe y otorga SOLO a DUEÑO_LOCAL", () => {
  const sql = readFileSync(
    "prisma/migrations/20260905150000_permiso_medios_cobro_dueno_local/migration.sql",
    "utf8"
  );
...
test("los medios de cobro NUNCA se deciden por el NOMBRE del rol", () => {
  // ── LA PRIMERA VERSIÓN DE ESTE CANDADO AFIRMABA UNA MENTIRA ──────────────
  //
  // Pedía que NINGÚN archivo de `app/` nombrara a DUEÑO_LOCAL, y se puso en rojo
  // con once archivos preexistentes: login, el cierre de turno, las
  // transferencias, la edición de roles. Esos usos son de otra cosa —quién es
  // dueño para eximirlo del operario, qué roles no se pueden borrar— y son
  // legítimos. El candado no encontró un problema: pedía algo que el repo nunca
  // prometió.
  //
  // Lo que sí hay que sostener es más chico y más preciso: que la decisión de
  // quién configura los medios de cobro se tome SIEMPRE por el permiso. Si un
  // archivo pregunta por `config_local.medios_cobro` y además por el nombre del
  // rol, el permiso dejó de ser configurable: sacarlo desde la pantalla de Roles
  // no cambiaría nada y la pantalla estaría mintiendo.
  let archivos = "";
  try {
    // `git grep` sale con 1 cuando NO encuentra nada. Sin atajarlo,
    // `execFileSync` tira y el candado se rompe justo cuando el repo está bien.
    archivos = execFileSync(
      "git",
      // `--untracked`: sin eso `git grep` mira solo lo trackeado y un lector
      // nuevo sin commitear no entraría en el censo. Agujero del 2026-09-14.
      [
        "grep",
        "--untracked",
        "-l",
  ...
  assert.notEqual(archivos, "", "nadie pregunta por el permiso: algo se rompió");

  const culpables = [];
  for (const archivo of archivos.split("\n").filter(Boolean)) {
    // La matriz de roles de sistema es el ÚNICO lugar donde las dos cosas
    // conviven legítimamente: es justamente la tabla que dice qué rol arranca
    // con qué permiso. Y los candados hablan de los dos por definición.
    if (archivo === "lib/rbac/systemRoles.js" || archivo.endsWith(".test.mjs")) continue;

    // Se lee del DISCO y no de `git show HEAD:…`: un cambio sin commitear tiene
    // que ponerse en rojo ahora, no en la corrida siguiente.
    const texto = readFileSync(archivo, "utf8")
      // Sin comentarios: acá se mira lo que el código hace. Varios de estos
      // archivos explican en prosa por qué DUEÑO_LOCAL recibe el permiso, y un
      // candado que busca texto encuentra los comentarios.
      .replace(/\/\/[^\n]*/g, "")
      .replace(/\/\*[\s\S]*?\*\//g, "");
    if (/DUEÑO_LOCAL|DUENO_LOCAL/.test(texto)) culpables.push(archivo);
  }

  assert.deepEqual(culpables, [], `deciden por el nombre del rol: ${culpables.join(", ")}`);
});
