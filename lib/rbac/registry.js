...
export const PERMISSION_REGISTRY = [
    ...
    // Corrección de ventas (dos modalidades, permisos separados).
    // - corregir_simple: solo cliente/observaciones/referencia interna (NO toca stock/dinero/CC).
    // - corregir_completa: modalidad completa (Fase B). NO habilitada en UI hasta que Fase B exista.
    // - corregir_turno_cerrado: permite corregir aunque el turno original esté cerrado (movimientos compensatorios).
    { code: "ventas.corregir_simple", label: "Corregir venta (simple: cliente/observaciones)", group: "pos", order: 70, deprecated: false },
    { code: "ventas.corregir_completa", label: "Corregir venta completa (productos/pagos/stock)", group: "pos", order: 80, deprecated: false },
    { code: "ventas.corregir_turno_cerrado", label: "Corregir ventas con turno cerrado", group: "pos", order: 90, deprecated: false },
    ...
    // ── OFERTAS ───────────────────────────────────────────────────────────
    //
    // Cinco permisos y no uno, porque son cinco decisiones de distinto peso y
    // en el mostrador las toma gente distinta. Mirar qué está en oferta lo
    // necesita cualquiera que atienda; crear una cambia lo que se cobra;
    // FINALIZAR va separado de EDITAR a propósito —bajar una promoción antes de
    // tiempo es una decisión comercial, no una corrección— y ELIMINAR es el
    // único que destruye una fila y por eso no viaja con ningún otro.
    //
    // Ninguno se ata acá a un rol: los roles los reciben desde el sistema de
    // permisos existente, como todos los demás.
    { code: "ofertas.ver", label: "Ver ofertas", group: "ofertas", order: 10, deprecated: false },
    { code: "ofertas.crear", label: "Crear ofertas", group: "ofertas", order: 20, deprecated: false },
    { code: "ofertas.editar", label: "Editar ofertas", group: "ofertas", order: 30, deprecated: false },
    { code: "ofertas.finalizar", label: "Finalizar ofertas", group: "ofertas", order: 40, deprecated: false },
    { code: "ofertas.eliminar", label: "Eliminar ofertas", group: "ofertas", order: 50, deprecated: false },
    ...
    // ── LA SEGUNDA REVISIÓN ES UN CONTROL, Y POR ESO ES OTRO PERMISO ──────
    //
    // El personal recibe y ahí entra todo: stock y costo. Después alguien
    // distinto revisa. Si revisar usara el MISMO permiso con el que se recibe,
    // el que carga la mercadería podría darse el visto bueno a sí mismo, y la
    // segunda revisión dejaría de ser un control para pasar a ser un trámite.
    //
    // Que sean dos permisos es lo que permite que sean dos personas.
    { code: "compras.revisar", label: "Revisar lo recibido", group: "compras", order: 40, deprecated: false },
    // ── LAS FOTOS DE COMPROBANTES VAN DESENGANCHADAS DE LOS COSTOS ────────────
    //
    // Antes esto se apoyaba en `costos.ver` porque hoy lo tienen justo los
    // mismos. Pero que alguien pueda ver márgenes no tiene por qué darle los
    // comprobantes del proveedor: son dos cosas distintas que coincidían por
    // casualidad, y esa casualidad se rompe el día que alguien reparta permisos
    // sin saber que estaban atados.
    { code: "comprobantes.ver", label: "Ver fotos de comprobantes", group: "compras", order: 50, deprecated: false },
    ...
    // El RECARGO que el local le cobra al CLIENTE por medio de pago. NO es la
    // comisión bancaria: esa se configura por grupo y sigue donde estaba. El
    // permiso es propio porque cambiar este número cambia lo que paga la gente
    // en la caja, y quien configura el ticket no tiene por qué poder hacerlo.
    { code: "config_local.recargos_pago", label: "Configurar recargos por medio de pago del local", group: "config_local", order: 80, deprecated: false },
    // Los MEDIOS DE COBRO del POS: cuáles se ven, cómo se llaman, en qué orden,
    // de qué tipo contable y por qué procesador.
    //
    // Es un permiso propio y no `config_local.pos` porque lo que se toca acá es
    // por dónde entra la plata: apagar un medio deja de poder cobrarse con él, y
    // cambiar un tipo contable cambia contra qué se concilia. Quien configura las
    // reglas de cierre de venta no tiene por qué poder hacer eso.
    //
    // Sigue la convención `config_local.<área>` del resto de la familia. No se
    // creó una familia `config_pos.*`: habría dos convenciones para lo mismo.
    { code: "config_local.medios_cobro", label: "Configurar medios de cobro del POS", group: "config_local", order: 85, deprecated: false },
