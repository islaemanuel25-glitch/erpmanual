...
//
// ANCLADO EN globalThis, por la misma razón que el AsyncLocalStorage de
// `contexto.js`: Turbopack re-evalúa este módulo y cada evaluación arranca con
// sus variables en cero. En desarrollo `lib/prisma.js` cachea el cliente
// extendido en `global.prisma`, así que el cliente sobrevive a la re-evaluación
// y esta referencia no: quedaba en null en la instancia donde corre el flush.
//
// El síntoma no era un error sino silencio. Medido con el módulo instrumentado:
// `auditoriaExtension` evaluada dos veces en una sola corrida, `programarFlush`
// con baseRef en false y el `after()` saliendo por la puerta del `if (baseRef)`
// sin escribir nada. Ninguna edición de producto llegaba a la bitácora en
// desarrollo, y eso impedía distinguir un bug del entorno.
const refs = globalThis.__auditoriaRefs || (globalThis.__auditoriaRefs = { base: null });
...
const ALLOW = {
  ...
  // `updateMany` entró el 2026-09-05 y tapa un agujero que ya existía: el
  // aumento masivo de precios de `/api/productos/precios/apply` escribe
  // `precio_costo` y `precio_venta` con `productoBase.updateMany`, y NADA de eso
  // llegaba a la bitácora. La operación que cambia el costo de cientos de
  // productos de una vez era, hasta hoy, la única que no dejaba rastro.
  //
  // Lo encontró el relevamiento de escritores de costo para las alertas de
  // ofertas, no una revisión de auditoría.
  ProductoBase: ["update", "updateMany"],
  ...
  // La segunda revisión de una recepción por comprobante. El costo en sí ya
  // queda auditado por ProductoBase/ProductoLocal —es por donde pasa toda
  // escritura—, pero sin esto no se vería QUÉ línea lo movió ni que se revirtió.
  ComprobanteLinea: ["update"],
  ComprobanteProveedor: ["update"],
};
...
const DOMINIOS = {
  ...
  // Las dos se agrupan bajo el comprobante: una corrección de línea se lee
  // junto al comprobante que la trajo, no suelta.
  ComprobanteProveedor: { dominio: "Comprobante", grupoKey: (e) => `Comprobante:${e.id}`,            refId: (e) => Number(e.id) },
  ComprobanteLinea:     { dominio: "Comprobante", grupoKey: (e) => `Comprobante:${e.comprobanteId}`, refId: (e) => Number(e.comprobanteId) },
};
...
export function programarFlush(store) {
  ...
  try {
    after(async () => {
      if (refs.base) await flushAuditoria(refs.base, store);
    });
  } catch {
