...
export const ESTADOS = ["abiertas", "en_preparacion", "cerradas", "anuladas", "todas"];

/**
 * Un turno tiene CUATRO estados, no tres.
 *
 * El anulado quedó con `cierre` seteado (se cerró para liberar el local), así que
 * sin distinguirlo aparece mezclado entre los cierres reales. Y el que tomó su
 * corte de cierre tiene `cierre` en null, así que sin distinguirlo aparece
 * mezclado entre las cajas abiertas —llamando "Abierta" a una que ya no vende y
 * que está esperando que alguien termine de contarla—.
 *
...
export function normalizarEstado(raw) {
  ...
  if (v === "en_preparacion" || v === "en preparacion") return "en_preparacion";
  if (v === "cerradas" || v === "cerrado") return "cerradas";
  if (v === "anuladas" || v === "anulado") return "anuladas";
  return "todas";
}

/** Condición Prisma del estado. Los cuatro son mutuamente excluyentes. */
export function condicionEstado(estado) {
  switch (normalizarEstado(estado)) {
    case "abiertas":
      // `cierreEnPreparacionEn: null` es lo que impide que una caja congelada se
      // liste como abierta. Sin esa condición, el listado seguiría diciendo
      // "Abierta" sobre una caja que ya cortó y no admite ni una venta más.
      return { cierre: null, cierreEnPreparacionEn: null, anuladoEn: null };
    case "en_preparacion":
      return { cierre: null, cierreEnPreparacionEn: { not: null }, anuladoEn: null };
    case "cerradas":
