...
// ═══════════════════════════════════════════════════════════════════════════
// SunmiSelectAdv — LA CADENA QUE SALE, con las declaraciones REALES
// ═══════════════════════════════════════════════════════════════════════════
//
// La pieza pasó a negociar el ancho con `componerClaseInput` en la fase 4. Estas
// son las declaraciones que hoy le pasan sus 39 consumidores, leídas del primer
// nivel de la etiqueta. No hay ninguna inventada.
//
// El envoltorio no tiene otras clases propias, así que la base es vacía.
const SELECT = [
  // Las que DECLARAN ancho: la pieza no tiene que poner el suyo.
  ["w-[85px]", true],
  ["w-[140px]", true],
  ["w-44", true],
  // Las que NO: la pieza pone `w-full`.
  ["[&_.sunmi-select-trigger]:!border-[var(--pos-link)]", false],
  ["min-w-[180px] !py-2 text-[13px] [&_.sunmi-select-trigger]:!border-[var(--pos-link)]", false],
  ["", false],
];

test("SunmiSelectAdv: NUNCA DOS ANCHOS en la cadena que sale", () => {
  for (const [pedido] of SELECT) {
    const salida = componerClaseInput(pedido, "").split(/\s+/).filter(Boolean);
    assert.ok(
      salida.filter((t) => declaraAncho(t)).length <= 1,
      `${pedido} -> ${salida.join(" ")}`
    );
  }
});

test("SunmiSelectAdv: quien declara ancho se queda con el suyo", () => {
  for (const [pedido, declara] of SELECT.filter(([, d]) => d)) {
    const salida = componerClaseInput(pedido, "");
    assert.ok(salida.includes(pedido), `${pedido} -> ${salida}`);
    assert.ok(!tieneFull(salida), `${pedido} no debería recibir w-full -> ${salida}`);
    assert.equal(declara, true);
  }
});

test("SunmiSelectAdv: quien NO declara ancho recibe w-full", () => {
  for (const [pedido] of SELECT.filter(([, d]) => !d)) {
    assert.ok(tieneFull(componerClaseInput(pedido, "")), `${pedido}`);
  }
});

// `min-w-` NO es un ancho: es un piso, otra propiedad CSS. Si contara, esa
// pantalla perdería el `w-full` y el select se encogería al contenido.
test("SunmiSelectAdv: min-w- no cuenta como ancho declarado", () => {
  assert.equal(declaraAncho("min-w-[180px]"), false);
  assert.ok(tieneFull(componerClaseInput("min-w-[180px]", "")));
});

// EL PARCHE QUE DEJÓ DE HACER FALTA. `!w-44` ganaba por `!important` y por eso
// funcionaba; ahora `w-44` gana porque la pieza cede. El candado afirma que las
// dos formas terminan igual, que es lo que hace seguro haber sacado el `!`.
test("SunmiSelectAdv: con o sin `!`, el ancho de la pantalla gana igual", () => {
  const conBang = componerClaseInput("!w-44", "");
  const sinBang = componerClaseInput("w-44", "");
  assert.ok(!tieneFull(conBang), conBang);
  assert.ok(!tieneFull(sinBang), sinBang);
});
