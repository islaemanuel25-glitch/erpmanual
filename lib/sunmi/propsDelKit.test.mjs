...
const DEUDA = {
  ...
  "SunmiCardHeader.color": 2,
  // 10 → 8 al migrar ModalRol y ModalLocal, y 8 → 7 al migrar ModalUsuario. Los
  // de rol y operador salieron con su encabezado viejo; los de local y usuario
  // se conservan a propósito —anotados en el roadmap— pero pasaron a declararse
  // una sola vez, en la pieza.
  "SunmiCardHeader.subtitle": 7,
  "SunmiHeader.subtitle": 5,
  // `titulo` y `subtitulo` salieron: eran los dos de arqueo-caja, en español, y
  // se descartaron a propósito —el título ya aparece veinte líneas más abajo—.
  // El motivo largo está en el roadmap para que nadie rehaga el análisis.
