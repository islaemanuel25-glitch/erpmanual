...
test("SunmiSeparator: las quince declaraciones siguen ahí, contadas una por una", async () => {
  ...
  for (const [, pedido, veces] of SEPARADOR) {
    const salida = execSync(
      // `--untracked` y no a secas: esto CUENTA apariciones y afirma un número
      // exacto. Un archivo nuevo sin commitear que declare el separador no se
      // contaría, y el candado pasaría en verde justo cuando acaba de aparecer
      // uno más. Agujero del 2026-09-14.
      `git grep --untracked -o -F -- ${JSON.stringify('className="' + pedido + '"')} -- app components || true`,
      { encoding: "utf8" }
    );
...
test("SunmiCard: las declaraciones de la lista siguen existiendo en el repo", async () => {
  ...
  for (const [donde, pedido] of CARD) {
    ...
    // `--untracked` acá es el caso CONTRARIO al del separador de arriba: esto
    // afirma que una pantalla conocida SIGUE declarando algo, así que no ver lo
    // sin commitear daría un rojo falso —no un verde falso— mientras se está
    // moviendo un archivo. Se pone igual para que las tres enumeraciones de este
    // archivo miren lo mismo.
    const hay = execSync(
      `git grep --untracked -c -F -- ${JSON.stringify(marca)} -- ${JSON.stringify(donde + "*")} || true`,
      { encoding: "utf8" }
    ).trim();
...
test("SunmiCard: las cinco pantallas del cero siguen pidiendo cero", async () => {
  ...
  for (const [donde, pedido] of CARD_CERO) {
    const hay = execSync(
      `git grep --untracked -c -F -- ${JSON.stringify(pedido)} -- ${JSON.stringify(donde + "*")} || true`,
      { encoding: "utf8" }
    ).trim();
