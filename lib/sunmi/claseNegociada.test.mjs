...
// La MISMA expresión que `SunmiTableRow.jsx`.
function componerFila(pedido, { onClick = true, selected = false, tono = null, intensidad = "ambiente" } = {}) {
  const fondoDelKit = tono
    ? `sunmi-fila sunmi-fila-${tono} sunmi-fila-${intensidad}${selected ? " sunmi-fila-seleccionada" : ""}`
    : selected
      ? FONDO_KIT
      : HOVER_KIT;
  const cedeFondo = tono || selected ? declaraSuperficie(pedido) : declaraHover(pedido);
  const fondo = cedeFondo ? "" : fondoDelKit;
  const tamano = declaraTamanoDeLetra(pedido) ? "" : TAMANO_FILA;
  const cursor = onClick && !declaraCursor(pedido) ? "cursor-pointer" : "";
  return `${tamano} ${cursor} ${fondo} ${pedido}`.replace(/\s+/g, " ").trim();
}
...
test("SunmiCard: el radio no se pierde nunca, y el padding tampoco ante un pedido de AIRE", () => {
  // El radio no se negocia en ningún caso. El padding cede SOLO ante un pedido
  // de cero, así que ante estas nueve —que piden `p-3`, `p-4`, `p-5` o nada— la
  // pieza tiene que seguir poniendo el suyo. Si algún día desaparece acá, es que
  // la negociación del cero se comió también al aire.
  for (const [donde, pedido] of CARD) {
    const salida = componerCard(CARD_CLARO, pedido).split(/\s+/);
    assert.ok(salida.includes("rounded-xl"), `${donde}: se perdió el radio`);
    assert.ok(salida.includes("p-6"), `${donde}: se perdió el padding del kit`);
  }
});
...
test("SunmiCard: la pieza NEGOCIA de verdad, no solo en la copia del candado", async () => {
  ...
  for (const [eje, patron] of [
    ...
    ["padding cero", /declaraPaddingCero\s*\(\s*className\s*\)\s*\?\s*""\s*:\s*"p-6"/],
  ]) {
  ...
  assert.ok(
    !/\}\s*p-6\s/.test(fuente),
    "el p-6 volvió a estar concatenado: las siete tablas pierden su borde a borde"
  );
});
...
// ═══════════════════════════════════════════════════════════════════════════
//
// De las 108 declaraciones de padding, ocho piden cero. Se migraron siete: las
// que envuelven una TABLA, donde el cero es una intención estructural —el
// contenido va de borde a borde y el `overflow-hidden` recorta las esquinas—.
//
// La octava, `CardDefaultDeposito`, es un FORMULARIO: ahí el cero no dibuja una
// tabla, aprieta los campos contra el marco. Quedó afuera declarando el `p-6`
// que ya venía dibujando, y por eso este candado la lista APARTE: si alguien le
// vuelve a poner `p-0`, se entera.
//
// Las otras 100 declaraciones —49 `p-3`, 30 `p-4` y el resto— siguen sin
// negociar y siguen perdiendo, que es lo de hoy. Eso lo afirma el test del radio
// y el padding, unas líneas más arriba.
const CARD_CERO = [
...
// NUNCA DOS PADDINGS — PERO SOLO EN LAS DEL CERO, Y NO ES UNA CONCESIÓN.
//
// Escribí este candado sobre las dos listas y salió ROJO en la primera corrida:
// `p-6 p-3` en reportes-ventas. Tenía razón el rojo. Las 100 del aire SALEN con
// dos paddings hoy, a propósito: la pieza pone el suyo, la pantalla pone el suyo
// y gana el de la pieza por el orden de la hoja. Esa es exactamente la deuda que
// falta pagar, no un defecto de la migración de ahora.
//
// Afirmarlo sobre las dos listas dejaba dos salidas y las dos malas: aflojar el
// candado, o migrar el aire de apuro para ponerlo en verde — que es tomar por
// error una decisión que no es mía. Se acota al cero, que es lo que esta tanda
// sí hizo, y el día que el aire se decida este candado se abre a `CARD`.
test("SunmiCard: NUNCA DOS PADDINGS en las que piden cero", () => {
  for (const card of [CARD_OSCURO, CARD_CLARO]) {
    for (const [donde, pedido] of CARD_CERO) {
      const salida = componerCard(card, pedido).split(/\s+/).filter(Boolean);
...
test("SunmiCard: el FORMULARIO que quedó afuera no pide cero", async () => {
  ...
  for (const etiqueta of etiquetas) {
    ...
    if (!cn) continue;
    assert.ok(
      !declaraPaddingCero(cn[1]),
      `volvió a pedir cero padding: ${JSON.stringify(cn[1])}. Es un formulario, no una tabla.`
    );
  }

  // Y el positivo: el predicado SÍ ve el cero cuando está, y la extracción lo
  // saca de una etiqueta de verdad. Sin esto, cualquier cambio que rompa el
  // recorte deja el candado en verde afirmando nada.
  const deMentira = '<SunmiCard className="p-0 overflow-hidden">';
  assert.equal(
    declaraPaddingCero(deMentira.match(/className\s*=\s*"([^"]*)"/)[1]),
    true
  );
});

// EL PREDICADO NO SE PUEDE ABLANDAR HASTA VOLVERSE EL DEL EJE ENTERO.
//
// Si alguien lo cambia por "declara algún padding", las 108 se migran de una
// vez y la decisión del aire —que es de Emanuel y todavía no está tomada— se
// toma sola y en silencio. Estos tres casos son exactamente esa diferencia.
test("declaraPaddingCero: el CERO no es lo mismo que declarar padding", () => {
  assert.equal(declaraPaddingCero("p-0 overflow-hidden"), true);
  assert.equal(declaraPaddingCero("hidden lg:block p-0 overflow-hidden"), true);
  assert.equal(declaraPaddingCero("px-0 py-0"), true);
  assert.equal(declaraPaddingCero("!p-0"), true, "el `!` no cambia lo que se pide");

  assert.equal(declaraPaddingCero("p-3"), false, "un p-3 NO es cero");
  assert.equal(declaraPaddingCero("p-4 border"), false);
  assert.equal(declaraPaddingCero("p-2 lg:p-3"), false);
  assert.equal(declaraPaddingCero(""), false, "sin padding no hay pedido de cero");
  assert.equal(declaraPaddingCero("overflow-hidden"), false);

  // Un cero MEZCLADO con un no-cero no es "de borde a borde": es un padding
  // chico de un lado. Si esto diera true, la pieza le sacaría el p-6 y la
  // tarjeta quedaría con padding en un solo eje.
  assert.equal(declaraPaddingCero("p-0 pt-2"), false);
  assert.equal(declaraPaddingCero("px-0 py-3"), false);

  // Y lo que no es de la familia no se cuenta: `pb-0` sí, `pointer-events-none`
  // no, aunque empiece con `p`.
  assert.equal(declaraPaddingCero("pointer-events-none"), false);
  assert.equal(declaraPaddingCero("pb-0"), true);
});
