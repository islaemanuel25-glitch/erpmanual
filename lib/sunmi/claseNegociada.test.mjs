...
const componerCard = (card, pedido) =>
  ...
  } p-6 ${declaraDifuminado(pedido) ? "" : "backdrop-blur-sm"} ${pedido}`
    .replace(/\s+/g, " ")
    .trim();
...
test("SunmiCard: el radio y el padding nunca se pierden", () => {
  // Los dos ejes que esta tanda NO negocia. Si alguno desaparece, la pieza dejó
  // de poner algo que sí es suyo.
  for (const [donde, pedido] of CARD) {
...
test("SunmiCard: la pieza NEGOCIA de verdad, no solo en la copia del candado", async () => {
  ...
  // Y los dos ejes que esta tanda NO negocia siguen puestos sin condición.
  assert.match(fuente, /rounded-xl/, "SunmiCard perdió el radio");
  assert.match(fuente, /p-6/, "SunmiCard perdió el padding del kit");

  // Nada de los cuatro ejes puede volver a la cadena fija.
