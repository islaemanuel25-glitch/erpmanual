...
// UN COLOR FIJO SE LEE COMO TAMAÑO. Está mal y está afirmado a propósito.
//
// `declaraTamanoDeLetra` cuenta cualquier `text-…` que no sea alineación, así que
// `text-blue-600` hace ceder el `text-[12px]` y la tabla se queda con el tamaño
// heredado. No se arregla desde acá: el predicado lo comparten el botón y el
// renglón chico de las celdas, y tocarlo de un lado solo es lo que la regla 1
// prohíbe — primero hay que buscar todos sus lectores.
//
// Se afirma como comportamiento CONOCIDO por dos motivos: para que nadie lo
// redescubra, y para que el día que alguien arregle el predicado este candado se
// ponga rojo y le muestre qué más estaba tocando.
//
// Lo que hoy lo mantiene lejos es el trinquete de hardcodeo, que prohíbe slate,
// red, amber, cyan, emerald, green, orange y gray. `blue` NO está en esa lista.
test("CONOCIDO Y MAL: un color fijo de Tailwind hace ceder el tamaño", () => {
  assert.equal(tiene(claseDeTabla("text-blue-600"), "text-[12px]"), false);
  // Los del tema NO tienen el problema, y la alineación tampoco. Ésta es la
  // mitad que hace que el candado signifique algo: sin ella, "cede con todo"
  // no se distingue de "cede con un color".
