...
// UN COLOR YA NO HACE CEDER EL TAMAÑO. Era el defecto y está arreglado.
//
// ── LO QUE ESTE CANDADO DECÍA ANTES, PORQUE IMPORTA ────────────────────────
//
// Hasta el arreglo esta prueba afirmaba lo CONTRARIO, bajo el título "CONOCIDO Y
// MAL": que `text-blue-600` hacía ceder el `text-[12px]` y la tabla se quedaba
// sin tamaño propio. Estaba escrito así a propósito, para que el día que alguien
// arreglara el predicado esto se pusiera rojo y le mostrara qué estaba tocando.
//
// Funcionó: al cambiar el predicado, este candado fue el que se puso rojo.
//
// ── LO QUE AFIRMA AHORA ───────────────────────────────────────────────────
//
// `declaraTamanoDeLetra` pasó a reconocer la escala en vez de aceptar cualquier
// `text-…`. Eran CINCO familias mal leídas y no sólo los colores: también
// `text-nowrap` y sus hermanas, `text-ellipsis`, `text-clip` y `text-opacity-*`.
// Ninguna hace ceder ya.
test("un color NO hace ceder el tamaño, y las otras cuatro familias tampoco", () => {
  // Los colores, incluidos los que el trinquete de hardcodeo no persigue.
  for (const c of ["text-blue-600", "text-white", "text-[#ff0000]", "text-[var(--pos-accent)]"]) {
    assert.equal(tiene(claseDeTabla(c), "text-[12px]"), true, `${c} no debería hacer ceder: ${claseDeTabla(c)}`);
  }
  // Las cuatro familias que NO son colores y que ninguna lista de colores vería.
  for (const c of ["text-nowrap", "text-balance", "text-ellipsis", "text-opacity-50"]) {
    assert.equal(tiene(claseDeTabla(c), "text-[12px]"), true, `${c} no debería hacer ceder: ${claseDeTabla(c)}`);
  }
  // Y la otra mitad, sin la cual "no cede con nada" no se distingue de "no cede
  // nunca": un tamaño de verdad SÍ tiene que hacer ceder.
  assert.equal(tiene(claseDeTabla("text-xs"), "text-[12px]"), false);
  assert.equal(tiene(claseDeTabla("text-[10px]"), "text-[12px]"), false);
  // Los del tema y la alineación nunca tuvieron el problema.
  assert.equal(tiene(claseDeTabla("sunmi-text-muted"), "text-[12px]"), true);
  assert.equal(tiene(claseDeTabla("text-right"), "text-[12px]"), true);
});
