...
import { etiquetasDeApertura } from "../hardcodeo/contador.mjs";
...
test("NINGÚN CONSUMIDOR DECLARA UN RADIO PARCIAL, que cedería ocho esquinas y repondría cuatro", () => {
  // `--others --exclude-standard`: un consumidor nuevo sin commitear no está en
  // el índice, así que `git ls-files` a secas lo dejaría afuera y este candado
  // daría verde sobre un radio parcial recién escrito. Agujero del 2026-09-14.
  const archivos = execSync("git ls-files --cached --others --exclude-standard app components", {
    cwd: RAIZ,
    encoding: "utf8",
  })
    .split("\n")
