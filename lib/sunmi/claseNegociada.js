...
/**
 * ¿Este `className` pide CERO padding?
 *
 * O sea: declara padding, y TODO lo que declara es cero. Un `p-0 pt-2` no entra
 * —eso no es "de borde a borde", es un padding chico— y por eso no alcanza con
 * preguntar si aparece un `p-0`.
 *
 * ── POR QUÉ EXISTE ESTE PREDICADO Y NO EL DEL EJE ENTERO ────────────────────
 *
 * El eje del padding tiene 108 declaraciones y las 108 pierden hoy: la pieza
 * concatena su `p-6` y entre dos utilidades de la misma familia decide el orden
 * de la hoja de Tailwind, no el del atributo. Negociar el eje completo cambiaría
 * las 108 de una vez, y eso es una decisión de aspecto de toda la aplicación.
 *
 * Pero las 108 no son todas la misma clase de pedido, y ahí está el criterio:
 *
 *   · Un `p-0` con `overflow-hidden` sobre una TABLA es una intención
 *     estructural declarada. Dice "el contenido va de borde a borde y las
 *     esquinas se recortan". No es una preferencia de aire: sin eso la tabla
 *     queda flotando adentro de un marco y las filas no llegan a los bordes.
 *   · Un `p-3` o un `p-4` es estética: la misma tarjeta se ve bien con 21 px y
 *     con 14, y cuál se elige es una decisión de quien mira la aplicación.
 *
 * Por eso este predicado es el CERO y no el eje: aplica la intención estructural
 * sin tomar la decisión estética. El día que el aire se decida, esto se
 * reemplaza por `declaraPaddingX || declaraPaddingY` y este predicado se va.
 *
 * ── Y EL CORTE NO ES "TODO p-0" ────────────────────────────────────────────
 *
 * De las ocho tarjetas que piden cero, SIETE son tablas y una —la de
 * `CardDefaultDeposito`— es un FORMULARIO. Ahí el cero no dibuja una tabla de
 * borde a borde: aprieta los campos contra el marco. Esa quedó afuera a mano,
 * declarando el `p-6` que ya venía dibujando, y va a decidirse con el grupo del
 * aire. La regla que salió de eso: `p-0` con `overflow-hidden` sobre una tabla
 * es intención; `p-0` sobre un formulario, no.
 */
export function declaraPaddingCero(className) {
  const dePadding = tokens(className)
    .map(pelar)
    .map((t) => t.replace(/^[a-z]+:/, ""))
    .filter((t) => /^(p|px|py|pt|pr|pb|pl|ps|pe)-\S/.test(t));

  return dePadding.length > 0 && dePadding.every((t) => t.endsWith("-0"));
}
