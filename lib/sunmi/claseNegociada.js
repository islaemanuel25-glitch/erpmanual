...
/**
 * LOS NOMBRES DE LA ESCALA, y por qué están enumerados acá.
 *
 * Son los de Tailwind más los tres que agrega `tailwind.config.js` —`xs2` de
 * 10px, `sm2` de 11px y el `base` pisado a 14px—. `2xl` hasta `9xl` no hacen
 * falta uno por uno: los reconoce la forma.
 *
 * **La lista NO se mantiene a mano, aunque esté escrita a mano.** Quien la
 * mantiene es `escalaDeTexto.test.mjs`, que LEE `tailwind.config.js` y el tema
 * por defecto de Tailwind y se pone rojo si aparece un tamaño que esto no
 * reconoce. Agregar un `xs3` a la config sin tocar acá deja el candado en rojo
 * nombrándolo.
 *
 * ── POR QUÉ NO SE LEE LA CONFIG DESDE ACÁ ──────────────────────────────────
 *
 * Porque este módulo lo importan `SunmiTable`, `SunmiTableRow`, `SunmiButton` y
 * `SunmiPar`, que son todos `"use client"`. Un `require("tailwindcss/defaultTheme")`
 * acá arrastraría Tailwind entero al bundle del navegador para preguntar por
 * quince nombres. El candado corre en node y puede leer lo que quiera; el módulo
 * de runtime se queda puro, que es lo que lo hace ejercitable.
 */
// `md2` y `lg2` los trajo el rediseño V16 de la recepción: 16 y 17 px, que no
// caen en la escala porque este proyecto redefine `base` a 14 y el salto siguiente
// es `lg` (18). Se suman ACÁ el mismo día que entran a `tailwind.config.js` — el
// candado de `escalaDeTexto.test.mjs` se pone rojo nombrándolos si no se hace, y
// un tamaño que el predicado no reconoce deja a la pieza y a la pantalla poniendo
// el suyo, que es el defecto original que toda esta fase vino a terminar.
// Y `base2` y `xl2` los trajo la limpieza V26 de la misma pantalla: 15 y 22 px,
// por el mismo motivo —el salto de 14 a 18 no tiene escalones—. El candado los
// nombró en rojo apenas entraron al config, que es exactamente para lo que está.
// Y `sm3`, `lg3` y `xl3` los trajo el rediseño de Transferencias: 13, 19 y 28 px,
// los tres protagonistas de esas pantallas. Los espaciados y los radios de esa
// especificación NO entraron al config —venían de la grilla de 16 de Figma y se
// ajustaron a la del proyecto, que es `1rem = 14px`—; los tamaños sí, porque seis
// píxeles en el importe grande se ven. Mismo día que el config, como siempre.
const ESCALA = new Set([
  "xs", "sm", "base", "lg", "xl",
  "xs2", "sm2", "base2", "md2", "lg2", "xl2",
  "sm3", "lg3", "xl3",
]);

/** `2xl`, `3xl`… `9xl`, y también `xl` a secas ya cubierto por ESCALA. */
const ESCALA_XL = /^\d+xl$/;

/** `text-[12px]`, `text-[9.5rem]`. Un valor arbitrario CON UNIDAD de longitud. */
const ARBITRARIO = /^\[[\d.]+(px|rem|em|pt|%|ch|ex|vw|vh)\]$/;

/**
 * ¿Este token es un TAMAÑO DE LETRA?
 *
 * Se exporta para que el candado de la escala pueda preguntarle por cada nombre
 * que encuentre en la config, en vez de reimplementar la regla al lado.
 */
export function esTamanoDeLetra(bruto) {
  const t = pelar(String(bruto));
  if (!t.startsWith("text-")) return false;
  const resto = t.slice("text-".length);
  if (resto === "" || ALINEACION.has(resto)) return false;
  return ESCALA.has(resto) || ESCALA_XL.test(resto) || ARBITRARIO.test(resto);
}

/**
 * ¿Este `className` declara un TAMAÑO DE LETRA?
 *
 * Cuenta `text-xs`, `text-2xl`, `text-xs2`, `text-[9.5px]`, con o sin `!`.
 *
 * ── LO QUE NO CUENTA, Y ES POR LO QUE ESTA FUNCIÓN SE REESCRIBIÓ ───────────
 *
 * Antes decía "cualquier `text-…` que no sea alineación", y eso metía adentro
 * CINCO familias que no son un tamaño. Medido pasándole cada clase, no deducido:
 *
 *   · COLORES — `text-blue-600`, `text-white`, `text-transparent`, `text-current`,
 *     `text-[#ff0000]`, `text-[var(--pos-accent)]`.
 *   · AJUSTE DE LÍNEA — `text-wrap`, `text-nowrap`, `text-balance`, `text-pretty`.
 *   · DESBORDE — `text-ellipsis`, `text-clip`.
 *   · OPACIDAD — `text-opacity-50`.
 *   · y cualquier `text-` que Tailwind agregue mañana.
 *
 * El daño no era teórico: la pieza CEDÍA su tamaño ante una de ésas y se quedaba
 * sin ninguno, heredando. El botón perdía sus 13px, `SunmiPar` sus 10.
 *
 * Que sean cuatro familias sin relación con los colores es lo que hace que el
 * trinquete de hardcodeo no pueda cubrir esto: `text-nowrap` no es hardcodeo de
 * nada y ninguna lista de colores lo ve.
 *
 * Los colores del tema —`sunmi-text-*`— nunca tuvieron el problema, y la
 * alineación tampoco. Un token con variante —`sm:text-sm`— tampoco cuenta, por el
 * motivo que está escrito más abajo: ceder un default incondicional por un pedido
 * condicional mueve píxeles.
 */
export function declaraTamanoDeLetra(className) {
  return tokens(className).some(esTamanoDeLetra);
}
...
/**
 * LA CLASE DEL `<tr>` DE `SunmiTableRow`, NEGOCIADA POR EJE.
 *
 * ── POR QUÉ ESTA FUNCIÓN EXISTE, Y NO ES UN REFACTOR DE GUSTO ──────────────
 *
 * Hasta hoy `SunmiTableRow.jsx` llamaba a `declaraTamanoDeLetra` DIRECTO, en su
 * propio archivo, sin pasar por ninguna función de este módulo. Era el cuarto
 * lector del predicado y el único que no se ve abriendo este archivo: quien
 * fuera a cambiar el predicado tenía a los otros tres a la vista y a éste no.
 * Aparecía sólo buscando el nombre del predicado por todo el repo.
 *
 * Y ya se cobró algo: la primera enumeración de los lectores dijo "dos", escrita
 * mirando este módulo. Esa vez lo que se salvó fue un número.
 *
 * Documentar la trampa no alcanzaba, así que se sacó: la expresión de la fila
 * vive acá, en un solo lugar, y la pieza la invoca. El predicado vuelve a tener
 * todos sus lectores adentro de este archivo.
 *
 * ── Y EL CANDADO PASA A EJERCER ESTO, QUE ES LO OTRO QUE FALTABA ───────────
 *
 * `claseNegociada.test.mjs` tenía una función `componerFila` copiada a mano, con
 * un comentario que decía "la MISMA expresión que `SunmiTableRow.jsx`". O sea que
 * el candado probaba la copia: el día que la pieza cambiara, seguiría verde. Con
 * la expresión acá, el candado ejerce lo que la pieza corre de verdad.
 *
 * ── LOS CUATRO EJES, Y CADA UNO CEDE POR SEPARADO ─────────────────────────
 *
 * El fondo del kit es de UN eje o del otro según el caso: con `tono` o `selected`
 * pinta el fondo normal, y sin nada pinta sólo el hover. Por eso se pregunta por
 * el eje que corresponde y no siempre por el mismo.
 *
 * @param {object} p
 * @param {string} p.pedido       lo que declaró la pantalla en `className`.
 * @param {string|null} p.tono    "ok" | "alerta" | "atencion" | "apagado" | null
 * @param {string} p.intensidad   "ambiente" | "fuerte"
 * @param {boolean} p.selected
 * @param {boolean} p.onClick     si la fila es clickeable.
 * @returns {string} la cadena completa del atributo `class` del `<tr>`.
 */
export function claseDeFila({
  pedido = "",
  tono = null,
  intensidad = "ambiente",
  selected = false,
  ...
  const cedeFondo = tono || selected ? declaraSuperficie(pide) : declaraHover(pide);
  const fondo = cedeFondo ? "" : fondoDelKit;

  const tamano = declaraTamanoDeLetra(pide) ? "" : TAMANO_DE_FILA;
  const cursor = onClick && !declaraCursor(pide) ? "cursor-pointer" : "";

  return `${tamano} ${cursor} ${fondo} ${pide}`.replace(/\s+/g, " ").trim();
}

/** El tamaño de letra de una fila. Lo mismo que ponía `SunmiTableRow` a mano. */
export const TAMANO_DE_FILA = "text-[12px]";
 ...
 *
 * ── UN LÍMITE HEREDADO, MEDIDO Y CON CANDADO ───────────────────────────────
 *
 * `declaraTamanoDeLetra` cuenta cualquier `text-…` que no sea alineación, así que
 * un COLOR FIJO de Tailwind lo lee como si fuera un tamaño: con
 * `className="text-blue-600"` la tabla CEDE el `text-[12px]` y se queda con el
 * tamaño heredado, que no es lo que nadie pidió. Comprobado, no deducido.
 *
 * Los colores del tema NO tienen el problema —`sunmi-text-muted` no matchea— y
 * `text-right` tampoco, que es alineación.
 *
 * No se arregla acá, y ese es el punto: el predicado lo comparten el botón y el
 * renglón chico de las celdas, y tocarlo de un lado solo es exactamente lo que la
 * regla 1 prohíbe. Hace falta buscar todos sus lectores primero.
 *
 * Lo que hoy lo mantiene lejos es el trinquete de hardcodeo, que prohíbe slate,
 * red, amber, cyan, emerald, green, orange y —desde el 2026-08-17— gray. Pero
 * **`blue` no está en esa lista**, así que el agujero es alcanzable. Queda
 * afirmado en el candado como comportamiento CONOCIDO para que nadie lo
 * redescubra, y para que el día que se arregle el predicado se vea qué más toca.
 *
...
/**
 * QUÉ EJE DESPLAZA EL ENVOLTORIO DE LA TABLA, Y SI EL ENCABEZADO SE PEGA.
 *
 * Son DOS cadenas y una sola decisión, y por eso se calculan juntas: el
 * encabezado pegajoso solo tiene sentido cuando hay debajo un contenedor que se
 * desplaza vertical. Calculadas por separado se pueden contradecir —un `thead`
 * declarado `sticky` colgando de un envoltorio que ya no scrollea— y esa
 * contradicción no rompe nada, no se ve en el diff y miente en el CSS.
 *
 * ── LOS TRES MODOS ────────────────────────────────────────────────────────
 *
 * · **default** (sin `stickyHeader`): el envoltorio existe para el scroll
 *   HORIZONTAL, crece con su contenido y no se encoge. Es lo de siempre.
 * · **`stickyHeader`**: se declara un tope de alto y el envoltorio se queda con
 *   los dos ejes; el encabezado queda fijo contra ÉL. Es lo de siempre.
 * · **`altoLibre`**: la tabla crece con su contenido y el desplazamiento
 *   vertical se lo queda quien esté afuera —en el ERP, el `<main>` del layout—.
 *   El encabezado NO se declara pegajoso, y no es una omisión: el envoltorio
 *   conserva `overflow-x` para las columnas que no entran, y un elemento con
 *   overflow en cualquier eje se vuelve el ámbito de lo pegajoso. El `thead`
 *   quedaría pegado a algo que ya no se desplaza.
 *
 * `altoLibre` MANDA sobre `stickyHeader`: pedir alto libre es pedir exactamente
 * lo que hace el default, así que devuelve la misma cadena y no una parecida.
 *
 * @returns {{ envoltorio: string, pegajoso: boolean }}
 */
export function scrollDeLaTabla({
  stickyHeader = false,
  altoLibre = false,
  maxHeightClass = "max-h-[70dvh]",
} = {}) {
  const conTope = stickyHeader === true && altoLibre !== true;
  return {
    envoltorio: conTope ? `overflow-auto ${maxHeightClass}` : "overflow-x-auto shrink-0",
    pegajoso: conTope,
  };
}
