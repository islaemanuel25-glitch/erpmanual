...
test("perfil: local con exigirOperador=false exime a cualquiera (CAJERO)", () => {
  assert.equal(
    perfilExentoDeOperador({ esAdmin: false, esDuenoLocal: false, exigirOperador: false }),
    true
  );
});

test("perfil: exigirOperador=true o ausente NO exime por sí solo", () => {
  // true explícito: CAJERO sigue el flujo normal.
  assert.equal(
    perfilExentoDeOperador({ esAdmin: false, esDuenoLocal: false, exigirOperador: true }),
    false
  );
  // ausente = obligatorio (no exento): comportamiento histórico.
  assert.equal(perfilExentoDeOperador({ esAdmin: false, esDuenoLocal: false }), false);
});

test("perfil: exigirOperador debe ser === false (no truthy/falsy raros)", () => {
  // 0 no es false estricto → no exime.
  assert.equal(
    perfilExentoDeOperador({ esAdmin: false, esDuenoLocal: false, exigirOperador: 0 }),
    false
  );
});

test("perfil: rol exime aunque el local exija operario", () => {
  // Admin/dueño quedan exentos por rol sin importar la config del local.
  assert.equal(perfilExentoDeOperador({ esAdmin: true, exigirOperador: true }), true);
  assert.equal(perfilExentoDeOperador({ esDuenoLocal: true, exigirOperador: true }), true);
});
