...
export async function propagarCostoALocales(
  db,
  { baseId, costo, margenBase, redondeo100 = false, soloLocalId = null } = {}
) {
  ...
  const locales = await db.productoLocal.findMany({
    where,
    select: { id: true, localId: true, margen: true, precio_venta: true },
  });
  ...
  for (const local of locales) {
    // Precedencia: margen de la ubicación; si no tiene, el de la ficha.
    const margenConfigurado = local.margen != null ? local.margen : margenBase;
    ...
    if (data.precio_venta !== undefined) {
      actualizadas.push({
        ...
        margenConfigurado: Number(margenConfigurado),
