...
test("la tarjeta del catálogo rotula con la escala en la que se VENDE", () => {
  // ── ESTE CANDADO CAMBIÓ DE OBJETIVO EL 2026-08-19, Y NO ES UN AFLOJE ────
  //
  // Pedía `etiquetaEscalaPrecio(p.unidadMedida)`, que rotula con la escala en la
  // que está GUARDADO el precio — o sea, cómo se COMPRA. Eso era mejor que el
  // "/ un" fijo que arregló en su momento, pero seguía siendo la escala
  // equivocada: medido, **5.450 de 10.521 filas activas** nombraban una escala
  // distinta de la que el POS usa para vender.
  //
  // Ahora se exige lo que corresponde: que el rótulo salga de `escalaDeVentaDe`,
  // la pieza que comparte con el POS. Exigir la función vieja sería exigir que
  // vuelva el defecto.
  const src = sinComentarios("app/modulos/productos/page.jsx");
  assert.match(
    src,
    /escalaDeVentaDe\(/,
    "el rótulo tiene que salir de la escala de VENTA, la misma que usa el POS"
  );
  assert.doesNotMatch(
    src,
    /etiquetaEscalaPrecio\(/,
    "volvió a rotular con la escala en la que está guardado el precio"
  );
  // El defecto original de esta afirmación, que sigue vigente.
  assert.doesNotMatch(
    src,
...
test("el andamio deriva sus textos del dato, como la pantalla real", () => {
  ...
  //
  // ── LA EQUIVALENCIA SE FUE DE LA PIEZA, ASÍ QUE SE FUE DE ACÁ ───────────
  //
  // `SunmiProductoCard` dejó de tener la franja: la presentación viaja pegada al
  // precio y la otra escala vive en el dorso, que administra el envoltorio del
  // catálogo. El andamio monta la PIEZA pelada, así que ya no hay equivalencia
  // que derivar en este archivo.
  //
  // Lo que el candado defiende NO cambió y por eso no se borra: que los textos
  // del andamio salgan de una función y no de una cadena escrita al lado. Queda
  // sobre el sufijo, que es el que sigue estando, y se agrega la prohibición
  // simétrica para la equivalencia — que no vuelva como texto suelto.
  ...
  assert.doesNotMatch(
    src,
    /equivalencia=/,
    "el andamio volvió a pasarle una equivalencia a la pieza: ese bloque no existe"
  );
});
