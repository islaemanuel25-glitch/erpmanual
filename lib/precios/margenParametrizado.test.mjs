...
//
// La resolución vive en UN solo lugar, lib/precios/propagarCostoALocales:
//   local.margen != null ? local.margen : margenBase
// Antes estaba escrita dos veces —en costoMaestro y en syncFromBaseToLocales— y esa
// duplicación permitió que los caminos divergieran. Acá se prueba la REGLA; más
// abajo, pruebas estructurales confirman que ese módulo la implementa así y le pasa
// el resultado al helper.
...
test("6c. el margen llega SIEMPRE desde el producto, no desde una constante", () => {
  // costoMaestro sigue calculando el precio de la FICHA con el margen de la ficha.
  ...
  // La resolución "margen del local, y si no el de la ficha" es ahora una sola, en
  // el módulo de propagación que comparten el editor y el flujo de compras. Antes
  // estaba escrita dos veces y por eso los dos caminos podían divergir.
  const propagacion = leerSinComentarios("lib/precios/propagarCostoALocales.js");
  assert.ok(
    /local\.margen\s*!=\s*null\s*\?\s*local\.margen\s*:\s*margenBase/.test(propagacion),
    "la propagación ya no resuelve override → base"
  );
  assert.ok(
    /precioDesdeMargen\(\{[\s\S]{0,140}margenConfigurado,/.test(propagacion),
    "la propagación no le pasa el margen resuelto al helper"
  );
