...
test("y los servicios NO llevan marca", () => {
  ...
  //
  // Este candado exigía la condición EXACTA `esProductoServicio(p) ? null`, y se
  // puso rojo el 2026-08-19 cuando se le sumó un segundo caso sin marca: las
  // ubicaciones que venden con lista al costo, donde el porcentaje afirmaría un
  // margen que no existe y la línea "Costo" repetiría el número grande.
  //
  // Se reescribió el patrón, NO se aflojó lo que afirma. Lo que exigía —que un
  // servicio no llegue nunca al bloque de la marca— lo sigue exigiendo igual: si
  // alguien saca `esProductoServicio(p)` de la condición, este candado se pone
  // rojo lo mismo. Lo único que dejó de exigir es que sea el ÚNICO caso, que era
  // una propiedad de la pantalla de aquel día y no de la regla.
  //
  // El segundo caso tiene su propio candado, en
  // `lib/precios/ubicacionVendeAlCosto.test.mjs`.
  // ── Y LA RANURA CAMBIÓ DE NOMBRE CON LAS DOS CARAS ──────────────────────
  //
  // Era `marca`, un nodo armado en la pantalla con el costo y la regla juntos.
  // Con frente y dorso el costo depende de la cara —cada una en su escala— así
  // que la pantalla pasa la regla por su lado y el envoltorio la dibuja en las
  // dos. La ranura se llama `regla`.
  //
  // Lo que este candado exige no cambió: que un servicio no llegue nunca a
  // mostrar una regla de ganancia. Si alguien saca `esProductoServicio(p)` de esa
  // condición, se pone rojo igual.
  const pagina = leer("app/modulos/productos/page.jsx");
  const i = pagina.indexOf("regla={");
  assert.notEqual(i, -1, "desapareció la ranura de la regla en el catálogo");
  const bloque = pagina.slice(i, i + 260);
  assert.match(
    bloque, /esProductoServicio\(p\)/,
