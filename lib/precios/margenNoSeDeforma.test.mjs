...
test("5. los cuatro consumidores importan lib/precios/precioDesdeMargen", () => {
  const consumidores = [
  ...
  for (const rel of consumidores) {
    ...
    assert.ok(
      /precios\/precioDesdeMargen/.test(src),
...
// ── 6. syncFromBaseToLocales ya no cae a margen 0 ───────────────────────────

test("6. syncFromBaseToLocales no usa el fallback `margen ?? 0`", () => {
  const src = leerSinComentarios("app/api/productos/editar/[id]/route.js");
  const inicio = src.indexOf("async function syncFromBaseToLocales");
  assert.notEqual(inicio, -1);
  const fin = src.indexOf("export async function PUT", inicio);
...
test("6b. sin regla automática, syncFromBaseToLocales no escribe precio_venta", () => {
  const src = leerSinComentarios("app/api/productos/editar/[id]/route.js");
  const inicio = src.indexOf("async function syncFromBaseToLocales");
  const fin = src.indexOf("export async function PUT", inicio);
  const cuerpo = src.slice(inicio, fin);
  // La única asignación de precio_venta debe estar dentro del bloque guardado.
