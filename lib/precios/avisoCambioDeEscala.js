 ...
 *
 * Réplica MÍNIMA de la condición de `esFiambreFijo`, y a propósito no se importa
 * de `lib/conversiones/stock.js`: acá se evalúa un estado HIPOTÉTICO —lo que el
 * formulario está por guardar— y no un producto de la base. Lo que se compara son
 * dos fotos, la de antes y la de después, y ninguna de las dos existe todavía
 * como fila.
 */
function esPiezaFija(e = {}) {
  const um = String(e.unidad_medida ?? e.unidadMedida ?? "").toLowerCase();
  if (um !== "kg") return false;
  const peso = num(e.pesoReferenciaKg);
  if (peso === null || peso <= 0) return false;
  if (e.modoVentaDeposito) return e.modoVentaDeposito === "PIEZA";
  return e.pesoEsFijo === true;
}
