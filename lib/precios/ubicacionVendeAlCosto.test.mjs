...
test("la tarjeta muestra el precio que cobra el POS, no la columna", () => {
  ...
  assert.match(
    ...
    "el número grande dejó de derivar del precio base elegido"
  );
  assert.match(
    src,
...
test("Y LA EQUIVALENCIA SALE DEL MISMO PRECIO, no de la columna de venta", () => {
  ...
  const i = src.indexOf("lineaDeEquivalencia({");
  assert.notEqual(i, -1, "se fue la línea de equivalencia de la tarjeta");
  const bloque = src.slice(i, src.indexOf("})", i));

  assert.match(
    bloque,
    /precio:\s*precioBaseDelPos/,
    "la equivalencia volvió a derivar de un precio distinto del número grande"
  );
  assert.match(
    bloque,
    /redondeo100:\s*redondeoDelPos/,
    "la equivalencia volvió a usar un redondeo distinto del número grande"
  );
  assert.doesNotMatch(
    bloque,
    /precio:\s*p\.precioVenta/,
    "volvió `p.precioVenta` a la equivalencia: es exactamente el defecto"
  );
});
