...
test("LA PANTALLA LO USA: el catálogo no redondea por su cuenta", () => {
  ...
  const src = leer("app/modulos/productos/page.jsx")
    .replace(/\{\s*\/\*[\s\S]*?\*\/\s*\}/g, "")
    .replace(/\/\/[^\n]*/g, "");
  // ── LA PANTALLA DEJÓ DE LLAMARLO DIRECTO, Y NO ES UN AFLOJE ────────────
  //
  // Pedía `precioEnEscalaQueSeCobra(` escrito en la página. Desde el 2026-08-19
  // el catálogo llama a `valorEnLaEscalaDeVenta`, que es quien elige entre este
  // helper y el unitario según la escala en la que se VENDE — y que además sabe
  // la pieza del fiambre, que este helper no sabe.
  //
  // Lo que el candado defiende no cambió: que el número grande NO salga de la
  // columna cruda. Se sigue exigiendo abajo, y se agrega que salga de la pieza
  // compartida en vez de una cuenta propia de la pantalla.
  // ── Y SE MUDÓ OTRA VEZ, UN NIVEL MÁS ADENTRO ──────────────────────────
  //
  // La pantalla llamaba a `valorEnLaEscalaDeVenta` directo. Con las dos caras esa
  // llamada pasó a `carasDeTarjeta`, que la hace una vez por escala —el frente y
  // el dorso— y devuelve los dos importes ya resueltos.
  //
  // Lo que el candado defiende sigue siendo lo mismo: que el número NO salga de
  // una cuenta escrita en la pantalla. Se exige la pieza que la pantalla usa hoy,
  // y por eso se agrega la contraprueba: que la pantalla no vuelva a hacer la
  // cuenta por su cuenta al lado.
  assert.match(
    src, /carasDeTarjeta\(/,
    "el número grande tiene que salir de la pieza compartida, no de una cuenta propia"
  );
  assert.doesNotMatch(
    src, /precioUnitarioQueSeCobra\(|precioEnEscalaQueSeCobra\(|redondear100\(/,
    "la pantalla volvió a redondear por su cuenta, al lado de la pieza"
  );
  // ── EL REDONDEO SE MUDÓ A UNA CONSTANTE, Y SIGUE SIENDO EL DEL PRODUCTO ──
  //
  // Pedía `redondeo100: p.redondeo100` escrito en la llamada. El 2026-08-21 eso
  // pasó a `redondeoDelPos`, porque el número grande y la línea de equivalencia
  // tienen que llevar el MISMO redondeo y antes cada uno traía el suyo.
  //
  // Lo que el candado defiende no cambió —la venta lleva el redondeo del
  // producto— y ahora se exige sobre la definición, que además fija la otra
  // mitad: bajo lista al costo el redondeo es el de la LISTA, que es lo que
  // aplica el POS. Un `redondeoDelPos` que dijera cualquier otra cosa se ve acá.
  assert.match(
    src,
    /const redondeoDelPos = vendeConListaAlCosto\s*\?\s*listaAlCostoRedondea100\s*:\s*p\.redondeo100/,
    "y la venta tiene que llevar el redondeo del producto (y el costo, el de la lista)"
  );
  assert.doesNotMatch(
    src,
