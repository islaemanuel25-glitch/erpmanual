...
/**
 * ¿ESTE PRODUCTO SE ESTÁ VENDIENDO SIN GANANCIA?
 *
 * ── QUÉ CONTESTA, Y QUÉ NO ────────────────────────────────────────────────
 *
 * Contesta por el HECHO ya ocurrido: la venta no le saca nada al costo. No
 * contesta si el producto tiene o no una regla de precio configurada, que es una
 * pregunta distinta y una población distinta — medido contra producción el
 * 2026-08-18: **429 filas se venden al costo o por debajo**, y **1.691 no tienen
 * porcentaje asignado**, con poca superposición. Marcar las 1.691 en la tarjeta
 * sería marcar el 16 % del catálogo por algo que todavía no pasó.
 *
 * ── SE APOYA EN `margenEfectivoDe` A PROPÓSITO ────────────────────────────
 *
 * No compara costo contra venta por su cuenta: usa la función que ya existe y
 * que ya tiene sus candados. Dos formas de calcular lo mismo no se rompen el día
 * que se escriben, se rompen el día que una cambia.
 *
 * Y hereda su prudencia: **sin costo cargado devuelve `false`, no `true`**. Un
 * costo en cero no significa que se venda sin ganancia, significa que no se sabe
 * cuánto costó — y avisar sobre lo que no se sabe es ruido que enseña a ignorar
 * el aviso.
 *
 * @param {object} p
 * @param {number|string|null} p.costo   el costo guardado.
 * @param {number|string|null} p.venta   el precio de venta.
 * @returns {boolean} true solo si el margen efectivo se puede calcular y no es positivo.
 */
export function seVendeSinGanancia({ costo, venta } = {}) {
  // La variable se llama `efectivo` y NO `margen` a propósito, y no es cosmético:
  // el candado 6d de `margenParametrizado.test.mjs` prohíbe escribir el campo
  // `margen` a partir de un margen efectivo, y se puso rojo cuando esto se
  // llamaba `margen`. Tenía razón — acá no se persiste nada, pero el nombre
  // decía lo contrario, y un nombre que miente es exactamente lo que ese candado
  // existe para atajar.
  const efectivo = margenEfectivoDe(costo, venta);
  if (efectivo === null) return false;
  // El umbral es un centésimo de punto: 0,000001 % es ruido de coma flotante,
  // no una ganancia.
  return efectivo <= 0.01;
}
