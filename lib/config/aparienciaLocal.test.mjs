...
test("LA PANTALLA APAGA POR DEFECTO, y eso se lee `=== true`", () => {
  ...
  // ── ACÁ SE PEDÍAN LAS DOS Y AHORA SE PIDE UNA. NO ES UN AFLOJE ──────────
  //
  // El 2026-08-19 la tarjeta pasó a mostrar la escala en la que se VENDE —la
  // que decide el POS— en vez de la escala en la que está guardado el precio.
  // Desde entonces el número y su rótulo ya no se eligen: salen de
  // `escalaDeVentaDe`. `tarjetaPrecioUnitario` dejó de tener efecto y la página
  // dejó de leerlo, así que exigir esa lectura sería exigir que vuelva el
  // defecto.
  //
  // Lo que NO se aflojó: el contexto sigue resolviendo las DOS con `=== true`
  // —las dos primeras afirmaciones—, porque ahí es donde vive el default
  // apagado. Y `tarjetaOcultarEquivalencia` sigue en uso y se sigue exigiendo.
