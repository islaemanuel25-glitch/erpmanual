...
test("LA PREFERENCIA QUE QUEDA APAGA POR DEFECTO, y eso se lee `=== true`", () => {
  // `!== false` daría VERDADERO para un local que nunca la tocó (null) y para
  // un servidor viejo que no la mande, y les cambiaría el catálogo a todos los
  ...
  // ── ERAN DOS Y QUEDÓ UNA, EN DOS PASOS Y NINGUNO ES UN AFLOJE ───────────
  //
  ...
  //
  // Después se sacó el interruptor, porque uno que no hace nada es peor que no
  // tenerlo — se toca, no pasa nada, y lo próximo que se reporta es "el sistema
  // anda mal". La COLUMNA se conserva; el motivo está en `aparienciaLocal.js`.
  //
  // Estas dos afirmaciones son las que impiden que vuelva por la ventana.
  assert.doesNotMatch(
  ...
  assert.doesNotMatch(
    apariencia, /tarjetaPrecioUnitario/,
    "volvió el interruptor sin efecto a la pantalla de apariencia"
  );

  const pagina = leer("app/modulos/productos/page.jsx");
  assert.match(pagina, /tarjetaOcultarEquivalencia === true/);
  assert.doesNotMatch(
    pagina, /tarjetaPrecioUnitario/,
    "la página volvió a dejar que una preferencia pise la escala de venta"
  );
});
