...
import { CONDICION_PAGO_LABEL, CONDICION_PAGO_OFERTA } from "./vigencia.js";
 ...
 */
/**
 * Los datos de la línea ÚNICA, o `null` en los tres si hay más de una.
 *
 * Aparte de `ofertaParaLista` para poder ejercer el caso de varias sin armar una
 * oferta entera: es el que decide entre mostrar un precio y no mostrar ninguno.
 */
function unicaLinea(lineas, oferta) {
  const total = oferta?._count?.lineas ?? lineas.length;
  const l = total === 1 ? lineas[0] : null;
  if (!l) return { producto: null, precioOferta: null, precioNormal: null };
  return {
    producto: l.productoLocal?.nombre || l.productoLocal?.base?.nombre || null,
    precioOferta: l.precioOferta == null ? null : Number(l.precioOferta),
    precioNormal:
      l.precioNormalReferencia == null ? null : Number(l.precioNormalReferencia),
  };
}
...
export function ofertaParaLista(oferta, ahora = new Date()) {
  ...
  return {
    ...
    // ── LOS NÚMEROS QUE MUESTRA LA TARJETA DEL CELULAR ───────────────────
    //
    // Una oferta del flujo móvil es UN producto: el nombre de la oferta ES el
    // del producto y la tarjeta muestra su precio contra el normal. Por eso los
    // tres campos salen de la ÚNICA línea, y vienen en `null` cuando hay más de
    // una.
    //
    // `null` y no "el de la primera": con dos productos, mostrar el precio de
    // uno solo como si fuera el de la oferta es una afirmación falsa sobre el
    // otro. La tarjeta, sin estos datos, dice cuántos productos tiene y no
    // inventa un precio.
    //
    // No se recalcula nada: `precioNormalReferencia` es la foto del momento en
    // que se cargó la línea, que es contra lo que la oferta se decidió. El
    // precio normal de HOY puede ser otro, y compararlos es justo de lo que se
    // ocupa el aviso de revisar.
    ...unicaLinea(lineas, oferta),
    soloEfectivo: oferta.condicionPago === CONDICION_PAGO_OFERTA.SOLO_EFECTIVO,
 ...
 */
export function lineaParaDetalle(
  linea,
  { precioActual = null, costoActual = null, stockActual = null, escalaActual = null } = {}
) {
  ...
  return {
    ...
    // EL STOCK DE HOY, para la tarjeta del producto del detalle. No se congela
    // en ningún lado: es un hecho del momento en que se mira, a diferencia de
    // las dos referencias de arriba.
    stock: stockActual == null ? null : Number(stockActual),
    escala: escalaActual ?? null,
...
export function ofertaParaDetalle(oferta, { actualesPorProductoLocal = {}, ahora = new Date() } = {}) {
  ...
  const lineas = (Array.isArray(oferta.lineas) ? oferta.lineas : []).map((l) => {
    ...
    return lineaParaDetalle(l, {
      ...
      // El stock y la escala son de HOY y salen de la misma lectura que el
      // precio y el costo actuales: una sola consulta, una sola fuente.
      stockActual: actual.stock ?? null,
      escalaActual: actual.escala ?? null,
    });
  });
