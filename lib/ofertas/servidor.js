...
import { orCodigoBarraProductoLocal } from "@/lib/productos/busquedaCodigoBarra";
...
export async function referenciasDeProducto(db, { localId, productoLocalIds }) {
  ...
  const filas = await db.productoLocal.findMany({
    ...
    select: {
      ...
      precio_venta: true,
      precio_costo: true,
      // ── EL STOCK VIAJA, Y NO SE CONGELA EN NINGÚN LADO ──────────────────
      //
      // El detalle de la oferta muestra la MISMA tarjeta de producto que la
      // pantalla de crear, y esa tarjeta dice el stock de hoy. Sin esto había
      // que pedírselo al buscador POR NOMBRE, que es otra consulta y otra
      // fuente para el mismo hecho.
      //
      // Es un dato de HOY: a diferencia del precio normal y el costo, que se
      // congelan en la fila de la oferta, éste no se guarda en ninguna parte.
  ...
  for (const pl of filas) {
    ...
    mapa[pl.id] = {
      ...
      stock: Number(pl.stock?.[0]?.cantidad ?? 0),
    };
  }
  return mapa;
}
...
export async function buscarProductosOfertables(db, { localId, q = "", limite = 30 }) {
  ...
  const filas = await db.productoLocal.findMany({
    where: {
      ...
      ...(texto
        ? {
            // ── EL SECUNDARIO FALTABA, Y SE DEVOLVÍA IGUAL ──────────────
            //
            // El OR estaba escrito a mano y miraba el propio y el principal,
            // pero no `codigo_barra_secundario` — que este mismo endpoint SÍ
            // devuelve en su respuesta. Escanear un código secundario armando
            // una oferta no encontraba el producto.
            //
            // `orCodigoBarraProductoLocal` es la definición canónica de "qué
            // código se puede escanear" y ya la usan el POS y Stock. Escribirla
            // al lado es cómo se empezó este defecto.
            OR: [
              { nombre: { contains: texto, mode: "insensitive" } },
              { base: { nombre: { contains: texto, mode: "insensitive" } } },
              ...orCodigoBarraProductoLocal(texto),
            ],
          }
        : {}),
    },
