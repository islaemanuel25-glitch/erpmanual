...
export async function buscarProductosOfertables(db, { localId, q = "", limite = 30 }) {
  ...
  const filas = await db.productoLocal.findMany({
    ...
    select: {
      ...
      precio_costo: true,
      base: {
        select: { nombre: true, precio_venta: true, precio_costo: true, es_combo: true },
      },
    },
  });
  ...
  return filas.map((pl) => ({
    ...
    productoBaseId: pl.baseId,
    nombre: pl.nombre || pl.base?.nombre || "",
    precioNormal: Number(precioDeLaUbicacion(pl.base?.precio_venta, pl.precio_venta)) || 0,
    costo: Number(precioDeLaUbicacion(pl.base?.precio_costo, pl.precio_costo)) || 0,
    esCombo: pl.base?.es_combo === true,
  }));
}
