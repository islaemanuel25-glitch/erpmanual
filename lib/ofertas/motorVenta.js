...
import { recargoDeCondiciones, importeRecargo } from "@/lib/recargos-pago/recargoPago.js";
import { condicionLegacy } from "@/lib/pos-ventas/modalidadesDeMedio.js";
 ...
 * @param {Array} args.condicionesDeCobro
 *   CONDICIONES YA RESUELTAS por el servidor, una por tender. Es la entrada del
 *   camino de modalidades y tiene prioridad sobre las dos anteriores.
 *
 *   Por qué hizo falta: `mediosUsados` + `recargosPorMedio` identifican una
 *   condición por su `MedioPago`, y desde que un medio puede tener modalidades
 *   dos condiciones distintas comparten ese enum —"Crédito 1 pago" al 4 % y
 *   "Crédito cuotas" al 8 % son las dos CREDITO—. Un mapa no puede tener las dos.
 *
 *   NO son dos motores: cuando no viene, se ARMA desde `mediosUsados` y
 *   `recargosPorMedio` con el adaptador legacy, y el resto del cálculo es el
 *   mismo código para los dos caminos.
...
export function calcularVentaComercial({
  ...
  mediosUsados = [],
  recargosPorMedio = {},
  condicionesDeCobro = null,
  descuentos = {},
  subtotalServicios = 0,
} = {}) {
  // ── UNA SOLA REPRESENTACIÓN, DOS ENTRADAS ────────────────────────────────
  //
  // Todo lo que sigue razona sobre `condiciones`. El camino legacy es una
  // conversión de la entrada y no una rama del cálculo: si mañana cambia la regla
  // del recargo, cambia para los dos a la vez porque hay una sola.
  const condiciones =
    Array.isArray(condicionesDeCobro) && condicionesDeCobro.length > 0
      ? condicionesDeCobro
      : [...new Set((Array.isArray(mediosUsados) ? mediosUsados : []).filter(Boolean))].map((m) =>
          condicionLegacy(m, { recargosPorMedio })
        );

  // Los TIPOS CONTABLES efectivos. Con modalidades no salen de lo que declaró el
  // cliente sino del `tipoContable` de la modalidad, que es lo que la venta va a
  // congelar — y es lo que tiene que mirar una oferta SOLO_EFECTIVO: una
  // modalidad de tipo EFECTIVO cuenta como efectivo, igual que cualquier otro
  // camino hacia ese enum.
  const medios = [...new Set(condiciones.map((c) => c?.medio).filter(Boolean))];
  ...
  for (const linea of lineas) {
    ...
    const { precio, oferta: aplicada, motivo } = resolverLinea(linea, oferta, medios);
  ...
  const recargo = recargoDeCondiciones(condiciones);
  ...
  return {
    ...
    recargoPagoImporte,
    total,
    // ── QUIÉN IMPUSO LA CONDICIÓN, ADEMÁS DE CUÁNTO ────────────────────────
    //
    // El mismo cálculo que decidió el porcentaje dice quién ganó. No se
    // reconstruye después buscando en la configuración: esa segunda búsqueda
    // podría dar otro resultado —un empate resuelto distinto, una modalidad
    // editada en el medio— y la venta guardaría un ganador que no es el que
    // produjo el número.
    //
    // Los cuatro en null cuando el ganador no tiene identidad configurable (el
    // camino legacy) o cuando no hubo recargo. Alimentan directamente las
    // columnas nuevas de `Venta`.
    recargoPagoMedioCobroLocalId: ganador?.medioCobroLocalId ?? null,
    recargoPagoMedioNombre: ganador?.medioNombre ?? null,
    recargoPagoModalidadId: ganador?.modalidadId ?? null,
    recargoPagoModalidadNombre: ganador?.modalidadNombre ?? null,
