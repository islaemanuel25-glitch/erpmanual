...
import { claveCobro, opcionesDeCobro } from "@/lib/pos-ventas/modalidadesDeMedio.js";
...
export function totalParaMedios({
  ...
  return calcularVentaComercial({
    ...
    recargosPorMedio,
    condicionesDeCobro: condiciones,
    descuentos,
    subtotalServicios,
  });
}
...
/**
 * EL TOTAL DE UNA OPCIÓN DE COBRO CONCRETA — este botón, esta modalidad.
 *
 * Sigue sin hacer matemática propia: arma la lista de condiciones y llama al
 * MISMO `calcularVentaComercial` que corre en `pos-ventas/crear`. Es toda la
 * garantía de que el preview y el backend den el mismo número, y por eso no hay
 * una versión "rápida" de esto en ningún lado.
 *
 * @param {object} args
 * @param {Array} args.condiciones  una o varias condiciones ya resueltas
 */
export function totalParaCondiciones({ condiciones = [], ...resto } = {}) {
  return totalParaMedios({ ...resto, condiciones });
}

/**
 * EL TOTAL DE CADA OPCIÓN QUE ESTE LOCAL PUEDE COBRAR.
 *
 * ── POR QUÉ NO ALCANZA `totalesPorMedio` ─────────────────────────────────
 *
 * Aquél devuelve un objeto indexado por `MedioPago`. Con modalidades eso pisa
 * datos: "Crédito 1 pago" al 4 % y "Crédito cuotas" al 8 % son las dos CREDITO,
 * así que la segunda sobrescribe a la primera y el cajero ve un solo total —el
 * equivocado para una de las dos— sin ningún síntoma.
 *
 * Acá la clave es la de `claveCobro`: la modalidad si hay, el medio si no. Dos
 * modalidades del mismo tipo contable son dos entradas distintas con dos totales
 * distintos, que es el punto de toda la tanda.
 *
 * `totalesPorMedio` se conserva intacto para los llamadores que todavía razonan
 * en tipos contables. No se lo "arregla" para que devuelva las dos cosas: un
 * mismo objeto con dos formas de clave sería exactamente la confusión que esto
 * viene a evitar.
 *
 * @param {Array} args.medios  los del local, compuestos y con modalidades
 * @returns {Record<string, {clave, condicion, total, ...}>}
 */
export function totalesPorOpcionDeCobro({
  carrito = [],
  medios = [],
  descuentos = {},
  subtotalServicios = 0,
} = {}) {
  const comun = { carrito, descuentos, subtotalServicios };
  const salida = {};

  for (const opcion of opcionesDeCobro(medios)) {
    const { clave, ...condicion } = opcion;
    const r = totalParaCondiciones({ ...comun, condiciones: [condicion] });
    salida[clave] = {
      clave,
      condicion,
      medio: condicion.medio,
      medioCobroLocalId: condicion.medioCobroLocalId,
      medioNombre: condicion.medioNombre,
      modalidadId: condicion.modalidadId,
      modalidadNombre: condicion.modalidadNombre,
      total: r.total,
      subtotal: r.subtotal,
  ...
  // Para el panel de "Dividir pago", donde el conjunto lo arma la persona y no se
  // puede saber de antemano. No enumerable, igual que en `totalesPorMedio`.
  Object.defineProperty(salida, "__paraCondiciones", {
    value: (condiciones) => totalParaCondiciones({ ...comun, condiciones }),
    enumerable: false,
  });
  Object.defineProperty(salida, "__clave", { value: claveCobro, enumerable: false });
