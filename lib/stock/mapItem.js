...
// Los tres datos identificatorios que necesita la tarjeta compartida. Salen del
// MISMO `base` que ya trae el listado: no hay una consulta por card.
function identificacionParaTarjeta(base) {
  const codigos = base.codigosProveedor ?? [];
  const principal = codigos.find((c) => c.proveedorId === base.proveedor_id);
  return {
    proveedorNombre: base.proveedor?.nombre ?? null,
    codigoInterno: (principal ?? codigos[0])?.codigoInterno ?? null,
...
export function mapStockItemLocal(pl, base, stock) {
  ...
  return {
    ...
    ...identificacionParaTarjeta(base),
