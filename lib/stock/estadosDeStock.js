...
export function estadosDeLaFila(fila) {
  // ── LA CANTIDAD SE LLAMA DISTINTO SEGÚN DE DÓNDE VENGA, Y ESO MORDIÓ ────
  //
  // En la fila cruda de `StockLocal` el campo es `cantidad`; en el ítem que
  // devuelve `mapItem` —que es lo que ve la pantalla y lo que devuelve la API—
  // se llama `stock`.
  //
  // Esta función leía solo `cantidad`, así que contra un ítem REAL tomaba 0 y
  // contestaba cualquier cosa: un producto con 90 unidades y máximo 50 salía
  // como "sin stock" y "bajo mínimo" a la vez. Y no se veía, porque los primeros
  // candados le pasaban objetos hechos a mano con `cantidad` — probaban una
  // forma que la aplicación no produce nunca.
  //
  // Lo destapó ejercer `mapStockItemDeposito` de verdad. Se aceptan las dos
  // formas y `mapItemDeposito.test.mjs` la ejerce contra ítems mapeados.
  const cantidad = Number(fila?.cantidad ?? fila?.stock ?? 0);
