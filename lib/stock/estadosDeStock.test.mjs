...
test("EST6bis. LA CANTIDAD SE LEE EN LAS DOS FORMAS QUE EXISTEN", () => {
  // ── ESTE CANDADO NACIÓ DE UN DEFECTO QUE LOS OTROS NO VEÍAN ────────────
  //
  // La fila cruda de `StockLocal` llama `cantidad` a la cantidad; el ítem que
  // devuelve `mapItem` —lo que ve la pantalla y lo que sale por la API— la llama
  // `stock`. La función leía solo la primera, así que contra un ítem real tomaba
  // 0: un producto con 90 unidades y máximo 50 salía a la vez como "sin stock" y
  // "bajo mínimo".
  //
  // Los demás candados de este archivo le pasan objetos con `cantidad`, o sea
  // una forma que la aplicación no produce nunca. Por eso pasaban todos.
  const conStock = { stock: 90, stockMin: 5, stockMax: 50, limitesConfigurados: true };
  const e = estadosDeLaFila(conStock);
  assert.ok(e.includes(ESTADO_STOCK.SOBRE_MAXIMO), "no leyó `stock` y creyó que la cantidad era 0");
  assert.ok(!e.includes(ESTADO_STOCK.SIN_STOCK), "un producto con 90 unidades salió como 'sin stock'");
  assert.ok(!e.includes(ESTADO_STOCK.BAJO_MINIMO));

  // Y la forma cruda sigue funcionando: las dos conviven.
  assert.deepEqual(
    estadosDeLaFila({ cantidad: 90, stockMin: 5, stockMax: 50, limitesConfigurados: true }),
    e,
    "las dos formas de nombrar la cantidad dejaron de contestar lo mismo"
  );
});
