...
test("ningún archivo compara contra una unidad que no está en el enum", () => {
  ...
  const archivos = execFileSync(
    // `--others --exclude-standard`: sin eso un archivo nuevo sin commitear no
    // entra y el candado lo deja pasar. Agujero del 2026-09-14.
    "git", ["ls-files", "--cached", "--others", "--exclude-standard", "*.js", "*.jsx"],
