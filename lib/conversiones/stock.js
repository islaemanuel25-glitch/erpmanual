...
/**
 * ¿EL DEPÓSITO DESPACHA ESTO POR PIEZA?
 *
 * ── POR QUÉ ESTA MITAD EXISTE APARTE ────────────────────────────────────────
 *
 * `esFiambreFijo` mezcla dos preguntas de relaciones comerciales distintas:
 * cómo el depósito COMPRA al proveedor —`modoCompraProveedor`— y cómo el
 * depósito DESPACHA al local —`modoVentaDeposito`—. Para el stock esa mezcla
 * está bien y no se toca: describe un producto concreto del que se sabe todo.
 *
 * Para la RECEPCIÓN DEL LOCAL no: ahí la única pregunta válida es cómo salió
 * del depósito. Con la mezcla, un fiambre de pieza fija que el depósito le
 * COMPRE por bulto al proveedor dejaría de verse como PIEZA en la recepción y
 * pasaría a verse como KG — una decisión de la relación proveedor→depósito
 * cambiando lo que ve el local. Hoy no ocurre en los datos, porque los 40
 * productos de pieza tienen `modoCompraProveedor = UNIDAD`, pero está latente y
 * es exactamente la mezcla que la recepción no puede hacer.
 *
 * Así que la mitad de VENTA se escribe una vez acá y la consumen los dos. No es
 * una copia: `esFiambreFijo` pasa a componerse con ésta, así que sigue dando lo
 * mismo que daba y la aritmética de fiambres no se mueve.
 *
 * @param {Object} base - ProductoBase
 * @returns {boolean}
 */
export function elDepositoDespachaPorPieza(base) {
  if (!base) return false;
  const um = (base.unidad_medida || "").toLowerCase();
  const peso = Number(base.pesoReferenciaKg ?? 0);
  if (um !== "kg" || !(peso > 0)) return false;
 ...
 *
 * SIGUE SIENDO EL PREDICADO DEL STOCK y no cambió de valor: es la conjunción de
 * lo que ya exigía —incluido `modoCompraProveedor`— con la mitad de venta, que
 * ahora vive aparte para que la recepción pueda preguntar solo por ésa.
 *
 * @param {Object} base - ProductoBase
 * @returns {boolean}
 */
export function esFiambreFijo(base) {
  ...
  return elDepositoDespachaPorPieza(base);
}
