...
function archivosDelRepo() {
  // `--others --exclude-standard`: este candado existe para que NO aparezca una
  // copia nueva del predicado, y una copia nueva vive en un archivo nuevo — que
  // es justo lo que `git ls-files` a secas no ve. Agujero del 2026-09-14.
  const salida = execFileSync("git", ["ls-files", "--cached", "--others", "--exclude-standard", "*.js", "*.jsx", "*.mjs"], {
    cwd: RAIZ,
    encoding: "utf8",
